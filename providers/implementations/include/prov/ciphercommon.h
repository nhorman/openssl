...
typedef int(PROV_CIPHER_HW_FN)(PROV_CIPHER_CTX *dat, unsigned char *out, const unsigned char *in,
                               size_t len);
...
void ossl_cipher_generic_initkey(void *vctx, size_t kbits, size_t blkbits, size_t ivbits,
                                 unsigned int mode, uint64_t flags, const PROV_CIPHER_HW *hw,
                                 void *provctx);

#define IMPLEMENT_generic_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits,   \
                                      typ)                                                         \
...
#define IMPLEMENT_var_keylen_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits,        \
                                         ivbits, typ)                                              \
    const OSSL_DISPATCH ossl_##alg##kbits##lcmode##_functions[] = {                                \
        {OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))alg##_##kbits##_##lcmode##_newctx},              \
        {OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))alg##_freectx},                                 \
...
#define IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits,  \
                                       typ)                                                        \
    ...
    {                                                                                              \
        ...
        if (ctx != NULL) {                                                                         \
            ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE,     \
                                        flags, ossl_prov_cipher_hw_##alg##_##lcmode(kbits),        \
                                        provctx);                                                  \
        }                                                                                          \
        return ctx;                                                                                \
    }
...
#define IMPLEMENT_generic_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ)   \
    IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ) \
        IMPLEMENT_generic_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits,   \
                                      typ)

#define IMPLEMENT_var_keylen_cipher(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits,     \
                                    typ)                                                           \
    IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits, ivbits, typ) \
        IMPLEMENT_var_keylen_cipher_func(alg, UCALG, lcmode, UCMODE, flags, kbits, blkbits,        \
                                         ivbits, typ)
...
#define IMPLEMENT_CIPHER_HW_OFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)                       \
    static int cipher_hw_##NAME##_##MODE##_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out,        \
...
size_t ossl_cipher_fillblock(unsigned char *buf, size_t *buflen, size_t blocksize,
