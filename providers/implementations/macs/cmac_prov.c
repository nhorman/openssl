 ...
 */
static int
tdes_check_param(struct cmac_data_st *macctx, OSSL_PARAM *p, int *state)
{
    ...
    *state = OSSL_FIPS_IND_STATE_UNKNOWN;
    if (EVP_CIPHER_is_a(cipher, "DES-EDE3-CBC")) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(macctx, OSSL_FIPS_IND_SETTABLE0, libctx, "CMAC",
                                         "Triple-DES", ossl_fips_config_tdes_encrypt_disallowed))
            return 0;
...
static int
cmac_setkey(struct cmac_data_st *macctx, const unsigned char *key, size_t keylen)
{
    int rv;
    OSSL_PARAM *p = NULL;
#ifdef FIPS_MODULE
...
static int
cmac_init(void *vmacctx, const unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    struct cmac_data_st *macctx = vmacctx;
...
static int
cmac_update(void *vmacctx, const unsigned char *data, size_t datalen)
{
    struct cmac_data_st *macctx = vmacctx;
...
static int
cmac_final(void *vmacctx, unsigned char *out, size_t *outl, size_t outsize)
{
    struct cmac_data_st *macctx = vmacctx;
...
cmac_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx)
{
    return known_gettable_ctx_params;
}

static int
cmac_get_ctx_params(void *vmacctx, OSSL_PARAM params[])
{
    OSSL_PARAM *p;
...
static const OSSL_PARAM *
cmac_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx)
{
 ...
 */
static int
cmac_set_ctx_params(void *vmacctx, const OSSL_PARAM params[])
{
    struct cmac_data_st *macctx = vmacctx;
