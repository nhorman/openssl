...
static int
gmac_setkey(struct gmac_data_st *macctx, const unsigned char *key, size_t keylen)
{
...
static int
gmac_init(void *vmacctx, const unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    struct gmac_data_st *macctx = vmacctx;
...
static int
gmac_update(void *vmacctx, const unsigned char *data, size_t datalen)
{
    struct gmac_data_st *macctx = vmacctx;
    EVP_CIPHER_CTX *ctx = macctx->ctx;
    int outlen;
...
static int
gmac_final(void *vmacctx, unsigned char *out, size_t *outl, size_t outsize)
{
...
gmac_gettable_params(void *provctx)
{
    return known_gettable_params;
}

static int
gmac_get_params(OSSL_PARAM params[])
{
    OSSL_PARAM *p;
...
static const OSSL_PARAM *
gmac_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx)
{
 ...
 */
static int
gmac_set_ctx_params(void *vmacctx, const OSSL_PARAM params[])
{
    struct gmac_data_st *macctx = vmacctx;
    EVP_CIPHER_CTX *ctx = macctx->ctx;
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_CIPHER)) != NULL) {
        ...
        if (!EVP_EncryptInit_ex(ctx, ossl_prov_cipher_cipher(&macctx->cipher),
                                ossl_prov_cipher_engine(&macctx->cipher), NULL, NULL))
            return 0;
    }
