...
static int
write_msblob(struct key2ms_ctx_st *ctx, OSSL_CORE_BIO *cout, EVP_PKEY *pkey, int ispub)
{
...
static int
write_pvk(struct key2ms_ctx_st *ctx, OSSL_CORE_BIO *cout, EVP_PKEY *pkey)
{
    BIO *out = NULL;
    int ret;
    ...
    if (out == NULL)
        return 0;
    ret = i2b_PVK_bio_ex(out, pkey, ctx->pvk_encr_level, ossl_pw_pvk_password, &ctx->pwdata, libctx,
                         NULL);
    BIO_free(out);
    return ret;
}
...
static int
key2pvk_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    struct key2ms_ctx_st *ctx = vctx;
    const OSSL_PARAM *p;
...
static int
key2ms_does_selection(void *vctx, int selection)
{
...
static int
key2msblob_encode(void *vctx, const void *key, int selection, OSSL_CORE_BIO *cout,
...
static int
key2pvk_encode(void *vctx, const void *key, int selection, OSSL_CORE_BIO *cout,
               evp_pkey_set1_fn *set1_key, OSSL_PASSPHRASE_CALLBACK *pw_cb, void *pw_cbarg)
{
    struct key2ms_ctx_st *ctx = vctx;
    EVP_PKEY *pkey = NULL;
...
#define MAKE_MS_ENCODER(impl, output, type)                                                        \
    ...
    static int impl##2##output##_encode(void *vctx, OSSL_CORE_BIO *cout, const void *key,          \
