...
static int
read_pem(PROV_CTX *provctx, OSSL_CORE_BIO *cin, char **pem_name, char **pem_header,
...
static int
pem2der_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    struct pem2der_ctx_st *ctx = vctx;
    const OSSL_PARAM *p;
    char *str;
...
static int
pem2der_pass_helper(char *buf, int num, int w, void *data)
{
...
static int
pem2der_decode(void *vctx, OSSL_CORE_BIO *cin, int selection, OSSL_CALLBACK *data_cb,
...
#define SPKI_LAST_IDX 2
    ...
    if (i < OSSL_NELEM(pem_name_map)) {
        ...
        if (i <= PKCS8_LAST_IDX &&
            ...
            ok = ossl_epki2pki_der_decode(der, der_len, selection, data_cb, data_cbarg, pw_cb,
        ...
        if (i <= SPKI_LAST_IDX &&
            ...
            ok = ossl_spki2typespki_der_decode(der, der_len, selection, data_cb, data_cbarg, pw_cb,
                                               pw_cbarg, PROV_LIBCTX_OF(ctx->provctx), ctx->propq);
            goto end;
        }
