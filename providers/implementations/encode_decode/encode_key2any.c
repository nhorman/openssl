...
typedef int key_to_der_fn(BIO *out, const void *key, int key_nid, const char *pemname,
...
static X509_SIG *
p8info_to_encp8(PKCS8_PRIV_KEY_INFO *p8info, KEY2ANY_CTX *ctx)
{
    X509_SIG *p8 = NULL;
...
static X509_SIG *
key_to_encp8(const void *key, int key_nid, void *params, int params_type, OSSL_i2d_of_void_ctx *k2d,
             KEY2ANY_CTX *ctx)
{
...
static X509_PUBKEY *
key_to_pubkey(const void *key, int key_nid, void *params, int params_type,
              OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    /* der, derlen store the key DER output and its length */
    unsigned char *der = NULL;
...
static int
key_to_epki_der_priv_bio(BIO *out, const void *key, int key_nid, ossl_unused const char *pemname,
                         key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    int ret = 0;
    void *str = NULL;
...
static int
key_to_epki_pem_priv_bio(BIO *out, const void *key, int key_nid, ossl_unused const char *pemname,
                         key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    int ret = 0;
    void *str = NULL;
...
static int
key_to_pki_der_priv_bio(BIO *out, const void *key, int key_nid, ossl_unused const char *pemname,
                        key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    int ret = 0;
    void *str = NULL;
...
static int
key_to_pki_pem_priv_bio(BIO *out, const void *key, int key_nid, ossl_unused const char *pemname,
                        key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    int ret = 0;
    void *str = NULL;
...
static int
key_to_spki_der_pub_bio(BIO *out, const void *key, int key_nid, ossl_unused const char *pemname,
                        key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    int ret = 0;
    void *str = NULL;
...
static int
key_to_spki_pem_pub_bio(BIO *out, const void *key, int key_nid, ossl_unused const char *pemname,
                        key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    int ret = 0;
    void *str = NULL;
 ...
 */
static int
key_to_type_specific_der_bio(BIO *out, const void *key, int key_nid,
                             ossl_unused const char *pemname, key_to_paramstring_fn *p2s,
                             OSSL_i2d_of_void_ctx *k2d, KEY2ANY_CTX *ctx)
{
    unsigned char *der = NULL;
    int derlen;
...
static int
key_to_type_specific_pem_bio_cb(BIO *out, const void *key, int key_nid, const char *pemname,
                                key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d,
                                KEY2ANY_CTX *ctx, pem_password_cb *cb, void *cbarg)
{
    return PEM_ASN1_write_bio_ctx(k2d, (void *)ctx, pemname, out, key, ctx->cipher, NULL, 0, cb,
                                  cbarg) > 0;
}

static int
key_to_type_specific_pem_priv_bio(BIO *out, const void *key, int key_nid, const char *pemname,
                                  key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d,
                                  KEY2ANY_CTX *ctx)
{
    return key_to_type_specific_pem_bio_cb(out, key, key_nid, pemname, p2s, k2d, ctx,
                                           ossl_pw_pem_password, &ctx->pwdata);
}

static int
key_to_type_specific_pem_pub_bio(BIO *out, const void *key, int key_nid, const char *pemname,
                                 key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d,
                                 KEY2ANY_CTX *ctx)
{
...
static int
key_to_type_specific_pem_param_bio(BIO *out, const void *key, int key_nid, const char *pemname,
                                   key_to_paramstring_fn *p2s, OSSL_i2d_of_void_ctx *k2d,
                                   KEY2ANY_CTX *ctx)
{
    return key_to_type_specific_pem_bio_cb(out, key, key_nid, pemname, p2s, k2d, ctx, NULL, NULL);
...
#define k2d_NOCTX(n, f)                                                                            \
    static int n##_k2d(const void *key, unsigned char **pder, ossl_unused void *ctx)               \
    {                                                                                              \
        return f(key, pder);                                                                       \
    }
...
static int
prepare_dh_params(const void *dh, int nid, int save, void **pstr, int *pstrtype)
{
...
static int
dh_spki_pub_to_der(const void *dh, unsigned char **pder, ossl_unused void *ctx)
{
...
static int
dh_pki_priv_to_der(const void *dh, unsigned char **pder, ossl_unused void *ctx)
{
    const BIGNUM *bn = NULL;
...
static int
dh_type_specific_params_to_der(const void *dh, unsigned char **pder, ossl_unused void *ctx)
{
...
static int
dh_check_key_type(const void *dh, int expected_type)
{
...
static int
encode_dsa_params(const void *dsa, int nid, void **pstr, int *pstrtype)
{
    ASN1_STRING *params = ASN1_STRING_new();
...
static int
prepare_dsa_params(const void *dsa, int nid, int save, void **pstr, int *pstrtype)
{
...
static int
dsa_spki_pub_to_der(const void *dsa, unsigned char **pder, ossl_unused void *ctx)
{
    const BIGNUM *bn = NULL;
    ASN1_INTEGER *pub_key = NULL;
    int ret;
...
static int
dsa_pki_priv_to_der(const void *dsa, unsigned char **pder, ossl_unused void *ctx)
{
    const BIGNUM *bn = NULL;
    ASN1_INTEGER *priv_key = NULL;
    int ret;
 ...
 */
static int
prepare_ec_params(const void *eckey, int nid, int save, void **pstr, int *pstrtype)
{
    int curve_nid;
...
static int
ec_spki_pub_to_der(const void *eckey, unsigned char **pder, ossl_unused void *ctx)
{
...
static int
ec_pki_priv_to_der(const void *veckey, unsigned char **pder, ossl_unused void *ctx)
{
...
static int
ecx_pki_priv_to_der(const void *vecxkey, unsigned char **pder, ossl_unused void *ctx)
{
    const ECX_KEY *ecxkey = vecxkey;
...
static int
ml_dsa_spki_pub_to_der(const void *vkey, unsigned char **pder, ossl_unused void *ctx)
{
    return ossl_ml_dsa_i2d_pubkey(vkey, pder);
}

static int
ml_dsa_pki_priv_to_der(const void *vkey, unsigned char **pder, void *vctx)
{
...
static int
ml_kem_spki_pub_to_der(const void *vkey, unsigned char **pder, ossl_unused void *ctx)
{
    return ossl_ml_kem_i2d_pubkey(vkey, pder);
}

static int
ml_kem_pki_priv_to_der(const void *vkey, unsigned char **pder, void *vctx)
{
    KEY2ANY_CTX *ctx = vctx;
...
static int
prepare_rsa_params(const void *rsa, int nid, int save, void **pstr, int *pstrtype)
{
...
static int
slh_dsa_spki_pub_to_der(const void *vkey, unsigned char **pder, ossl_unused void *ctx)
{
...
static int
slh_dsa_pki_priv_to_der(const void *vkey, unsigned char **pder, ossl_unused void *ctx)
{
    const SLH_DSA_KEY *key = vkey;
    size_t len;
...
static int
key2any_encode(KEY2ANY_CTX *ctx, OSSL_CORE_BIO *cout, const void *key, int type,
               const char *pemname, check_key_type_fn *checker, key_to_der_fn *writer,
               OSSL_PASSPHRASE_CALLBACK *pwcb, void *pwcbarg,
               key_to_paramstring_fn *key2paramstring, OSSL_i2d_of_void_ctx *key2der)
{
    int ret = 0;
    ...
    if (key == NULL) {
        ...
        if (out != NULL &&
            (pwcb == NULL || ossl_pw_set_ossl_passphrase_cb(&ctx->pwdata, pwcb, pwcbarg)))
...
#define DO_PRIVATE_KEY(impl, type, kind, output)                                                   \
    if ((selection & DO_PRIVATE_KEY_selection_mask) != 0)                                          \
        return key2any_encode(ctx, cout, key, impl##_evp_type, impl##_pem_type " PRIVATE KEY",     \
                              type##_check_key_type, key_to_##kind##_##output##_priv_bio, cb,      \
...
#define DO_PUBLIC_KEY(impl, type, kind, output)                                                    \
    if ((selection & DO_PUBLIC_KEY_selection_mask) != 0)                                           \
        return key2any_encode(ctx, cout, key, impl##_evp_type, impl##_pem_type " PUBLIC KEY",      \
                              type##_check_key_type, key_to_##kind##_##output##_pub_bio, cb,       \
...
#define DO_PARAMETERS(impl, type, kind, output)                                                    \
    if ((selection & DO_PARAMETERS_selection_mask) != 0)                                           \
        return key2any_encode(ctx, cout, key, impl##_evp_type, impl##_pem_type " PARAMETERS",      \
...
#define DO_type_specific_selection_mask                                                            \
    (DO_type_specific_keypair_selection_mask | DO_type_specific_params_selection_mask)
#define DO_type_specific(impl, type, output)                                                       \
    DO_type_specific_keypair(impl, type, output) DO_type_specific_params(impl, type, output)
...
#define MAKE_ENCODER(impl, type, kind, output)                                                     \
    ...
    static OSSL_FUNC_encoder_encode_fn impl##_to_##kind##_##output##_encode;                       \
                                                                                                   \
    static void *impl##_to_##kind##_##output##_import_object(void *vctx, int selection,            \
                                                             const OSSL_PARAM params[])            \
    {                                                                                              \
        KEY2ANY_CTX *ctx = vctx;                                                                   \
                                                                                                   \
        return ossl_prov_import_key(ossl_##impl##_keymgmt_functions, ctx->provctx, selection,      \
                                    params);                                                       \
    }                                                                                              \
    ...
    static int impl##_to_##kind##_##output##_encode(                                               \
        void *ctx, OSSL_CORE_BIO *cout, const void *key, const OSSL_PARAM key_abstract[],          \
