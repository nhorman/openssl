...
static int
pvk2key_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    struct pvk2key_ctx_st *ctx = vctx;
    const OSSL_PARAM *p;
    char *str = ctx->propq;
...
static int
pvk2key_does_selection(void *provctx, int selection)
{
    if (selection == 0)
        return 1;
...
static int
pvk2key_decode(void *vctx, OSSL_CORE_BIO *cin, int selection, OSSL_CALLBACK *data_cb,
...
static int
pvk2key_export_object(void *vctx, const void *reference, size_t reference_sz,
...
static void
rsa_adjust(void *key, struct pvk2key_ctx_st *ctx)
{
...
#define IMPLEMENT_MS(KEYTYPE, keytype)                                                             \
    static const struct keytype_desc_st pvk2##keytype##_desc = {                                   \
        EVP_PKEY_##KEYTYPE,        #KEYTYPE,         ossl_##keytype##_keymgmt_functions,           \
        keytype##_private_key_bio, keytype##_adjust, keytype##_free};                              \
    ...
    const OSSL_DISPATCH ossl_##pvk_to_##keytype##_decoder_functions[] = {                          \
        ...
        {OSSL_FUNC_DECODER_SETTABLE_CTX_PARAMS, (void (*)(void))pvk2key_settable_ctx_params},      \
