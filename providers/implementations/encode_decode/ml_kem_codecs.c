...
ML_KEM_KEY *
ossl_ml_kem_d2i_PUBKEY(const uint8_t *pubenc, int publen, int evp_type, PROV_CTX *provctx,
                       const char *propq)
{
    ...
    if (!ossl_ml_kem_parse_public_key(pubenc, (size_t)publen, ret)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING,
                       "errror parsing %s public key from input SPKI", v->algorithm_name);
...
ML_KEM_KEY *
ossl_ml_kem_d2i_PKCS8(const uint8_t *prvenc, int prvlen, int evp_type, PROV_CTX *provctx,
                      const char *propq)
{
    const ML_KEM_VINFO *v;
    const ML_COMMON_CODEC *codec;
    ...
    if (p8fmt == NULL || (p8fmt->seed_length > 0 && p8fmt->seed_length != ML_KEM_SEED_BYTES) ||
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_ML_KEM_NO_FORMAT,
                       "no matching enabled %s private key input formats", v->algorithm_name);
        goto end;
    }
...
ossl_ml_kem_i2d_prvkey(const ML_KEM_KEY *key, uint8_t **out, PROV_CTX *provctx)
{
    ...
    if (p8fmt == NULL || (p8fmt->seed_length > 0 && p8fmt->seed_length != ML_KEM_SEED_BYTES) ||
        ...
        (p8fmt->pub_length > 0 && p8fmt->pub_length != v->pubkey_bytes)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_ML_KEM_NO_FORMAT,
                       "no matching enabled %s private key output formats", v->algorithm_name);
        goto end;
    }
    len = p8fmt->p8_bytes;
    ...
    case 4:
        break;
    default:
        ERR_raise_data(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR, "error encoding %s private key",
                       v->algorithm_name);
        goto end;
    }
...
ossl_ml_kem_key_to_text(BIO *out, const ML_KEM_KEY *key, int selection)
{
    ...
    if (ret == 0)
        ERR_raise_data(ERR_LIB_PROV, PROV_R_MISSING_KEY, "no %s key material available",
                       type_label);
