...
void asm_aescbc_sha512_hmac(const uint8_t *csrc, uint8_t *cdst, uint64_t clen, uint8_t *dsrc,
                            uint8_t *ddst, uint64_t dlen, CIPH_DIGEST *arg);
void asm_sha512_hmac_aescbc_dec(const uint8_t *csrc, uint8_t *cdst, uint64_t clen, uint8_t *dsrc,
                                uint8_t *ddst, uint64_t dlen, CIPH_DIGEST *arg);
...
static int
hwaes_cbc_hmac_sha512_init_key(PROV_CIPHER_CTX *vctx, const unsigned char *key, size_t keylen)
{
    int ret;
...
static int
hwaes_cbc_hmac_sha512_etm(PROV_CIPHER_CTX *vctx, unsigned char *out, const unsigned char *in,
                          size_t len)
{
    PROV_AES_HMAC_SHA_ETM_CTX *ctx = (PROV_AES_HMAC_SHA_ETM_CTX *)vctx;
    CIPH_DIGEST arg = {0};
...
static int
hwaes_cbc_hmac_sha512_cipher(PROV_CIPHER_CTX *vctx, unsigned char *out, const unsigned char *in,
                             size_t len)
{
...
static void
hwaes_cbc_hmac_sha512_set_mac_key(void *vctx, const unsigned char *mackey, size_t len)
{
