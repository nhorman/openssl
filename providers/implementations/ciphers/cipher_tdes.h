...
#define IMPLEMENT_tdes_cipher(type, UCTYPE, lcmode, UCMODE, flags,             \
        ...
        static OSSL_FUNC_cipher_newctx_fn tdes_ ## type ## _ ## lcmode ## \
        _newctx;             \
        static void *tdes_ ## type ## _ ## lcmode ## _newctx(void *provctx)                    \
        {                                                                              \
            return ossl_tdes_newctx(provctx, EVP_CIPH_ ## UCMODE ## _MODE, \
                                    kbits, blkbits, \
                                    ivbits, flags,                                          \
                                    ossl_prov_cipher_hw_tdes_ ## type ## _ ## lcmode());          \
        }                                                                              \
        static OSSL_FUNC_cipher_get_params_fn tdes_ ## type ## _ ## lcmode ## \
        _get_params;     \
        static int tdes_ ## type ## _ ## lcmode ## _get_params( \
            OSSL_PARAM params[])            \
        {                                                                              \
...
#define PROV_CIPHER_HW_tdes_mode(type, mode)                                   \
        ...
        const PROV_CIPHER_HW *ossl_prov_cipher_hw_tdes_ ## type ## _ ## mode( \
            void)           \
        {                                                                              \
            return &type ## _ ## mode;                                                     \
        }
