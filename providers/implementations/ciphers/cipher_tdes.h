...
#define IMPLEMENT_tdes_cipher(type, UCTYPE, lcmode, UCMODE, flags, kbits, blkbits, ivbits, block)  \
    ...
    {                                                                                              \
        return ossl_tdes_get_params(params, EVP_CIPH_##UCMODE##_MODE, flags, kbits, blkbits,       \
                                    ivbits);                                                       \
    }                                                                                              \
...
void *ossl_tdes_newctx(void *provctx, int mode, size_t kbits, size_t blkbits, size_t ivbits,
                       uint64_t flags, const PROV_CIPHER_HW *hw);
int ossl_tdes_get_params(OSSL_PARAM params[], unsigned int md, uint64_t flags, size_t kbits,
...
#define PROV_CIPHER_HW_tdes_mode(type, mode)                                                       \
    static const PROV_CIPHER_HW type##_##mode = {ossl_cipher_hw_tdes_##type##_initkey,             \
                                                 ossl_cipher_hw_tdes_##mode,                       \
                                                 ossl_cipher_hw_tdes_copyctx};                     \
    const PROV_CIPHER_HW *ossl_prov_cipher_hw_tdes_##type##_##mode(void) { return &type##_##mode; }
...
int ossl_cipher_hw_tdes_cbc(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in,
                            size_t inl);
int ossl_cipher_hw_tdes_ecb(PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in,
                            size_t len);
