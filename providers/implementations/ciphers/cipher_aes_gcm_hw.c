...
static int
generic_aes_gcm_cipher_update(PROV_GCM_CTX *ctx, const unsigned char *in, size_t len,
                              unsigned char *out)
{
    if (ctx->enc) {
        if (ctx->ctr != NULL) {
            ...
            if (len >= AES_GCM_ENC_BYTES && AES_GCM_ASM(ctx)) {
                ...
                bulk = AES_gcm_encrypt(in + res, out + res, len - res, ctx->gcm.key, ctx->gcm.Yi.c,
                                       ctx->gcm.Xi.u);
        ...
        if (ctx->ctr != NULL) {
            ...
            if (len >= AES_GCM_DEC_BYTES && AES_GCM_ASM(ctx)) {
                ...
                bulk = AES_gcm_decrypt(in + res, out + res, len - res, ctx->gcm.key, ctx->gcm.Yi.c,
                                       ctx->gcm.Xi.u);
