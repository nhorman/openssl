...
static int
rc2_einit(void *ctx, const unsigned char *key, size_t keylen, const unsigned char *iv, size_t ivlen,
...
static int
rc2_dinit(void *ctx, const unsigned char *key, size_t keylen, const unsigned char *iv, size_t ivlen,
          const OSSL_PARAM params[])
{
...
rc2_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (p->data_type != OSSL_PARAM_OCTET_STRING || ctx->base.ivlen > sizeof(iv) ||
            ...
            ((size_t)ASN1_TYPE_get_int_octetstring(type, &num, iv, ctx->base.ivlen) !=
             ctx->base.ivlen) ||
    ...
    {                                                                                              \
        ...
        if (ctx != NULL) {                                                                         \
            ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE,     \
                                        flags, ossl_prov_cipher_hw_##alg##_##lcmode(kbits), NULL); \
            ctx->key_bits = kbits;                                                                 \
        }                                                                                          \
        return ctx;                                                                                \
    }                                                                                              \
