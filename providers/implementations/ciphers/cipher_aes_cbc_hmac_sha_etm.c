...
static int
aes_einit(void *ctx, const unsigned char *key, size_t keylen, const unsigned char *iv, size_t ivlen,
...
static int
aes_dinit(void *ctx, const unsigned char *key, size_t keylen, const unsigned char *iv, size_t ivlen,
          const OSSL_PARAM params[])
{
...
base_ctx_init(void *provctx, PROV_AES_HMAC_SHA_ETM_CTX *ctx,
              const PROV_CIPHER_HW_AES_HMAC_SHA_ETM *meths, size_t kbits, size_t blkbits,
              size_t ivbits, uint64_t flags)
{
    ossl_cipher_generic_initkey(&ctx->base, kbits, blkbits, ivbits, EVP_CIPH_CBC_MODE, flags,
                                &meths->base, provctx);
    ctx->hw = (PROV_CIPHER_HW_AES_HMAC_SHA_ETM *)ctx->base.hw;
}

static void *
aes_cbc_hmac_sha1_etm_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits,
                             uint64_t flags)
{
    ...
    if (ctx != NULL)
        base_ctx_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha1_etm(), kbits,
...
static void *
aes_cbc_hmac_sha256_etm_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits,
                               uint64_t flags)
{
    ...
    ctx = OPENSSL_zalloc(sizeof(*ctx));
    if (ctx != NULL)
        base_ctx_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha256_etm(), kbits,
                      blkbits, ivbits, flags);
    return ctx;
}
...
static void *
aes_cbc_hmac_sha512_etm_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits,
                               uint64_t flags)
{
    ...
    ctx = OPENSSL_zalloc(sizeof(*ctx));
    if (ctx != NULL)
        base_ctx_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha512_etm(), kbits,
                      blkbits, ivbits, flags);
    return ctx;
}
...
# define IMPLEMENT_CIPHER(nm, sub, kbits, blkbits, ivbits, flags)                                  \
     ...
     {                                                                                             \
         return ossl_cipher_generic_get_params(params, EVP_CIPH_CBC_MODE, flags, kbits, blkbits,   \
                                               ivbits);                                            \
     }                                                                                             \
