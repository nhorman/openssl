 ...
 */
static int
aes_xts_init(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
aes_xts_einit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
aes_xts_dinit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
              size_t ivlen, const OSSL_PARAM params[])
{
#ifdef AES_XTS_S390X
...
static void *
aes_xts_newctx(void *provctx, unsigned int mode, uint64_t flags, size_t kbits, size_t blkbits,
               size_t ivbits)
{
               ...
               const unsigned char *in, size_t inl)
{
    ...
    if (!ossl_prov_is_running() || ctx->xts.key1 == NULL || ctx->xts.key2 == NULL ||
        !ctx->base.iv_set || out == NULL || in == NULL || inl < AES_BLOCK_SIZE)
        return 0;
...
static int
aes_xts_stream_update(void *vctx, unsigned char *out, size_t *outl, size_t outsize,
                      const unsigned char *in, size_t inl)
{
    PROV_AES_XTS_CTX *ctx = (PROV_AES_XTS_CTX *)vctx;
...
static int
aes_xts_stream_final(void *vctx, unsigned char *out, size_t *outl, size_t outsize)
{
    if (!ossl_prov_is_running())
        return 0;
    *outl = 0;
