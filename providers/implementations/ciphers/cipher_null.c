...
static int
null_einit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
null_dinit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
           size_t ivlen, const OSSL_PARAM params[])
{
    if (!ossl_prov_is_running())
        return 0;
...
static int
null_cipher(void *vctx, unsigned char *out, size_t *outl, size_t outsize, const unsigned char *in,
            size_t inl)
{
    PROV_CIPHER_NULL_CTX *ctx = (PROV_CIPHER_NULL_CTX *)vctx;
