 ...
 */
static int
sm4_xts_init(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
sm4_xts_einit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
sm4_xts_dinit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
              size_t ivlen, const OSSL_PARAM params[])
{
...
static void *
sm4_xts_newctx(void *provctx, unsigned int mode, uint64_t flags, size_t kbits, size_t blkbits,
               size_t ivbits)
{
               ...
               const unsigned char *in, size_t inl)
{
    ...
    if (!ossl_prov_is_running() || ctx->xts.key1 == NULL || ctx->xts.key2 == NULL ||
        !ctx->base.iv_set || out == NULL || in == NULL || inl < SM4_BLOCK_SIZE)
        return 0;
...
static int
sm4_xts_stream_update(void *vctx, unsigned char *out, size_t *outl, size_t outsize,
                      const unsigned char *in, size_t inl)
{
    PROV_SM4_XTS_CTX *ctx = (PROV_SM4_XTS_CTX *)vctx;
...
static int
sm4_xts_stream_final(void *vctx, unsigned char *out, size_t *outl, size_t outsize)
{
    if (!ossl_prov_is_running())
        return 0;
    *outl = 0;
...
#define IMPLEMENT_cipher(lcmode, UCMODE, kbits, flags)                                             \
    ...
    {                                                                                              \
        return ossl_cipher_generic_get_params(params, EVP_CIPH_##UCMODE##_MODE, flags, 2 * kbits,  \
