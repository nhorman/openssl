...
PROV_CIPHER_FUNC(int, ocb_cipher,
                 (PROV_AES_OCB_CTX * ctx, const unsigned char *in, unsigned char *out,
                  size_t nextblock));
...
static ossl_inline int
aes_generic_ocb_setaad(PROV_AES_OCB_CTX *ctx, const unsigned char *aad, size_t alen)
{
...
static ossl_inline int
aes_generic_ocb_cipher(PROV_AES_OCB_CTX *ctx, const unsigned char *in, unsigned char *out,
                       size_t len)
{
    if (ctx->base.enc) {
 ...
 */
static int
aes_ocb_init(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
aes_ocb_einit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
...
static int
aes_ocb_dinit(void *vctx, const unsigned char *key, size_t keylen, const unsigned char *iv,
              size_t ivlen, const OSSL_PARAM params[])
{
 ...
 */
static int
aes_ocb_block_update_internal(PROV_AES_OCB_CTX *ctx, unsigned char *buf, size_t *bufsz,
...
static int
cipher_updateaad(PROV_AES_OCB_CTX *ctx, const unsigned char *in, unsigned char *out, size_t len)
{
...
static int
aes_ocb_block_update(void *vctx, unsigned char *out, size_t *outl, size_t outsize,
...
static int
aes_ocb_block_final(void *vctx, unsigned char *out, size_t *outl, size_t outsize)
{
    PROV_AES_OCB_CTX *ctx = (PROV_AES_OCB_CTX *)vctx;
...
static void *
aes_ocb_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits, unsigned int mode,
               uint64_t flags)
{
...
#define IMPLEMENT_cipher(mode, UCMODE, flags, kbits, blkbits, ivbits)                              \
    ...
    {                                                                                              \
        return ossl_cipher_generic_get_params(params, EVP_CIPH_##UCMODE##_MODE, flags, kbits,      \
                                              blkbits, ivbits);                                    \
    }                                                                                              \
    static OSSL_FUNC_cipher_newctx_fn aes_##kbits##_##mode##_newctx;                               \
    static void *aes_##kbits##_##mode##_newctx(void *provctx)                                      \
    {                                                                                              \
        return aes_##mode##_newctx(provctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE,      \
                                   flags);                                                         \
    }                                                                                              \
