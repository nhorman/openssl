...
int common_get_params(void *key, OSSL_PARAM params[], int sm2)
{
    ...
    if ((p =
             OSSL_PARAM_locate(params,
                               OSSL_PKEY_PARAM_SECURITY_BITS)) != NULL) {
        int ecbits, sec_bits;
    ...
    if (!sm2) {
        if ((p =
                 OSSL_PARAM_locate(params,
                                   OSSL_PKEY_PARAM_DEFAULT_DIGEST)) != NULL
...
int ec_validate(const void *keydata, int selection, int checktype)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
        ...
        if ((flags & EC_FLAG_CHECK_NAMED_GROUP) != 0)
            ok = ok && EC_GROUP_check_named_curve(EC_KEY_get0_group(
                                                      eck),
                                                  (flags &
                                                   EC_FLAG_CHECK_NAMED_GROUP_NIST) != 0,
                                                  ctx) > 0;
        else
            ok = ok && EC_GROUP_check(EC_KEY_get0_group(eck), ctx);
    }
