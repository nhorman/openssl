...
static int
ml_kem_pairwise_test(const ML_KEM_KEY *key, int key_flags)
{
#ifdef FIPS_MODULE
    ...
    OSSL_SELF_TEST_onbegin(st, OSSL_SELF_TEST_TYPE_PCT, OSSL_SELF_TEST_DESC_PCT_ML_KEM);
#endif /* FIPS_MODULE */
    ...
    if (key_flags & ML_KEM_KEY_RANDOM_PCT) {
        ...
        operation_result = ossl_ml_kem_encap_seed(ctext, v->ctext_bytes, secret, sizeof(secret),
    ...
    operation_result = ossl_ml_kem_decap(out, sizeof(out), ctext, v->ctext_bytes, key);
...
err:
    ...
    if (ret == 0) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                       "public part of %s private key fails to match private", v->algorithm_name);
    }
#endif
    OPENSSL_free(ctext);
    return ret;
...
ML_KEM_KEY *
ossl_prov_ml_kem_new(PROV_CTX *ctx, const char *propq, int evp_type)
{
    ML_KEM_KEY *key;
    ...
    if ((key = ossl_ml_kem_key_new(PROV_LIBCTX_OF(ctx), propq, evp_type)) != NULL) {
        const char *pct_type =
            ossl_prov_ctx_get_param(ctx, OSSL_PKEY_PARAM_ML_KEM_IMPORT_PCT_TYPE, "random");

        if (ossl_prov_ctx_get_bool_param(ctx, OSSL_PKEY_PARAM_ML_KEM_RETAIN_SEED, 1))
            key->prov_flags |= ML_KEM_KEY_RETAIN_SEED;
        else
            key->prov_flags &= ~ML_KEM_KEY_RETAIN_SEED;
        if (ossl_prov_ctx_get_bool_param(ctx, OSSL_PKEY_PARAM_ML_KEM_PREFER_SEED, 1))
...
static int
ml_kem_has(const void *vkey, int selection)
{
...
static int
ml_kem_match(const void *vkey1, const void *vkey2, int selection)
{
...
static int
ml_kem_validate(const void *vkey, int selection, int check_type)
{
    const ML_KEM_KEY *key = vkey;
...
static int
ml_kem_export(void *vkey, int selection, OSSL_CALLBACK *param_cb, void *cbarg)
{
    ML_KEM_KEY *key = vkey;
    ...
    if (!ossl_ml_kem_have_pubkey(key)) {
        ...
        if (pubenc == NULL || !ossl_ml_kem_encode_public_key(pubenc, v->pubkey_bytes, key))
            goto err;
    }
    ...
    if (seedenc != NULL && !ossl_param_build_set_octet_string(
                               tmpl, params, OSSL_PKEY_PARAM_ML_KEM_SEED, seedenc, seedlen))
        goto err;

    /* The private key in the FIPS 203 |dk| format, when requested. */
    if (prvenc != NULL &&
        !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY, prvenc, prvlen))
        goto err;

    /* The public key on request; it is always available when either is */
    if (pubenc != NULL && !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_PUB_KEY,
...
ml_kem_imexport_types(int selection)
{
...
static int
check_seed(const uint8_t *seed, const uint8_t *prvenc, ML_KEM_KEY *key)
{
    size_t zlen = ML_KEM_RANDOM_BYTES;

    if (memcmp(seed + ML_KEM_SEED_BYTES - zlen, prvenc + key->vinfo->prvkey_bytes - zlen, zlen) ==
        0)
        return 1;
    ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
...
static int
check_prvenc(const uint8_t *prvenc, ML_KEM_KEY *key)
{
    ...
    if (buf != NULL)
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                       "explicit %s private key does not match seed", key->vinfo->algorithm_name);
    ossl_ml_kem_key_reset(key);
    return 0;
}

static int
ml_kem_key_fromdata(ML_KEM_KEY *key, const OSSL_PARAM params[], int include_private)
{
    ...
    if (include_private) {
        ...
        if (p != NULL && OSSL_PARAM_get_octet_string_ptr(p, &seedenc, &seedlen) != 1)
            return 0;
    ...
    if (publen > 0 && prvlen > 0) {
        ...
        if (memcmp(pubenc, (unsigned char *)prvenc + puboff, publen) != 0) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                           "explicit %s public key does not match private", v->algorithm_name);
            return 0;
        }
    }

    if (seedlen != 0 && (prvlen == 0 || (key->prov_flags & ML_KEM_KEY_PREFER_SEED))) {
        if (prvlen != 0 && !check_seed(seedenc, prvenc, key))
            return 0;
        if (!ossl_ml_kem_set_seed(seedenc, seedlen, key) || !ossl_ml_kem_genkey(NULL, 0, key))
            return 0;
...
static int
ml_kem_import(void *vkey, int selection, const OSSL_PARAM params[])
{
    ML_KEM_KEY *key = vkey;
    int include_private;
    int res;
    ...
    if (res > 0 && include_private && !ml_kem_pairwise_test(key, key->prov_flags)) {
#ifdef FIPS_MODULE
...
static const OSSL_PARAM *
ml_kem_gettable_params(void *provctx)
{
...
#ifndef FIPS_MODULE
void *
ml_kem_load(const void *reference, size_t reference_sz)
{
    ML_KEM_KEY *key = NULL;
    ...
    if (ossl_prov_is_running() && reference_sz == sizeof(key)) {
        ...
        *(ML_KEM_KEY **)reference = NULL;
        if (encoded_dk != NULL && ossl_ml_kem_encode_seed(seed, sizeof(seed), key) &&
        ...
        if (ossl_ml_kem_have_seed(key) &&
            ...
            if (!ossl_ml_kem_parse_private_key(encoded_dk, key->vinfo->prvkey_bytes, key)) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY, "error parsing %s private key",
                               key->vinfo->algorithm_name);
                goto err;
            }
 ...
 */
static int
ml_kem_get_params(void *vkey, OSSL_PARAM params[])
{
    ML_KEM_KEY *key = vkey;
    const ML_KEM_VINFO *v = ossl_ml_kem_key_vinfo(key);
    OSSL_PARAM *p;
    const char *pubparams[] = {OSSL_PKEY_PARAM_PUB_KEY, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY};
    int i;
...
static const OSSL_PARAM *
ml_kem_settable_params(void *provctx)
{
    static const OSSL_PARAM arr[] = {
...
static int
ml_kem_set_params(void *vkey, const OSSL_PARAM params[])
{
    ML_KEM_KEY *key = vkey;
    const OSSL_PARAM *p;
    ...
    if (ossl_ml_kem_have_pubkey(key)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE,
...
static int
ml_kem_gen_set_params(void *vgctx, const OSSL_PARAM params[])
{
...
static void *
ml_kem_gen_init(void *provctx, int selection, const OSSL_PARAM params[], int evp_type)
{
...
static const OSSL_PARAM *
ml_kem_gen_settable_params(ossl_unused void *vgctx, ossl_unused void *provctx)
{
    static OSSL_PARAM settable[] = {OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_ML_KEM_SEED, NULL, 0),
                                    OSSL_PARAM_END};
    return settable;
}

static void *
ml_kem_gen(void *vgctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
    PROV_ML_KEM_GEN_CTX *gctx = vgctx;
    ML_KEM_KEY *key;
    uint8_t *nopub = NULL;
...
static void *
ml_kem_dup(const void *vkey, int selection)
{
    const ML_KEM_KEY *key = vkey;
...
#define DECLARE_VARIANT(bits)                                                                      \
    ...
    static void *ml_kem_##bits##_gen_init(void *provctx, int selection, const OSSL_PARAM params[]) \
    {                                                                                              \
        return ml_kem_gen_init(provctx, selection, params, EVP_PKEY_ML_KEM_##bits);                \
    }                                                                                              \
