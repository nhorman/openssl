...
static int
dsa_key_todata(DSA *dsa, OSSL_PARAM_BLD *bld, OSSL_PARAM params[], int include_private)
{
...
dsa_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL &&
        (p->data_type != OSSL_PARAM_OCTET_STRING || !dsa_set_gen_seed(gctx, p->data, p->data_size)))
        return 0;
...
{
    static const OSSL_PARAM dsa_gen_gettable_params_table[] = {OSSL_FIPS_IND_GETTABLE_CTX_PARAM()
                                                                   OSSL_PARAM_END};
...
dsa_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
    ...
    if (gctx->gen_type == DSA_PARAMGEN_TYPE_FIPS_DEFAULT)
        gctx->gen_type =
            (gctx->pbits >= 2048 ? DSA_PARAMGEN_TYPE_FIPS_186_4 : DSA_PARAMGEN_TYPE_FIPS_186_2);
    ...
    if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
        ...
        if (ossl_dsa_generate_ffc_parameters(dsa, gctx->gen_type, gctx->pbits, gctx->qbits,
                                             gencb) <= 0)
            goto end;
    }
