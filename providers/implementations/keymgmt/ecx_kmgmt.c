...
ecx_match(const void *keydata1, const void *keydata2, int selection)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
        ...
        if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
            ...
            if (pa != NULL && pb != NULL) {
                ok =
                    ok && key1->type == key2->type && pal == pbl && CRYPTO_memcmp(pa, pb, pal) == 0;
                key_checked = 1;
            }
        }
...
static int
key_to_params(ECX_KEY *key, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[], int include_private)
{
    if (key == NULL)
        return 0;

    if (!ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_PUB_KEY, key->pubkey,
                                           key->keylen))
        return 0;

    if (include_private && key->privkey != NULL &&
        !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY, key->privkey,
                                           key->keylen))
        return 0;
...
static int
ecx_get_params(void *key, OSSL_PARAM params[], int bits, int secbits, int size)
{
    ECX_KEY *ecx = key;
    OSSL_PARAM *p;
...
static int
ed_get_params(void *key, OSSL_PARAM params[])
{
    OSSL_PARAM *p;
...
{
    return ecx_get_params(key, params, ED448_BITS, ED448_SECURITY_BITS, ED448_SIGSIZE) &&
           ed_get_params(key, params);
}
...
static void *
ecx_gen_init(void *provctx, int selection, const OSSL_PARAM params[], ECX_KEY_TYPE type,
             const char *algdesc)
{
...
ecd_fips140_pairwise_test(const ECX_KEY *ecx, int type, int self_test)
{
    ...
    if (is_ed25519)
        operation_result = ossl_ed25519_sign(sig, msg, msg_len, ecx->pubkey, ecx->privkey, 0, 0, 0,
                                             NULL, 0, ecx->libctx, ecx->propq);
    else
        operation_result = ossl_ed448_sign(ecx->libctx, sig, msg, msg_len, ecx->pubkey,
    ...
    if (is_ed25519)
        operation_result = ossl_ed25519_verify(msg, msg_len, sig, ecx->pubkey, 0, 0, 0, NULL, 0,
                                               ecx->libctx, ecx->propq);
    else
        operation_result =
            ossl_ed448_verify(ecx->libctx, msg, msg_len, sig, ecx->pubkey, NULL, 0, 0, ecx->propq);
    if (operation_result != 1)
        goto err;
...
#define MAKE_KEYMGMT_FUNCTIONS(alg)                                                                \
    const OSSL_DISPATCH ossl_##alg##_keymgmt_functions[] = {                                       \
        ...
        {OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS, (void (*)(void))ecx_gen_settable_params},          \
