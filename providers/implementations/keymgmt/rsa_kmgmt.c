...
static int
pss_params_fromdata(RSA_PSS_PARAMS_30 *pss_params, int *defaults_set, const OSSL_PARAM params[],
...
rsa_import(void *keydata, int selection, const OSSL_PARAM params[])
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_OTHER_PARAMETERS) != 0)
        ok = ok && pss_params_fromdata(ossl_rsa_get0_pss_params_30(rsa), &pss_defaults_set, params,
...
static int
rsa_export(void *keydata, int selection, OSSL_CALLBACK *param_callback, void *cbarg)
{
    RSA *rsa = keydata;
...
rsa_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
    ...
    if (gctx->rsa_type == RSA_FLAG_TYPE_RSASSAPSS &&
        !pss_params_fromdata(&gctx->pss_params, &gctx->pss_defaults_set, params, gctx->rsa_type,
                             gctx->libctx))
        return 0;
#if defined(FIPS_MODULE) && !defined(OPENSSL_NO_ACVP_TESTS)
...
rsa_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
    ...
    if (!RSA_generate_multi_prime_key(rsa_tmp, (int)gctx->nbits, (int)gctx->primes, gctx->pub_exp,
                                      gencb))
        goto err;
