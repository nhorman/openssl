 ...
 */
static int
SSKDF_hash_kdm(const EVP_MD *kdf_md, const unsigned char *z, size_t z_len,
               const unsigned char *info, size_t info_len, unsigned int append_ctr,
...
static int
kmac_init(EVP_MAC_CTX *ctx, const unsigned char *custom, size_t custom_len, size_t kmac_out_len,
          size_t derived_key_len, unsigned char **out)
{
    ...
    params[0] =
        OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_CUSTOM, (void *)custom, custom_len);
    ...
    else if (!(kmac_out_len == derived_key_len || kmac_out_len == 20 || kmac_out_len == 28 ||
               kmac_out_len == 32 || kmac_out_len == 48 || kmac_out_len == 64))
        return 0;
 ...
 */
static int
SSKDF_mac_kdm(EVP_MAC_CTX *ctx_init, const unsigned char *kmac_custom, size_t kmac_custom_len,
              size_t kmac_out_len, const unsigned char *salt, size_t salt_len,
              const unsigned char *z, size_t z_len, const unsigned char *info, size_t info_len,
              unsigned char *derived_key, size_t derived_key_len)
{
    int ret = 0;
...
#ifdef FIPS_MODULE
static int
fips_sskdf_key_check_passed(KDF_SSKDF *ctx)
{
...
static int
sskdf_derive(void *vctx, unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    KDF_SSKDF *ctx = (KDF_SSKDF *)vctx;
    const EVP_MD *md;
    ...
    if (ctx->macctx != NULL) {
        ...
        ret = SSKDF_mac_kdm(ctx->macctx, custom, custom_len, ctx->out_len, ctx->salt, ctx->salt_len,
        ...
        return SSKDF_hash_kdm(md, ctx->secret, ctx->secret_len, ctx->info, ctx->info_len, 0, key,
                              keylen);
    }
}
...
#ifdef FIPS_MODULE
static int
fips_x963kdf_digest_check_passed(KDF_SSKDF *ctx, const EVP_MD *md)
{
    OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
    /*
...
static int
fips_x963kdf_key_check_passed(KDF_SSKDF *ctx)
{
    OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
    int key_approved = ossl_kdf_check_key_size(ctx->secret_len);

    if (!key_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE1, libctx, "X963KDF",
                                         "Key size", ossl_fips_config_x963kdf_key_check)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY_LENGTH);
            return 0;
        }
    }
...
static int
x963kdf_derive(void *vctx, unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    KDF_SSKDF *ctx = (KDF_SSKDF *)vctx;
    const EVP_MD *md;
    ...
    return SSKDF_hash_kdm(md, ctx->secret, ctx->secret_len, ctx->info, ctx->info_len, 1, key,
                          keylen);
}

static int
sskdf_common_set_ctx_params(KDF_SSKDF *ctx, const OSSL_PARAM params[])
{
    const OSSL_PARAM *p;
    OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
    const EVP_MD *md = NULL;
...
static int
sskdf_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    KDF_SSKDF *ctx = (KDF_SSKDF *)vctx;
...
sskdf_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx)
{
...
static int
sskdf_common_get_ctx_params(KDF_SSKDF *ctx, OSSL_PARAM params[])
{
    OSSL_PARAM *p;
...
static int
sskdf_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    KDF_SSKDF *ctx = (KDF_SSKDF *)vctx;
...
static const OSSL_PARAM *
sskdf_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx)
{
...
static int
x963kdf_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    KDF_SSKDF *ctx = (KDF_SSKDF *)vctx;
...
static int
x963kdf_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    KDF_SSKDF *ctx = (KDF_SSKDF *)vctx;
