...
static int pbkdf2_derive(KDF_PBKDF2 *ctx, const char *pass, size_t passlen,
                         ...
                         const EVP_MD *digest, unsigned char *key, size_t keylen,
                         int lower_bound_checks);

static void kdf_pbkdf2_init(KDF_PBKDF2 *ctx);

static void *
kdf_pbkdf2_new_no_init(void *provctx)
{
    KDF_PBKDF2 *ctx;
...
static void
kdf_pbkdf2_cleanup(KDF_PBKDF2 *ctx)
{
...
static void
kdf_pbkdf2_init(KDF_PBKDF2 *ctx)
{
...
static int
pbkdf2_set_membuf(unsigned char **buffer, size_t *buflen, const OSSL_PARAM *p)
{
...
static int
pbkdf2_lower_bound_check_passed(int saltlen, uint64_t iter, size_t keylen, int *error,
                                const char **desc)
{
...
#ifdef FIPS_MODULE
static int
fips_lower_bound_check_passed(KDF_PBKDF2 *ctx, int saltlen, uint64_t iter, size_t keylen)
{
...
static int
lower_bound_check_passed(KDF_PBKDF2 *ctx, int saltlen, uint64_t iter, size_t keylen,
                         int lower_bound_checks)
{
#ifdef FIPS_MODULE
...
static int
kdf_pbkdf2_derive(void *vctx, unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    KDF_PBKDF2 *ctx = (KDF_PBKDF2 *)vctx;
    const EVP_MD *md;
    ...
    return pbkdf2_derive(ctx, (char *)ctx->pass, ctx->pass_len, ctx->salt, ctx->salt_len, ctx->iter,
                         md, key, keylen, ctx->lower_bound_checks);
}

static int
kdf_pbkdf2_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    const OSSL_PARAM *p;
    KDF_PBKDF2 *ctx = vctx;
    OSSL_LIB_CTX *provctx = PROV_LIBCTX_OF(ctx->provctx);
...
static int
kdf_pbkdf2_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    OSSL_PARAM *p;
 ...
 */
static int
pbkdf2_derive(KDF_PBKDF2 *ctx, const char *pass, size_t passlen, const unsigned char *salt,
              int saltlen, uint64_t iter, const EVP_MD *digest, unsigned char *key, size_t keylen,
              int lower_bound_checks)
{
    int ret = 0;
