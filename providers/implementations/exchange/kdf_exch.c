...
#define KDF_NEWCTX(funcname, kdfname)                                                              \
    static void *kdf_##funcname##_newctx(void *provctx) { return kdf_newctx(kdfname, provctx); }
...
static int
kdf_derive(void *vpkdfctx, unsigned char *secret, size_t *secretlen, size_t outlen)
{
    PROV_KDF_CTX *pkdfctx = (PROV_KDF_CTX *)vpkdfctx;
    size_t kdfsize;
    int ret;
...
kdf_settable_ctx_params(ossl_unused void *vpkdfctx, void *provctx, const char *kdfname)
{
...
static const OSSL_PARAM *
kdf_gettable_ctx_params(ossl_unused void *vpkdfctx, void *provctx, const char *kdfname)
{
    EVP_KDF *kdf = EVP_KDF_fetch(PROV_LIBCTX_OF(provctx), kdfname, NULL);
    const OSSL_PARAM *params;
