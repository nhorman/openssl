...
#ifdef FIPS_MODULE
static int
dh_check_key(PROV_DH_CTX *ctx)
{
    int key_approved = ossl_dh_check_key(ctx->dh);

    if (!key_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE0, ctx->libctx, "DH Init",
                                         "DH Key", ossl_fips_config_securitycheck_enabled)) {
...
static int
digest_check(PROV_DH_CTX *ctx, const EVP_MD *md)
{
    return ossl_fips_ind_digest_exch_check(OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE1,
                                           ctx->libctx, md, "DH Set Ctx");
}
#endif

static int
dh_init(void *vpdhctx, void *vdh, const OSSL_PARAM params[])
{
...
static int
dh_match_params(DH *priv, DH *peer)
{
    int ret;
...
static int
dh_set_peer(void *vpdhctx, void *vdh)
{
    PROV_DH_CTX *pdhctx = (PROV_DH_CTX *)vpdhctx;
...
static int
dh_plain_derive(void *vpdhctx, unsigned char *secret, size_t *secretlen, size_t outlen,
                unsigned int pad)
{
    PROV_DH_CTX *pdhctx = (PROV_DH_CTX *)vpdhctx;
    int ret;
...
static int
dh_X9_42_kdf_derive(void *vpdhctx, unsigned char *secret, size_t *secretlen, size_t outlen)
{
    PROV_DH_CTX *pdhctx = (PROV_DH_CTX *)vpdhctx;
    ...
    if (pdhctx->kdf_type == PROV_DH_KDF_X9_42_ASN1) {
        if (!ossl_dh_kdf_X9_42_asn1(secret, pdhctx->kdf_outlen, stmp, stmplen, pdhctx->kdf_cekalg,
                                    pdhctx->kdf_ukm, pdhctx->kdf_ukmlen, pdhctx->kdf_md,
                                    pdhctx->libctx, NULL))
            goto err;
    }
    *secretlen = pdhctx->kdf_outlen;
...
static int
dh_derive(void *vpdhctx, unsigned char *secret, size_t *psecretlen, size_t outlen)
{
    PROV_DH_CTX *pdhctx = (PROV_DH_CTX *)vpdhctx;
...
static int
dh_set_ctx_params(void *vpdhctx, const OSSL_PARAM params[])
{
    PROV_DH_CTX *pdhctx = (PROV_DH_CTX *)vpdhctx;
    const OSSL_PARAM *p;
    unsigned int pad;
...
dh_settable_ctx_params(ossl_unused void *vpdhctx, ossl_unused void *provctx)
{
...
static const OSSL_PARAM *
dh_gettable_ctx_params(ossl_unused void *vpdhctx, ossl_unused void *provctx)
{
    return known_gettable_ctx_params;
}

static int
dh_get_ctx_params(void *vpdhctx, OSSL_PARAM params[])
{
    PROV_DH_CTX *pdhctx = (PROV_DH_CTX *)vpdhctx;
    OSSL_PARAM *p;
