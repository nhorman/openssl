...
static void
winstore_win_reset(struct winstore_ctx_st *ctx)
{
    if (ctx->win_ctx != NULL) {
...
static void
winstore_win_advance(struct winstore_ctx_st *ctx)
{
    ...
    ctx->win_ctx =
        (name.cbData == 0
             ? NULL
             : CertFindCertificateInStore(ctx->win_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
...
static void *
winstore_open(void *provctx, const char *uri)
{
...
static void *
winstore_attach(void *provctx, OSSL_CORE_BIO *cin)
{
...
static int
winstore_set_ctx_params(void *loaderctx, const OSSL_PARAM params[])
{
...
static int
load_construct(OSSL_DECODER_INSTANCE *decoder_inst, const OSSL_PARAM *params, void *construct_data)
{
...
static void
load_cleanup(void *construct_data)
{
    /* No-op. */
}

static int
setup_decoder(struct winstore_ctx_st *ctx)
{
...
static int
winstore_load_using(struct winstore_ctx_st *ctx, OSSL_CALLBACK *object_cb, void *object_cbarg,
                    OSSL_PASSPHRASE_CALLBACK *pw_cb, void *pw_cbarg, const void *der,
                    size_t der_len)
{
...
static int
winstore_load(void *loaderctx, OSSL_CALLBACK *object_cb, void *object_cbarg,
