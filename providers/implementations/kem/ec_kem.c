...
static int
eckey_check(const EC_KEY *ec, int requires_privatekey)
{
    int rv = 0;
    BN_CTX *bnctx = NULL;
    BIGNUM *rem = NULL;
...
static const char *
ec_curvename_get0(const EC_KEY *ec)
{
    const EC_GROUP *group = EC_KEY_get0_group(ec);
 ...
 */
static int
recipient_key_set(PROV_EC_CTX *ctx, EC_KEY *ec)
{
 ...
 */
static int
sender_authkey_set(PROV_EC_CTX *ctx, EC_KEY *ec)
{
 ...
 */
static EC_KEY *
eckey_frompub(EC_KEY *in, const unsigned char *pubbuf, size_t pubbuflen)
{
    EC_KEY *key;
 ...
 */
static int
ecpubkey_todata(const EC_KEY *ec, unsigned char *out, size_t *outlen, size_t maxoutlen)
{
    const EC_POINT *pub;
...
static int
ossl_ec_match_params(const EC_KEY *key1, const EC_KEY *key2)
{
    int ret;
    BN_CTX *ctx = NULL;
...
static int
eckem_init(void *vctx, int operation, void *vec, void *vauth, const OSSL_PARAM params[])
{
    int rv;
...
static int
eckem_encapsulate_init(void *vctx, void *vec, const OSSL_PARAM params[])
{
    return eckem_init(vctx, EVP_PKEY_OP_ENCAPSULATE, vec, NULL, params);
}

static int
eckem_decapsulate_init(void *vctx, void *vec, const OSSL_PARAM params[])
{
    return eckem_init(vctx, EVP_PKEY_OP_DECAPSULATE, vec, NULL, params);
}

static int
eckem_auth_encapsulate_init(void *vctx, void *vecx, void *vauthpriv, const OSSL_PARAM params[])
{
    return eckem_init(vctx, EVP_PKEY_OP_ENCAPSULATE, vecx, vauthpriv, params);
}

static int
eckem_auth_decapsulate_init(void *vctx, void *vecx, void *vauthpub, const OSSL_PARAM params[])
{
    return eckem_init(vctx, EVP_PKEY_OP_DECAPSULATE, vecx, vauthpub, params);
}

static int
eckem_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    PROV_EC_CTX *ctx = (PROV_EC_CTX *)vctx;
    const OSSL_PARAM *p;
    int mode;
...
eckem_settable_ctx_params(ossl_unused void *vctx, ossl_unused void *provctx)
{
 ...
 */
static int
dhkem_extract_and_expand(EVP_KDF_CTX *kctx, unsigned char *okm, size_t okmlen, uint16_t kemid,
                         ...
                         size_t kemctxlen)
{
    ...
    ret = ossl_hpke_labeled_extract(kctx, prk, prklen, NULL, 0, LABEL_KEM, suiteid, sizeof(suiteid),
          ...
          ossl_hpke_labeled_expand(kctx, okm, okmlen, prk, prklen, LABEL_KEM, suiteid,
                                   sizeof(suiteid), OSSL_DHKEM_LABEL_SHARED_SECRET, kemctx,
                                   kemctxlen);
 ...
 */
int
ossl_ec_dhkem_derive_private(EC_KEY *ec, BIGNUM *priv, const unsigned char *ikm, size_t ikmlen)
{
    int ret = 0;
    ...
    if (ikmlen < info->Nsk) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_INPUT_LENGTH,
                       "ikm length is :%zu, should be at least %zu", ikmlen, info->Nsk);
        goto err;
    }
    ...
    if (!ossl_hpke_labeled_extract(kdfctx, prk, info->Nsecret, NULL, 0, LABEL_KEM, suiteid,
                                   sizeof(suiteid), OSSL_DHKEM_LABEL_DKP_PRK, ikm, ikmlen))
        goto err;

    order = EC_GROUP_get0_order(EC_KEY_get0_group(ec));
    do {
        if (!ossl_hpke_labeled_expand(kdfctx, privbuf, info->Nsk, prk, info->Nsecret, LABEL_KEM,
                                      suiteid, sizeof(suiteid), OSSL_DHKEM_LABEL_CANDIDATE,
                                      &counter, 1))
            goto err;
 ...
 */
static EC_KEY *
derivekey(PROV_EC_CTX *ctx, const unsigned char *ikm, size_t ikmlen)
{
    int ret = 0;
    EC_KEY *key;
 ...
 */
static int
check_publickey(const EC_KEY *pub)
{
    int ret = 0;
 ...
 */
static int
generate_ecdhkm(const EC_KEY *sender, const EC_KEY *peer, unsigned char *out, size_t maxout,
 ...
 */
static int
derive_secret(PROV_EC_CTX *ctx, unsigned char *secret, const EC_KEY *privkey1,
              ...
              const unsigned char *sender_pub, const unsigned char *recipient_pub)
{
    ...
    if (auth) {
        ...
        if (!ecpubkey_todata(ctx->sender_authkey, sender_authpub, &sender_authpublen,
                             sizeof(sender_authpub)))
            goto err;
        if (sender_authpublen != encodedpublen) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY, "Invalid sender auth public key");
            goto err;
        }
        if (!generate_ecdhkm(privkey2, peerkey2, dhkm + dhkmlen, sizeof(dhkm) - dhkmlen,
                             encodedprivlen))
            goto err;
        dhkmlen += encodedprivlen;
    ...
    if (kdfctx == NULL)
        goto err;
    if (!dhkem_extract_and_expand(kdfctx, secret, info->Nsecret, info->kem_id, dhkm, dhkmlen,
                                  kemctx, kemctxlen))
        goto err;
    ret = 1;
err:
 ...
 */
static int
dhkem_encap(PROV_EC_CTX *ctx, unsigned char *enc, size_t *enclen, unsigned char *secret,
            size_t *secretlen)
{
    int ret = 0;
    ...
    if (!derive_secret(ctx, secret, sender_ephemkey, ctx->recipient_key, ctx->sender_authkey,
                       ctx->recipient_key, sender_pub, recipient_pub))
        goto err;
 ...
 */
static int
dhkem_decap(PROV_EC_CTX *ctx, unsigned char *secret, size_t *secretlen, const unsigned char *enc,
            size_t enclen)
{
    int ret = 0;
    ...
    if (!derive_secret(ctx, secret, ctx->recipient_key, sender_ephempubkey, ctx->recipient_key,
                       ctx->sender_authkey, enc, recipient_pub))
        goto err;
    *secretlen = info->Nsecret;
    ret = 1;
err:
...
static int
eckem_encapsulate(void *vctx, unsigned char *out, size_t *outlen, unsigned char *secret,
                  size_t *secretlen)
{
    PROV_EC_CTX *ctx = (PROV_EC_CTX *)vctx;
...
static int
eckem_decapsulate(void *vctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                  size_t inlen)
{
    PROV_EC_CTX *ctx = (PROV_EC_CTX *)vctx;
