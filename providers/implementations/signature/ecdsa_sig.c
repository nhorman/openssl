...
static void *
ecdsa_newctx(void *provctx, const char *propq)
{
    PROV_ECDSA_CTX *ctx;
...
static int
ecdsa_setup_md(PROV_ECDSA_CTX *ctx, const char *mdname, const char *mdprops, const char *desc)
{
    EVP_MD *md = NULL;
    size_t mdname_len;
    int md_nid, md_size;
    ...
    if (md_size <= 0) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST, "%s has invalid md size %d", mdname,
                       md_size);
        goto err;
    }
...
static int
ecdsa_sign_init(void *vctx, void *ec, const OSSL_PARAM params[])
{
    ...
    ctx->verify_message = 1;
#endif
    return ecdsa_signverify_init(ctx, ec, ecdsa_set_ctx_params, params, EVP_PKEY_OP_SIGN,
                                 "ECDSA Sign Init");
}
 ...
 */
static int
ecdsa_sign_directly(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize,
                    const unsigned char *tbs, size_t tbslen)
{
    ...
    if (ctx->nonce_type != 0) {
        ...
        if (ctx->mdname[0] != '\0')
            mdname = ctx->mdname;
        ret = ossl_ecdsa_deterministic_sign(tbs, tbslen, sig, &sltmp, ctx->ec, ctx->nonce_type,
                                            mdname, ctx->libctx, ctx->propq);
    } else {
...
static int
ecdsa_signverify_message_update(void *vctx, const unsigned char *data, size_t datalen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int
ecdsa_sign_message_final(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
 ...
 */
static int
ecdsa_sign(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs,
           size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int
ecdsa_verify_init(void *vctx, void *ec, const OSSL_PARAM params[])
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
#endif
    return ecdsa_signverify_init(ctx, ec, ecdsa_set_ctx_params, params, EVP_PKEY_OP_VERIFY,
                                 "ECDSA Verify Init");
}

static int
ecdsa_verify_directly(void *vctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                      size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int
ecdsa_verify_set_sig(void *vctx, const unsigned char *sig, size_t siglen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    OSSL_PARAM params[2];
...
static int
ecdsa_verify_message_final(void *vctx)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    unsigned char digest[EVP_MAX_MD_SIZE];
    unsigned int dlen = 0;
 ...
 */
static int
ecdsa_verify(void *vctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
             size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int
ecdsa_digest_signverify_init(void *vctx, const char *mdname, void *ec, const OSSL_PARAM params[],
...
static int
ecdsa_digest_sign_init(void *vctx, const char *mdname, void *ec, const OSSL_PARAM params[])
{
    return ecdsa_digest_signverify_init(vctx, mdname, ec, params, EVP_PKEY_OP_SIGNMSG,
                                        "ECDSA Digest Sign Init");
}

static int
ecdsa_digest_signverify_update(void *vctx, const unsigned char *data, size_t datalen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
int
ecdsa_digest_sign_final(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    int ok = 0;
...
static int
ecdsa_digest_verify_init(void *vctx, const char *mdname, void *ec, const OSSL_PARAM params[])
{
    return ecdsa_digest_signverify_init(vctx, mdname, ec, params, EVP_PKEY_OP_VERIFYMSG,
                                        "ECDSA Digest Verify Init");
}

int
ecdsa_digest_verify_final(void *vctx, const unsigned char *sig, size_t siglen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    int ok = 0;
...
static int
ecdsa_get_ctx_params(void *vctx, OSSL_PARAM *params)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    OSSL_PARAM *p;
    ...
    if (p != NULL &&
        !OSSL_PARAM_set_octet_string(p, ctx->aid_len == 0 ? NULL : ctx->aid_buf, ctx->aid_len))
        return 0;
    ...
    if (p != NULL &&
        !OSSL_PARAM_set_utf8_string(p, ctx->md == NULL ? ctx->mdname : EVP_MD_get0_name(ctx->md)))
        return 0;
...
ecdsa_gettable_ctx_params(ossl_unused void *vctx, ossl_unused void *provctx)
{
 ...
 */
static int
ecdsa_common_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    const OSSL_PARAM *p;
...
static int
ecdsa_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    const OSSL_PARAM *p;
    size_t mdsize = 0;
...
static const OSSL_PARAM *
ecdsa_settable_ctx_params(void *vctx, ossl_unused void *provctx)
{
    return settable_ctx_params;
}

static int
ecdsa_get_ctx_md_params(void *vctx, OSSL_PARAM *params)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int
ecdsa_set_ctx_md_params(void *vctx, const OSSL_PARAM params[])
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
 ...
 */
static int
ecdsa_sigalg_signverify_init(void *vctx, void *vec,
                             OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
...
static const char **
ecdsa_sigalg_query_key_types(void)
{
...
static int
ecdsa_sigalg_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    const OSSL_PARAM *p;
    int ret;
...
#define IMPL_ECDSA_SIGALG(md, MD)                                                                  \
    ...
    static OSSL_FUNC_signature_sign_message_init_fn ecdsa_##md##_sign_message_init;                \
    static OSSL_FUNC_signature_verify_init_fn ecdsa_##md##_verify_init;                            \
    static OSSL_FUNC_signature_verify_message_init_fn ecdsa_##md##_verify_message_init;            \
                                                                                                   \
    static int ecdsa_##md##_sign_init(void *vctx, void *vec, const OSSL_PARAM params[])            \
    {                                                                                              \
        static const char desc[] = "ECDSA-" MD " Sign Init";                                       \
                                                                                                   \
        return ecdsa_sigalg_signverify_init(vctx, vec, ecdsa_sigalg_set_ctx_params, params, MD,    \
                                            EVP_PKEY_OP_SIGN, desc);                               \
    }                                                                                              \
                                                                                                   \
    static int ecdsa_##md##_sign_message_init(void *vctx, void *vec, const OSSL_PARAM params[])    \
    {                                                                                              \
        ...
        return ecdsa_sigalg_signverify_init(vctx, vec, ecdsa_sigalg_set_ctx_params, params, MD,    \
                                            EVP_PKEY_OP_SIGNMSG, desc);                            \
                                                                                                   ...
                                                                                                   \
    static int ecdsa_##md##_verify_init(void *vctx, void *vec, const OSSL_PARAM params[])          \
    {                                                                                              \
        ...
        return ecdsa_sigalg_signverify_init(vctx, vec, ecdsa_sigalg_set_ctx_params, params, MD,    \
                                            EVP_PKEY_OP_VERIFY, desc);                             \
                                                                                                   ...
                                                                                                   \
    static int ecdsa_##md##_verify_message_init(void *vctx, void *vec, const OSSL_PARAM params[])  \
    {                                                                                              \
    ...
    const OSSL_DISPATCH ossl_ecdsa_##md##_signature_functions[] = {                                \
        ...
        {OSSL_FUNC_SIGNATURE_SIGN, (void (*)(void))ecdsa_sign},                                    \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_INIT, (void (*)(void))ecdsa_##md##_sign_message_init},   \
        ...
        {OSSL_FUNC_SIGNATURE_FREECTX, (void (*)(void))ecdsa_freectx},                              \
        {OSSL_FUNC_SIGNATURE_DUPCTX, (void (*)(void))ecdsa_dupctx},                                \
        {OSSL_FUNC_SIGNATURE_QUERY_KEY_TYPES, (void (*)(void))ecdsa_sigalg_query_key_types},       \
        {OSSL_FUNC_SIGNATURE_GET_CTX_PARAMS, (void (*)(void))ecdsa_get_ctx_params},                \
        {OSSL_FUNC_SIGNATURE_GETTABLE_CTX_PARAMS, (void (*)(void))ecdsa_gettable_ctx_params},      \
        {OSSL_FUNC_SIGNATURE_SET_CTX_PARAMS, (void (*)(void))ecdsa_sigalg_set_ctx_params},         \
