...
static size_t
rsa_get_md_size(const PROV_RSA_CTX *prsactx)
{
    int md_size;
...
static int
rsa_check_padding(const PROV_RSA_CTX *prsactx, const char *mdname, const char *mgf1_mdname,
                  int mdnid)
{
...
static int
rsa_check_parameters(PROV_RSA_CTX *prsactx, int min_saltlen)
{
...
static void *
rsa_newctx(void *provctx, const char *propq)
{
...
static int
rsa_pss_compute_saltlen(PROV_RSA_CTX *ctx)
{
    ...
    if (saltlen < 0) {
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_PSS_SALTLEN_TOO_SMALL,
                       "minimum salt length: %d, actual salt length: %d", ctx->min_saltlen,
                       saltlen);
        return -1;
    }
    return saltlen;
...
rsa_generate_signature_aid(PROV_RSA_CTX *ctx, unsigned char *aid_buf, size_t buf_len,
                           size_t *aid_len)
{
    WPACKET pkt;
    ...
    case RSA_PKCS1_PADDING:
        ...
        ERR_raise_data(ERR_LIB_PROV, ERR_R_UNSUPPORTED, "Algorithm ID generation - md NID: %d",
                       ctx->mdnid);
        goto cleanup;
    case RSA_PKCS1_PSS_PADDING:
        ...
        if (!ossl_rsa_pss_params_30_set_defaults(&pss_params) ||
            ...
            !ossl_DER_w_algorithmIdentifier_RSA_PSS(&pkt, -1, RSA_FLAG_TYPE_RSASSAPSS,
                                                    &pss_params)) {
            ERR_raise(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR);
            goto cleanup;
        }
        break;
    default:
        ERR_raise_data(ERR_LIB_PROV, ERR_R_UNSUPPORTED, "Algorithm ID generation - pad mode: %d",
                       ctx->pad_mode);
        goto cleanup;
    }
...
static int
rsa_setup_md(PROV_RSA_CTX *ctx, const char *mdname, const char *mdprops, const char *desc)
{
    EVP_MD *md = NULL;
    ...
    if (mdname != NULL) {
...
#ifdef FIPS_MODULE
        {
            ...
            if (!ossl_fips_ind_digest_sign_check(OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE1,
        ...
        if (mdname_len >= sizeof(ctx->mdname)) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST, "%s exceeds name buffer length",
                           mdname);
            goto err;
        }
        ...
        if (!ctx->flag_allow_md) {
            if (ctx->mdname[0] != '\0' && !EVP_MD_is_a(md, ctx->mdname)) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_DIGEST_NOT_ALLOWED, "digest %s != %s", mdname,
                               ctx->mdname);
                goto err;
            }
            EVP_MD_free(md);
...
static int
rsa_setup_mgf1_md(PROV_RSA_CTX *ctx, const char *mdname, const char *mdprops)
{
    size_t len;
    EVP_MD *md = NULL;
    int mdnid;
                    ...
                    const OSSL_PARAM params[], int operation, const char *desc)
{
    ...
    case RSA_FLAG_TYPE_RSASSAPSS:
...

        {
            ...
            if (!ossl_rsa_pss_params_30_is_unrestricted(pss)) {
                ...
                len =
                    OPENSSL_strlcpy(prsactx->mgf1_mdname, mgf1mdname, sizeof(prsactx->mgf1_mdname));
    ...
    if (!set_ctx_params(prsactx, params))
        return 0;
#ifdef FIPS_MODULE
    if (!ossl_fips_ind_rsa_key_check(OSSL_FIPS_IND_GET(prsactx), OSSL_FIPS_IND_SETTABLE0,
                                     prsactx->libctx, prsactx->rsa, desc, protect))
        return 0;
#endif
    return 1;
}
...
#ifdef FIPS_MODULE
static int
rsa_pss_saltlen_check_passed(PROV_RSA_CTX *ctx, const char *algoname, int saltlen)
{
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE3, ctx->libctx, algoname,
                                         "PSS Salt Length",
...
static int
rsa_sign_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    ...
    return rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, EVP_PKEY_OP_SIGN,
                               "RSA Sign Init");
}
 ...
 */
static int
rsa_sign_directly(PROV_RSA_CTX *prsactx, unsigned char *sig, size_t *siglen, size_t sigsize,
                  const unsigned char *tbs, size_t tbslen)
{
    ...
    if (mdsize != 0) {
        ...
        case RSA_X931_PADDING:
            if ((size_t)RSA_size(prsactx->rsa) < tbslen + 1) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_KEY_SIZE_TOO_SMALL,
                               "RSA key size = %d, expected minimum = %d", RSA_size(prsactx->rsa),
                               tbslen + 1);
                return 0;
            }
            ...
            ret =
                RSA_private_encrypt(tbslen + 1, prsactx->tbuf, sig, prsactx->rsa, RSA_X931_PADDING);
            clean_tbuf(prsactx);
            break;
        case RSA_PKCS1_PADDING: {
            unsigned int sltmp;
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            if (!ossl_rsa_padding_add_PKCS1_PSS_mgf1(prsactx->rsa, prsactx->tbuf, tbs, prsactx->md,
            ...
            if (!rsa_pss_saltlen_check_passed(prsactx, "RSA Sign", saltlen))
                return 0;
#endif
            ret = RSA_private_encrypt(RSA_size(prsactx->rsa), prsactx->tbuf, sig, prsactx->rsa,
                                      RSA_NO_PADDING);
            clean_tbuf(prsactx);
        } break;
...
static int
rsa_signverify_message_update(void *vprsactx, const unsigned char *data, size_t datalen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_sign_message_final(void *vprsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
 ...
 */
static int
rsa_sign(void *vprsactx, unsigned char *sig, size_t *siglen, size_t sigsize,
         const unsigned char *tbs, size_t tbslen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_verify_recover_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
 ...
 */
static int
rsa_verify_recover(void *vprsactx, unsigned char *rout, size_t *routlen, size_t routsize,
        ...
        case RSA_X931_PADDING:
            ...
            if (ret != EVP_MD_get_size(prsactx->md)) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST_LENGTH,
                               "Should be %d, but got %d", EVP_MD_get_size(prsactx->md), ret);
                return 0;
            }
...
static int
rsa_verify_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    ...
    return rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, EVP_PKEY_OP_VERIFY,
                               "RSA Verify Init");
}

static int
rsa_verify_directly(PROV_RSA_CTX *prsactx, const unsigned char *sig, size_t siglen,
                    const unsigned char *tbs, size_t tbslen)
{
    size_t rslen;
    ...
    if (prsactx->md != NULL) {
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            saltlen = prsactx->saltlen;
            ret = ossl_rsa_verify_PKCS1_PSS_mgf1(prsactx->rsa, tbs, prsactx->md, prsactx->mgf1_md,
...
static int
rsa_verify_set_sig(void *vprsactx, const unsigned char *sig, size_t siglen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    OSSL_PARAM params[2];
...
static int
rsa_verify_message_final(void *vprsactx)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    unsigned char digest[EVP_MAX_MD_SIZE];
    unsigned int dlen = 0;
 ...
 */
static int
rsa_verify(void *vprsactx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
           size_t tbslen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_digest_signverify_init(void *vprsactx, const char *mdname, void *vrsa,
                           const OSSL_PARAM params[], int operation, const char *desc)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_digest_sign_init(void *vprsactx, const char *mdname, void *vrsa, const OSSL_PARAM params[])
{
    if (!ossl_prov_is_running())
        return 0;
    return rsa_digest_signverify_init(vprsactx, mdname, vrsa, params, EVP_PKEY_OP_SIGNMSG,
                                      "RSA Digest Sign Init");
}

static int
rsa_digest_sign_update(void *vprsactx, const unsigned char *data, size_t datalen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_digest_sign_final(void *vprsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    int ok = 0;
...
static int
rsa_digest_verify_init(void *vprsactx, const char *mdname, void *vrsa, const OSSL_PARAM params[])
{
    if (!ossl_prov_is_running())
        return 0;
    return rsa_digest_signverify_init(vprsactx, mdname, vrsa, params, EVP_PKEY_OP_VERIFYMSG,
                                      "RSA Digest Verify Init");
}

static int
rsa_digest_verify_update(void *vprsactx, const unsigned char *data, size_t datalen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
int
rsa_digest_verify_final(void *vprsactx, const unsigned char *sig, size_t siglen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    int ok = 0;
...
static int
rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    OSSL_PARAM *p;
...
rsa_gettable_ctx_params(ossl_unused void *vprsactx, ossl_unused void *provctx)
{
...
#ifdef FIPS_MODULE
static int
rsa_x931_padding_allowed(PROV_RSA_CTX *ctx)
{
    int approved = ((ctx->operation & EVP_PKEY_OP_SIGN) == 0);

    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE2, ctx->libctx,
...
static int
rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[])
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    const OSSL_PARAM *p;
    int pad_mode;
    ...
    if (p != NULL) {
        ...
        case RSA_PKCS1_OAEP_PADDING:
            ...
            goto bad_pad;
        case RSA_PKCS1_PSS_PADDING:
            if ((prsactx->operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_SIGNMSG | EVP_PKEY_OP_VERIFY |
...
static const OSSL_PARAM *
rsa_settable_ctx_params(void *vprsactx, ossl_unused void *provctx)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_get_ctx_md_params(void *vprsactx, OSSL_PARAM *params)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int
rsa_set_ctx_md_params(void *vprsactx, const OSSL_PARAM params[])
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
 ...
 */
static int
rsa_sigalg_signverify_init(void *vprsactx, void *vrsa,
                           OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                           const OSSL_PARAM params[], const char *mdname, int operation,
                           int pad_mode, const char *desc)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static const char **
rsa_sigalg_query_key_types(void)
{
...
static int
rsa_sigalg_set_ctx_params(void *vprsactx, const OSSL_PARAM params[])
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    const OSSL_PARAM *p;
...
#define IMPL_RSA_SIGALG(md, MD)                                                                    \
    ...
    static OSSL_FUNC_signature_sign_message_init_fn rsa_##md##_sign_message_init;                  \
    static OSSL_FUNC_signature_verify_init_fn rsa_##md##_verify_init;                              \
    static OSSL_FUNC_signature_verify_message_init_fn rsa_##md##_verify_message_init;              \
                                                                                                   \
    static int rsa_##md##_sign_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])         \
    {                                                                                              \
        static const char desc[] = "RSA Sigalg Sign Init";                                         \
                                                                                                   \
        return rsa_sigalg_signverify_init(vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD,   \
                                          EVP_PKEY_OP_SIGN, RSA_PKCS1_PADDING, desc);              \
    }                                                                                              \
                                                                                                   \
    static int rsa_##md##_sign_message_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[]) \
    {                                                                                              \
        ...
        return rsa_sigalg_signverify_init(vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD,   \
                                          EVP_PKEY_OP_SIGNMSG, RSA_PKCS1_PADDING, desc);           \
                                                                                                   ...
                                                                                                   \
    static int rsa_##md##_verify_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])       \
    {                                                                                              \
    ...
    const OSSL_DISPATCH ossl_rsa_##md##_signature_functions[] = {                                  \
        ...
        {OSSL_FUNC_SIGNATURE_SIGN, (void (*)(void))rsa_sign},                                      \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_INIT, (void (*)(void))rsa_##md##_sign_message_init},     \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_UPDATE, (void (*)(void))rsa_signverify_message_update},  \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_FINAL, (void (*)(void))rsa_sign_message_final},          \
        {OSSL_FUNC_SIGNATURE_VERIFY_INIT, (void (*)(void))rsa_##md##_verify_init},                 \
        {OSSL_FUNC_SIGNATURE_VERIFY, (void (*)(void))rsa_verify},                                  \
        {OSSL_FUNC_SIGNATURE_VERIFY_MESSAGE_INIT, (void (*)(void))rsa_##md##_verify_message_init}, \
        {OSSL_FUNC_SIGNATURE_VERIFY_MESSAGE_UPDATE,                                                \
         (void (*)(void))rsa_signverify_message_update},                                           \
        {OSSL_FUNC_SIGNATURE_VERIFY_MESSAGE_FINAL, (void (*)(void))rsa_verify_message_final},      \
        {OSSL_FUNC_SIGNATURE_VERIFY_RECOVER_INIT, (void (*)(void))rsa_##md##_verify_recover_init}, \
        {OSSL_FUNC_SIGNATURE_VERIFY_RECOVER, (void (*)(void))rsa_verify_recover},                  \
        {OSSL_FUNC_SIGNATURE_FREECTX, (void (*)(void))rsa_freectx},                                \
        {OSSL_FUNC_SIGNATURE_DUPCTX, (void (*)(void))rsa_dupctx},                                  \
        {OSSL_FUNC_SIGNATURE_QUERY_KEY_TYPES, (void (*)(void))rsa_sigalg_query_key_types},         \
        {OSSL_FUNC_SIGNATURE_GET_CTX_PARAMS, (void (*)(void))rsa_get_ctx_params},                  \
        {OSSL_FUNC_SIGNATURE_GETTABLE_CTX_PARAMS, (void (*)(void))rsa_gettable_ctx_params},        \
        {OSSL_FUNC_SIGNATURE_SET_CTX_PARAMS, (void (*)(void))rsa_sigalg_set_ctx_params},           \
        {OSSL_FUNC_SIGNATURE_SETTABLE_CTX_PARAMS, (void (*)(void))rsa_sigalg_settable_ctx_params}, \
        OSSL_DISPATCH_END}
