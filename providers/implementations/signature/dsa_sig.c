...
static size_t
dsa_get_md_size(const PROV_DSA_CTX *pdsactx)
{
    int md_size;
...
static void *
dsa_newctx(void *provctx, const char *propq)
{
...
static int
dsa_setup_md(PROV_DSA_CTX *ctx, const char *mdname, const char *mdprops, const char *desc)
{
    EVP_MD *md = NULL;
    ...
    if (mdname != NULL) {
        ...
        if (WPACKET_init_der(&pkt, ctx->aid_buf, sizeof(ctx->aid_buf)) &&
            ossl_DER_w_algorithmIdentifier_DSA_with_MD(&pkt, -1, ctx->dsa, md_nid) &&
            WPACKET_finish(&pkt)) {
...
static int
dsa_sign_check_approved(PROV_DSA_CTX *ctx, int signing, const char *desc)
{
    /* DSA Signing is not approved in FIPS 140-3 */
    if (signing && !OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE2, ctx->libctx, desc,
                                                "DSA", ossl_fips_config_dsa_sign_disallowed))
        return 0;
    return 1;
}

static int
dsa_check_key(PROV_DSA_CTX *ctx, int sign, const char *desc)
{
...
static int
dsa_sign_init(void *vpdsactx, void *vdsa, const OSSL_PARAM params[])
{
    return dsa_signverify_init(vpdsactx, vdsa, dsa_set_ctx_params, params, EVP_PKEY_OP_SIGN,
                               "DSA Sign Init");
}
 ...
 */
static int
dsa_sign_directly(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize,
                  const unsigned char *tbs, size_t tbslen)
{
    ...
    ret = ossl_dsa_sign_int(0, tbs, tbslen, sig, &sltmp, pdsactx->dsa, pdsactx->nonce_type,
                            pdsactx->mdname, pdsactx->libctx, pdsactx->propq);
    if (ret <= 0)
        return 0;
...
static int
dsa_signverify_message_update(void *vpdsactx, const unsigned char *data, size_t datalen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_sign_message_final(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
 ...
 */
static int
dsa_sign(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize,
         const unsigned char *tbs, size_t tbslen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_verify_init(void *vpdsactx, void *vdsa, const OSSL_PARAM params[])
{
    return dsa_signverify_init(vpdsactx, vdsa, dsa_set_ctx_params, params, EVP_PKEY_OP_VERIFY,
                               "DSA Verify Init");
}

static int
dsa_verify_directly(void *vpdsactx, const unsigned char *sig, size_t siglen,
                    const unsigned char *tbs, size_t tbslen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    size_t mdsize = dsa_get_md_size(pdsactx);
...
static int
dsa_verify_set_sig(void *vpdsactx, const unsigned char *sig, size_t siglen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    OSSL_PARAM params[2];
...
static int
dsa_verify_message_final(void *vpdsactx)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    unsigned char digest[EVP_MAX_MD_SIZE];
    unsigned int dlen = 0;
 ...
 */
static int
dsa_verify(void *vpdsactx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
           size_t tbslen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_digest_signverify_init(void *vpdsactx, const char *mdname, void *vdsa,
                           const OSSL_PARAM params[], int operation, const char *desc)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_digest_sign_init(void *vpdsactx, const char *mdname, void *vdsa, const OSSL_PARAM params[])
{
    return dsa_digest_signverify_init(vpdsactx, mdname, vdsa, params, EVP_PKEY_OP_SIGNMSG,
                                      "DSA Digest Sign Init");
}

static int
dsa_digest_signverify_update(void *vpdsactx, const unsigned char *data, size_t datalen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_digest_sign_final(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    int ok = 0;
...
static int
dsa_digest_verify_init(void *vpdsactx, const char *mdname, void *vdsa, const OSSL_PARAM params[])
{
    return dsa_digest_signverify_init(vpdsactx, mdname, vdsa, params, EVP_PKEY_OP_VERIFYMSG,
                                      "DSA Digest Verify Init");
}

int
dsa_digest_verify_final(void *vpdsactx, const unsigned char *sig, size_t siglen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    int ok = 0;
...
static int
dsa_get_ctx_params(void *vpdsactx, OSSL_PARAM *params)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    OSSL_PARAM *p;
...
dsa_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx)
{
 ...
 */
static int
dsa_common_set_ctx_params(void *vpdsactx, const OSSL_PARAM params[])
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    const OSSL_PARAM *p;
...
static int
dsa_set_ctx_params(void *vpdsactx, const OSSL_PARAM params[])
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    const OSSL_PARAM *p;
    int ret;
...
static const OSSL_PARAM *
dsa_settable_ctx_params(void *vpdsactx, ossl_unused void *provctx)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_get_ctx_md_params(void *vpdsactx, OSSL_PARAM *params)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int
dsa_set_ctx_md_params(void *vpdsactx, const OSSL_PARAM params[])
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
 ...
 */
static int
dsa_sigalg_signverify_init(void *vpdsactx, void *vdsa,
                           OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
...
static const char **
dsa_sigalg_query_key_types(void)
{
...
static int
dsa_sigalg_set_ctx_params(void *vpdsactx, const OSSL_PARAM params[])
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    const OSSL_PARAM *p;
    int ret;
...
#define IMPL_DSA_SIGALG(md, MD)                                                                    \
    ...
    static OSSL_FUNC_signature_sign_message_init_fn dsa_##md##_sign_message_init;                  \
    static OSSL_FUNC_signature_verify_init_fn dsa_##md##_verify_init;                              \
    static OSSL_FUNC_signature_verify_message_init_fn dsa_##md##_verify_message_init;              \
                                                                                                   \
    static int dsa_##md##_sign_init(void *vpdsactx, void *vdsa, const OSSL_PARAM params[])         \
    {                                                                                              \
        static const char desc[] = "DSA-" MD " Sign Init";                                         \
                                                                                                   \
        return dsa_sigalg_signverify_init(vpdsactx, vdsa, dsa_sigalg_set_ctx_params, params, MD,   \
                                          EVP_PKEY_OP_SIGN, desc);                                 \
    }                                                                                              \
                                                                                                   \
    static int dsa_##md##_sign_message_init(void *vpdsactx, void *vdsa, const OSSL_PARAM params[]) \
    {                                                                                              \
        ...
        return dsa_sigalg_signverify_init(vpdsactx, vdsa, dsa_sigalg_set_ctx_params, params, MD,   \
                                          EVP_PKEY_OP_SIGNMSG, desc);                              \
                                                                                                   ...
                                                                                                   \
    static int dsa_##md##_verify_init(void *vpdsactx, void *vdsa, const OSSL_PARAM params[])       \
    {                                                                                              \
    ...
    const OSSL_DISPATCH ossl_dsa_##md##_signature_functions[] = {                                  \
        ...
        {OSSL_FUNC_SIGNATURE_SIGN, (void (*)(void))dsa_sign},                                      \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_INIT, (void (*)(void))dsa_##md##_sign_message_init},     \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_UPDATE, (void (*)(void))dsa_signverify_message_update},  \
        {OSSL_FUNC_SIGNATURE_SIGN_MESSAGE_FINAL, (void (*)(void))dsa_sign_message_final},          \
        {OSSL_FUNC_SIGNATURE_VERIFY_INIT, (void (*)(void))dsa_##md##_verify_init},                 \
        {OSSL_FUNC_SIGNATURE_VERIFY, (void (*)(void))dsa_verify},                                  \
        {OSSL_FUNC_SIGNATURE_VERIFY_MESSAGE_INIT, (void (*)(void))dsa_##md##_verify_message_init}, \
        {OSSL_FUNC_SIGNATURE_VERIFY_MESSAGE_UPDATE,                                                \
         (void (*)(void))dsa_signverify_message_update},                                           \
        {OSSL_FUNC_SIGNATURE_VERIFY_MESSAGE_FINAL, (void (*)(void))dsa_verify_message_final},      \
        {OSSL_FUNC_SIGNATURE_FREECTX, (void (*)(void))dsa_freectx},                                \
        {OSSL_FUNC_SIGNATURE_DUPCTX, (void (*)(void))dsa_dupctx},                                  \
        {OSSL_FUNC_SIGNATURE_QUERY_KEY_TYPES, (void (*)(void))dsa_sigalg_query_key_types},         \
        {OSSL_FUNC_SIGNATURE_GET_CTX_PARAMS, (void (*)(void))dsa_get_ctx_params},                  \
        {OSSL_FUNC_SIGNATURE_GETTABLE_CTX_PARAMS, (void (*)(void))dsa_gettable_ctx_params},        \
        {OSSL_FUNC_SIGNATURE_SET_CTX_PARAMS, (void (*)(void))dsa_sigalg_set_ctx_params},           \
        {OSSL_FUNC_SIGNATURE_SETTABLE_CTX_PARAMS, (void (*)(void))dsa_sigalg_settable_ctx_params}, \
        OSSL_DISPATCH_END}
