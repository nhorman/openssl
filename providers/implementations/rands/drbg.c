 ...
 */
size_t
ossl_drbg_get_seed(void *vdrbg, unsigned char **pout, int entropy, size_t min_len, size_t max_len,
...
static size_t
get_entropy(PROV_DRBG *drbg, unsigned char **pout, int entropy, size_t min_len, size_t max_len,
            int prediction_resistance)
{
     ...
     *       intentional and correct here.
     */
    bytes = drbg->parent_get_seed(drbg->parent, pout, entropy > 0 ? entropy : (int)drbg->strength,
...
static size_t
prov_drbg_get_nonce(PROV_DRBG *drbg, unsigned char **pout, size_t min_len, size_t max_len)
{
    size_t ret = 0, n;
 ...
 */
int
ossl_prov_drbg_instantiate(PROV_DRBG *drbg, unsigned int strength, int prediction_resistance,
                           const unsigned char *pers, size_t perslen)
{
    ...
    if (drbg->min_noncelen > 0) {
        if (drbg->parent_nonce != NULL) {
            noncelen = drbg->parent_nonce(drbg->parent, NULL, drbg->strength, drbg->min_noncelen,
                                          drbg->max_noncelen);
            if (noncelen == 0) {
    ...
    entropylen = get_entropy(drbg, &entropy, min_entropy, min_entropylen, max_entropylen,
...
static int
ossl_prov_drbg_reseed_unlocked(PROV_DRBG *drbg, int prediction_resistance, const unsigned char *ent,
 ...
 */
int
ossl_prov_drbg_reseed(PROV_DRBG *drbg, int prediction_resistance, const unsigned char *ent,
                      size_t ent_len, const unsigned char *adin, size_t adinlen)
{
    int ret;
 ...
 */
int
ossl_prov_drbg_generate(PROV_DRBG *drbg, unsigned char *out, size_t outlen, unsigned int strength,
 ...
 */
PROV_DRBG *
ossl_rand_drbg_new(void *provctx, void *parent, const OSSL_DISPATCH *p_dispatch,
 ...
 */
int
ossl_drbg_get_ctx_params(PROV_DRBG *drbg, OSSL_PARAM params[])
{
    OSSL_PARAM *p;
 ...
 */
int
ossl_drbg_get_ctx_params_no_lock(PROV_DRBG *drbg, OSSL_PARAM params[], int *complete)
{
    size_t cnt = 0;
    OSSL_PARAM *p;
...
int
ossl_drbg_set_ctx_params(PROV_DRBG *drbg, const OSSL_PARAM params[])
{
    const OSSL_PARAM *p;
