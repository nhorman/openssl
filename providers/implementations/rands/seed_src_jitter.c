 ...
 */
static size_t
ossl_prov_acquire_entropy_from_jitter(PROV_JITTER *s, RAND_POOL *pool)
{
    size_t bytes_needed;
...
static size_t
get_jitter_random_value(PROV_JITTER *s, unsigned char *buf, size_t len)
{
...
static void *
jitter_new(void *provctx, void *parent, const OSSL_DISPATCH *parent_dispatch)
{
    PROV_JITTER *s;
...
static void
jitter_free(void *vseed)
{
    OPENSSL_free(vseed);
}

static int
jitter_instantiate(void *vseed, unsigned int strength, int prediction_resistance,
                   ...
                   ossl_unused const OSSL_PARAM params[])
{
    ...
    if ((ret = jent_entropy_init_ex(0, JENT_FORCE_FIPS)) != 0) {
        ERR_raise_data(ERR_LIB_RAND, RAND_R_ERROR_RETRIEVING_ENTROPY, "jent_entropy_init_ex (%d)",
                       ret);
        s->state = EVP_RAND_STATE_ERROR;
        return 0;
    }
...
static int
jitter_uninstantiate(void *vseed)
{
    PROV_JITTER *s = (PROV_JITTER *)vseed;
...
static int
jitter_generate(void *vseed, unsigned char *out, size_t outlen, unsigned int strength,
                ...
                ossl_unused size_t adin_len)
{
    ...
    if (s->state != EVP_RAND_STATE_READY) {
        ERR_raise(ERR_LIB_PROV, s->state == EVP_RAND_STATE_ERROR ? PROV_R_IN_ERROR_STATE
...
static int
jitter_reseed(void *vseed, ossl_unused int prediction_resistance,
...
static int
jitter_get_ctx_params(void *vseed, OSSL_PARAM params[])
{
    PROV_JITTER *s = (PROV_JITTER *)vseed;
    OSSL_PARAM *p;
...
jitter_gettable_ctx_params(ossl_unused void *vseed, ossl_unused void *provctx)
{
...
static int
jitter_verify_zeroization(ossl_unused void *vseed)
{
    return 1;
}

static size_t
jitter_get_seed(void *vseed, unsigned char **pout, int entropy, size_t min_len, size_t max_len,
...
size_t
ossl_rand_jitter_get_seed(unsigned char **pout, int entropy, size_t min_len, size_t max_len)
{
    size_t ret = 0;
...
static void
jitter_clear_seed(ossl_unused void *vdrbg, unsigned char *out, size_t outlen)
{
    OPENSSL_secure_clear_free(out, outlen);
}

static int
jitter_enable_locking(ossl_unused void *vseed)
{
    return 1;
}

int
jitter_lock(ossl_unused void *vctx)
{
    return 1;
}

void
jitter_unlock(ossl_unused void *vctx)
{
}
