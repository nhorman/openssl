 ...
 */
static size_t
prepare_item_list(const struct item_st *items_input, size_t items_input_num, ILE3 *items,
...
data_collect_method(RAND_POOL *pool)
{
    ...
    total_elems += prepare_item_list(DVI_item_data, OSSL_NELEM(DVI_item_data), DVI_items,
                                     &data.buffer[total_elems]);
    total_elems += prepare_item_list(JPI_item_data, OSSL_NELEM(JPI_item_data), JPI_items,
                                     &data.buffer[total_elems]);
    total_elems += prepare_item_list(RMI_item_data, OSSL_NELEM(RMI_item_data), RMI_items,
                                     &data.buffer[total_elems]);
    total_elems += prepare_item_list(SYI_item_data, OSSL_NELEM(SYI_item_data), SYI_items,
                                     &data.buffer[total_elems]);
    ...
    /* Fill data.buffer with various info bits from this process */
    {
        ...
        if ((status = sys$getdviw(EFN$C_ENF, 0, &SYSDEVICE, DVI_items, 0, 0, 0, 0, 0)) !=
            SS$_NORMAL) {
            lib$signal(status);
            return 0;
        }
    ...
    if (!ossl_assert(total_length >= bytes_needed)) {
        ERR_raise_data(ERR_LIB_RAND, RAND_R_RANDOM_POOL_UNDERFLOW, "Needed: %zu, Available: %zu",
