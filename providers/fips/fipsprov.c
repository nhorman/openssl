...
static int
fips_random_bytes(ossl_unused void *vprov, int which, void *buf, size_t n, unsigned int strength)
{
    OSSL_LIB_CTX *libctx;
...
#define OSSL_FIPS_PARAM(structname, paramname, initvalue)                                          \
    *p++ = OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_##paramname,                              \
...
fips_get_params(void *provctx, OSSL_PARAM params[])
{
    OSSL_PARAM *p;
    FIPS_GLOBAL *fgbl =
        ossl_lib_ctx_get_data(ossl_prov_ctx_get0_libctx(provctx), OSSL_LIB_CTX_FIPS_PROV_INDEX);
...
#endif
int
OSSL_provider_init_int(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
...
int
ossl_fips_intern_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
                               const OSSL_DISPATCH **out, void **provctx)
{
...
void *
CRYPTO_aligned_alloc(size_t num, size_t align, void **freeptr, const char *file, int line)
{
    return NULL;
}
...
void
OSSL_SELF_TEST_get_callback(OSSL_LIB_CTX *libctx, OSSL_CALLBACK **cb, void **cbarg)
{
    assert(libctx != NULL);
