...
static const unsigned char hmac_kat_digest[] = {
    0xf5, 0xf5, 0xe5, 0xf2, 0x66, 0x49, 0xe2, 0x40, 0xfc, 0x9e, 0x85, 0x7f, 0x2b, 0x9a, 0xbe, 0x28,
    0x20, 0x12, 0x00, 0x92, 0x82, 0x21, 0x3e, 0x51, 0x44, 0x5d, 0xe3, 0x31, 0x04, 0x01, 0x72, 0x6b};
 ...
 */
static int
verify_integrity(OSSL_CORE_BIO *bio, OSSL_FUNC_BIO_read_ex_fn read_ex_cb, unsigned char *expected,
                 size_t expected_len, OSSL_LIB_CTX *libctx, OSSL_SELF_TEST *ev,
...
SELF_TEST_post(SELF_TEST_POST_PARAMS *st, int on_demand_test)
{
    ...
    if (bio_module == NULL ||
        !verify_integrity(bio_module, st->bio_read_ex_cb, module_checksum, checksum_len, st->libctx,
    ...
    if (rng != NULL)
        if ((testrand = EVP_RAND_fetch(st->libctx, "TEST-RAND", NULL)) == NULL ||
            strcmp(EVP_RAND_get0_name(EVP_RAND_CTX_get0_rand(rng)), EVP_RAND_get0_name(testrand)) ==
                0) {
            ERR_raise(ERR_LIB_PROV, PROV_R_SELF_TEST_KAT_FAILURE);
            goto end;
        }
