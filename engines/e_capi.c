...
static EVP_PKEY *capi_load_privkey(ENGINE *eng, const char *key_id, UI_METHOD *ui_method,
                                   void *callback_data);
static int capi_rsa_sign(int dtype, const unsigned char *m, unsigned int m_len,
                         unsigned char *sigret, unsigned int *siglen, const RSA *rsa);
static int capi_rsa_priv_enc(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                             int padding);
static int capi_rsa_priv_dec(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                             int padding);
...
static const ENGINE_CMD_DEFN capi_cmd_defns[] = {
    ...
    {CAPI_CMD_KEYTYPE, "key_type", "Key type: 1=AT_KEYEXCHANGE (default), 2=AT_SIGNATURE",
     ENGINE_CMD_FLAG_NUMERIC},
    ...
    {CAPI_CMD_SET_CSP_NAME, "csp_name", "Set CSP name, (default CSP used if not specified)",
    ...
    {CAPI_CMD_STORE_NAME, "store_name", "certificate store name, default \"MY\"",
     ENGINE_CMD_FLAG_STRING},
    {CAPI_CMD_STORE_FLAGS, "store_flags", "Certificate store flags: 1 = system store",
     ENGINE_CMD_FLAG_NUMERIC},
...
# endif
    if (!ENGINE_set_id(e, engine_capi_id) || !ENGINE_set_name(e, engine_capi_name) ||
        ...
        !ENGINE_set_init_function(e, capi_init) || !ENGINE_set_finish_function(e, capi_finish) ||
        !ENGINE_set_destroy_function(e, capi_destroy) || !ENGINE_set_RSA(e, capi_rsa_method)
# ifndef OPENSSL_NO_DSA
...
static EVP_PKEY *
capi_load_privkey(ENGINE *eng, const char *key_id, UI_METHOD *ui_method, void *callback_data)
{
    CAPI_CTX *ctx;
    CAPI_KEY *key;
    EVP_PKEY *ret;
...
int
capi_rsa_priv_enc(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
...
int
capi_rsa_sign(int dtype, const unsigned char *m, unsigned int m_len, unsigned char *sigret,
...
int
capi_rsa_priv_dec(int flen, const unsigned char *from, unsigned char *to, RSA *rsa, int padding)
{
    int i;
...
capi_find_cert(CAPI_CTX *ctx, const char *id, HCERTSTORE hstore)
{
    ...
    case CAPI_LU_SUBSTR:
        return CertFindCertificateInStore(hstore, X509_ASN_ENCODING, 0, CERT_FIND_SUBJECT_STR_A, id,
                                          NULL);
    case CAPI_LU_FNAME:
        for (;;) {
            cert = CertEnumCertificatesInStore(hstore, cert);
...
static CAPI_KEY *
capi_get_key(CAPI_CTX *ctx, const WCHAR *contname, const WCHAR *provname, DWORD ptype,
             DWORD keyspec)
{
    DWORD dwFlags = 0;
...
capi_get_cert_key(CAPI_CTX *ctx, PCCERT_CONTEXT cert)
{
    ...
    if (pinfo != NULL)
        key = capi_get_key(ctx, pinfo->pwszContainerName, pinfo->pwszProvName, pinfo->dwProvType,
                           pinfo->dwKeySpec);
...
static int
capi_load_ssl_client_cert(ENGINE *e, SSL *ssl, STACK_OF(X509_NAME) * ca_dn, X509 **pcert,
