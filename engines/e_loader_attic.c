...
static char *
file_get_pass(const UI_METHOD *ui_method, char *pass, size_t maxsize, const char *desc,
              const char *info, void *data)
{
    UI *ui = UI_new();
    char *prompt = NULL;
    ...
    if ((prompt = UI_construct_prompt(ui, desc, info)) == NULL) {
        ATTICerr(0, ERR_R_UI_LIB);
        pass = NULL;
    } else if (UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, pass, 0, maxsize - 1) <=
               0) {
        ATTICerr(0, ERR_R_UI_LIB);
        pass = NULL;
    } else {
...
static int
file_fill_pem_pass_data(struct pem_pass_data *pass_data, const char *desc, const char *info,
...
file_get_pem_pass(char *buf, int num, int w, void *data)
{
    ...
    char *pass = file_get_pass(pass_data->ui_method, buf, num, pass_data->prompt_desc,
 ...
 *    an OSSL_STORE_INFO
 */
typedef OSSL_STORE_INFO *(*file_try_decode_fn)(const char *pem_name, const char *pem_header,
                                               const unsigned char *blob, size_t len,
                                               void **handler_ctx, int *matchcount,
                                               const UI_METHOD *ui_method, void *ui_data,
        ...
        if ((p12 = d2i_PKCS12(NULL, &blob, len)) != NULL) {
            ...
            *matchcount = 1;
            ...
            if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, "", 0) ||
                ...
                if ((pass = file_get_pass(ui_method, tpass, PEM_BUFSIZE, "PKCS12 import", uri,
                                          ui_data)) == NULL) {
                          ...
                          void *ui_data, const char *uri, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    *matchcount = 1;
    ...
    if ((pass = file_get_pass(ui_method, kbuf, PEM_BUFSIZE, "PKCS8 decrypt pass phrase", uri,
                              ui_data)) == NULL) {
    ...
    if (!PKCS12_pbe_crypt(dalg, pass, strlen(pass), doct->data, doct->length, &new_data,
                          &new_data_len, 0))
        goto nop8;
 ...
 */
static OSSL_STORE_INFO *
try_decode_X509CRL(const char *pem_name, const char *pem_header, const unsigned char *blob,
                   size_t len, void **pctx, int *matchcount, const UI_METHOD *ui_method,
                   void *ui_data, const char *uri, OSSL_LIB_CTX *libctx, const char *propq)
{
    OSSL_STORE_INFO *store_info = NULL;
...
file_open_ex(const OSSL_STORE_LOADER *loader, const char *uri, OSSL_LIB_CTX *libctx,
...
static OSSL_STORE_LOADER_CTX *
file_open(const OSSL_STORE_LOADER *loader, const char *uri, const UI_METHOD *ui_method,
          void *ui_data)
{
...
static OSSL_STORE_LOADER_CTX *
file_attach(const OSSL_STORE_LOADER *loader, BIO *bp, OSSL_LIB_CTX *libctx, const char *propq,
            const UI_METHOD *ui_method, void *ui_data)
{
    OSSL_STORE_LOADER_CTX *ctx = NULL;
...
static OSSL_STORE_INFO *
file_load_try_decode(OSSL_STORE_LOADER_CTX *ctx, const char *pem_name, const char *pem_header,
...
again: {
    ...
    *matchcount = 0;
    for (i = 0; i < OSSL_NELEM(file_handlers); i++) {
        ...
        ERR_set_mark();
        tmp_result =
            handler->try_decode(pem_name, pem_header, data, len, &tmp_handler_ctx, &try_matchcount,
...
static OSSL_STORE_INFO *
file_load_try_repeat(OSSL_STORE_LOADER_CTX *ctx, const UI_METHOD *ui_method, void *ui_data)
{
    OSSL_STORE_INFO *result = NULL;
    int try_matchcount = 0;
    ...
    if (ctx->_.file.last_handler != NULL) {
        result = ctx->_.file.last_handler->try_decode(
            NULL, NULL, NULL, 0, &ctx->_.file.last_handler_ctx, &try_matchcount, ui_method, ui_data,
            ctx->uri, ctx->libctx, ctx->propq);
...
static int
file_read_pem(BIO *bp, char **pem_name, char **pem_header, unsigned char **data, long *len,
...
static OSSL_STORE_INFO *
file_try_read_PVK(BIO *bp, const UI_METHOD *ui_method, void *ui_data, const char *uri,
                  int *matchcount)
{
    OSSL_STORE_INFO *result = NULL;
...
file_load(OSSL_STORE_LOADER_CTX *ctx, const UI_METHOD *ui_method, void *ui_data)
{
    ...
    again:
        ...
        do {
            ...
            matchcount = -1;
            if (ctx->type == is_pem) {
                if (!file_read_pem(ctx->_.file.file, &pem_name, &pem_header, &data, &len, ui_method,
                                   ui_data, ctx->uri, (ctx->flags & FILE_FLAG_SECMEM) != 0)) {
                    ctx->errcnt++;
                    goto endloop;
                }
            } else {
                if ((result = file_try_read_msblob(ctx->_.file.file, &matchcount)) != NULL ||
                    (result = file_try_read_PVK(ctx->_.file.file, ui_method, ui_data, ctx->uri,
                                                &matchcount)) != NULL)
                    goto endloop;
            ...
            result = file_load_try_decode(ctx, pem_name, pem_header, data, len, ui_method, ui_data,
                                          &matchcount);
