...
SSL_use_PrivateKey_ASN1(int type, SSL *ssl, const unsigned char *d, long len)
{
    ...
    p = d;
    if ((pkey = d2i_PrivateKey_ex(type, NULL, &p, (long)len, ssl->ctx->libctx, ssl->ctx->propq)) ==
        NULL) {
        ERR_raise(ERR_LIB_SSL, ERR_R_ASN1_LIB);
        return 0;
    }
...
int
SSL_CTX_use_PrivateKey_ASN1(int type, SSL_CTX *ctx, const unsigned char *d, long len)
{
    int ret;
    const unsigned char *p;
    EVP_PKEY *pkey;
...
static int
serverinfo_find_extension(const unsigned char *serverinfo, size_t serverinfo_length,
...
static int
serverinfo_srv_parse_cb(SSL *s, unsigned int ext_type, const unsigned char *in, size_t inlen,
                        int *al, void *arg)
{
...
static int
serverinfo_srv_add_cb(SSL *s, unsigned int ext_type, const unsigned char **out, size_t *outlen,
                      int *al, void *arg)
{
 ...
 */
static int
serverinfo_process_buffer(unsigned int version, const unsigned char *serverinfo,
                          size_t serverinfo_length, SSL_CTX *ctx)
{
    ...
    while (PACKET_remaining(&pkt)) {
        ...
        if ((version == SSL_SERVERINFOV2 && !PACKET_get_net_4(&pkt, &context)) ||
            !PACKET_get_net_2(&pkt, &ext_type) || !PACKET_get_length_prefixed_2(&pkt, &data))
            return 0;
...
int
SSL_CTX_use_serverinfo_ex(SSL_CTX *ctx, unsigned int version, const unsigned char *serverinfo,
...
int
SSL_CTX_use_serverinfo(SSL_CTX *ctx, const unsigned char *serverinfo, size_t serverinfo_length)
{
                     ...
                     STACK_OF(X509) * chain, int override)
{
    ...
    if (!override &&
        (c->pkeys[i].x509 != NULL || c->pkeys[i].privatekey != NULL || c->pkeys[i].chain != NULL)) {
...
int
SSL_use_cert_and_key(SSL *ssl, X509 *x509, EVP_PKEY *privatekey, STACK_OF(X509) * chain,
                     int override)
{
...
int
SSL_CTX_use_cert_and_key(SSL_CTX *ctx, X509 *x509, EVP_PKEY *privatekey, STACK_OF(X509) * chain,
                         int override)
{
