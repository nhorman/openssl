...
OSSL_QRL_ENC_LEVEL *
ossl_qrl_enc_level_set_get(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level, int require_prov)
{
...
int
ossl_qrl_enc_level_set_has_keyslot(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level,
...
static void
el_teardown_keyslot(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level, size_t keyslot)
{
    OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);
...
static int
el_setup_keyslot(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level, unsigned char tgt_state,
                 size_t keyslot, const unsigned char *secret, size_t secret_len)
{
    OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);
    ...
    if (!ossl_assert(el != NULL &&
                     ossl_qrl_enc_level_set_has_keyslot(els, enc_level, tgt_state, keyslot))) {
    ...
    /* Derive "quic iv" key. */
    if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, secret, quic_v1_iv_label,
                              sizeof(quic_v1_iv_label), NULL, 0, el->iv[keyslot], iv_len, 1))
        goto err;

    /* Derive "quic key" key. */
    if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, secret, quic_v1_key_label,
                              sizeof(quic_v1_key_label), NULL, 0, key, key_len, 1))
        goto err;
...
int
ossl_qrl_enc_level_set_provide_secret(OSSL_QRL_ENC_LEVEL_SET *els, OSSL_LIB_CTX *libctx,
                                      const char *propq, uint32_t enc_level, uint32_t suite_id,
                                      ...
                                      unsigned char init_key_phase_bit, int is_tx)
{
    ...
    /* Derive "quic hp" key. */
    if (!tls13_hkdf_expand_ex(libctx, propq, md, secret, quic_v1_hp_label, sizeof(quic_v1_hp_label),
                              NULL, 0, hpr_key, hpr_key_len, 1))
        goto err;

    /* Setup KS0 (or KS1 if init_key_phase_bit), our initial keyslot. */
    if (!el_setup_keyslot(els, enc_level, QRL_EL_STATE_PROV_NORMAL, init_keyslot, secret,
                          secret_len))
        goto err;
    ...
    if (enc_level == QUIC_ENC_LEVEL_1RTT) {
        ...
        if (!tls13_hkdf_expand_ex(libctx, propq, md, secret, quic_v1_ku_label,
                                  sizeof(quic_v1_ku_label), NULL, 0, is_tx ? el->ku : ku_key,
                                  secret_len, 1))
            goto err;

        if (!is_tx) {
            /* Setup KS1 (or KS0 if init_key_phase_bit), our next keyslot. */
            if (!el_setup_keyslot(els, enc_level, QRL_EL_STATE_PROV_NORMAL, !init_keyslot, ku_key,
                                  secret_len))
                goto err;
    ...
    if (!ossl_quic_hdr_protector_init(&el->hpr, libctx, propq,
...
int
ossl_qrl_enc_level_set_key_update(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level)
{
    OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);
    size_t secret_len;
    ...
    if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, el->ku, quic_v1_ku_label,
                              sizeof(quic_v1_ku_label), NULL, 0, new_ku, secret_len, 1))
        return 0;
...
int
ossl_qrl_enc_level_set_key_update_done(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level)
{
    OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);
 ...
 */
int
ossl_qrl_enc_level_set_key_cooldown_done(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level)
{
    OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);
    size_t secret_len;
    unsigned char new_ku[EVP_MAX_KEY_LENGTH];
    ...
    if (!el_setup_keyslot(els, enc_level, QRL_EL_STATE_PROV_NORMAL, ~el->key_epoch & 1, el->ku,
                          secret_len))
        return 0;
