...
void
ossl_qlog_event_connectivity_connection_started(QLOG *qlog, const QUIC_CONN_ID *init_dcid)
{
...
#ifndef OPENSSL_NO_QLOG
static const char *
map_state_to_qlog(uint32_t state, int handshake_complete, int handshake_confirmed)
{
    switch (state) {
    default:
...
void
ossl_qlog_event_connectivity_connection_state_updated(QLOG *qlog, uint32_t old_state,
                                                      uint32_t new_state, int handshake_complete,
...
void
ossl_qlog_event_connectivity_connection_closed(QLOG *qlog, const QUIC_TERMINATE_CAUSE *tcause)
{
#ifndef OPENSSL_NO_QLOG
...
log_frame_actual(QLOG *qlog_instance, PACKET *pkt, size_t *need_skip)
{
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI: {
        ...
        QLOG_STR("stream_type", frame_type == OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI
                                    ? "bidirectional"
                                    : "unidirectional");
        QLOG_U64("limit", x);
    } break;
...
static void
log_packet(QLOG *qlog_instance, const QUIC_PKT_HDR *hdr, QUIC_PN pn, const OSSL_QTX_IOVEC *iovec,
...
void
ossl_qlog_event_transport_packet_sent(QLOG *qlog, const QUIC_PKT_HDR *hdr, QUIC_PN pn,
...
void
ossl_qlog_event_transport_packet_received(QLOG *qlog, const QUIC_PKT_HDR *hdr, QUIC_PN pn,
                                          const OSSL_QTX_IOVEC *iovec, size_t num_iovec,
                                          uint64_t datagram_id)
{
#ifndef OPENSSL_NO_QLOG
