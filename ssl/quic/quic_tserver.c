               ...
               unsigned int inlen, void *arg)
{
    ...
    if (SSL_select_next_proto((unsigned char **)out, outlen, alpn, alpnlen, in, inlen) !=
...
QUIC_TSERVER *
ossl_quic_tserver_new(const QUIC_TSERVER_ARGS *args, const char *certfile, const char *keyfile)
{
...
int
ossl_quic_tserver_set_plain_packet_mutator(QUIC_TSERVER *srv, ossl_mutate_packet_cb mutatecb,
...
{
    return ossl_statem_set_mutator(ossl_quic_channel_get0_ssl(srv->ch), mutate_handshake_cb,
...
int
ossl_quic_tserver_read(QUIC_TSERVER *srv, uint64_t stream_id, unsigned char *buf, size_t buf_len,
                       size_t *bytes_read)
{
    int is_fin = 0;
    QUIC_STREAM *qs;
...
int
ossl_quic_tserver_write(QUIC_TSERVER *srv, uint64_t stream_id, const unsigned char *buf,
...
int
ossl_quic_tserver_stream_has_peer_stop_sending(QUIC_TSERVER *srv, uint64_t stream_id,
...
int
ossl_quic_tserver_stream_has_peer_reset_stream(QUIC_TSERVER *srv, uint64_t stream_id,
                                               uint64_t *app_error_code)
{
    QUIC_STREAM *qs;
...
int
ossl_quic_tserver_set_new_local_cid(QUIC_TSERVER *srv, const QUIC_CONN_ID *conn_id)
{
...
ossl_quic_tserver_set_msg_callback(QUIC_TSERVER *srv,
                                   void (*f)(int write_p, int version, int content_type,
...
void
ossl_quic_tserver_set_psk_find_session_cb(QUIC_TSERVER *srv, SSL_psk_find_session_cb_func cb)
{
