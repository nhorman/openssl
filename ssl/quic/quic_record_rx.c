...
static int qrx_relocate_buffer(OSSL_QRX *qrx, RXE **prxe, size_t *pi, const unsigned char **pptr,
                               size_t buf_len);
static int qrx_validate_hdr(OSSL_QRX *qrx, RXE *rxe);
static RXE *qrx_reserve_rxe(RXE_LIST *rxl, RXE *rxe, size_t n);
static int qrx_decrypt_pkt_body(OSSL_QRX *qrx, unsigned char *dst, const unsigned char *src,
                                size_t src_len, size_t *dec_len, const unsigned char *aad,
...
ossl_qrx_inject_urxe(OSSL_QRX *qrx, QUIC_URXE *urxe)
{
    ...
    if (qrx->msg_callback != NULL)
        qrx->msg_callback(0, OSSL_QUIC1_VERSION, SSL3_RT_QUIC_DATAGRAM, urxe + 1, urxe->data_len,
 ...
 */
static int
qrx_validate_initial_pkt(OSSL_QRX *qrx, QUIC_URXE *urxe, const QUIC_CONN_ID *first_dcid,
                         size_t datagram_len)
{
    PACKET pkt, orig_pkt;
    RXE *rxe;
    ...
    if (!qrx_decrypt_pkt_body(qrx, dst, rxe->hdr.data, rxe->hdr.len, &dec_len, sop, aad_len,
                              rxe->pn, QUIC_ENC_LEVEL_INITIAL, rxe->hdr.key_phase, &rx_key_epoch))
        goto malformed;
...
int
ossl_qrx_validate_initial_packet(OSSL_QRX *qrx, QUIC_URXE *urxe, const QUIC_CONN_ID *dcid)
{
    urxe->processed = 0;
    urxe->hpr_removed = 0;
    urxe->deferred = 0;
...
int
ossl_qrx_provide_secret(OSSL_QRX *qrx, uint32_t enc_level, uint32_t suite_id, EVP_MD *md,
                        const unsigned char *secret, size_t secret_len)
{
    if (enc_level >= QUIC_ENC_LEVEL_NUM)
        return 0;

    if (!ossl_qrl_enc_level_set_provide_secret(&qrx->el_set, qrx->libctx, qrx->propq, enc_level,
                                               suite_id, md, secret, secret_len,
 ...
 */
static int
qrx_relocate_buffer(OSSL_QRX *qrx, RXE **prxe, size_t *pi, const unsigned char **pptr,
                    size_t buf_len)
{
    RXE *rxe;
    unsigned char *dst;
...
qrx_validate_hdr(OSSL_QRX *qrx, RXE *rxe)
{
    ...
    if (!ossl_quic_wire_decode_pkt_hdr_pn(rxe->hdr.pn, rxe->hdr.pn_len, qrx->largest_pn[pn_space],
                                          &rxe->pn))
        return 0;
 ...
 */
static size_t
qrx_get_cipher_ctx_idx(OSSL_QRX *qrx, OSSL_QRL_ENC_LEVEL *el, uint32_t enc_level,
 ...
 */
static int
qrx_decrypt_pkt_body(OSSL_QRX *qrx, unsigned char *dst, const unsigned char *src, size_t src_len,
                ...
                QUIC_CONN_ID *first_dcid, size_t datagram_len)
{
    ...
    if (!ossl_quic_wire_decode_pkt_hdr(pkt, qrx->short_conn_id_len, need_second_decode, 0,
                                       &rxe->hdr, &ptrs, NULL))
        goto malformed;
    ...
    *pkt = orig_pkt;
    ...
    if (need_second_decode) {
        ...
        /* Decode the now unprotected header. */
        if (ossl_quic_wire_decode_pkt_hdr(pkt, qrx->short_conn_id_len, 0, 0, &rxe->hdr, NULL,
                                          NULL) != 1)
            goto malformed;
    }
    ...
    dst = (unsigned char *)rxe_data(rxe) + i;
    if (!qrx_decrypt_pkt_body(qrx, dst, rxe->hdr.data, rxe->hdr.len, &dec_len, sop, aad_len,
                              rxe->pn, enc_level, rxe->hdr.key_phase, &rx_key_epoch))
        goto malformed;
...
static int
qrx_process_datagram(OSSL_QRX *qrx, QUIC_URXE *e, const unsigned char *data, size_t data_len)
{
    int have_deferred = 0;
    PACKET pkt;
    size_t pkt_idx = 0;
...
int
ossl_qrx_set_late_validation_cb(OSSL_QRX *qrx, ossl_qrx_late_validation_cb *cb, void *cb_arg)
{
...
int
ossl_qrx_set_key_update_cb(OSSL_QRX *qrx, ossl_qrx_key_update_cb *cb, void *cb_arg)
{
...
void
ossl_qrx_set_msg_callback(OSSL_QRX *qrx, ossl_msg_cb msg_callback, SSL *msg_callback_ssl)
{
