...
{
    assert(n->prev != NULL && n->next != NULL && (n == l || (n->prev != n && n->next != n)) &&
...
#define ready_for_gc_next(l, s)                                                                    \
    list_next((l), &(s)->ready_for_gc_node, offsetof(QUIC_STREAM, ready_for_gc_node))
...
int
ossl_quic_stream_map_init(QUIC_STREAM_MAP *qsm, uint64_t (*get_stream_limit_cb)(int uni, void *arg),
...
void
ossl_quic_stream_map_visit(QUIC_STREAM_MAP *qsm, void (*visit_cb)(QUIC_STREAM *stream, void *arg),
                           void *visit_cb_arg)
{
...
ossl_quic_stream_map_alloc(QUIC_STREAM_MAP *qsm, uint64_t stream_id, int type)
{
    ...
    s->send_state = (ossl_quic_stream_is_local_init(s) || ossl_quic_stream_is_bidi(s))
                        ? QUIC_SSTREAM_STATE_READY
                        : QUIC_SSTREAM_STATE_NONE;
    s->recv_state = (!ossl_quic_stream_is_local_init(s) || ossl_quic_stream_is_bidi(s))
...
int
ossl_quic_stream_map_is_local_allowed_by_stream_limit(QUIC_STREAM_MAP *qsm, uint64_t stream_ordinal,
                                                      int is_uni)
{
...
ossl_quic_stream_map_update_state(QUIC_STREAM_MAP *qsm, QUIC_STREAM *s)
{
    ...
    if (ossl_quic_stream_is_server_init(s) == qsm->is_server) {
        ...
        allowed_by_stream_limit =
            ossl_quic_stream_map_is_local_allowed_by_stream_limit(qsm, stream_ordinal, is_uni);
    }
...
int
ossl_quic_stream_map_reset_stream_send_part(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs, uint64_t aec)
{
    switch (qs->send_state) {
    default:
    case QUIC_SSTREAM_STATE_NONE:
...
int
ossl_quic_stream_map_notify_reset_stream_acked(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs)
{
    switch (qs->send_state) {
    default:
        /* Wrong state - caller error. */
...
int
ossl_quic_stream_map_notify_size_known_recv_part(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs,
                                                 uint64_t final_size)
{
...
int
ossl_quic_stream_map_notify_totally_received(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs)
{
    switch (qs->recv_state) {
    default:
        /* Wrong state - caller error. */
...
int
ossl_quic_stream_map_notify_reset_recv_part(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs,
                                            uint64_t app_error_code, uint64_t final_size)
{
...
int
ossl_quic_stream_map_notify_app_read_reset_recv_part(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs)
{
    switch (qs->recv_state) {
    default:
        /* Wrong state - caller error. */
...
int
ossl_quic_stream_map_stop_sending_recv_part(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs, uint64_t aec)
{
    if (qs->stop_sending)
        return 0;
...
int
ossl_quic_stream_map_schedule_stop_sending(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs)
{
    if (!qs->stop_sending)
        return 0;
...
void
ossl_quic_stream_map_remove_from_accept_queue(QUIC_STREAM_MAP *qsm, QUIC_STREAM *s, OSSL_TIME rtt)
{
...
ossl_quic_stream_map_gc(QUIC_STREAM_MAP *qsm)
{
    ...
    for (qs = qs_head = ready_for_gc_head(&qsm->ready_for_gc_list); qs != NULL && qs != qs_head;
         qs = qsn) {
 ...
 */
void
ossl_quic_stream_iter_init(QUIC_STREAM_ITER *it, QUIC_STREAM_MAP *qsm, int advance_rr)
{
    it->qsm = qsm;
