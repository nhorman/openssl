...
static void
list_insert_after(QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *ref, QUIC_CFQ_ITEM_EX *n)
{
    n->prev = ref;
    n->next = ref->next;
    if (ref->next != NULL)
...
QUIC_CFQ_ITEM *
ossl_quic_cfq_add_frame(QUIC_CFQ *cfq, uint32_t priority, uint32_t pn_space, uint64_t frame_type,
...
QUIC_CFQ_ITEM *
ossl_quic_cfq_item_get_priority_next(const QUIC_CFQ_ITEM *item, uint32_t pn_space)
{
    QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
