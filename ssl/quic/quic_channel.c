...
static int ch_on_handshake_yield_secret(uint32_t prot_level, int direction, uint32_t suite_id,
...
static void ch_start_terminating(QUIC_CHANNEL *ch, const QUIC_TERMINATE_CAUSE *tcause,
                                 int force_immediate);
...
ch_init(QUIC_CHANNEL *ch)
{
    ...
    if (!ch->is_server &&
        !ossl_quic_gen_rand_conn_id(ch->port->engine->libctx, tx_init_dcid_len, &ch->init_dcid))
        goto err;
    ...
    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        if (!ossl_quic_rxfc_init_standalone(&ch->crypto_rxfc[pn_space], INIT_CRYPTO_RECV_BUF_LEN,
                                            get_time, ch))
            goto err;

    if (!ossl_quic_rxfc_init_standalone(&ch->max_streams_bidi_rxfc, DEFAULT_INIT_CONN_MAX_STREAMS,
                                        get_time, ch))
        goto err;

    if (!ossl_quic_rxfc_init_standalone(&ch->max_streams_uni_rxfc, DEFAULT_INIT_CONN_MAX_STREAMS,
                                        get_time, ch))
        goto err;
...
int
ossl_quic_channel_set_mutator(QUIC_CHANNEL *ch, ossl_mutate_packet_cb mutatecb,
...
int
ossl_quic_channel_schedule_new_token(QUIC_CHANNEL *ch, const unsigned char *token, size_t token_len)
{
    int rc = 0;
    ...
    cfq_item =
        ossl_quic_cfq_add_frame(ch->cfq, 1, QUIC_PN_SPACE_APP, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN, 0,
...
rxku_detected(QUIC_PN pn, void *arg)
{
    ...
    if (decision == DECISION_PROTOCOL_VIOLATION) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_KEY_UPDATE_ERROR, 0,
 ...
 */
static int
ch_on_crypto_send(const unsigned char *buf, size_t buf_len, size_t *consumed, void *arg)
{
    int ret;
    QUIC_CHANNEL *ch = arg;
...
static int
ch_on_crypto_recv_record(const unsigned char **buf, size_t *bytes_read, void *arg)
{
    QUIC_CHANNEL *ch = arg;
...
static int
ch_on_handshake_yield_secret(uint32_t prot_level, int direction, uint32_t suite_id, EVP_MD *md,
...
ch_on_handshake_alert(void *arg, unsigned char alert_code)
{
     ...
     */
        ...
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CRYPTO_ERR_BEGIN + alert_code, 0,
                                               "handshake alert");
...
static int
ch_on_transport_params(const unsigned char *params, size_t params_len, void *arg)
{
    QUIC_CHANNEL *ch = arg;
    PACKET pkt;
    uint64_t id, v;
    ...
    while (PACKET_remaining(&pkt) > 0) {
        ...
        case QUIC_TPARAM_MAX_IDLE_TIMEOUT:
            ...
            ch->max_idle_timeout =
                min_u64_ignore_0(ch->max_idle_timeout_local_req, ch->max_idle_timeout_remote_req);
...
ch_generate_transport_params(QUIC_CHANNEL *ch)
{
    ...
    if (ch->is_server) {
        ...
        if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_INITIAL_SCID,
            ...
            goto err;
        if (ch->odcid.id_len != 0)
            if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_RETRY_SCID,
                                                           &ch->init_dcid))
                goto err;
    } else {
        if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_INITIAL_SCID,
 ...
 */
void
ossl_quic_channel_subtick(QUIC_CHANNEL *ch, QUIC_TICK_RESULT *res, uint32_t flags)
{
    ...
    res->net_write_desired =
        (!ossl_quic_channel_is_terminated(ch) && ossl_qtx_get_queue_len_datagrams(ch->qtx) > 0);
...
ch_rx_handle_packet(QUIC_CHANNEL *ch, int channel_only)
{
    ...
    if (ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
         ...
         */
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CONNECTION_REFUSED, 0,
    ...
    ossl_qlog_event_transport_packet_received(ch_get_qlog(ch), ch->qrx_pkt->hdr, ch->qrx_pkt->pn,
                                              &iovec, 1, ch->qrx_pkt->datagram_id);
    ...
    case QUIC_PKT_TYPE_RETRY:
        ...
        if (!ossl_quic_validate_retry_integrity_tag(ch->port->engine->libctx,
                                                    ch->port->engine->propq, ch->qrx_pkt->hdr,
                                                    &ch->init_dcid))
    ...
    for (i = 0; i < QUIC_ENC_LEVEL_NUM; i++) {
        if (ossl_qtx_is_enc_level_provisioned(ch->qtx, i)) {
            deadline = ossl_time_min(
                deadline, ossl_ackm_get_ack_deadline(ch->ackm, ossl_quic_enc_level_to_pn_space(i)));
        }
    }
...
ch_record_state_transition(QUIC_CHANNEL *ch, uint32_t new_state)
{
    ...
    ossl_qlog_event_connectivity_connection_state_updated(
        ch_get_qlog(ch), old_state, new_state, ch->handshake_complete, ch->handshake_confirmed);
}
...
ossl_quic_channel_start(QUIC_CHANNEL *ch)
{
     ...
     */
    if (!ch->is_server &&
        ossl_quic_get_peer_token(ch->port->channel_ctx, &ch->cur_peer_addr, &token) &&
        !ossl_quic_tx_packetiser_set_initial_token(ch->txp, token->token, token->token_len,
                                                   free_peer_token, token))
        free_peer_token(NULL, 0, token);

    /* Plug in secrets for the Initial EL. */
    if (!ossl_quic_provide_initial_secret(ch->port->engine->libctx, ch->port->engine->propq,
...
void
ossl_quic_channel_local_close(QUIC_CHANNEL *ch, uint64_t app_error_code, const char *app_reason)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
...
static int
ch_retry(QUIC_CHANNEL *ch, const unsigned char *retry_token, size_t retry_token_len,
         const QUIC_CONN_ID *retry_scid, int drop_later_pn)
{
    void *buf;
    QUIC_PN pn = 0;
    ...
    if (!ossl_quic_tx_packetiser_set_initial_token(ch->txp, buf, retry_token_len, free_token,
                                                   NULL)) {
        /*
...
static void
ch_start_terminating(QUIC_CHANNEL *ch, const QUIC_TERMINATE_CAUSE *tcause, int force_immediate)
{
    ...
    case QUIC_CHANNEL_STATE_ACTIVE:
        ...
        if (!force_immediate) {
            ch_record_state_transition(ch, tcause->remote ? QUIC_CHANNEL_STATE_TERMINATING_DRAINING
...
/* For RXDP use. */
void
ossl_quic_channel_on_remote_conn_close(QUIC_CHANNEL *ch, OSSL_QUIC_FRAME_CONN_CLOSE *f)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
...
ch_enqueue_retire_conn_id(QUIC_CHANNEL *ch, uint64_t seq_num)
{
    ...
    if (ossl_quic_cfq_add_frame(ch->cfq, 1, QUIC_PN_SPACE_APP, OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID,
                                0, (unsigned char *)buf_mem->data, l, free_frame_data,
                                NULL) == NULL)
        goto err;
...
void
ossl_quic_channel_on_new_conn_id(QUIC_CHANNEL *ch, OSSL_QUIC_FRAME_NEW_CONN_ID *f)
{
...
void
ossl_quic_channel_raise_protocol_error_loc(QUIC_CHANNEL *ch, uint64_t error_code,
                                           uint64_t frame_type, const char *reason,
                                           ERR_STATE *err_state, const char *src_file, int src_line,
                                           const char *src_func)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
    int err_reason = error_code == OSSL_QUIC_ERR_INTERNAL_ERROR ? ERR_R_INTERNAL_ERROR
 ...
 */
static int
ch_on_new_conn_common(QUIC_CHANNEL *ch, const BIO_ADDR *peer, const QUIC_CONN_ID *peer_scid,
                      const QUIC_CONN_ID *peer_dcid, const QUIC_CONN_ID *peer_odcid)
{
    ...
    if (!ossl_quic_lcidm_enrol_odcid(ch->lcidm, ch,
                                     peer_odcid == NULL ? &ch->init_dcid : peer_odcid))
        return 0;
...
int
ossl_quic_channel_on_new_conn(QUIC_CHANNEL *ch, const BIO_ADDR *peer, const QUIC_CONN_ID *peer_scid,
 ...
 */
int
ossl_quic_bind_channel(QUIC_CHANNEL *ch, const BIO_ADDR *peer, const QUIC_CONN_ID *peer_scid,
                       const QUIC_CONN_ID *peer_dcid, const QUIC_CONN_ID *peer_odcid)
{
    if (peer_dcid == NULL)
        return 0;
...
static int
ch_init_new_stream(QUIC_CHANNEL *ch, QUIC_STREAM *qs, int can_send, int can_recv)
{
    uint64_t rxfc_wnd;
    ...
    if (!ossl_quic_rxfc_init(&qs->rxfc, &ch->conn_rxfc, rxfc_wnd,
                             DEFAULT_STREAM_RXFC_MAX_WND_MUL * rxfc_wnd, get_time, ch))
        goto err;
...
uint64_t
ossl_quic_channel_get_local_stream_count_avail(const QUIC_CHANNEL *ch, int is_uni)
{
...
uint64_t
ossl_quic_channel_get_remote_stream_count_avail(const QUIC_CHANNEL *ch, int is_uni)
{
...
ossl_quic_channel_new_stream_remote(QUIC_CHANNEL *ch, uint64_t stream_id)
{
    ...
    qs = ossl_quic_stream_map_alloc(
        &ch->qsm, stream_id, stream_id & (QUIC_STREAM_INITIATOR_MASK | QUIC_STREAM_DIR_MASK));
    if (qs == NULL)
        return NULL;
...
void
ossl_quic_channel_set_incoming_stream_auto_reject(QUIC_CHANNEL *ch, int enable, uint64_t aec)
{
...
int
ossl_quic_channel_replace_local_cid(QUIC_CHANNEL *ch, const QUIC_CONN_ID *conn_id)
{
...
void
ossl_quic_channel_set_txku_threshold_override(QUIC_CHANNEL *ch, uint64_t tx_pkt_threshold)
{
