...
wbuf_flush(struct json_write_buf *wbuf, int full)
{
    ...
    while (total_written < wbuf->cur) {
        if (!BIO_write_ex(wbuf->bio, wbuf->buf + total_written, wbuf->cur - total_written,
                          &written)) {
...
json_push(OSSL_JSON_ENC *json, unsigned int v)
{
    ...
    if (json->stack_end_byte >= json->stack_bytes) {
        size_t new_size =
            (json->stack_bytes == 0) ? OSSL_NELEM(json->stack_small) : (json->stack_bytes * 2);
...
void
ossl_json_bool(OSSL_JSON_ENC *json, int v)
{
    if (!json_pre_item(json))
        return;
...
static ossl_inline void
json_write_qstring_inner(OSSL_JSON_ENC *json, const char *str, size_t str_len, int nul_term)
{
    char c, *o, obuf[7];
    unsigned char *u_str;
    int i;
    ...
    for (j = nul_term ? strlen(str) : str_len; j > 0; str++, j--) {
        ...
        default:
            ...
            if (u_str[0] >= 0xf0 && u_str[0] <= 0xf4 && j >= 4 && u_str[1] >= 0x80 &&
                u_str[1] <= 0xbf && u_str[2] >= 0x80 && u_str[2] <= 0xbf && u_str[3] >= 0x80 &&
                u_str[3] <= 0xbf && !(u_str[0] == 0xf0 && u_str[1] <= 0x8f) &&
