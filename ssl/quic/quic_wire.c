...
{
    uint64_t frame_type =
        ack->ecn_present ? OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN : OSSL_QUIC_FRAME_TYPE_ACK_WITHOUT_ECN;
    ...
    if (ack->ecn_present)
        if (!WPACKET_quic_write_vlint(pkt, ack->ect0) ||
            !WPACKET_quic_write_vlint(pkt, ack->ect1) || !WPACKET_quic_write_vlint(pkt, ack->ecnce))
            return 0;
...
int
ossl_quic_wire_encode_frame_reset_stream(WPACKET *pkt, const OSSL_QUIC_FRAME_RESET_STREAM *f)
{
...
int
ossl_quic_wire_encode_frame_stop_sending(WPACKET *pkt, const OSSL_QUIC_FRAME_STOP_SENDING *f)
{
...
int
ossl_quic_wire_encode_frame_crypto_hdr(WPACKET *pkt, const OSSL_QUIC_FRAME_CRYPTO *f)
{
    if (!encode_frame_hdr(pkt, OSSL_QUIC_FRAME_TYPE_CRYPTO) ||
        !WPACKET_quic_write_vlint(pkt, f->offset) || !WPACKET_quic_write_vlint(pkt, f->len))
        return 0;
...
void *
ossl_quic_wire_encode_frame_crypto(WPACKET *pkt, const OSSL_QUIC_FRAME_CRYPTO *f)
{
...
int
ossl_quic_wire_encode_frame_new_token(WPACKET *pkt, const unsigned char *token, size_t token_len)
{
    if (!encode_frame_hdr(pkt, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN) ||
        !WPACKET_quic_write_vlint(pkt, token_len) || !WPACKET_memcpy(pkt, token, token_len))
        return 0;
...
int
ossl_quic_wire_encode_frame_stream_hdr(WPACKET *pkt, const OSSL_QUIC_FRAME_STREAM *f)
{
...
void *
ossl_quic_wire_encode_frame_stream(WPACKET *pkt, const OSSL_QUIC_FRAME_STREAM *f)
{
...
int
ossl_quic_wire_encode_frame_max_stream_data(WPACKET *pkt, uint64_t stream_id, uint64_t max_data)
{
    if (!encode_frame_hdr(pkt, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA) ||
        !WPACKET_quic_write_vlint(pkt, stream_id) || !WPACKET_quic_write_vlint(pkt, max_data))
        return 0;
...
int
ossl_quic_wire_encode_frame_max_streams(WPACKET *pkt, char is_uni, uint64_t max_streams)
{
...
int
ossl_quic_wire_encode_frame_stream_data_blocked(WPACKET *pkt, uint64_t stream_id,
...
int
ossl_quic_wire_encode_frame_streams_blocked(WPACKET *pkt, char is_uni, uint64_t max_streams)
{
...
int
ossl_quic_wire_encode_frame_new_conn_id(WPACKET *pkt, const OSSL_QUIC_FRAME_NEW_CONN_ID *f)
{
...
int
ossl_quic_wire_encode_frame_conn_close(WPACKET *pkt, const OSSL_QUIC_FRAME_CONN_CLOSE *f)
{
...
unsigned char *
ossl_quic_wire_encode_transport_param_bytes(WPACKET *pkt, uint64_t id, const unsigned char *value,
                                            size_t value_len)
{
...
int
ossl_quic_wire_encode_transport_param_int(WPACKET *pkt, uint64_t id, uint64_t value)
{
...
int
ossl_quic_wire_encode_transport_param_cid(WPACKET *wpkt, uint64_t id, const QUIC_CONN_ID *cid)
{
...
static int
expect_frame_header_mask(PACKET *pkt, uint64_t expected_frame_type, uint64_t mask_bits,
...
int
ossl_quic_wire_peek_frame_ack_num_ranges(const PACKET *orig_pkt, uint64_t *total_ranges)
{
    PACKET pkt = *orig_pkt;
...
int
ossl_quic_wire_decode_frame_ack(PACKET *pkt, uint32_t ack_delay_exponent, OSSL_QUIC_FRAME_ACK *ack,
...
int
ossl_quic_wire_decode_frame_reset_stream(PACKET *pkt, OSSL_QUIC_FRAME_RESET_STREAM *f)
{
...
int
ossl_quic_wire_decode_frame_stop_sending(PACKET *pkt, OSSL_QUIC_FRAME_STOP_SENDING *f)
{
...
int
ossl_quic_wire_decode_frame_crypto(PACKET *pkt, int nodata, OSSL_QUIC_FRAME_CRYPTO *f)
{
    if (!expect_frame_header(pkt, OSSL_QUIC_FRAME_TYPE_CRYPTO) ||
        !PACKET_get_quic_vlint(pkt, &f->offset) || !PACKET_get_quic_vlint(pkt, &f->len) ||
        f->len > SIZE_MAX /* sizeof(uint64_t) > sizeof(size_t)? */)
        return 0;
...
int
ossl_quic_wire_decode_frame_new_token(PACKET *pkt, const unsigned char **token, size_t *token_len)
{
    uint64_t token_len_;
...
int
ossl_quic_wire_decode_frame_stream(PACKET *pkt, int nodata, OSSL_QUIC_FRAME_STREAM *f)
{
    uint64_t frame_type;
                                            ...
                                            uint64_t *max_stream_data)
{
    if (!expect_frame_header(pkt, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA) ||
        !PACKET_get_quic_vlint(pkt, stream_id) || !PACKET_get_quic_vlint(pkt, max_stream_data))
        return 0;
...
int
ossl_quic_wire_decode_frame_stream_data_blocked(PACKET *pkt, uint64_t *stream_id,
                                                uint64_t *max_stream_data)
{
...
int
ossl_quic_wire_decode_frame_new_conn_id(PACKET *pkt, OSSL_QUIC_FRAME_NEW_CONN_ID *f)
{
    unsigned int len;
...
int
ossl_quic_wire_decode_frame_conn_close(PACKET *pkt, OSSL_QUIC_FRAME_CONN_CLOSE *f)
{
...
const unsigned char *
ossl_quic_wire_decode_transport_param_bytes(PACKET *pkt, uint64_t *id, size_t *len)
{
    uint64_t len_;
...
int
ossl_quic_wire_decode_transport_param_int(PACKET *pkt, uint64_t *id, uint64_t *value)
{
    PACKET sub;
...
int
ossl_quic_wire_decode_transport_param_cid(PACKET *pkt, uint64_t *id, QUIC_CONN_ID *cid)
{
...
int
ossl_quic_wire_decode_transport_param_preferred_addr(PACKET *pkt, QUIC_PREFERRED_ADDR *p)
{
    const unsigned char *body;
    uint64_t id;
    size_t len = 0;
    ...
    if (body == NULL || len < QUIC_MIN_ENCODED_PREFERRED_ADDR_LEN ||
        len > QUIC_MAX_ENCODED_PREFERRED_ADDR_LEN || id != QUIC_TPARAM_PREFERRED_ADDR)
        return 0;
    ...
    if (!PACKET_copy_bytes(&pkt2, p->ipv4, sizeof(p->ipv4)) ||
        ...
        cidl > QUIC_MAX_CONN_ID_LEN || !PACKET_copy_bytes(&pkt2, p->cid.id, cidl) ||
