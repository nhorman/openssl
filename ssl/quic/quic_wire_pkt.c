...
int
ossl_quic_hdr_protector_init(QUIC_HDR_PROTECTOR *hpr, OSSL_LIB_CTX *libctx, const char *propq,
...
static int
hdr_generate_mask(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample, size_t sample_len,
                  unsigned char *mask)
{
    int l = 0;
...
int
ossl_quic_hdr_protector_decrypt(QUIC_HDR_PROTECTOR *hpr, QUIC_PKT_HDR_PTRS *ptrs)
{
    return ossl_quic_hdr_protector_decrypt_fields(hpr, ptrs->raw_sample, ptrs->raw_sample_len,
                                                  ptrs->raw_start, ptrs->raw_pn);
}

int
ossl_quic_hdr_protector_decrypt_fields(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample,
...
int
ossl_quic_hdr_protector_encrypt(QUIC_HDR_PROTECTOR *hpr, QUIC_PKT_HDR_PTRS *ptrs)
{
    return ossl_quic_hdr_protector_encrypt_fields(hpr, ptrs->raw_sample, ptrs->raw_sample_len,
                                                  ptrs->raw_start, ptrs->raw_pn);
}

int
ossl_quic_hdr_protector_encrypt_fields(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample,
                                       size_t sample_len, unsigned char *first_byte,
                                       unsigned char *pn_bytes)
{
    unsigned char mask[5], pn_len, i;
...
int
ossl_quic_wire_decode_pkt_hdr(PACKET *pkt, size_t short_conn_id_len, int partial, int nodata,
                              QUIC_PKT_HDR *hdr, QUIC_PKT_HDR_PTRS *ptrs, uint64_t *fail_cause)
{
    unsigned int b0;
    ...
    if ((b0 & 0x80) == 0) {
        ...
        if (!PACKET_get_1(pkt, &dst_conn_id_len) || dst_conn_id_len > QUIC_MAX_CONN_ID_LEN ||
            ...
            !PACKET_get_1(pkt, &src_conn_id_len) || src_conn_id_len > QUIC_MAX_CONN_ID_LEN ||
...
int
ossl_quic_wire_encode_pkt_hdr(WPACKET *pkt, size_t short_conn_id_len, const QUIC_PKT_HDR *hdr,
                              QUIC_PKT_HDR_PTRS *ptrs)
{
    ...
    if (hdr->partial ||
        (hdr->type == QUIC_PKT_TYPE_1RTT && hdr->dst_conn_id.id_len != short_conn_id_len))
        return 0;
    ...
    if (hdr->type == QUIC_PKT_TYPE_1RTT) {
        ...
        if (hdr->dst_conn_id.id_len != short_conn_id_len ||
            short_conn_id_len > QUIC_MAX_CONN_ID_LEN || hdr->pn_len < 1 || hdr->pn_len > 4)
            return 0;
...
int
ossl_quic_wire_get_encoded_pkt_hdr_len(size_t short_conn_id_len, const QUIC_PKT_HDR *hdr)
{
    size_t len = 0, enclen;
...
int
ossl_quic_wire_get_pkt_hdr_dst_conn_id(const unsigned char *buf, size_t buf_len,
...
int
ossl_quic_wire_decode_pkt_hdr_pn(const unsigned char *enc_pn, size_t enc_pn_len, QUIC_PN largest_pn,
                                 QUIC_PN *res_pn)
{
...
int
ossl_quic_wire_encode_pkt_hdr_pn(QUIC_PN pn, unsigned char *enc_pn, size_t enc_pn_len)
{
    switch (enc_pn_len) {
    case 1:
...
int
ossl_quic_validate_retry_integrity_tag(OSSL_LIB_CTX *libctx, const char *propq,
...
int
ossl_quic_calculate_retry_integrity_tag(OSSL_LIB_CTX *libctx, const char *propq,
                                        const QUIC_PKT_HDR *hdr,
                                        const QUIC_CONN_ID *client_initial_dcid, unsigned char *tag)
{
    ...
    if (!EVP_CipherInit_ex(cctx, cipher, NULL, retry_integrity_key, retry_integrity_nonce,
                           /*enc=*/1)) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }
