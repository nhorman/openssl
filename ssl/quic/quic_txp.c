...
static int
tx_helper_init(struct tx_helper *h, OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level,
 ...
 */
static int
tx_helper_append_iovec(struct tx_helper *h, const unsigned char *buf, size_t buf_len)
{
...
static void on_regen_notify(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
                            void *arg);
static void on_confirm_notify(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
                              void *arg);
static void on_sstream_updated(uint64_t stream_id, void *arg);
static int sstream_is_pending(QUIC_SSTREAM *sstream);
static int txp_should_try_staging(OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level,
...
static int txp_determine_ppl_from_pl(OSSL_QUIC_TX_PACKETISER *txp, size_t pl, uint32_t enc_level,
                                     size_t hdr_len, size_t *r);
static size_t txp_get_mdpl(OSSL_QUIC_TX_PACKETISER *txp);
static int txp_generate_for_el(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                               int chosen_for_conn_close);
static int txp_pkt_init(struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level,
                        uint32_t archetype, size_t running_total);
...
static int txp_pkt_append_padding(struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp,
                                  size_t num_bytes);
static int txp_pkt_commit(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt, uint32_t archetype,
 ...
 */
void
ossl_quic_tx_packetiser_add_unvalidated_credit(OSSL_QUIC_TX_PACKETISER *txp, size_t credit)
{
 ...
 */
void
ossl_quic_tx_packetiser_consume_unvalidated_credit(OSSL_QUIC_TX_PACKETISER *txp, size_t credit)
{
    if (txp->unvalidated_credit != SIZE_MAX) {
 ...
 */
int
ossl_quic_tx_packetiser_check_unvalidated_credit(OSSL_QUIC_TX_PACKETISER *txp, size_t req_credit)
{
...
ossl_quic_tx_packetiser_new(const OSSL_QUIC_TX_PACKETISER_ARGS *args)
{
    ...
    if (args == NULL || args->qtx == NULL || args->txpim == NULL || args->cfq == NULL ||
        args->ackm == NULL || args->qsm == NULL || args->conn_txfc == NULL ||
        args->conn_rxfc == NULL || args->max_streams_bidi_rxfc == NULL ||
    ...
    if (!ossl_quic_fifd_init(&txp->fifd, txp->args.cfq, txp->args.ackm, txp->args.txpim,
...
int
ossl_quic_tx_packetiser_set_initial_token(OSSL_QUIC_TX_PACKETISER *txp, const unsigned char *token,
                                          size_t token_len,
...
int
ossl_quic_tx_packetiser_set_cur_dcid(OSSL_QUIC_TX_PACKETISER *txp, const QUIC_CONN_ID *dcid)
{
    if (dcid == NULL) {
        ERR_raise(ERR_LIB_SSL, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
...
int
ossl_quic_tx_packetiser_set_cur_scid(OSSL_QUIC_TX_PACKETISER *txp, const QUIC_CONN_ID *scid)
{
    if (scid == NULL) {
        ERR_raise(ERR_LIB_SSL, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
...
int
ossl_quic_tx_packetiser_set_peer(OSSL_QUIC_TX_PACKETISER *txp, const BIO_ADDR *peer)
{
    if (peer == NULL) {
...
void
ossl_quic_tx_packetiser_set_qlog_cb(OSSL_QUIC_TX_PACKETISER *txp, QLOG *(*get_qlog_cb)(void *arg),
...
int
ossl_quic_tx_packetiser_discard_enc_level(OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level)
{
...
void
ossl_quic_tx_packetiser_schedule_ack_eliciting(OSSL_QUIC_TX_PACKETISER *txp, uint32_t pn_space)
{
    txp->force_ack_eliciting |= (1UL << pn_space);
}

void
ossl_quic_tx_packetiser_schedule_ack(OSSL_QUIC_TX_PACKETISER *txp, uint32_t pn_space)
{
 ...
 */
int
ossl_quic_tx_packetiser_generate(OSSL_QUIC_TX_PACKETISER *txp, QUIC_TXP_STATUS *status)
{
    /*
    ...
    for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM; ++enc_level) {
        ...
        pkt[enc_level].geom.hwm =
            running_total + pkt[enc_level].h.bytes_appended + pkt[enc_level].geom.pkt_overhead;
    }
    ...
    for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM; ++enc_level) {
        ...
        if (rc) {
            status->sent_ack_eliciting =
                status->sent_ack_eliciting || pkt[enc_level].tpkt->ackm_pkt.is_ack_eliciting;
...
static int
txp_get_archetype_data(uint32_t enc_level, uint32_t archetype, struct archetype_data *a)
{
...
static int
txp_determine_geometry(OSSL_QUIC_TX_PACKETISER *txp, uint32_t archetype, uint32_t enc_level,
...
static int
txp_should_try_staging(OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level, uint32_t archetype,
                       uint64_t cc_limit, uint32_t *conn_close_enc_level)
{
    ...
    if (a.allow_conn_fc &&
        (txp->want_max_data || ossl_quic_rxfc_has_cwm_changed(txp->args.conn_rxfc, 0)))
        return 1;
...
static int
txp_determine_ppl_from_pl(OSSL_QUIC_TX_PACKETISER *txp, size_t pl, uint32_t enc_level,
                          size_t hdr_len, size_t *r)
{
    if (pl < hdr_len)
        return 0;
...
static void
on_regen_notify(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
    OSSL_QUIC_TX_PACKETISER *txp = arg;
...
static int
txp_need_ping(OSSL_QUIC_TX_PACKETISER *txp, uint32_t pn_space, const struct archetype_data *adata)
{
    return adata->allow_ping &&
           (adata->require_ack_eliciting || (txp->force_ack_eliciting & (1UL << pn_space)) != 0);
}

static int
txp_pkt_init(struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level,
             uint32_t archetype, size_t running_total)
{
    uint32_t pn_space = ossl_quic_enc_level_to_pn_space(enc_level);
...
static int
txp_pkt_postgen_update_pkt_overhead(struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp)
{
    /*
    ...
    if (!ossl_qtx_calculate_ciphertext_payload_len(txp->args.qtx, pkt->h.enc_level,
                                                   pkt->h.bytes_appended, &ciphertext_len))
        return 0;
...
static void
on_confirm_notify(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
    OSSL_QUIC_TX_PACKETISER *txp = arg;
...
static int
txp_pkt_append_padding(struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp, size_t num_bytes)
{
    WPACKET *wpkt;
...
void
ossl_quic_tx_packetiser_record_received_closing_bytes(OSSL_QUIC_TX_PACKETISER *txp, size_t n)
{
    txp->closing_bytes_recv += n;
}

static int
txp_generate_pre_token(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt, int chosen_for_conn_close,
...
static int
try_len(size_t space_left, size_t orig_len, size_t base_hdr_len, size_t lenbytes, uint64_t maxn,
...
static int
determine_len(size_t space_left, size_t orig_len, size_t base_hdr_len, uint64_t *hlen,
              uint64_t *len)
{
    int ok = 0;
    ...
    valid[0] = try_len(space_left, orig_len, base_hdr_len, 1, OSSL_QUIC_VLINT_1B_MAX, &hdr_len[0],
                       &payload_len[0]);
    valid[1] = try_len(space_left, orig_len, base_hdr_len, 2, OSSL_QUIC_VLINT_2B_MAX, &hdr_len[1],
                       &payload_len[1]);
    valid[2] = try_len(space_left, orig_len, base_hdr_len, 4, OSSL_QUIC_VLINT_4B_MAX, &hdr_len[2],
                       &payload_len[2]);
    valid[3] = try_len(space_left, orig_len, base_hdr_len, 8, OSSL_QUIC_VLINT_8B_MAX, &hdr_len[3],
                       &payload_len[3]);
 ...
 */
static int
determine_crypto_len(struct tx_helper *h, OSSL_QUIC_FRAME_CRYPTO *chdr, size_t space_left,
...
static int
determine_stream_len(struct tx_helper *h, OSSL_QUIC_FRAME_STREAM *shdr, size_t space_left,
                     uint64_t *hlen, uint64_t *len)
{
    size_t orig_len;
                           ...
                           int *have_ack_eliciting)
{
    ...
    for (i = 0;; ++i) {
        ...
        if (!ossl_quic_sstream_get_stream_frame(txp->args.crypto[pn_space], i, &shdr, iov,
                                                &num_stream_iovec))
...
static int
txp_plan_stream_chunk(OSSL_QUIC_TX_PACKETISER *txp, struct tx_helper *h, QUIC_SSTREAM *sstream,
                      QUIC_TXFC *stream_txfc, size_t skip, struct chunk_info *chunk,
                      uint64_t consumed)
{
    uint64_t fc_credit, fc_swm, fc_limit;

    chunk->num_stream_iovec = OSSL_NELEM(chunk->iov);
    chunk->valid = ossl_quic_sstream_get_stream_frame(sstream, skip, &chunk->shdr, chunk->iov,
 ...
 */
static int
txp_generate_stream_frames(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt, uint64_t id,
                           ...
                           uint64_t conn_consumed)
{
    ...
    for (i = 0;; ++i) {
        ...
        if (i > 0)
            ...
            if (!txp_plan_stream_chunk(txp, h, sstream, stream_txfc, i + 1, &chunks[(i + 1) % 2],
                                       conn_consumed))
                goto err;
                            ...
                            int *have_ack_eliciting, QUIC_STREAM **tmp_head)
{
    ...
    for (ossl_quic_stream_iter_init(&it, txp->args.qsm, 1); it.stream != NULL;) {
        ...
        if (ossl_quic_stream_has_send_buffer(stream) && !ossl_quic_stream_send_is_reset(stream)) {
            ...
            if (!txp_generate_stream_frames(txp, pkt, stream->id, stream->sstream, &stream->txfc,
                                            snext, have_ack_eliciting, &packet_full,
...
static int
txp_generate_for_el(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt, int chosen_for_conn_close)
{
 ...
 */
static int
txp_pkt_commit(OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt, uint32_t archetype,
               int *txpim_pkt_reffed)
{
    int rc = 1;
...
void
ossl_quic_tx_packetiser_set_msg_callback(OSSL_QUIC_TX_PACKETISER *txp, ossl_msg_cb msg_callback,
                                         SSL *msg_callback_ssl)
{
...
void
ossl_quic_tx_packetiser_set_msg_callback_arg(OSSL_QUIC_TX_PACKETISER *txp, void *msg_callback_arg)
{
    txp->msg_callback_arg = msg_callback_arg;
}

QUIC_PN
ossl_quic_tx_packetiser_get_next_pn(OSSL_QUIC_TX_PACKETISER *txp, uint32_t pn_space)
{
...
ossl_quic_tx_packetiser_get_deadline(OSSL_QUIC_TX_PACKETISER *txp)
{
    ...
    for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM; ++enc_level)
        if (ossl_qtx_is_enc_level_provisioned(txp->args.qtx, enc_level)) {
            ...
            deadline =
                ossl_time_min(deadline, ossl_ackm_get_ack_deadline(txp->args.ackm, pn_space));
        }

    /* When will CC let us send more? */
    if (txp->args.cc_method->get_tx_allowance(txp->args.cc_data) == 0)
        deadline =
            ossl_time_min(deadline, txp->args.cc_method->get_wakeup_deadline(txp->args.cc_data));
