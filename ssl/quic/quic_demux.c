...
QUIC_DEMUX *
ossl_quic_demux_new(BIO *net_bio, size_t short_conn_id_len, OSSL_TIME (*now)(void *arg),
                    void *now_arg)
{
    QUIC_DEMUX *demux;
...
void
ossl_quic_demux_set_default_handler(QUIC_DEMUX *demux, ossl_quic_demux_cb_fn *cb, void *cb_arg)
{
    demux->default_cb = cb;
...
static int
demux_identify_conn_id(QUIC_DEMUX *demux, QUIC_URXE *e, QUIC_CONN_ID *dst_conn_id)
{
    return ossl_quic_wire_get_pkt_hdr_dst_conn_id(ossl_quic_urxe_data(e), e->data_len,
                                                  demux->short_conn_id_len, dst_conn_id);
}
...
int
ossl_quic_demux_inject(QUIC_DEMUX *demux, const unsigned char *buf, size_t buf_len,
