...
int
ossl_qlog_event_try_begin(QLOG *qlog, uint32_t event_type, const char *event_cat,
                          const char *event_name, const char *event_combined_name)
{
    if (qlog == NULL)
        return 0;
...
void
ossl_qlog_str_len(QLOG *qlog, const char *name, const char *value, size_t value_len)
{
    if (name != NULL)
        ossl_json_key(&qlog->json, name);
...
void
ossl_qlog_bool(QLOG *qlog, const char *name, int value)
{
    if (name != NULL)
        ossl_json_key(&qlog->json, name);
...
filter_match_event(const char *cat, size_t cat_l, const char *event, size_t event_l,
 ...
 */
static void
filter_apply(size_t *enabled, int add, const char *cat, size_t cat_l, const char *event,
             size_t event_l)
{
