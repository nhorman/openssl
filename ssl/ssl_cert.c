...
static int ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx, int op, int bits,
...
DEFINE_RUN_ONCE_STATIC(ssl_x509_store_ctx_init)
{
    ssl_x509_store_ctx_idx =
        X509_STORE_CTX_get_ex_new_index(0, "SSL for verify callback", NULL, NULL, NULL);
...
const STACK_OF(X509_NAME) * SSL_CTX_get0_CA_list(const SSL_CTX *ctx) { return ctx->ca_names; }
    ...
    while (!OSSL_STORE_eof(ctx) && !OSSL_STORE_error(ctx)) {
        ...
        if (infotype == OSSL_STORE_INFO_NAME) {
            ...
            if ((x = OSSL_STORE_INFO_get0_CERT(info)) == NULL ||
                (xn = X509_get_subject_name(x)) == NULL || (xn = X509_NAME_dup(xn)) == NULL)
                goto err;
...
int
SSL_add_store_cert_subjects_to_stack(STACK_OF(X509_NAME) * stack, const char *store)
{
...
static int
ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid,
                              void *other, void *ex)
{
...
{
    return s->cert->sec_cb(SSL_CONNECTION_GET_USER_SSL(s), NULL, op, bits, nid, other,
                           s->cert->sec_ex);
}
