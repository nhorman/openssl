...
#define SSL_CBC                                                                                    \
    (SSL_DES | SSL_3DES | SSL_RC2 | SSL_IDEA | SSL_AES128 | SSL_AES256 | SSL_CAMELLIA128 |         \
...
struct ssl_method_st {
    ...
    int (*ssl_read_bytes)(SSL *s, uint8_t type, uint8_t *recvd_type, unsigned char *buf, size_t len,
...
struct ssl_ctx_st {
    ...
    SSL_SESSION *(*get_session_cb)(struct ssl_st *ssl, const unsigned char *data, int len,
                                   int *copy);
    struct {
    ...
    struct {
        ...
        int (*ticket_key_cb)(SSL *ssl, unsigned char *name, unsigned char *iv, EVP_CIPHER_CTX *ectx,
...
typedef struct ossl_quic_tls_callbacks_st {
    int (*crypto_send_cb)(SSL *s, const unsigned char *buf, size_t buf_len, size_t *consumed,
                          void *arg);
    ...
    int (*yield_secret_cb)(SSL *s, uint32_t prot_level, int direction, const unsigned char *secret,
                           size_t secret_len, void *arg);
    int (*got_transport_params_cb)(SSL *s, const unsigned char *params, size_t params_len,
                                   void *arg);
...
struct ssl_connection_st {
    ...
    struct {
        ...
        void (*debug_cb)(SSL *s, int client_server, int type, const unsigned char *data, int len,
                         void *arg);
        void *debug_arg;
        char *hostname;
        /* certificate status request info */
...
typedef struct cert_st {
    ...
    /* Security callback */
    int (*sec_cb)(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other,
                  void *ex);
    /* Security level */
    int sec_level;
    void *sec_ex;
...
typedef struct ssl3_enc_method {
    ...
    int (*generate_master_secret)(SSL_CONNECTION *, unsigned char *, unsigned char *, size_t,
                                  size_t *);
...
#define SSL_USE_PSS(s)                                                                             \
    (s->s3.tmp.peer_sigalg != NULL && s->s3.tmp.peer_sigalg->sig == EVP_PKEY_RSA_PSS)
...
tls1_get_peer_groups(SSL_CONNECTION *s, const uint16_t **pgroups, size_t *pgroupslen)
{
...
__owur SSL_SESSION *lookup_sess_in_cache(SSL_CONNECTION *s, const unsigned char *sess_id,
                                         size_t sess_id_len);
...
__owur int ossl_bytes_to_cipher_list(SSL_CONNECTION *s, PACKET *cipher_suites,
                                     STACK_OF(SSL_CIPHER) * *skp, STACK_OF(SSL_CIPHER) * *scsvs,
                                     int sslv2format, int fatal);
void ssl_update_cache(SSL_CONNECTION *s, int mode);
__owur int ssl_cipher_get_evp_cipher(SSL_CTX *ctx, const SSL_CIPHER *sslc, const EVP_CIPHER **enc);
__owur int ssl_cipher_get_evp_md_mac(SSL_CTX *ctx, const SSL_CIPHER *sslc, const EVP_MD **md,
                                     int *mac_pkey_type, size_t *mac_secret_size);
__owur int ssl_cipher_get_evp(SSL_CTX *ctxc, const SSL_SESSION *s, const EVP_CIPHER **enc,
                              const EVP_MD **md, int *mac_pkey_type, size_t *mac_secret_size,
                              SSL_COMP **comp, int use_etm);
__owur int ssl_cipher_get_overhead(const SSL_CIPHER *c, size_t *mac_overhead, size_t *int_overhead,
                                   size_t *blocksize, size_t *ext_overhead);
__owur int ssl_cert_is_disabled(SSL_CTX *ctx, size_t idx);
__owur const SSL_CIPHER *ssl_get_cipher_by_char(SSL_CONNECTION *ssl, const unsigned char *ptr,
                                                int all);
...
__owur int ssl_fill_hello_random(SSL_CONNECTION *s, int server, unsigned char *field, size_t len,
                                 DOWNGRADE dgrd);
__owur int ssl_generate_master_secret(SSL_CONNECTION *s, unsigned char *pms, size_t pmslen,
                                      int free_pms);
...
__owur int ssl_decapsulate(SSL_CONNECTION *s, EVP_PKEY *privkey, const unsigned char *ct,
                           size_t ctlen, int gensecret);
__owur int ssl_encapsulate(SSL_CONNECTION *s, EVP_PKEY *pubkey, unsigned char **ctp, size_t *ctlenp,
                           int gensecret);
__owur EVP_PKEY *ssl_dh_to_pkey(DH *dh);
__owur int ssl_set_tmp_ecdh_groups(uint16_t **pext, size_t *pextlen, uint16_t **ksext,
                                   size_t *ksextlen, size_t **tplext, size_t *tplextlen, void *key);
...
__owur int ssl3_generate_master_secret(SSL_CONNECTION *s, unsigned char *out, unsigned char *p,
...
__owur size_t ssl3_final_finish_mac(SSL_CONNECTION *s, const char *sender, size_t slen,
                                    unsigned char *p);
...
__owur unsigned long ssl3_output_cert_chain(SSL_CONNECTION *s, WPACKET *pkt, CERT_PKEY *cpk,
                                            int for_comp);
__owur const SSL_CIPHER *ssl3_choose_cipher(SSL_CONNECTION *s, STACK_OF(SSL_CIPHER) * clnt,
...
__owur int ssl_get_min_max_version(const SSL_CONNECTION *s, int *min_version, int *max_version,
                                   int *real_max);

__owur OSSL_TIME tls1_default_timeout(void);
__owur int dtls1_do_write(SSL_CONNECTION *s, uint8_t type);
void dtls1_set_message_header(SSL_CONNECTION *s, unsigned char mt, size_t len, size_t frag_off,
                              size_t frag_len);
...
__owur size_t tls1_final_finish_mac(SSL_CONNECTION *s, const char *str, size_t slen,
                                    unsigned char *p);
__owur int tls1_generate_master_secret(SSL_CONNECTION *s, unsigned char *out, unsigned char *p,
                                       size_t len, size_t *secret_size);
...
__owur int tls13_hkdf_expand(SSL_CONNECTION *s, const EVP_MD *md, const unsigned char *secret,
                             const unsigned char *label, size_t labellen, const unsigned char *data,
                             size_t datalen, unsigned char *out, size_t outlen, int fatal);
__owur int tls13_hkdf_expand_ex(OSSL_LIB_CTX *libctx, const char *propq, const EVP_MD *md,
...
__owur int tls13_derive_key(SSL_CONNECTION *s, const EVP_MD *md, const unsigned char *secret,
                            unsigned char *key, size_t keylen);
__owur int tls13_derive_iv(SSL_CONNECTION *s, const EVP_MD *md, const unsigned char *secret,
                           unsigned char *iv, size_t ivlen);
__owur int tls13_derive_finishedkey(SSL_CONNECTION *s, const EVP_MD *md,
                                    const unsigned char *secret, unsigned char *fin, size_t finlen);
...
__owur int tls13_generate_handshake_secret(SSL_CONNECTION *s, const unsigned char *insecret,
                                           size_t insecretlen);
__owur int tls13_generate_master_secret(SSL_CONNECTION *s, unsigned char *out, unsigned char *prev,
                                        size_t prevlen, size_t *secret_size);
...
__owur int tls13_export_keying_material(SSL_CONNECTION *s, unsigned char *out, size_t olen,
                                        ...
                                        int use_context);
__owur int tls13_export_keying_material_early(SSL_CONNECTION *s, unsigned char *out, size_t olen,
                                              const char *label, size_t llen,
                                              const unsigned char *context, size_t contextlen);
...
__owur int tls1_get0_implemented_groups(int min_proto_version, int max_proto_version,
...
__owur int tls1_set_groups(uint16_t **grpext, size_t *grpextlen, uint16_t **ksext, size_t *ksextlen,
                           size_t **tplext, size_t *tplextlen, int *curves, size_t ncurves);
__owur int tls1_set_groups_list(SSL_CTX *ctx, uint16_t **grpext, size_t *grpextlen,
...
__owur int tls_valid_group(SSL_CONNECTION *s, uint16_t group_id, int minversion, int maxversion,
...
__owur int tls12_copy_sigalgs(SSL_CONNECTION *s, WPACKET *pkt, const uint16_t *psig,
                              size_t psiglen);
...
__owur int ssl_handshake_hash(SSL_CONNECTION *s, unsigned char *out, size_t outlen,
                              size_t *hashlen);
 ...
 */
__owur int ssl_log_secret(SSL_CONNECTION *s, const char *label, const uint8_t *secret,
                          size_t secret_len);
...
custom_ext_method *custom_ext_find(const custom_ext_methods *exts, ENDPOINT role,
                                   unsigned int ext_type, size_t *idx);

void custom_ext_init(custom_ext_methods *meths);

int ossl_tls_add_custom_ext_intern(SSL_CTX *ctx, custom_ext_methods *exts, ENDPOINT role,
...
__owur int custom_ext_parse(SSL_CONNECTION *s, unsigned int context, unsigned int ext_type,
                            ...
                            size_t chainidx);
__owur int custom_ext_add(SSL_CONNECTION *s, int context, WPACKET *pkt, X509 *x, size_t chainidx,
                          int maxversion);
...
void ossl_ssl_set_custom_record_layer(SSL_CONNECTION *s, const OSSL_RECORD_METHOD *meth,
                                      void *rlarg);
...
#define OSSL_LEGACY_SSL_OPTIONS                                                                    \
    (SSL_OP_NETSCAPE_REUSE_CIPHER_CHANGE_BUG | SSL_OP_MICROSOFT_BIG_SSLV3_BUFFER |                 \
     SSL_OP_SSLEAY_080_CLIENT_DH_BUG | SSL_OP_TLS_D5_BUG | SSL_OP_TLS_BLOCK_PADDING_BUG |          \
     SSL_OP_MSIE_SSLV2_RSA_PADDING | SSL_OP_SSLREF2_REUSE_CERT_TYPE_BUG |                          \
     SSL_OP_MICROSOFT_SESS_ID_BUG | SSL_OP_NETSCAPE_CHALLENGE_BUG | SSL_OP_PKCS1_CHECK_1 |         \
...
#define OSSL_TLS1_2_OPTIONS                                                                        \
    (SSL_OP_CRYPTOPRO_TLSEXT_BUG | SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS |                            \
     SSL_OP_ALLOW_CLIENT_RENEGOTIATION | SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION |                \
     SSL_OP_NO_COMPRESSION | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 |               \
     ...
     SSL_OP_NO_RENEGOTIATION | SSL_OP_NO_EXTENDED_MASTER_SECRET | SSL_OP_NO_ENCRYPT_THEN_MAC |     \
     SSL_OP_COOKIE_EXCHANGE | SSL_OP_LEGACY_SERVER_CONNECT | SSL_OP_IGNORE_UNEXPECTED_EOF)

/* Total mask of connection-level options permitted or ignored under QUIC. */
#define OSSL_QUIC_PERMITTED_OPTIONS_CONN                                                           \
    (OSSL_LEGACY_SSL_OPTIONS | OSSL_TLS1_2_OPTIONS | SSL_OP_CIPHER_SERVER_PREFERENCE |             \
     SSL_OP_DISABLE_TLSEXT_CA_NAMES | SSL_OP_NO_TX_CERTIFICATE_COMPRESSION |                       \
     SSL_OP_NO_RX_CERTIFICATE_COMPRESSION | SSL_OP_PRIORITIZE_CHACHA | SSL_OP_NO_QUERY_MTU |       \
...
#define OSSL_QUIC_SUPPORTED_DOMAIN_FLAGS                                                           \
    (SSL_DOMAIN_FLAG_SINGLE_THREAD | SSL_DOMAIN_FLAG_MULTI_THREAD |                                \
     SSL_DOMAIN_FLAG_THREAD_ASSISTED | SSL_DOMAIN_FLAG_BLOCKING | SSL_DOMAIN_FLAG_LEGACY_BLOCKING)
