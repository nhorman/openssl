...
static int
ssl3_set_crypto_state(OSSL_RECORD_LAYER *rl, int level, unsigned char *key, size_t keylen,
 ...
 */
static int
ssl3_cipher(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *inrecs, size_t n_recs, int sending,
            SSL_MAC_BUF *mac, size_t macsize)
{
    ...
    if (provided) {
        ...
        if (!sending && mac != NULL) {
            ...
            *p++ = OSSL_PARAM_construct_octet_ptr(OSSL_CIPHER_PARAM_TLS_MAC, (void **)&mac->mac,
                                                  macsize);
...
static int
ssl3_mac(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rec, unsigned char *md, int sending)
{
    ...
    if (!sending && EVP_CIPHER_CTX_get_mode(rl->enc_ctx) == EVP_CIPH_CBC_MODE &&
        ...
        if (ssl3_cbc_digest_record(EVP_MD_CTX_get0_md(hash), md, &md_size, header, rec->input,
                                   rec->length, rec->orig_len, mac_sec, md_size, 1) <= 0)
            return 0;
#endif
    } else {
        ...
        if (EVP_MD_CTX_copy_ex(md_ctx, hash) <= 0 ||
            ...
            EVP_DigestUpdate(md_ctx, seq, 8) <= 0 || EVP_DigestUpdate(md_ctx, &rec_char, 1) <= 0 ||
            EVP_DigestUpdate(md_ctx, md, 2) <= 0 ||
            EVP_DigestUpdate(md_ctx, rec->input, rec->length) <= 0 ||
            EVP_DigestFinal_ex(md_ctx, md, NULL) <= 0 || EVP_MD_CTX_copy_ex(md_ctx, hash) <= 0 ||
            EVP_DigestUpdate(md_ctx, mac_sec, md_size) <= 0 ||
...
const struct record_functions_st ssl_3_0_funcs = {
    ...
    tls_default_post_process_record, tls_get_max_records_default, tls_write_records_default,
