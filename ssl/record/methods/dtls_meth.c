...
static DTLS_BITMAP *
dtls_get_bitmap(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rr, unsigned int *is_next_epoch)
{
    *is_next_epoch = 0;
...
static int
dtls_rlayer_buffer_record(OSSL_RECORD_LAYER *rl, struct pqueue_st *queue, unsigned char *priority)
{
...
static int
dtls_retrieve_rlayer_buffered_record(OSSL_RECORD_LAYER *rl, struct pqueue_st *queue)
{
    pitem *item;
...
again:
    ...
    if ((rl->rstate != SSL_ST_READ_BODY) || (rl->packet_length < DTLS1_RT_HEADER_LENGTH)) {
        rret =
            rl->funcs->read_n(rl, DTLS1_RT_HEADER_LENGTH, TLS_BUFFER_get_len(&rl->rbuf), 0, 1, &n);
        ...
        if (ssl_major !=
            (rl->version == DTLS_ANY_VERSION ? DTLS1_VERSION_MAJOR : rl->version >> 8)) {
...
static int
dtls_new_record_layer(OSSL_LIB_CTX *libctx, const char *propq, int vers, int role, int direction,
...
int
dtls_prepare_record_header(OSSL_RECORD_LAYER *rl, WPACKET *thispkt, OSSL_RECORD_TEMPLATE *templ,
                           uint8_t rectype, unsigned char **recdata)
{
    ...
    *recdata = NULL;
    ...
    if (!WPACKET_put_bytes_u8(thispkt, rectype) ||
        ...
        !WPACKET_memcpy(thispkt, &(rl->sequence[2]), 6) || !WPACKET_start_sub_packet_u16(thispkt) ||
