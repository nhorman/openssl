...
dtls_buffer_record(SSL_CONNECTION *s, TLS_RECORD *rec)
{
    ...
    *rdata = *rec;
    ...
    if (BIO_dgram_is_sctp(s->rbio) && (ossl_statem_get_state(s) == TLS_ST_SR_FINISHED ||
                                       ossl_statem_get_state(s) == TLS_ST_CR_FINISHED)) {
        BIO_ctrl(s->rbio, BIO_CTRL_DGRAM_SCTP_GET_RCVINFO, sizeof(rdata->recordinfo),
                 &rdata->recordinfo);
    }
#endif
...
dtls_unbuffer_record(SSL_CONNECTION *s)
{
    ...
    if (item != NULL) {
        ...
        if (BIO_dgram_is_sctp(s->rbio)) {
            BIO_ctrl(s->rbio, BIO_CTRL_DGRAM_SCTP_SET_RCVINFO, sizeof(rdata->recordinfo),
                     &rdata->recordinfo);
        }
#endif
 ...
 */
int
dtls1_read_bytes(SSL *s, uint8_t type, uint8_t *recvd_type, unsigned char *buf, size_t len,
    ...
    if (sc->rlayer.curr_rec >= sc->rlayer.num_recs) {
        ...
        do {
            ...
            ret = HANDLE_RLAYER_READ_RETURN(
                sc, sc->rlayer.rrlmethod->read_record(sc->rlayer.rrl, &rr->rechandle, &rr->version,
                                                      &rr->type, &rr->data, &rr->length, &rr->epoch,
                                                      rr->seq_num));
            if (ret <= 0) {
 ...
 */
int
dtls1_write_bytes(SSL_CONNECTION *s, uint8_t type, const void *buf, size_t len, size_t *written)
{
    int i;
...
int
do_dtls1_write(SSL_CONNECTION *sc, uint8_t type, const unsigned char *buf, size_t len,
               size_t *written)
{
    int i;
