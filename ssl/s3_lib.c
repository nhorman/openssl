...
static int
sslcon_undefined_function_1(SSL_CONNECTION *sc, unsigned char *r, size_t s, const char *t, size_t u,
...
ssl3_ctrl(SSL *s, int cmd, long larg, void *parg)
{
    ...
    case SSL_CTRL_SET_GROUPS:
        return tls1_set_groups(&sc->ext.supportedgroups, &sc->ext.supportedgroups_len,
                               &sc->ext.keyshares, &sc->ext.keyshares_len, &sc->ext.tuples,
                               &sc->ext.tuples_len, parg, larg);

    case SSL_CTRL_SET_GROUPS_LIST:
        return tls1_set_groups_list(s->ctx, &sc->ext.supportedgroups, &sc->ext.supportedgroups_len,
                                    &sc->ext.keyshares, &sc->ext.keyshares_len, &sc->ext.tuples,
                                    &sc->ext.tuples_len, parg);
...
ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg)
{
    ...
    case SSL_CTRL_SET_GROUPS_LIST:
        return tls1_set_groups_list(ctx, &ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len,
                                    &ctx->ext.keyshares, &ctx->ext.keyshares_len, &ctx->ext.tuples,
                                    &ctx->ext.tuples_len, parg);

    case SSL_CTRL_GET0_IMPLEMENTED_GROUPS:
        return tls1_get0_implemented_groups(ctx->min_proto_version, ctx->max_proto_version,
...
{
    return ssl3_get_cipher_by_id(SSL3_CK_CIPHERSUITE_FLAG | ((uint32_t)p[0] << 8L) |
                                 (uint32_t)p[1]);
}
 ...
 */
const SSL_CIPHER *
ssl3_choose_cipher(SSL_CONNECTION *s, STACK_OF(SSL_CIPHER) * clnt, STACK_OF(SSL_CIPHER) * srvr)
{
...
ssl3_read_internal(SSL *s, void *buf, size_t len, int peek, size_t *readbytes)
{
    ...
    if ((ret == -1) && (sc->s3.in_read_app_data == 2)) {
        ...
        ret =
            s->method->ssl_read_bytes(s, SSL3_RT_APPLICATION_DATA, NULL, buf, len, peek, readbytes);
 ...
 */
int
ssl_fill_hello_random(SSL_CONNECTION *s, int server, unsigned char *result, size_t len,
                      DOWNGRADE dgrd)
{
    int send_time = 0, ret;
...
int
ssl_generate_master_secret(SSL_CONNECTION *s, unsigned char *pms, size_t pmslen, int free_pms)
{
    ...
    if (alg_k & SSL_PSK) {
        ...
        if (!ssl->method->ssl3_enc->generate_master_secret(
                s, s->session->master_key, pskpms, pskpmslen, &s->session->master_key_length)) {
...
int
ssl_derive(SSL_CONNECTION *s, EVP_PKEY *privkey, EVP_PKEY *pubkey, int gensecret)
{
    int rv = 0;
...
int
ssl_decapsulate(SSL_CONNECTION *s, EVP_PKEY *privkey, const unsigned char *ct, size_t ctlen,
                int gensecret)
{
    int rv = 0;
    unsigned char *pms = NULL;
...
int
ssl_encapsulate(SSL_CONNECTION *s, EVP_PKEY *pubkey, unsigned char **ctp, size_t *ctlenp,
                int gensecret)
{
    int rv = 0;
    ...
    if (EVP_PKEY_encapsulate_init(pctx, NULL) <= 0 ||
        EVP_PKEY_encapsulate(pctx, NULL, &ctlen, NULL, &pmslen) <= 0 || pmslen == 0 || ctlen == 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
    }
