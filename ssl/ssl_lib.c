...
static int
ssl_undefined_function_3(SSL_CONNECTION *sc, unsigned char *r, unsigned char *s, size_t t,
                         size_t *u)
{
...
static size_t
ssl_undefined_function_5(SSL_CONNECTION *sc, const char *r, size_t s, unsigned char *t)
{
    return ssl_undefined_function(SSL_CONNECTION_GET_SSL(sc));
}
...
static int
ssl_undefined_function_7(SSL_CONNECTION *sc, unsigned char *r, size_t s, const char *t, size_t u,
...
ssl_dane_dup(SSL_CONNECTION *to, SSL_CONNECTION *from)
{
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (SSL_dane_tlsa_add(SSL_CONNECTION_GET_SSL(to), t->usage, t->selector, t->mtype, t->data,
                              t->dlen) <= 0)
            return 0;
    }
    return 1;
...
static int
dane_mtype_set(struct dane_ctx_st *dctx, const EVP_MD *md, uint8_t mtype, uint8_t ord)
{
    int i;
...
SSL_CTX_set_ssl_version(SSL_CTX *ctx, const SSL_METHOD *meth)
{
    ...
    sk = ssl_create_cipher_list(ctx, ctx->tls13_ciphersuites, &(ctx->cipher_list),
                                &(ctx->cipher_list_by_id), OSSL_default_cipher_list(), ctx->cert);
...
SSL *
ossl_ssl_connection_new_int(SSL_CTX *ctx, SSL *user_ssl, const SSL_METHOD *method)
{
    SSL_CONNECTION *s;
    SSL *ssl;
    ...
    if (ctx->ext.supportedgroups != NULL) {
        ...
        s->ext.supportedgroups =
            OPENSSL_memdup(ctx->ext.supportedgroups, (ctx->ext.supportedgroups_len + add) *
    ...
    if (ctx->ext.keyshares != NULL) {
        ...
        if (ctx->ext.keyshares_len == 0)
            /* Add 1 so allocation won't fail */
            add = 1;
        s->ext.keyshares = OPENSSL_memdup(ctx->ext.keyshares, (ctx->ext.keyshares_len + add) *
    ...
    if (ctx->ext.tuples != NULL) {
        ...
        if (ctx->ext.tuples_len == 0)
            ...
            add = 1;
        s->ext.tuples =
            OPENSSL_memdup(ctx->ext.tuples, (ctx->ext.tuples_len + add) * sizeof(*ctx->ext.tuples));
...
int
SSL_CTX_set_session_id_context(SSL_CTX *ctx, const unsigned char *sid_ctx, unsigned int sid_ctx_len)
{
...
int
SSL_dane_tlsa_add(SSL *s, uint8_t usage, uint8_t selector, uint8_t mtype, const unsigned char *data,
                  size_t dlen)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_CTX_dane_mtype_set(SSL_CTX *ctx, const EVP_MD *md, uint8_t mtype, uint8_t ord)
{
...
SSL_set_read_ahead(SSL *s, int yes)
{
    ...
    *opts++ =
        OSSL_PARAM_construct_int(OSSL_LIBSSL_RECORD_LAYER_PARAM_READ_AHEAD, &sc->rlayer.read_ahead);
...
int
SSL_get_changed_async_fds(SSL *s, OSSL_ASYNC_FD *addfd, size_t *numaddfds, OSSL_ASYNC_FD *delfd,
                          size_t *numdelfds)
{
    ASYNC_WAIT_CTX *ctx;
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(s);
...
ssl_start_async_job(SSL *s, struct ssl_async_args *args, int (*func)(void *))
{
    ...
    sc->rwstate = SSL_NOTHING;
    switch (
        ASYNC_start_job(&sc->job, sc->waitctx, &ret, func, args, sizeof(struct ssl_async_args))) {
    case ASYNC_ERR:
        sc->rwstate = SSL_NOTHING;
...
int
ssl_write_internal(SSL *s, const void *buf, size_t num, uint64_t flags, size_t *written)
{
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_write_ex2(SSL *s, const void *buf, size_t num, uint64_t flags, size_t *written)
{
...
SSL_new_session_ticket(SSL *s)
{
    ...
    if ((SSL_in_init(s) && sc->ext.extra_tickets_expected == 0) || SSL_IS_FIRST_HANDSHAKE(sc) ||
        !sc->server || !SSL_CONNECTION_IS_TLS13(sc))
        return 0;
 ...
 */
int
SSL_select_next_proto(unsigned char **out, unsigned char *outlen, const unsigned char *server,
                      unsigned int server_len, const unsigned char *client, unsigned int client_len)
{
    PACKET cpkt, csubpkt, spkt, ssubpkt;

    if (!PACKET_buf_init(&cpkt, client, client_len) ||
        !PACKET_get_length_prefixed_1(&cpkt, &csubpkt) || PACKET_remaining(&csubpkt) == 0) {
        *out = NULL;
        *outlen = 0;
 ...
 */
void
SSL_CTX_set_npn_advertised_cb(SSL_CTX *ctx, SSL_CTX_npn_advertised_cb_func cb, void *arg)
{
    if (IS_QUIC_CTX(ctx))
 ...
 */
void
SSL_CTX_set_npn_select_cb(SSL_CTX *ctx, SSL_CTX_npn_select_cb_func cb, void *arg)
{
    if (IS_QUIC_CTX(ctx))
        /* NPN not allowed for QUIC */
        return;
 ...
 */
void
SSL_CTX_set_alpn_select_cb(SSL_CTX *ctx, SSL_CTX_alpn_select_cb_func cb, void *arg)
{
...
int
SSL_export_keying_material(SSL *s, unsigned char *out, size_t olen, const char *label, size_t llen,
                           const unsigned char *context, size_t contextlen, int use_context)
{
    ...
    return sc->ssl.method->ssl3_enc->export_keying_material(sc, out, olen, label, llen, context,
                                                            contextlen, use_context);
}

int
SSL_export_keying_material_early(SSL *s, unsigned char *out, size_t olen, const char *label,
 ...
 */
static long
check_keylog_bio_free(BIO *b, int oper, const char *argp, size_t len, int argi, long argl, int ret,
                      size_t *processed)
{
...
SSL_CTX_new_ex(OSSL_LIB_CTX *libctx, const char *propq, const SSL_METHOD *meth)
{
    ...
    if (!ssl_create_cipher_list(ret, ret->tls13_ciphersuites, &ret->cipher_list,
                                &ret->cipher_list_by_id, OSSL_default_cipher_list(), ret->cert) ||
    ...
    if (RAND_priv_bytes_ex(libctx, ret->ext.cookie_hmac_key, sizeof(ret->ext.cookie_hmac_key), 0) <=
        0) {
...
# if defined(OPENSSL_THREADS)
        if (meth == OSSL_QUIC_client_thread_method())
            ret->domain_flags = SSL_DOMAIN_FLAG_MULTI_THREAD | SSL_DOMAIN_FLAG_THREAD_ASSISTED |
...
int
ssl_get_server_cert_serverinfo(SSL_CONNECTION *s, const unsigned char **serverinfo,
...
int
SSL_SESSION_set1_master_key(SSL_SESSION *sess, const unsigned char *in, size_t len)
{
...
void
SSL_CTX_set_psk_find_session_callback(SSL_CTX *ctx, SSL_psk_find_session_cb_func cb)
{
...
void
SSL_CTX_set_psk_use_session_callback(SSL_CTX *ctx, SSL_psk_use_session_cb_func cb)
{
    ctx->psk_use_session_cb = cb;
}

void
SSL_CTX_set_msg_callback(SSL_CTX *ctx, void (*cb)(int write_p, int version, int content_type,
...
void
SSL_set_msg_callback(SSL *ssl, void (*cb)(int write_p, int version, int content_type,
                                          const void *buf, size_t len, SSL *ssl, void *arg))
{
...
void
SSL_CTX_set_not_resumable_session_callback(SSL_CTX *ctx, int (*cb)(SSL *ssl, int is_forward_secure))
{
    SSL_CTX_callback_ctrl(ctx, SSL_CTRL_SET_NOT_RESUMABLE_SESS_CB, (void (*)(void))cb);
}

void
SSL_set_not_resumable_session_callback(SSL *ssl, int (*cb)(SSL *ssl, int is_forward_secure))
{
...
int
SSL_CTX_set_block_padding_ex(SSL_CTX *ctx, size_t app_block_size, size_t hs_block_size)
{
...
int
SSL_set_record_padding_callback(SSL *ssl, size_t (*cb)(SSL *ssl, int type, size_t len, void *arg))
{
    BIO *b;
    SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL_ONLY(ssl);
...
int
ssl_handshake_hash(SSL_CONNECTION *s, unsigned char *out, size_t outlen, size_t *hashlen)
{
    EVP_MD_CTX *ctx = NULL;
...
void
SSL_set_security_callback(SSL *s, int (*cb)(const SSL *s, const SSL_CTX *ctx, int op, int bits,
...
void
SSL_CTX_set_security_callback(SSL_CTX *ctx, int (*cb)(const SSL *s, const SSL_CTX *ctx, int op,
...
static int
ct_permissive(const CT_POLICY_EVAL_CTX *ctx, const STACK_OF(SCT) * scts, void *unused_arg)
{
    return 1;
}

static int
ct_strict(const CT_POLICY_EVAL_CTX *ctx, const STACK_OF(SCT) * scts, void *unused_arg)
{
...
SSL_set_ct_validation_callback(SSL *s, ssl_ct_validation_cb callback, void *arg)
{
     ...
     */
    if (callback != NULL &&
        SSL_CTX_has_client_custom_ext(s->ctx, TLSEXT_TYPE_signed_certificate_timestamp)) {
...
int
SSL_CTX_set_ct_validation_callback(SSL_CTX *ctx, ssl_ct_validation_cb callback, void *arg)
{
    /*
     ...
     */
    if (callback != NULL &&
        SSL_CTX_has_client_custom_ext(ctx, TLSEXT_TYPE_signed_certificate_timestamp)) {
        ERR_raise(ERR_LIB_SSL, SSL_R_CUSTOM_EXT_HANDLER_ALREADY_INSTALLED);
        return 0;
    }
...
void
SSL_CTX_set_new_pending_conn_cb(SSL_CTX *c, SSL_new_pending_conn_cb_fn cb, void *arg)
{
...
static int
nss_keylog_int(const char *prefix, SSL_CONNECTION *sc, const uint8_t *parameter_1,
               size_t parameter_1_len, const uint8_t *parameter_2, size_t parameter_2_len)
{
    char *out = NULL;
    char *cursor = NULL;
...
int
ssl_log_rsa_client_key_exchange(SSL_CONNECTION *sc, const uint8_t *encrypted_premaster,
...
int
ssl_log_secret(SSL_CONNECTION *sc, const char *label, const uint8_t *secret, size_t secret_len)
{
...
int
SSL_bytes_to_cipher_list(SSL *s, const unsigned char *bytes, size_t len, int isv2format,
...
int
ossl_bytes_to_cipher_list(SSL_CONNECTION *s, PACKET *cipher_suites, STACK_OF(SSL_CIPHER) * *skp,
...
int
SSL_CTX_set_session_ticket_cb(SSL_CTX *ctx, SSL_CTX_generate_session_ticket_fn gen_cb,
...
void
SSL_CTX_set_allow_early_data_cb(SSL_CTX *ctx, SSL_allow_early_data_cb_fn cb, void *arg)
{
...
int
SSL_shutdown_ex(SSL *ssl, uint64_t flags, const SSL_SHUTDOWN_EX_ARGS *args, size_t args_len)
{
#ifndef OPENSSL_NO_QUIC
    if (!IS_QUIC(ssl))
...
SSL_add_expected_rpk(SSL *s, EVP_PKEY *rpk)
{
    ...
    ret = SSL_dane_tlsa_add(s, DANETLS_USAGE_DANE_EE, DANETLS_SELECTOR_SPKI, DANETLS_MATCHING_FULL,
                            data, (size_t)ret) > 0;
    OPENSSL_free(data);
    return ret;
}
...
static int
set_cert_type(unsigned char **cert_type, size_t *cert_type_len, const unsigned char *val,
              size_t len)
{
...
int
SSL_CTX_set1_client_cert_type(SSL_CTX *ctx, const unsigned char *val, size_t len)
{
...
int
SSL_CTX_get0_client_cert_type(const SSL_CTX *ctx, unsigned char **t, size_t *len)
{
    if (t == NULL || len == NULL)
        return 0;
...
int
SSL_CTX_get0_server_cert_type(const SSL_CTX *ctx, unsigned char **t, size_t *len)
{
    if (t == NULL || len == NULL)
        return 0;
