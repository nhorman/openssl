...
static int
ssl_cipher_info_find(const ssl_cipher_table *table, size_t table_cnt, uint32_t mask)
{
    size_t i;
...
static const SSL_CIPHER cipher_aliases[] = {
    ...
    {0, SSL3_TXT_EDH_DSS_DES_192_CBC3_SHA, NULL, 0, SSL_kDHE, SSL_aDSS, SSL_3DES, SSL_SHA1, 0, 0, 0,
     0, SSL_HIGH | SSL_FIPS},
    {0, SSL3_TXT_EDH_RSA_DES_192_CBC3_SHA, NULL, 0, SSL_kDHE, SSL_aRSA, SSL_3DES, SSL_SHA1, 0, 0, 0,
     0, SSL_HIGH | SSL_FIPS},
                   ...
                   int *mac_pkey_type, size_t *mac_secret_size, SSL_COMP **comp, int use_etm)
{
    ...
    if ((*enc != NULL) &&
        (*md != NULL || (EVP_CIPHER_get_flags(*enc) & EVP_CIPH_FLAG_AEAD_CIPHER)) &&
...
static void
ssl_cipher_collect_ciphers(const SSL_METHOD *ssl_method, int num_of_ciphers, uint32_t disabled_mkey,
...
static void
ssl_cipher_apply_rule(uint32_t cipher_id, uint32_t alg_mkey, uint32_t alg_auth, uint32_t alg_enc,
                      ...
                      int32_t strength_bits, CIPHER_ORDER **head_p, CIPHER_ORDER **tail_p)
{
    ...
    OSSL_TRACE_BEGIN(TLS_CIPHER)
    {
        BIO_printf(trc_out, "Applying rule %d with %08x/%08x/%08x/%08x/%08x %08x (%d)\n", rule,
                   (unsigned int)alg_mkey, (unsigned int)alg_auth, (unsigned int)alg_enc,
                   (unsigned int)alg_mac, min_tls, (unsigned int)algo_strength, (int)strength_bits);
    }
        ...
        if (strength_bits >= 0) {
            ...
            if (trc_out != NULL) {
                BIO_printf(trc_out,
                           ...
                           (unsigned int)cp->algorithm_auth, (unsigned int)cp->algorithm_enc,
...
static int
ssl_cipher_process_rulestr(const char *rule_str, CIPHER_ORDER **head_p, CIPHER_ORDER **tail_p,
                           const SSL_CIPHER **ca_list, CERT *c)
{
    ...
    for (;;) {
        ...
        if (rule == CIPHER_SPECIAL) { /* special command */
            ...
            ssl_cipher_apply_rule(cipher_id, alg_mkey, alg_auth, alg_enc, alg_mac, min_tls,
                                  algo_strength, rule, -1, head_p, tail_p);
        } else {
            while ((*l != '\0') && !ITEM_SEP(*l))
                l++;
        }
...
static int
check_suiteb_cipher_list(const SSL_METHOD *meth, CERT *c, const char **prule_str)
{
...
set_ciphersuites(STACK_OF(SSL_CIPHER) * *currciphers, const char *str)
{
    ...
    if (*str != '\0' && (CONF_parse_list(str, ':', 1, ciphersuite_cb, newciphers) <= 0 ||
                           ...
                           STACK_OF(SSL_CIPHER) * *cipher_list_by_id, const char *rule_str, CERT *c)
{
    ...
    ssl_cipher_collect_ciphers(ssl_method, num_of_ciphers, disabled_mkey, disabled_auth,
                               disabled_enc, disabled_mac, co_list, &head, &tail);
    ...
    ssl_cipher_apply_rule(0, SSL_kDHE | SSL_kECDHE, 0, 0, SSL_AEAD, 0, 0, CIPHER_BUMP, -1, &head,
                          &tail);
    ...
    ssl_cipher_collect_aliases(ca_list, num_of_group_aliases, disabled_mkey, disabled_auth,
                               disabled_enc, disabled_mac, head);
...
int
ssl_cipher_get_overhead(const SSL_CIPHER *c, size_t *mac_overhead, size_t *int_overhead,
