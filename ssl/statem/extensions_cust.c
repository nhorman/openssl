 ...
 */
static int
custom_ext_add_old_cb_wrap(SSL *s, unsigned int ext_type, unsigned int context,
...
static void
custom_ext_free_old_cb_wrap(SSL *s, unsigned int ext_type, unsigned int context,
...
static int
custom_ext_parse_old_cb_wrap(SSL *s, unsigned int ext_type, unsigned int context,
 ...
 */
custom_ext_method *
custom_ext_find(const custom_ext_methods *exts, ENDPOINT role, unsigned int ext_type, size_t *idx)
{
    size_t i;
...
int
custom_ext_parse(SSL_CONNECTION *s, unsigned int context, unsigned int ext_type,
                 const unsigned char *ext_data, size_t ext_size, X509 *x, size_t chainidx)
{
    ...
    if (meth->parse_cb(SSL_CONNECTION_GET_USER_SSL(s), ext_type, context, ext_data, ext_size, x,
                       chainidx, &al, meth->parse_arg) <= 0) {
 ...
 */
int
custom_ext_add(SSL_CONNECTION *s, int context, WPACKET *pkt, X509 *x, size_t chainidx,
               int maxversion)
{
    custom_ext_methods *exts = &s->cert->custext;
    custom_ext_method *meth;
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if ((context & (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO |
                        ...
                        SSL_EXT_TLS1_3_RAW_PUBLIC_KEY | SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST)) != 0) {
        ...
        if (meth->add_cb != NULL) {
            int cb_retval = meth->add_cb(SSL_CONNECTION_GET_USER_SSL(s), meth->ext_type, context,
        ...
        if (!WPACKET_put_bytes_u16(pkt, meth->ext_type) || !WPACKET_start_sub_packet_u16(pkt) ||
            ...
            if (meth->free_cb != NULL)
                meth->free_cb(SSL_CONNECTION_GET_USER_SSL(s), meth->ext_type, context, out,
                              meth->add_arg);
            if (!for_comp)
...
int
ossl_tls_add_custom_ext_intern(SSL_CTX *ctx, custom_ext_methods *exts, ENDPOINT role,
                               ...
                               void *add_arg, SSL_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
    ...
    if (ext_type == TLSEXT_TYPE_signed_certificate_timestamp &&
        (context & SSL_EXT_CLIENT_HELLO) != 0 && ctx != NULL && SSL_CTX_ct_is_enabled(ctx))
        return 0;
#endif
...
static int
add_old_custom_ext(SSL_CTX *ctx, ENDPOINT role, unsigned int ext_type, unsigned int context,
                   ...
                   custom_ext_parse_cb parse_cb, void *parse_arg)
{
    ...
    ret = ossl_tls_add_custom_ext_intern(ctx, NULL, role, ext_type, context,
                                         custom_ext_add_old_cb_wrap, custom_ext_free_old_cb_wrap,
...
int
SSL_CTX_add_client_custom_ext(SSL_CTX *ctx, unsigned int ext_type, custom_ext_add_cb add_cb,
...
int
SSL_CTX_add_server_custom_ext(SSL_CTX *ctx, unsigned int ext_type, custom_ext_add_cb add_cb,
                              custom_ext_free_cb free_cb, void *add_arg,
                              custom_ext_parse_cb parse_cb, void *parse_arg)
{
...
int
SSL_CTX_add_custom_ext(SSL_CTX *ctx, unsigned int ext_type, unsigned int context,
                       SSL_custom_ext_add_cb_ex add_cb, SSL_custom_ext_free_cb_ex free_cb,
                       void *add_arg, SSL_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
    return ossl_tls_add_custom_ext_intern(ctx, NULL, ENDPOINT_BOTH, ext_type, context, add_cb,
