    ...
    case TLS_ST_CR_SRVR_HELLO:
        if (s->hit) {
            ...
            if (SSL_CONNECTION_IS_DTLS(s) && mt == DTLS1_MT_HELLO_VERIFY_REQUEST) {
                st->hand_state = DTLS_ST_CR_HELLO_VERIFY_REQUEST;
                return 1;
            } else if (s->version >= TLS1_VERSION && s->ext.session_secret_cb != NULL &&
                       s->session->ext.tick != NULL && mt == SSL3_MT_CHANGE_CIPHER_SPEC) {
                /*
 ...
 */
int
ossl_statem_client_construct_message(SSL_CONNECTION *s, confunc_f *confunc, int *mt)
{
    OSSL_STATEM *st = &s->statem;
...
tls_process_server_hello(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls_parse_extension(s, TLSEXT_IDX_psk, SSL_EXT_TLS1_3_SERVER_HELLO, extensions, NULL,
                                 0)) {
            /* SSLfatal() already called */
            goto err;
        }
        ...
        if (s->version >= TLS1_VERSION && s->ext.session_secret_cb != NULL &&
            ...
            if (s->ext.session_secret_cb(ussl, s->session->master_key, &master_key_length, NULL,
                                         &pref_cipher, s->ext.session_secret_cb_arg) &&
                master_key_length > 0) {
    ...
    if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
        ...
        if (SSL_export_keying_material(ssl, sctpauthkey, sizeof(sctpauthkey), labelbuffer, labellen,
                                       NULL, 0, 0) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }

        BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_SCTP_ADD_AUTH_KEY, sizeof(sctpauthkey),
                 sctpauthkey);
    }
#endif
...
tls_process_as_hello_retry_request(SSL_CONNECTION *s, PACKET *extpkt)
{
    ...
    if (!tls_collect_extensions(s, extpkt, SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST, &extensions, NULL,
                                1) ||
...
tls_post_process_server_rpk(SSL_CONNECTION *sc, WORK_STATE wst)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(sc) &&
        !ssl_handshake_hash(sc, sc->cert_verify_hash, sizeof(sc->cert_verify_hash),
...
tls_process_server_certificate(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    for (chainidx = 0; PACKET_remaining(pkt); chainidx++) {
        ...
        if (SSL_CONNECTION_IS_TLS13(s)) {
            ...
            if (!tls_collect_extensions(s, &extensions, SSL_EXT_TLS1_3_CERTIFICATE, &rawexts, NULL,
                                        chainidx == 0) ||
...
tls_process_ske_srp(SSL_CONNECTION *s, PACKET *pkt, EVP_PKEY **pkey)
{
    ...
    if ((s->srp_ctx.N = BN_bin2bn(PACKET_data(&prime), (int)PACKET_remaining(&prime), NULL)) ==
            NULL ||
        (s->srp_ctx.g =
             BN_bin2bn(PACKET_data(&generator), (int)PACKET_remaining(&generator), NULL)) == NULL ||
        (s->srp_ctx.s = BN_bin2bn(PACKET_data(&salt), (int)PACKET_remaining(&salt), NULL)) ==
            NULL ||
        (s->srp_ctx.B = BN_bin2bn(PACKET_data(&server_pub), (int)PACKET_remaining(&server_pub),
                                  NULL)) == NULL) {
...
tls_process_key_exchange(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (pkey != NULL) {
        ...
        if (EVP_DigestVerifyInit_ex(md_ctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md),
        ...
        tbslen =
            construct_key_exchange_tbs(s, &tbs, PACKET_data(&params), PACKET_remaining(&params));
        if (tbslen == 0) {
            /* SSLfatal() already called */
            goto err;
        }

        rv = EVP_DigestVerify(md_ctx, PACKET_data(&signature), PACKET_remaining(&signature), tbs,
                              tbslen);
        OPENSSL_free(tbs);
        if (rv <= 0) {
...
tls_process_certificate_request(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls_collect_extensions(s, &extensions, SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, &rawexts,
                                    NULL, 1) ||
...
tls_process_new_session_ticket(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls_collect_extensions(s, &extpkt, SSL_EXT_TLS1_3_NEW_SESSION_TICKET, &exts, NULL,
                                    1) ||
     ...
     */
    if (!EVP_Digest(s->session->ext.tick, ticklen, s->session->session_id, &sess_len, sha256,
                    NULL)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
        goto err;
    }
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls13_hkdf_expand(s, md, s->resumption_master_secret, nonce_label,
                               ...
                               PACKET_remaining(&nonce), s->session->master_key, hashlen, 1)) {
            /* SSLfatal() already called */
            goto err;
        }
 ...
 */
int
tls_process_cert_status_body(SSL_CONNECTION *s, PACKET *pkt)
{
    size_t resplen;
    unsigned int type;
...
tls_construct_cke_psk_preamble(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    psklen = s->psk_client_callback(SSL_CONNECTION_GET_USER_SSL(s), s->session->psk_identity_hint,
...
tls_construct_cke_gost(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    ukm_hash = NULL;
    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_ENCRYPT, EVP_PKEY_CTRL_SET_IV, 8, shared_ukm) <=
        0) {
...
tls_construct_cke_gost18(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_ENCRYPT, EVP_PKEY_CTRL_SET_IV, 32, rnd_dgst) <=
        0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
        goto err;
    }

    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_ENCRYPT, EVP_PKEY_CTRL_CIPHER, cipher_nid,
                          NULL) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
        goto err;
    }
...
tls_construct_cke_srp(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (s->srp_ctx.A == NULL ||
        !WPACKET_sub_allocate_bytes_u16(pkt, BN_num_bytes(s->srp_ctx.A), &abytes)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
...
tls_construct_client_compressed_certificate(SSL_CONNECTION *sc, WPACKET *pkt)
{
    ...
    if (SSL_IS_FIRST_HANDSHAKE(sc) &&
        ...
        (!ssl->method->ssl3_enc->change_cipher_state(sc, SSL3_CC_HANDSHAKE |
                                                             SSL3_CHANGE_CIPHER_CLIENT_WRITE))) {
        /*
...
tls_process_encrypted_extensions(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (!tls_collect_extensions(s, &extensions, SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS, &rawexts, NULL,
                                1) ||
...
int
ssl_cipher_list_to_bytes(SSL_CONNECTION *s, STACK_OF(SSL_CIPHER) * sk, WPACKET *pkt)
{
    int i;
...
# endif
    ...
    if (totlen != 0) {
        ...
        if (s->mode & SSL_MODE_SEND_FALLBACK_SCSV) {
            static const SSL_CIPHER scsv = {0, NULL, NULL, SSL3_CK_FALLBACK_SCSV, 0, 0, 0, 0, 0, 0,
                                            0, 0,    0};
            if (!ssl->method->put_cipher_by_char(&scsv, pkt, &len)) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
