...
ossl_statem_server_post_work(SSL_CONNECTION *s, WORK_STATE wst)
{
    ...
    case TLS_ST_SW_SRVR_HELLO:
        ...
        if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
            ...
            if (SSL_export_keying_material(ssl, sctpauthkey, sizeof(sctpauthkey), labelbuffer,
                                           labellen, NULL, 0, 0) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return WORK_ERROR;
            }
...
#endif
        if (!SSL_CONNECTION_IS_TLS13(s) || ((s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0 &&
                                            s->hello_retry_request != SSL_HRR_COMPLETE))
            break;
        /* Fall through */
    ...
    case TLS_ST_SW_FINISHED:
        ...
        if (SSL_CONNECTION_IS_TLS13(s)) {
            ...
            if (!ssl->method->ssl3_enc->generate_master_secret(s, s->master_secret,
                ...
                !ssl->method->ssl3_enc->change_cipher_state(s, SSL3_CC_APPLICATION |
 ...
 */
int
ossl_statem_server_construct_message(SSL_CONNECTION *s, confunc_f *confunc, int *mt)
{
    OSSL_STATEM *st = &s->statem;
...
int
dtls_raw_hello_verify_request(WPACKET *pkt, unsigned char *cookie, size_t cookie_len)
{
...
tls_process_client_hello(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (clienthello->isv2) {
        ...
        if (!PACKET_copy_bytes(&challenge, clienthello->random + SSL3_RANDOM_SIZE - challenge_len,
                               challenge_len)
...
tls_early_post_process_client_hello(SSL_CONNECTION *s)
{
    ...
    if (!tls_parse_extension(s, TLSEXT_IDX_extended_master_secret, SSL_EXT_CLIENT_HELLO,
     ...
     */
    if (clienthello->isv2 ||
        (s->new_session && (s->options & SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION))) {
    ...
    /* TLS extensions */
    if (!tls_parse_all_extensions(s, SSL_EXT_CLIENT_HELLO, clienthello->pre_proc_exts, NULL, 0,
                                  1)) {
        /* SSLfatal() already called */
        goto err;
    }
    ...
    if (!s->hit && s->version >= TLS1_VERSION && !SSL_CONNECTION_IS_TLS13(s) &&
        ...
        if (s->ext.session_secret_cb(ussl, s->session->master_key, &master_key_length, ciphers,
...
tls_handle_alpn(SSL_CONNECTION *s)
{
    ...
    if (sctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
        int r = sctx->ext.alpn_select_cb(SSL_CONNECTION_GET_USER_SSL(s), &selected, &selected_len,
...
tls_construct_server_hello(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, version)
         ...
         */
        || !WPACKET_memcpy(
               pkt, s->hello_retry_request == SSL_HRR_PENDING ? hrrrandom : s->s3.server_random,
               SSL3_RANDOM_SIZE)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return CON_FUNC_ERROR;
    }
...
tls_construct_server_key_exchange(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (lu != NULL) {
        ...
        if (EVP_DigestSignInit_ex(md_ctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md),
...
tls_construct_certificate_request(SSL_CONNECTION *s, WPACKET *pkt)
{
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (s->post_handshake_auth == SSL_PHA_REQUEST_PENDING) {
            ...
            if (RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, s->pha_context, s->pha_context_len,
                              0) <= 0 ||
...
tls_process_cke_psk_preamble(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    psklen = s->psk_server_callback(SSL_CONNECTION_GET_USER_SSL(s), s->session->psk_identity, psk,
                                    sizeof(psk));
...
tls_process_cke_rsa(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION,
    ...
    if ((s->options & SSL_OP_TLS_ROLLBACK_BUG) != 0)
        *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION,
...
tls_process_cke_gost18(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_DECRYPT, EVP_PKEY_CTRL_SET_IV, 32, rnd_dgst) <=
        0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
        goto err;
    }

    if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_DECRYPT, EVP_PKEY_CTRL_CIPHER, cipher_nid,
                          NULL) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
        goto err;
    }
    ...
    if (wst == WORK_MORE_A) {
        if (SSL_CONNECTION_IS_DTLS(s)) {
            ...
            if (SSL_export_keying_material(SSL_CONNECTION_GET_SSL(s), sctpauthkey,
                                           sizeof(sctpauthkey), labelbuffer, labellen, NULL, 0,
                                           0) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return WORK_ERROR;
            }
...
tls_process_client_rpk(SSL_CONNECTION *sc, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(sc)) {
        ...
        if (!ssl_handshake_hash(sc, sc->cert_verify_hash, sizeof(sc->cert_verify_hash),
...
tls_construct_server_compressed_certificate(SSL_CONNECTION *sc, WPACKET *pkt)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, alg) || !WPACKET_put_bytes_u24(pkt, cc->orig_len) ||
        !WPACKET_start_sub_packet_u24(pkt) || !WPACKET_memcpy(pkt, cc->data, cc->len) ||
        !WPACKET_close(pkt))
        return 0;
...
static int
create_ticket_prequel(SSL_CONNECTION *s, WPACKET *pkt, uint32_t age_add, unsigned char *tick_nonce)
{
                          ...
                          unsigned char *tick_nonce)
{
    ...
    if (!WPACKET_memcpy(pkt, s->session->session_id, s->session->session_id_length) ||
        !WPACKET_close(pkt)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
...
tls_construct_new_session_ticket(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, age_add_u.age_add_c, sizeof(age_add_u),
                          0) <= 0) {
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            goto err;
        }
        ...
        if (!tls13_hkdf_expand(s, md, s->resumption_master_secret, nonce_label,
                               sizeof(nonce_label) - 1, tick_nonce, TICKET_NONCE_SIZE,
 ...
 */
int
tls_construct_cert_status_body(SSL_CONNECTION *s, WPACKET *pkt)
{
...
tls_process_next_proto(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (!PACKET_get_length_prefixed_1(pkt, &next_proto) ||
        !PACKET_get_length_prefixed_1(pkt, &padding) || PACKET_remaining(pkt) > 0) {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
        return MSG_PROCESS_ERROR;
    }
