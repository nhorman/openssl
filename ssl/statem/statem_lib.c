...
int
ossl_statem_set_mutator(SSL *s, ossl_statem_mutate_handshake_cb mutate_handshake_cb,
...
ssl3_do_write(SSL_CONNECTION *s, uint8_t type)
{
    ...
    if (s->statem.mutate_handshake_cb != NULL && !s->statem.write_in_progress &&
        ...
        if (!s->statem.mutate_handshake_cb((unsigned char *)s->init_buf->data, s->init_num, &msg,
                                           &msglen, s->statem.mutatearg))
            return -1;
    ...
    if (type == SSL3_RT_HANDSHAKE)
         ...
         */
        if (!SSL_CONNECTION_IS_TLS13(s) || (s->statem.hand_state != TLS_ST_SW_SESSION_TICKET &&
    ...
    if (written == s->init_num) {
        ...
        if (s->msg_callback)
            s->msg_callback(1, s->version, type, s->init_buf->data,
                            (size_t)(s->init_off + s->init_num), ussl, s->msg_callback_arg);
        return 1;
    }
    s->init_off += written;
    s->init_num -= written;
...
static int
get_cert_verify_tbs_data(SSL_CONNECTION *s, unsigned char *tls13tbs, void **hdata, size_t *hdatalen)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        } else if (!ssl_handshake_hash(s, tls13tbs + TLS13_TBS_PREAMBLE_SIZE, EVP_MAX_MD_SIZE,
                                       &hashlen)) {
            /* SSLfatal() already called */
            return 0;
        }
...
tls_construct_cert_verify(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (EVP_DigestSignInit_ex(mctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md), sctx->libctx,
...
tls_process_cert_verify(SSL_CONNECTION *s, PACKET *pkt)
{
...
#ifndef OPENSSL_NO_GOST
    if (!SSL_USE_SIGALGS(s) &&
        ((PACKET_remaining(pkt) == 64 && (EVP_PKEY_get_id(pkey) == NID_id_GostR3410_2001 ||
    ...
    if (EVP_DigestVerifyInit_ex(mctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md), sctx->libctx,
                                sctx->propq, pkey, NULL) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_EVP_LIB);
        goto err;
    }
...
tls_construct_finished(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s) && !s->server &&
        ...
        s->s3.tmp.cert_req == 0 &&
        (!ssl->method->ssl3_enc->change_cipher_state(s, SSL3_CC_HANDSHAKE |
...
tls_process_change_cipher_spec(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_DTLS(s)) {
         ...
         * SCTP is used
         */
        BIO_ctrl(SSL_get_wbio(SSL_CONNECTION_GET_SSL(s)), BIO_CTRL_DGRAM_SCTP_AUTH_CCS_RCVD, 1,
                 NULL);
#endif
    }
...
tls_process_finished(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        if (s->server) {
            ...
            size_t dummy;
            if (!ssl->method->ssl3_enc->generate_master_secret(s, s->master_secret,
                                                               s->handshake_secret, 0, &dummy)) {
                /* SSLfatal() already called */
                return MSG_PROCESS_ERROR;
            }
...
static int
ssl_add_cert_to_wpacket(SSL_CONNECTION *s, WPACKET *pkt, X509 *x, int chain, int for_comp)
{
    int len;
...
static int
ssl_add_cert_chain(SSL_CONNECTION *s, WPACKET *pkt, CERT_PKEY *cpk, int for_comp)
{
    int i, chain_count;
    X509 *x;
...
tls_process_rpk(SSL_CONNECTION *sc, PACKET *pkt, EVP_PKEY **peer_rpk)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(sc)) {
        ...
        if (!tls_collect_extensions(sc, &extensions, SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, &rawexts, NULL,
                                    1)) {
            /* SSLfatal already called */
            goto err;
        }
...
unsigned long
ssl3_output_cert_chain(SSL_CONNECTION *s, WPACKET *pkt, CERT_PKEY *cpk, int for_comp)
{
    if (!WPACKET_start_sub_packet_u24(pkt)) {
        if (!for_comp)
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
 ...
 */
WORK_STATE
tls_finish_handshake(SSL_CONNECTION *s, ossl_unused WORK_STATE wst, int clearbufs, int stop)
{
        ...
        if (!s->server)
            if (s->statem.hand_state != TLS_ST_OK && p[0] == SSL3_MT_HELLO_REQUEST)
                ...
                if (p[1] == 0 && p[2] == 0 && p[3] == 0) {
                    ...
                    if (s->msg_callback)
                        s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, p, SSL3_HM_HEADER_LENGTH,
...
tls_get_message_body(SSL_CONNECTION *s, size_t *len)
{
    ...
    while (n > 0) {
        i = ssl->method->ssl_read_bytes(ssl, SSL3_RT_HANDSHAKE, NULL, &p[s->init_num], n, 0,
                                        &readbytes);
        if (i <= 0) {
            s->rwstate = SSL_READING;
            *len = 0;
    ...
    if (RECORD_LAYER_is_sslv2_record(&s->rlayer)) {
        ...
        if (s->msg_callback)
            s->msg_callback(0, SSL2_VERSION, 0, s->init_buf->data, (size_t)s->init_num, ussl,
                            s->msg_callback_arg);
    } else {
        /*
 ...
 */
int
ssl_version_supported(const SSL_CONNECTION *s, int version, const SSL_METHOD **meth)
{
    ...
    for (vent = table; vent->version != 0 && ssl_version_cmp(s, version, vent->version) <= 0;
         ++vent) {
...
ssl_set_version_bound(int method_version, int version, int *bound)
{
    ...
    valid_dtls =
        ...
        (version == DTLS1_BAD_VER) || (DTLS_VERSION_LE(version, DTLS_MAX_VERSION_INTERNAL) &&
 ...
 */
int
ssl_choose_server_version(SSL_CONNECTION *s, CLIENTHELLO_MSG *hello, DOWNGRADE *dgrd)
{
    /*-
 ...
 */
int
ssl_choose_client_version(SSL_CONNECTION *s, int version, RAW_EXTENSION *extensions)
{
    const version_info *vent;
    const version_info *table;
 ...
 */
int
ssl_get_min_max_version(const SSL_CONNECTION *s, int *min_version, int *max_version, int *real_max)
{
 ...
 */
int
check_in_list(SSL_CONNECTION *s, uint16_t group_id, const uint16_t *groups, size_t num_groups,
...
int
create_synthetic_message_hash(SSL_CONNECTION *s, const unsigned char *hashval, size_t hashlen,
                              const unsigned char *hrr, size_t hrrlen)
{
     ...
     */
    if (hrr != NULL && (!ssl3_finish_mac(s, hrr, hrrlen) ||
...
int
construct_ca_names(SSL_CONNECTION *s, const STACK_OF(X509_NAME) * ca_sk, WPACKET *pkt)
{
...
size_t
construct_key_exchange_tbs(SSL_CONNECTION *s, unsigned char **ptbs, const void *param,
                           size_t paramlen)
{
...
MSG_PROCESS_RETURN
tls13_process_compressed_certificate(SSL_CONNECTION *sc, PACKET *pkt, PACKET *tmppkt, BUF_MEM *buf)
{
    MSG_PROCESS_RETURN ret = MSG_PROCESS_ERROR;
    int comp_alg;
