...
int
set_cert_key_stuff(SSL_CTX *ctx, X509 *cert, EVP_PKEY *key, STACK_OF(X509) * chain, int build_chain)
{
...
ssl_print_tmp_key(BIO *out, SSL *s)
{
    ...
    case EVP_PKEY_EC: {
        ...
        if (!EVP_PKEY_get_utf8_string_param(key, OSSL_PKEY_PARAM_GROUP_NAME, name, sizeof(name),
                                            &name_len))
            strcpy(name, "?");
    ...
    case (BIO_CB_READ | BIO_CB_RETURN):
        if (ret > 0 && processed != NULL) {
            BIO_printf(out, "read from %p [%p] (%zu bytes => %zu (0x%zX))\n", (void *)bio,
                       (void *)argp, len, *processed, *processed);
    ...
    case (BIO_CB_RECVMMSG | BIO_CB_RETURN):
        ...
        if (ret > 0) {
            for (i = 0; i < *(mmsgargs->msgs_processed); i++) {
                ...
                BIO_printf(out, "read from %p [%p] (%zu bytes => %zu (0x%zX))\n", (void *)bio,
                           (void *)msg->data, msg->data_len, msg->data_len, msg->data_len);
...
msg_cb(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl, void *arg)
{
    ...
    if (version == SSL3_VERSION || version == TLS1_VERSION || version == TLS1_1_VERSION ||
        version == TLS1_2_VERSION || version == TLS1_3_VERSION || version == DTLS1_VERSION ||
...
void
tlsext_cb(SSL *s, int client_server, int type, const unsigned char *data, int len, void *arg)
{
    BIO *bio = arg;
...
generate_stateless_cookie_callback(SSL *ssl, unsigned char *cookie, size_t *cookie_len)
{
    ...
    if (EVP_Q_mac(NULL, "HMAC", NULL, "SHA1", NULL, cookie_secret, COOKIE_SECRET_LENGTH, buffer,
                  length, cookie, DTLS1_COOKIE_LENGTH, cookie_len) == NULL) {
...
int
ssl_load_stores(SSL_CTX *ctx, const char *vfyCApath, const char *vfyCAfile, const char *vfyCAstore,
                const char *chCApath, const char *chCAfile, const char *chCAstore,
...
typedef struct {
    BIO *out;
    int verbose;
    int (*old_cb)(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other,
                  void *ex);
} security_debug_ex;
...
static int
security_callback_debug(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other,
                        void *ex)
{
