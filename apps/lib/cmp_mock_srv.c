...
int
ossl_cmp_mock_srv_set1_chainOut(OSSL_CMP_SRV_CTX *srv_ctx, STACK_OF(X509) * chain)
{
    mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
...
int
ossl_cmp_mock_srv_set1_caPubsOut(OSSL_CMP_SRV_CTX *srv_ctx, STACK_OF(X509) * caPubs)
{
    mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
...
int
ossl_cmp_mock_srv_set_statusInfo(OSSL_CMP_SRV_CTX *srv_ctx, int status, int fail_info,
                                 const char *text)
{
    mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
    OSSL_CMP_PKISI *si;
...
static int
refcert_cmp(const X509 *refcert, const X509_NAME *issuer, const ASN1_INTEGER *serial)
{
...
static int
clean_transaction(OSSL_CMP_SRV_CTX *srv_ctx, ossl_unused const ASN1_OCTET_STRING *id)
{
    mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
    ...
    *caPubs = NULL;
    ...
    if (central_keygen == 1 &&
        (ctx->keyOut == NULL || (keyOut = EVP_PKEY_dup(ctx->keyOut)) == NULL ||
         !OSSL_CMP_CTX_set0_newPkey(OSSL_CMP_SRV_CTX_get0_cmp_ctx(srv_ctx), 1 /* priv */,
                                    keyOut))) {
...
static OSSL_CMP_PKISI *
process_rr(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *rr, const X509_NAME *issuer,
...
static int
check_client_crl(const STACK_OF(OSSL_CMP_CRLSTATUS) * crlStatusList, const X509_CRL *crl)
{
...
process_genm_itav(mock_srv_ctx *ctx, int req_nid, const OSSL_CMP_ITAV *req)
{
    ...
    case NID_id_it_rootCaCert: {
        ...
        if (rootcacert != NULL && X509_NAME_cmp(X509_get_subject_name(rootcacert),
    ...
    case NID_id_it_certReqTemplate: {
        ...
        if (!OSSL_CRMF_CERTTEMPLATE_fill(reqtemp, NULL, NULL, X509_get_issuer_name(ctx->refCert),
                                         NULL))
            goto crt_err;
        ...
        (void)X509_ALGOR_set0(keyalg, OBJ_nid2obj(NID_X9_62_id_ecPublicKey), V_ASN1_UNDEF,
                              NULL); /* cannot fail */
...
static int
process_certConf(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *certConf, ossl_unused int certReqId,
                 const ASN1_OCTET_STRING *certHash, const OSSL_CMP_PKISI *si)
{
    ...
    if (ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(certConf) ||
        ctx->certOut == NULL) {
        ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
        return 0;
    }
...
static int
process_pollReq(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *pollReq, ossl_unused int certReqId,
...
ossl_cmp_mock_srv_new(OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (srv_ctx != NULL && ctx != NULL &&
        OSSL_CMP_SRV_CTX_init(srv_ctx, ctx, process_cert_request, process_rr, process_genm,
                              process_error, process_certConf, process_pollReq) &&
