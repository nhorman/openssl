...
static int
opt_found(const char *name, unsigned int *result, const OPT_PAIR pairs[], unsigned int nbelem)
{
    unsigned int idx;
...
static const OPT_PAIR rsa_choices[RSA_NUM] = {{"rsa512", R_RSA_512},    {"rsa1024", R_RSA_1024},
...
static int
mac_setup(const char *name, EVP_MAC **mac, OSSL_PARAM params[], loopargs_t *loopargs,
...
init_evp_cipher_ctx(const char *ciphername, const unsigned char *key, int keylen)
{
...
run_benchmark(int async_jobs, int (*loop_function)(void *), loopargs_t *loopargs)
{
    ...
    for (i = 0; i < async_jobs && !error; i++) {
        ...
        ret = ASYNC_start_job(&loopargs[i].inprogress_job, loopargs[i].wait_ctx, &job_op_count,
                              loop_function, (void *)&looparg_item, sizeof(looparg_item));
        switch (ret) {
        case ASYNC_PAUSE:
            ++num_inprogress;
            break;
    ...
    while (num_inprogress > 0) {
        ...
        for (i = 0; i < async_jobs; i++) {
            ...
            if (num_job_fds == 1 && !PeekNamedPipe(job_fd, NULL, 0, NULL, &avail, NULL) &&
                avail > 0)
                continue;
#endif
...
speed_main(int argc, char **argv)
{
...
#endif
    long op_count = 1;
    openssl_speed_sec_t seconds = {SECONDS,      RSA_SECONDS,   DSA_SECONDS, ECDSA_SECONDS,
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_PRIMES:
            ...
            break;
        case OPT_SECONDS:
            seconds.sym = seconds.rsa = seconds.dsa = seconds.ecdsa = seconds.ecdh = seconds.eddsa =
                seconds.sm2 = seconds.ffdh = seconds.kem = seconds.sig = opt_int_arg();
            break;
        case OPT_BYTES:
    ...
    if (doit[D_GHASH]) {
        ...
        params[1] = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_IV, (char *)gmac_iv,
                                                      sizeof(gmac_iv) - 1);
        ...
        if (evp_cipher != NULL) {
            ...
            for (testnum = 0; testnum < size_num; testnum++) {
                ...
                for (k = 0; k < loopargs_len; k++) {
                    ...
                    if (!ae_mode) {
                        if (!EVP_CipherInit_ex(loopargs[k].ctx, NULL, NULL, loopargs[k].key, iv,
                                               -1)) {
                    ...
                    if (ae_mode && decrypt) {
                        ...
                        if (!EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_GET_TAG, TAG_LEN,
                                                 &loopargs[k].tag)) {
    ...
    for (testnum = 0; testnum < RSA_NUM; testnum++) {
        ...
        if (primes > RSA_DEFAULT_PRIME_NUM) {
            ...
            st = (rsa_key = d2i_PrivateKey(EVP_PKEY_RSA, NULL, &p, rsa_keys[testnum].length)) !=
                 NULL;
        }
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].rsa_encrypt_ctx[testnum] == NULL ||
                ...
                EVP_PKEY_encrypt(loopargs[i].rsa_encrypt_ctx[testnum], loopargs[i].buf2,
                                 &loopargs[i].encsize, loopargs[i].buf, 36) <= 0)
                st = 0;
        }
        if (!st) {
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].rsa_decrypt_ctx[testnum] == NULL ||
                ...
                EVP_PKEY_decrypt(loopargs[i].rsa_decrypt_ctx[testnum], loopargs[i].buf, &declen,
                                 loopargs[i].buf2, loopargs[i].encsize) <= 0)
                st = 0;
        }
        if (!st) {
    ...
    for (testnum = 0; testnum < ECDSA_NUM; testnum++) {
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].ecdsa_verify_ctx[testnum] == NULL ||
                ...
                EVP_PKEY_verify(loopargs[i].ecdsa_verify_ctx[testnum], loopargs[i].buf2,
                                loopargs[i].sigsize, loopargs[i].buf, 20) <= 0)
                st = 0;
        }
        if (!st) {
    ...
    for (testnum = 0; testnum < EdDSA_NUM; testnum++) {
        ...
        if (st == 0) {
            ...
            if (st == 0) {
                ...
                pkey_print_message("sign", ed_curves[testnum].name, ed_curves[testnum].bits,
                                   seconds.eddsa);
                Time_F(START);
            ...
            if (st != 1) {
                ...
                pkey_print_message("verify", ed_curves[testnum].name, ed_curves[testnum].bits,
                                   seconds.eddsa);
                Time_F(START);
    ...
    for (testnum = 0; testnum < SM2_NUM; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if (!EVP_DigestSignInit(loopargs[i].sm2_ctx[testnum], NULL, EVP_sm3(), NULL, sm2_pkey))
                break;
            if (!EVP_DigestVerifyInit(loopargs[i].sm2_vfy_ctx[testnum], NULL, EVP_sm3(), NULL,
                                      sm2_pkey))
                break;
        ...
        if (st == 0) {
            ...
            if (st == 0) {
                ...
                pkey_print_message("sign", sm2_curves[testnum].name, sm2_curves[testnum].bits,
                                   seconds.sm2);
                Time_F(START);
            ...
            for (i = 0; i < loopargs_len; i++) {
                st = EVP_DigestVerify(loopargs[i].sm2_vfy_ctx[testnum], loopargs[i].buf2,
                                      loopargs[i].sigsize, loopargs[i].buf, 20);
                if (st != 1)
                    break;
            }
            if (st != 1) {
                ...
                pkey_print_message("verify", sm2_curves[testnum].name, sm2_curves[testnum].bits,
                                   seconds.sm2);
                Time_F(START);
    ...
    for (testnum = 0; testnum < kems_algs_len; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if (EVP_PKEY_decapsulate(kem_decaps_ctx, rcv_secret, &rcv_secret_len, out, out_len) <=
                    0 ||
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if (sig_gen_ctx == NULL)
                sig_gen_ctx = EVP_PKEY_CTX_new_from_name(
                    app_get0_libctx(), use_params == 1 ? "RSA" : sig_name, app_get0_propq());
            ...
            if (sig_verify_ctx == NULL || EVP_PKEY_verify_init(sig_verify_ctx) <= 0 ||
                (use_params == 1 &&
                 (EVP_PKEY_CTX_set_rsa_padding(sig_verify_ctx, RSA_PKCS1_PADDING) <= 0))) {
...
show_res:
    ...
    for (k = 0; k < OSSL_NELEM(ecdsa_doit); k++) {
        ...
        if (mr)
            ...
            printf("%4u bits ecdsa (%s) %8.4fs %8.4fs %8.1f %8.1f\n", ec_curves[k].bits,
                   ec_curves[k].name, 1.0 / ecdsa_results[k][0], 1.0 / ecdsa_results[k][1],
    ...
    for (k = 0; k < OSSL_NELEM(eddsa_doit); k++) {
        ...
        if (mr)
            ...
            printf("%4u bits EdDSA (%s) %8.4fs %8.4fs %8.1f %8.1f\n", ed_curves[k].bits,
                   ed_curves[k].name, 1.0 / eddsa_results[k][0], 1.0 / eddsa_results[k][1],
                   eddsa_results[k][0], eddsa_results[k][1]);
    }
#endif /* OPENSSL_NO_ECX */
    ...
    for (k = 0; k < OSSL_NELEM(sm2_doit); k++) {
        ...
        if (mr)
            ...
            printf("%4u bits SM2 (%s) %8.4fs %8.4fs %8.1f %8.1f\n", sm2_curves[k].bits,
                   sm2_curves[k].name, 1.0 / sm2_results[k][0], 1.0 / sm2_results[k][1],
                   sm2_results[k][0], sm2_results[k][1]);
    }
#endif
#ifndef OPENSSL_NO_DH
    ...
    for (k = 0; k < kems_algs_len; k++) {
        ...
        if (mr)
            ...
            printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n", kem_name,
                   1.0 / kems_results[k][0], 1.0 / kems_results[k][1], 1.0 / kems_results[k][2],
    ...
    for (k = 0; k < sigs_algs_len; k++) {
        ...
        if (mr)
            ...
            printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n", sig_name,
                   1.0 / sigs_results[k][0], 1.0 / sigs_results[k][1], 1.0 / sigs_results[k][2],
                   sigs_results[k][0], sigs_results[k][1], sigs_results[k][2]);
    }
    ret = 0;
...
{
    BIO_printf(bio_err, mr ? "+DTP:%d:%s:%s:%d\n" : "Doing %u bits %s %s ops for %ds: ", bits, str,
               str2, tm);
    (void)BIO_flush(bio_err);
    run = 1;
    alarm(tm);
...
multiblock_speed(const EVP_CIPHER *evp_cipher, int lengths_single,
                 const openssl_speed_sec_t *seconds)
{
    static const int mblengths_list[] = {8 * 1024, 2 * 8 * 1024, 4 * 8 * 1024, 8 * 8 * 1024,
                                         8 * 16 * 1024};
    ...
    for (j = 0; j < num; j++) {
        ...
        for (count = 0; run && COND(count); count++) {
            ...
            packlen = EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_AAD, sizeof(mb_param),
                                          &mb_param);
            ...
            if (packlen > 0) {
                ...
                mb_param.inp = inp;
                mb_param.len = len;
                (void)EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT, sizeof(mb_param),
                                          &mb_param);
            } else {
                int pad;
