...
static void user_data_init(struct user_data_st *user_data, SSL *con, char *buf, size_t bufmax,
                           int mode);
...
static unsigned int
psk_client_cb(SSL *ssl, const char *hint, char *identity, unsigned int max_identity_len,
...
static int
psk_use_session_cb(SSL *s, const EVP_MD *md, const unsigned char **id, size_t *idlen,
                   SSL_SESSION **sess)
{
...
static int
next_proto_cb(SSL *s, unsigned char **out, unsigned char *outlen, const unsigned char *in,
...
# define IS_PROT_FLAG(o)                                                                           \
     (o == OPT_SSL3 || o == OPT_TLS1 || o == OPT_TLS1_1 || o == OPT_TLS1_2 || o == OPT_TLS1_3 ||   \
      o == OPT_DTLS || o == OPT_DTLS1 || o == OPT_DTLS1_2 || o == OPT_QUIC)
...
# endif
        ...
        case OPT_MAXFRAGLEN:
            ...
            case 4096:
                ...
                break;
            default:
                BIO_printf(bio_err, "%s: Max Fragment Len %u is out of permitted values", prog,
                           len);
                goto opthelp;
            }
            break;
...
# endif
    ...
    for (i = 0; i < serverinfo_count; i++) {
        if (!SSL_CTX_add_client_custom_ext(ctx, serverinfo_types[i], NULL, NULL, NULL,
...
# ifndef OPENSSL_NO_QUIC
    ...
    if (isquic && alpn_in == NULL) {
        BIO_printf(bio_err,
                   "%s: QUIC requires ALPN to be specified (e.g. \"h3\" for HTTP/3) via the -alpn "
                   "option\n",
                   prog);
        goto end;
    }
...
re_start:
    ...
    peer_addr = NULL;
    if (init_client(&sock, host, port, bindhost, bindport, socket_family, socket_type, protocol,
                    tfo, !isquic, &peer_addr) == 0) {
...
# ifndef OPENSSL_NO_HTTP
    if (proxystr != NULL) {
        ...
        if (!OSSL_HTTP_proxy_connect(sbio, thost, tport, proxyuser, proxypass, 0 /* no timeout */,
                                     bio_err, prog))
            goto shut;
    }
# endif
    ...
    for (;;) {
        ...
        if ((isdtls || isquic) && SSL_get_event_timeout(con, &timeout, &is_infinite) &&
            !is_infinite)
            timeoutp = &timeout;
        else
            timeoutp = NULL;
...
static void
user_data_init(struct user_data_st *user_data, SSL *con, char *buf, size_t bufmax, int mode)
{
    user_data->con = con;
    user_data->buf = buf;
