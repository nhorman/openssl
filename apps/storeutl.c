...
static int process(const char *uri, const UI_METHOD *uimeth, PW_CB_DATA *uidata, int expected,
                   int criterion, OSSL_STORE_SEARCH *search, int text, int noout, int recursive,
                   int indent, const char *outfile, const char *prog, OSSL_LIB_CTX *libctx);
...
storeutl_main(int argc, char *argv[])
{
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_CRITERION_FINGERPRINT:
            if (criterion != 0 ||
                (criterion == OSSL_STORE_SEARCH_BY_KEY_FINGERPRINT && fingerprint != NULL)) {
                BIO_printf(bio_err, "%s: criterion already given.\n", prog);
                goto end;
            }
    ...
    if (criterion != 0) {
        ...
        case OSSL_STORE_SEARCH_BY_KEY_FINGERPRINT:
            if ((search = OSSL_STORE_SEARCH_by_key_fingerprint(digest, fingerprint,
                                                               fingerprintlen)) == NULL) {
                ERR_print_errors(bio_err);
                goto end;
            }
    ...
    ret = process(argv[0], get_ui_method(), &pw_cb_data, expected, criterion, search, text, noout,
                  recursive, 0, outfile, prog, libctx);
...
static int
process(const char *uri, const UI_METHOD *uimeth, PW_CB_DATA *uidata, int expected, int criterion,
        OSSL_STORE_SEARCH *search, int text, int noout, int recursive, int indent,
        const char *outfile, const char *prog, OSSL_LIB_CTX *libctx)
{
    OSSL_STORE_CTX *store_ctx = NULL;
    int ret = 1, items = 0;

    if ((store_ctx = OSSL_STORE_open_ex(uri, libctx, app_get0_propq(), uimeth, uidata, NULL, NULL,
                                        NULL)) == NULL) {
    ...
    for (;;) {
        ...
        case OSSL_STORE_INFO_NAME:
            if (recursive) {
                ...
                ret += process(suburi, uimeth, uidata, expected, criterion, search, text, noout,
                               recursive, indent + 2, outfile, prog, libctx);
            }
            break;
        ...
        case OSSL_STORE_INFO_PKEY:
            ...
            if (!noout)
                PEM_write_bio_PrivateKey(out, OSSL_STORE_INFO_get0_PKEY(info), NULL, NULL, 0, NULL,
                                         NULL);
            break;
