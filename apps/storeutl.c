    ...
    if (criterion != 0) {
        switch (criterion) {
            ...
            case OSSL_STORE_SEARCH_BY_ISSUER_SERIAL:
                ...
                if ((search =
                         OSSL_STORE_SEARCH_by_issuer_serial(issuer, serial))
                    == NULL) {
                    ERR_print_errors(bio_err);
                    goto end;
                }
                   ...
                   const char *prog, OSSL_LIB_CTX *libctx)
{
    ...
    if ((store_ctx =
             OSSL_STORE_open_ex(uri, libctx, app_get0_propq(), uimeth, uidata,
                                NULL, NULL, NULL))
        == NULL) {
    ...
    for (;;) {
        ...
        switch (type) {
            ...
            case OSSL_STORE_INFO_PARAMS:
                if (text)
                    EVP_PKEY_print_params(out, OSSL_STORE_INFO_get0_PARAMS(
                                              info),
                                          0, NULL);
                if (!noout)
            ...
            case OSSL_STORE_INFO_PUBKEY:
                if (text)
                    EVP_PKEY_print_public(out, OSSL_STORE_INFO_get0_PUBKEY(
                                              info),
                                          0, NULL);
                if (!noout)
                    PEM_write_bio_PUBKEY(out,
                                         OSSL_STORE_INFO_get0_PUBKEY(info));
                break;
            case OSSL_STORE_INFO_PKEY:
                if (text)
                    EVP_PKEY_print_private(out, OSSL_STORE_INFO_get0_PKEY(info),
                                           0, NULL);
                if (!noout)
                    PEM_write_bio_PrivateKey(out, OSSL_STORE_INFO_get0_PKEY(
                                                 info),
