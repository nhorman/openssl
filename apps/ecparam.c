...
ecparam_main(int argc, char **argv)
{
    ...
    if (curve_name != NULL) {
        ...
        *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, curve_name, 0);
        ...
        if (point_format != NULL)
            *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT,
                                                    point_format, 0);
        *p = OSSL_PARAM_construct_end();
        ...
        if (point_format &&
            !EVP_PKEY_set_utf8_string_param(params_key, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT,
                                            point_format)) {
    ...
    if (!noout) {
        ectx_params =
            OSSL_ENCODER_CTX_new_for_pkey(params_key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
    ...
    if (genkey) {
        ...
        ectx_key = OSSL_ENCODER_CTX_new_for_pkey(
            key, OSSL_KEYMGMT_SELECT_ALL, outformat == FORMAT_ASN1 ? "DER" : "PEM", NULL, NULL);
