...
load_content_info(int informat, BIO *in, int flags, BIO **indata, const char *name)
{
...
cms_main(int argc, char **argv)
{
    ...
    if (operation != SMIME_ENCRYPT && *argv != NULL)
        BIO_printf(bio_err, "Warning: recipient certificate file parameters ignored for operation "
                            "other than -encrypt\n");
    ...
    if (originatorfile != NULL) {
        if ((originator = load_cert(originatorfile, FORMAT_UNDEF, "originator certificate file")) ==
            NULL) {
            ERR_print_errors(bio_err);
            goto end;
        }
    ...
    if (operation == SMIME_SIGN_RECEIPT) {
        if ((signer = load_cert(signerfile, FORMAT_UNDEF, "receipt signer certificate file")) ==
            NULL) {
            ERR_print_errors(bio_err);
            goto end;
        }
    ...
    if (operation == SMIME_DATA_CREATE) {
        ...
        for (i = 0; i < sk_X509_num(encerts); i++) {
            ...
            res = EVP_PKEY_CTX_ctrl(pctx, -1, -1, EVP_PKEY_CTRL_CIPHER, EVP_CIPHER_get_nid(cipher),
                                    NULL);
            if (res <= 0 && res != -2)
                goto end;
        ...
        if (secret_key != NULL) {
            if (!CMS_add0_recipient_key(cms, NID_undef, secret_key, secret_keylen, secret_keyid,
        ...
        if (!(flags & CMS_STREAM)) {
            if (!CMS_final(cms, in, NULL, flags)) {
                if (originator != NULL && ERR_GET_REASON(ERR_peek_error()) ==
        ...
        cms = CMS_EncryptedData_encrypt_ex(in, cipher, secret_key, secret_keylen, flags, libctx,
                                           app_get0_propq());
        ...
        if (operation == SMIME_SIGN) {
            ...
            if (rr_to != NULL &&
                ((rr = make_receipt_request(rr_to, rr_allorfirst, rr_from)) == NULL)) {
        ...
        if (operation == SMIME_SIGN && digestbin != NULL && (flags & CMS_STREAM) == 0) {
                     ...
                     STACK_OF(OPENSSL_STRING) * rr_from)
{
    ...
    rr =
        CMS_ReceiptRequest_create0_ex(NULL, -1, rr_allorfirst, rct_from, rct_to, app_get0_libctx());
    if (rr == NULL)
        goto err;
    return rr;
err:
