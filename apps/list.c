...
static void
legacy_cipher_fn(const EVP_CIPHER *c, const char *from, const char *to, void *arg)
{
    if (select_name != NULL &&
        (c == NULL || OPENSSL_strcasecmp(select_name, EVP_CIPHER_get0_name(c)) != 0))
        return;
    if (c != NULL) {
    ...
    for (i = 0; i < sk_EVP_CIPHER_num(ciphers); i++) {
        ...
        if (names != NULL && EVP_CIPHER_names_do_all(c, collect_names, names)) {
            ...
            if (verbose) {
                ...
                print_param_types("retrievable algorithm parameters", EVP_CIPHER_gettable_params(c),
                                  4);
    ...
    for (i = 0; i < sk_EVP_MD_num(digests); i++) {
        ...
        if (names != NULL && EVP_MD_names_do_all(m, collect_names, names)) {
            ...
            if (verbose) {
                ...
                print_param_types("retrievable operation parameters", EVP_MD_gettable_ctx_params(m),
                                  4);
                print_param_types("settable operation parameters", EVP_MD_settable_ctx_params(m),
                                  4);
            }
        }
        sk_OPENSSL_CSTRING_free(names);
    ...
    for (i = 0; i < sk_EVP_MAC_num(macs); i++) {
        ...
        if (names != NULL && EVP_MAC_names_do_all(m, collect_names, names)) {
            ...
            if (verbose) {
                ...
                if (desc != NULL)
                    BIO_printf(bio_out, "    description: %s\n", desc);
                print_param_types("retrievable algorithm parameters", EVP_MAC_gettable_params(m),
                                  4);
                print_param_types("retrievable operation parameters",
                                  EVP_MAC_gettable_ctx_params(m), 4);
                print_param_types("settable operation parameters", EVP_MAC_settable_ctx_params(m),
                                  4);
            }
        }
        sk_OPENSSL_CSTRING_free(names);
    ...
    for (i = 0; i < sk_EVP_KDF_num(kdfs); i++) {
        ...
        if (names != NULL && EVP_KDF_names_do_all(k, collect_names, names)) {
            ...
            if (verbose) {
                ...
                if (desc != NULL)
                    BIO_printf(bio_out, "    description: %s\n", desc);
                print_param_types("retrievable algorithm parameters", EVP_KDF_gettable_params(k),
                                  4);
                print_param_types("retrievable operation parameters",
                                  EVP_KDF_gettable_ctx_params(k), 4);
                print_param_types("settable operation parameters", EVP_KDF_settable_ctx_params(k),
                                  4);
            }
        }
        sk_OPENSSL_CSTRING_free(names);
...
list_random_generators(void)
{
    ...
    for (i = 0; i < sk_EVP_RAND_num(rands); i++) {
        ...
        if (verbose) {
            ...
            print_param_types("retrievable operation parameters", EVP_RAND_gettable_ctx_params(m),
                              4);
    ...
    for (i = 0; i < sk_EVP_KEYMGMT_num(km_stack); i++) {
        ...
        if (names != NULL && EVP_KEYMGMT_names_do_all(k, collect_names, names)) {
            ...
            if (verbose) {
                ...
                print_param_types("settable operation parameters", EVP_KEYMGMT_settable_params(k),
                                  4);
                print_param_types("retrievable operation parameters",
    ...
    for (i = 0; i < sk_EVP_KEM_num(kem_stack); i++) {
        ...
        if (names != NULL && EVP_KEM_names_do_all(k, collect_names, names)) {
            ...
            if (verbose) {
                ...
                if (desc != NULL)
                    BIO_printf(bio_out, "    description: %s\n", desc);
                print_param_types("settable operation parameters", EVP_KEM_settable_ctx_params(k),
                                  4);
                print_param_types("retrievable operation parameters",
