    ...
    if (cipher != NULL) {
        if (str != NULL) { /* a passphrase is available */
            ...
            if (pbkdf2 == 1) {
                ...
                if (!PKCS5_PBKDF2_HMAC(str, str_len, sptr, islen, iter, dgst, iklen + ivlen,
                                       tmpkeyiv)) {
                ...
                if (!EVP_BytesToKey(cipher, dgst, sptr, (unsigned char *)str, str_len, 1, key,
                                    iv)) {
        ...
        if (rawkey_set) {
            if (!EVP_CipherInit_ex(ctx, cipher, e, key, (hiv == NULL && wrap == 1 ? NULL : iv),
                                   enc)) {
            ...
            params =
                app_params_new_from_opts(skeyopts, EVP_SKEYMGMT_get0_imp_settable_params(mgmt));
            if (params == NULL)
                goto end;
