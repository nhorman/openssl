...
static int
print_to_bio_out(const char *func, const char *file, int line, OSSL_CMP_severity level,
                 const char *msg)
{
...
static int
print_to_bio_err(const char *func, const char *file, int line, OSSL_CMP_severity level,
                 const char *msg)
{
...
static EVP_PKEY *
load_key_pwd(const char *uri, int format, const char *pass, ENGINE *eng, const char *desc)
{
...
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
            if (opt_server == NULL) {
                CMP_err("missing -server or -use_mock_srv option, or too few -rspin filename "
                        "arguments");
                goto err;
            }
            if (rspin_in_use)
...
#else
            CMP_err("-server not supported on no-sock/no-http build; missing -use_mock_srv option "
                    "or too few -rspin filename arguments");
#endif
        }
        rspin_in_use = 0;
    }
...
static int
set_name(const char *str, int (*set_fn)(OSSL_CMP_CTX *ctx, const X509_NAME *name),
...
static int
setup_cert(void *ctx, const char *file, const char *pass, const char *desc, add_X509_fn_t set1_fn)
{
    X509 *cert;
    int ok;
...
static int
setup_certs(char *files, const char *desc, void *ctx, add_X509_stack_fn_t set1_fn)
{
    STACK_OF(X509) * certs;
    int ok;
...
setup_srv_ctx(ENGINE *engine)
{
    ...
    if (opt_srv_secret != NULL) {
        ...
        CMP_err("server credentials (-srv_secret or -srv_cert) must be given if -use_mock_srv or "
                "-port is used");
        goto err;
    } else {
        CMP_warn("server will not be able to handle PBM-protected requests since -srv_secret is "
                 "not given");
    }
    ...
    if (!setup_cert(ctx, opt_srv_cert, opt_srv_keypass, "signer certificate of the mock server",
        ...
        goto err;
    if (opt_srv_key != NULL) {
        EVP_PKEY *pkey = load_key_pwd(opt_srv_key, opt_keyform, opt_srv_keypass, engine,
    ...
    if (opt_srv_trusted != NULL) {
        ...
        CMP_warn("mock server will not be able to handle signature-protected requests since "
                 "-srv_trusted is not given");
    }
    ...
    if (opt_rsp_key != NULL) {
        EVP_PKEY *pkey = load_key_pwd(opt_rsp_key, opt_keyform, opt_rsp_keypass, engine,
...
setup_protection_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_ref != NULL &&
        !OSSL_CMP_CTX_set1_referenceValue(ctx, (unsigned char *)opt_ref, strlen(opt_ref)))
        return 0;
...
setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL && opt_cert == NULL &&
    ...
    if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
        if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen && opt_key == NULL &&
            opt_csr == NULL && opt_oldcert == NULL) {
            CMP_err("missing -newkey (or -key) to be certified and no -csr, -oldcert, -cert, or "
                    "-reqin option given, which could provide fallback public key."
        ...
        if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE &&
            ...
            if (opt_csr != NULL) {
                CMP_err1("no -newkey option given with private key for POPO, -csr option provides "
                         "just public key%s",
                         opt_key == NULL ? "" : ", and -key option superseded by -csr");
                if (opt_reqin != NULL)
                    CMP_info("since -reqin is used, may use -popo -1 or -popo 0 to disable the "
                             "needless generation of a POPO");
                return 0;
            }
            if (opt_key == NULL) {
    ...
    if (opt_cmd == CMP_RR) {
        if (opt_issuer == NULL && opt_serial == NULL) {
            if (opt_oldcert == NULL && opt_csr == NULL) {
                CMP_err("missing -oldcert or -issuer and -serial for certificate to be revoked and "
                        "no -csr given");
                return 0;
            }
            if (opt_oldcert != NULL && opt_csr != NULL)
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
    ...
    if (opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL && opt_oldcert == NULL &&
        opt_cert == NULL)
        CMP_warn("missing -recipient, -srvcert, -issuer, -oldcert or -cert; recipient for any "
                 "requests not covered by -reqin will be set to \"NULL-DN\"");
    ...
    } else if (opt_reqin != NULL && opt_key == NULL && opt_csr == NULL && opt_oldcert == NULL &&
               !opt_centralkeygen) {
...
setup_client_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
...
#else
        CMP_err("missing -use_mock_srv or -rspin option; -server option is not supported due to "
                "no-sock build");
        goto err;
#endif
    }
    ...
    if (!OSSL_HTTP_parse_url(opt_server, &use_ssl, NULL /* user */, &host, &port, &portnum, &path,
                             NULL /* q */, NULL /* frag */)) {
    ...
    if (opt_no_proxy != NULL && !OSSL_CMP_CTX_set1_no_proxy(ctx, opt_no_proxy))
        goto oom;
    (void)BIO_snprintf(server_buf, sizeof(server_buf), "http%s://%s:%s/%s", opt_tls_used ? "s" : "",
                       host, port, *used_path == '/' ? used_path + 1 : used_path);
...
set_path:
    ...
    if (opt_total_timeout > 0 && opt_msg_timeout > 0 && opt_total_timeout < opt_msg_timeout) {
        CMP_err2("-total_timeout argument = %d must not be < %d (-msg_timeout)", opt_total_timeout,
                 opt_msg_timeout);
        goto err;
    }
    ...
    if (opt_reqin != NULL || opt_reqout != NULL || opt_rspin != NULL || opt_rspout != NULL ||
        opt_use_mock_srv)
...
delete_file(const char *file, const char *desc)
{
    ...
    if (unlink(file) != 0 && errno != ENOENT) {
        CMP_err2("Failed to delete %s, which should be done to indicate there is no %s", file,
                 desc);
        return 0;
    }
    return 1;
...
static int
conf_get_number_e(const CONF *conf_, const char *groups, const char *name, long *result)
{
...
read_config(void)
{
    ...
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL; i++, opt++)
        if (!strcmp(opt->name, OPT_SECTION_STR) || !strcmp(opt->name, OPT_MORE_STR))
            n_options--;
    OPENSSL_assert(OSSL_NELEM(cmp_vars) == n_options + OPT_PROV__FIRST + 1 - OPT_PROV__LAST +
...
get_opts(int argc, char **argv)
{
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_REVREASON:
            ...
            if (opt_revreason < CRL_REASON_NONE || opt_revreason > CRL_REASON_AA_COMPROMISE ||
                opt_revreason == 7) {
...
cmp_server(OSSL_CMP_CTX *srv_cmp_ctx)
{
    ...
    while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
        ...
        ret = http_server_get_asn1_req(ASN1_ITEM_rptr(OSSL_CMP_MSG), (ASN1_VALUE **)&req, &path,
                                       &cbio, acbio, &keep_alive, prog, 0, 0);
        ...
        if (req != NULL) {
            ...
            ret =
                http_server_send_asn1_resp(prog, cbio, keep_alive, "application/pkixcmp",
...
do_genm(OSSL_CMP_CTX *ctx)
{
    if (opt_infotype == NID_id_it_caCerts) {
        ...
        if (opt_oldwithold == NULL) {
            CMP_warn("No -oldwithold given, will use all certs given with -trusted as trust "
                     "anchors for verifying the newWithNew cert");
        } else {
        ...
        if (newwithnew == NULL)
            ...
            CMP_warn("oldWithNew certificate received in genp for verifying oldWithOld, but "
                     "oldWithOld was not provided");
    ...
    end_upd:
        ...
        if (opt_oldcrl != NULL) {
            oldcrl =
                load_crl(opt_oldcrl, FORMAT_UNDEF, 0, "CRL for genm with -infotype crlStatusList");
            if (oldcrl == NULL)
                goto end_crlupd;
        }
...
cmp_main(int argc, char **argv)
{
    ...
    if (configfile != NULL && configfile[0] != '\0' /* non-empty string */
        ...
        CMP_info2("using section(s) '%s' of OpenSSL configuration file '%s'", opt_section,
                  configfile);
...
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
    if (opt_tls_cert == NULL && opt_tls_key == NULL && opt_tls_keypass == NULL &&
        opt_tls_extra == NULL && opt_tls_trusted == NULL && opt_tls_host == NULL) {
        if (opt_tls_used)
...
# endif
    ...
    for (i = 0; i < opt_repeat; i++) {
        ...
        print_status();
        if (!save_cert_or_delete(OSSL_CMP_CTX_get0_validatedSrvCert(cmp_ctx), opt_srvcertout,
                                 "validated server"))
            ret = 0;
        if (!ret)
            goto err;
        ret = 0;
        if (save_free_certs(OSSL_CMP_CTX_get1_extraCertsIn(cmp_ctx), opt_extracertsout, "extra") <
            0)
            goto err;
        if (newcert != NULL && (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR ||
                                opt_cmd == CMP_P10CR)) {
            ...
            if (opt_centralkeygen) {
                ...
                if (opt_newkeypass != NULL) {
                    pass_string =
                        get_passwd(opt_newkeypass, "Centrally generated private key password");
