...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
        if (opt_newkey == NULL
            ...
            CMP_err(
                "missing -newkey (or -key) to be certified and no -csr, -oldcert, or -cert given for fallback public key");
            return 0;
        }
        if (opt_newkey == NULL
            ...
            if (opt_csr != NULL) {
                CMP_err1(
                    "no -newkey option given with private key for POPO, -csr option only provides public key%s",
                    opt_key == NULL ? "" :
...
opthelp:
            ...
            case OPT_REVREASON:
                ...
                if (opt_revreason < CRL_REASON_NONE
                    ...
                    || opt_revreason == 7) {
                    CMP_err(
                        "invalid revreason. Valid values are -1 .. 6, 8 .. 10");
                    goto opthelp;
                }
                break;
            case OPT_ISSUER:
