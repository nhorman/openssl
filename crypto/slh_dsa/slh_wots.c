 ...
 */
static int
slh_wots_chain(SLH_DSA_HASH_CTX *ctx, const uint8_t *in, uint8_t start_index, uint8_t steps,
 ...
 */
int
ossl_slh_wots_pk_gen(SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed, const uint8_t *pk_seed,
 ...
 */
int
ossl_slh_wots_sign(SLH_DSA_HASH_CTX *ctx, const uint8_t *msg, const uint8_t *sk_seed,
 ...
 */
int
ossl_slh_wots_pk_from_sig(SLH_DSA_HASH_CTX *ctx, PACKET *sig_rpkt, const uint8_t *msg,
    ...
    for (i = 0; i < len; ++i) {
        ...
        if (!PACKET_get_bytes(sig_rpkt, &sig_i, n) ||
            !slh_wots_chain(ctx, sig_i, msg_and_csum_nibbles[i],
                            NIBBLE_MASK - msg_and_csum_nibbles[i], pk_seed, adrs, tmp_pkt))
            goto err;
    }
