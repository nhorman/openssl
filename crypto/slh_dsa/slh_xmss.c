 ...
 */
int
ossl_slh_xmss_node(SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed, uint32_t node_id, uint32_t h,
                   const uint8_t *pk_seed, uint8_t *adrs, uint8_t *pk_out, size_t pk_out_len)
{
    ...
    if (h == 0) {
        ...
        if (!ossl_slh_xmss_node(ctx, sk_seed, 2 * node_id, h - 1, pk_seed, adrs, lnode,
                                sizeof(lnode)) ||
            !ossl_slh_xmss_node(ctx, sk_seed, 2 * node_id + 1, h - 1, pk_seed, adrs, rnode,
                                sizeof(rnode)))
            return 0;
 ...
 */
int
ossl_slh_xmss_sign(SLH_DSA_HASH_CTX *ctx, const uint8_t *msg, const uint8_t *sk_seed,
 ...
 */
int
ossl_slh_xmss_pk_from_sig(SLH_DSA_HASH_CTX *ctx, uint32_t node_id, PACKET *sig_rpkt,
