...
dl_load(DSO *dso)
{
    ...
    ptr = shl_load(filename,
                   BIND_IMMEDIATE | (dso->flags & DSO_FLAG_NO_NAME_TRANSLATION ? 0 : DYNAMIC_PATH),
                   0L);
    if (ptr == NULL) {
        char errbuf[160];
        ...
        if (openssl_strerror_r(errno, errbuf, sizeof(errbuf)))
            ...
            ERR_raise_data(ERR_LIB_DSO, DSO_R_LOAD_FAILED, "filename(%s): errno %d", filename,
                           errno);
        goto err;
    }
...
dl_name_converter(DSO *dso, const char *filename)
{
    ...
    if (transform)
        BIO_snprintf(translated, rsize,
                     (DSO_flags(dso) & DSO_FLAG_NAME_TRANSLATION_EXT_ONLY) == 0 ? "lib%s%s"
                                                                                : "%s%s",
