...
static int ts_compute_imprint(BIO *data, TS_TST_INFO *tst_info, X509_ALGOR **md_alg,
 ...
 */
int
TS_RESP_verify_signature(PKCS7 *token, STACK_OF(X509) * certs, X509_STORE *store, X509 **signer_out)
{
 ...
 */
static int
ts_verify_cert(X509_STORE *store, STACK_OF(X509) * untrusted, X509 *signer, STACK_OF(X509) * *chain)
{
 ...
 */
static int
int_ts_RESP_verify_token(TS_VERIFY_CTX *ctx, PKCS7 *token, TS_TST_INFO *tst_info)
{
    X509 *signer = NULL;
...
static int
ts_compute_imprint(BIO *data, TS_TST_INFO *tst_info, X509_ALGOR **md_alg, unsigned char **imprint,
...
static int
ts_check_imprints(X509_ALGOR *algor_a, const unsigned char *imprint_a, unsigned len_a,
                  TS_TST_INFO *tst_info)
{
