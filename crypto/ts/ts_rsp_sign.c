...
TS_RESP_CTX_set_accuracy(TS_RESP_CTX *ctx, int secs, int millis, int micros)
{
    ...
    if (secs &&
        ((ctx->seconds = ASN1_INTEGER_new()) == NULL || !ASN1_INTEGER_set(ctx->seconds, secs)))
        goto err;
    if (millis &&
        ((ctx->millis = ASN1_INTEGER_new()) == NULL || !ASN1_INTEGER_set(ctx->millis, millis)))
        goto err;
    if (micros &&
        ((ctx->micros = ASN1_INTEGER_new()) == NULL || !ASN1_INTEGER_set(ctx->micros, micros)))
        goto err;
...
static int
ossl_ess_add1_signing_cert_v2(PKCS7_SIGNER_INFO *si, const ESS_SIGNING_CERT_V2 *sc)
{
    ASN1_STRING *seq = NULL;
    ...
    OPENSSL_free(pp);
    return PKCS7_add_signed_attribute(si, NID_id_smime_aa_signingCertificateV2, V_ASN1_SEQUENCE,
                                      seq);
}
...
ts_RESP_sign(TS_RESP_CTX *ctx)
{
    ...
    if (ctx->ess_cert_id_digest == NULL || EVP_MD_is_a(ctx->ess_cert_id_digest, SN_sha1)) {
        ...
        sc2 =
            OSSL_ESS_signing_cert_v2_new_init(ctx->ess_cert_id_digest, ctx->signer_cert, certs, 0);
        if (sc2 == NULL)
            goto err;
...
TS_RESP_set_genTime_with_precision(ASN1_GENERALIZEDTIME *asn1_time, long sec, long usec,
                                   unsigned precision)
{
     ...
     */
    p += BIO_snprintf(p, p_end - p, "%04d%02d%02d%02d%02d%02d", tm->tm_year + 1900, tm->tm_mon + 1,
