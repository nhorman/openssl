...
static int
put_encoder_in_store(void *store, void *method, const OSSL_PROVIDER *prov, const char *names,
                     const char *propdef, void *data)
{
    ...
    return ossl_method_store_add(store, prov, id, propdef, method, ossl_encoder_up_ref,
                                 ossl_encoder_free);
}

/* Create and populate a encoder method */
static void *
encoder_from_algorithm(int id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
    OSSL_ENCODER *encoder = NULL;
 ...
 */
static void *
construct_encoder(const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov, void *data)
{
    /*
...
inner_ossl_encoder_fetch(struct encoder_data_st *methdata, const char *name, const char *properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = {get_tmp_encoder_store,   reserve_encoder_store,
        ...
        if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_ENCODER, &prov,
            ...
            ossl_method_store_cache_set(store, prov, id, propq, method, up_ref_encoder,
                                        free_encoder);
        }
...
OSSL_ENCODER *
OSSL_ENCODER_fetch(OSSL_LIB_CTX *libctx, const char *name, const char *properties)
{
...
OSSL_ENCODER_do_all_provided(OSSL_LIB_CTX *libctx,
                             void (*user_fn)(OSSL_ENCODER *encoder, void *arg), void *user_arg)
{
    struct encoder_data_st methdata;
...
int
OSSL_ENCODER_names_do_all(const OSSL_ENCODER *encoder, void (*fn)(const char *name, void *data),
                          void *data)
{
    if (encoder == NULL)
        return 0;
