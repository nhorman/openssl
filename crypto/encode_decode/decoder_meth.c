...
static int
put_decoder_in_store(void *store, void *method, const OSSL_PROVIDER *prov, const char *names,
                     const char *propdef, void *data)
{
    ...
    return ossl_method_store_add(store, prov, id, propdef, method, ossl_decoder_up_ref,
                                 ossl_decoder_free);
}

/* Create and populate a decoder method */
void *
ossl_decoder_from_algorithm(int id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
    OSSL_DECODER *decoder = NULL;
 ...
 */
static void *
construct_decoder(const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov, void *data)
{
    /*
...
inner_ossl_decoder_fetch(struct decoder_data_st *methdata, const char *name, const char *properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = {get_tmp_decoder_store,   reserve_decoder_store,
        ...
        if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_DECODER, &prov,
            ...
            if (id != 0)
                ossl_method_store_cache_set(store, prov, id, propq, method, up_ref_decoder,
                                            free_decoder);
        }
...
OSSL_DECODER *
OSSL_DECODER_fetch(OSSL_LIB_CTX *libctx, const char *name, const char *properties)
{
...
OSSL_DECODER_do_all_provided(OSSL_LIB_CTX *libctx,
                             void (*user_fn)(OSSL_DECODER *decoder, void *arg), void *user_arg)
{
    struct decoder_data_st methdata;
...
int
OSSL_DECODER_names_do_all(const OSSL_DECODER *decoder, void (*fn)(const char *name, void *data),
                          void *data)
{
    if (decoder == NULL)
        return 0;
