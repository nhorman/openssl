...
int
OSSL_DECODER_CTX_set_passphrase(OSSL_DECODER_CTX *ctx, const unsigned char *kstr, size_t klen)
{
    return ossl_pw_set_passphrase(&ctx->pwdata, kstr, klen);
}

int
OSSL_DECODER_CTX_set_passphrase_ui(OSSL_DECODER_CTX *ctx, const UI_METHOD *ui_method, void *ui_data)
{
    return ossl_pw_set_ui_method(&ctx->pwdata, ui_method, ui_data);
}

int
OSSL_DECODER_CTX_set_pem_password_cb(OSSL_DECODER_CTX *ctx, pem_password_cb *cb, void *cbarg)
{
    return ossl_pw_set_pem_password_cb(&ctx->pwdata, cb, cbarg);
}

int
OSSL_DECODER_CTX_set_passphrase_cb(OSSL_DECODER_CTX *ctx, OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg)
{
...
static int
decoder_construct_pkey(OSSL_DECODER_INSTANCE *decoder_inst, const OSSL_PARAM *params,
                       void *construct_data)
{
    ...
    if (i < end) {
        ...
        if (!EVP_KEYMGMT_up_ref(keymgmt))
            return 0;
    } else if ((keymgmt = EVP_KEYMGMT_fetch(data->libctx, data->object_type, data->propq)) !=
               NULL) {
        keymgmt_prov = EVP_KEYMGMT_get0_provider(keymgmt);
    }
...
static void
collect_decoder_keymgmt(EVP_KEYMGMT *keymgmt, OSSL_DECODER *decoder, void *provctx,
...
check_keymgmt(EVP_KEYMGMT *keymgmt, struct collect_data_st *data)
{
    ...
    if (!data->keytype_resolved) {
         ...
         */
        if (data->keytype_id != 0 && (strcmp(data->keytype, "id-ecPublicKey") == 0 ||
 ...
 */
static int
ossl_decoder_ctx_setup_for_pkey(OSSL_DECODER_CTX *ctx, const char *keytype, OSSL_LIB_CTX *libctx,
...
ossl_decoder_ctx_for_pkey_dup(OSSL_DECODER_CTX *src, EVP_PKEY **pkey, const char *input_type,
...
decoder_cache_entry_hash(const DECODER_CACHE_ENTRY *cache)
{
    ...
    hash = (hash * 23) +
           (cache->input_structure == NULL ? 0 : ossl_lh_strcasehash(cache->input_structure));
...
static int
decoder_cache_entry_cmp(const DECODER_CACHE_ENTRY *a, const DECODER_CACHE_ENTRY *b)
{
    int cmp;
...
OSSL_DECODER_CTX *
OSSL_DECODER_CTX_new_for_pkey(EVP_PKEY **pkey, const char *input_type, const char *input_structure,
                              ...
                              const char *propquery)
{
    ...
    if (propquery != NULL)
        decoder_params[i++] =
            OSSL_PARAM_construct_utf8_string(OSSL_DECODER_PARAM_PROPERTIES, (char *)propquery, 0);
