...
int
OSSL_ENCODER_to_data(OSSL_ENCODER_CTX *ctx, unsigned char **pdata, size_t *pdata_len)
{
    BIO *out;
    BUF_MEM *buf = NULL;
    int ret = 0;
...
int
OSSL_ENCODER_CTX_set_output_structure(OSSL_ENCODER_CTX *ctx, const char *output_structure)
{
...
static int
ossl_encoder_ctx_add_encoder_inst(OSSL_ENCODER_CTX *ctx, OSSL_ENCODER_INSTANCE *ei)
{
    int ok;
...
int
OSSL_ENCODER_CTX_add_extra(OSSL_ENCODER_CTX *ctx, OSSL_LIB_CTX *libctx, const char *propq)
{
    return 1;
}
...
int
OSSL_ENCODER_CTX_set_construct(OSSL_ENCODER_CTX *ctx, OSSL_ENCODER_CONSTRUCT *construct)
{
    if (!ossl_assert(ctx != NULL)) {
        ERR_raise(ERR_LIB_OSSL_ENCODER, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
...
int
OSSL_ENCODER_CTX_set_cleanup(OSSL_ENCODER_CTX *ctx, OSSL_ENCODER_CLEANUP *cleanup)
{
    if (!ossl_assert(ctx != NULL)) {
        ERR_raise(ERR_LIB_OSSL_ENCODER, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
    ...
    for (i = data->current_encoder_inst_index; i-- > 0;) {
        ...
        if (top) {
            if (data->ctx->output_type != NULL &&
                ...
                {
                    BIO_printf(trc_out,
                               "[%d]    Skipping because current encoder output type (%s) != "
            ...
            if (!OSSL_ENCODER_is_a(next_encoder, current_output_type)) {
                ...
                {
                    BIO_printf(trc_out,
                               "[%d]    Skipping because current encoder output type (%s) != name "
                               "of encoder %p\n",
        ...
        if (data->ctx->output_structure != NULL && current_output_structure != NULL) {
            if (OPENSSL_strcasecmp(data->ctx->output_structure, current_output_structure) != 0) {
                ...
                {
                    BIO_printf(trc_out,
                               "[%d]    Skipping because current encoder output structure (%s) != "
                               "ctx output structure (%s)\n",
        ...
        case 1:
...

            {
                ...
                *abstract_p++ = OSSL_PARAM_construct_octet_string(
                    OSSL_OBJECT_PARAM_DATA, data->running_output, data->running_output_length);
        ...
        if (ok) {
            ...
            if (ok) {
                ok = current_encoder->encode(current_encoder_ctx, cbio, original_data,
                                             ...
                                             ossl_pw_passphrase_callback_enc, &data->ctx->pwdata);
                OSSL_TRACE_BEGIN(ENCODER)
                {
...
ossl_bio_print_labeled_bignum(BIO *out, const char *label, const BIGNUM *bn)
{
    ...
    while (*p != '\0') {
        ...
        if (BIO_printf(out, "%s%c%c", use_sep ? ":" : "", tolower((unsigned char)p[0]),
...
int
ossl_bio_print_labeled_buf(BIO *out, const char *label, const unsigned char *buf, size_t buflen)
{
    size_t i;
