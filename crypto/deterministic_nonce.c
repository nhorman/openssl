 ...
 */
static int
bits2int_consttime(BIGNUM *out, int qlen_bits, const unsigned char *in, size_t inlen)
{
 ...
 */
static int
bits2octets(unsigned char *out, const BIGNUM *q, int qlen_bits, int rlen, const unsigned char *in,
            size_t inlen)
{
    int ret = 0;
    BIGNUM *z = BN_new();
 ...
 */
static EVP_KDF_CTX *
kdf_setup(const char *digestname, const unsigned char *entropy, size_t entropylen,
          const unsigned char *nonce, size_t noncelen, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_ENTROPY, (void *)entropy,
                                             entropylen);
    *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_NONCE, (void *)nonce, noncelen);
 ...
 */
int
ossl_gen_deterministic_nonce_rfc6979(BIGNUM *out, const BIGNUM *q, const BIGNUM *priv,
                                     const unsigned char *hm, size_t hmlen, const char *digestname,
                                     OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    do {
        if (!EVP_KDF_derive(kdfctx, rbits, rlen, NULL) ||
            ...
            goto end;
    } while (ossl_bn_is_word_fixed_top(out, 0) || ossl_bn_is_word_fixed_top(out, 1) ||
             BN_ucmp(out, q) >= 0);
#ifdef BN_DEBUG
