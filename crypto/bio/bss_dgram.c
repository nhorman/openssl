...
#  define BIO_CMSG_ALLOC_LEN                                                                       \
      BIO_MAX(BIO_CMSG_ALLOC_LEN_1, BIO_MAX(BIO_CMSG_ALLOC_LEN_2, BIO_CMSG_ALLOC_LEN_3))
# endif
...
static int dgram_sendmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
                          size_t *num_processed);
static int dgram_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
                          size_t *num_processed);
...
static long
dgram_get_mtu_overhead(bio_dgram_data *data)
{
    long ret;
...
# if OPENSSL_USE_IPV6 && defined(IPV6_MTU_DISCOVER) && defined(IPV6_PMTUDISC_DO)
        case AF_INET6:
            ...
            if ((ret = setsockopt(b->num, IPPROTO_IPV6, IPV6_MTU_DISCOVER, &sockopt_val,
                                  sizeof(sockopt_val))) < 0)
                ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
            break;
# endif
    ...
    case BIO_CTRL_DGRAM_QUERY_MTU:
        ...
        switch (addr.sa.sa_family) {
        case AF_INET:
            if ((ret = getsockopt(b->num, IPPROTO_IP, IP_MTU, (void *)&sockopt_val, &sockopt_len)) <
                    0 ||
                sockopt_val < 0) {
                ret = 0;
            } else {
...
# ifdef OPENSSL_SYS_WINDOWS
    {
        ...
        if ((ret = setsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout, sizeof(timeout))) <
            0)
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
    }
# else
...
# ifdef OPENSSL_SYS_WINDOWS
    {
        ...
        if ((ret = setsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout, sizeof(timeout))) <
            0)
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
    }
# else
...
# endif
    ...
    case BIO_CTRL_DGRAM_GET_EFFECTIVE_CAPS:
        ret = (long)(BIO_DGRAM_CAP_HANDLES_DST_ADDR | BIO_DGRAM_CAP_HANDLES_SRC_ADDR |
...
static void
translate_msg_win(BIO *b, WSAMSG *mh, WSABUF *iov, unsigned char *control, BIO_MSG *msg)
{
...
static void
translate_msg(BIO *b, struct msghdr *mh, struct iovec *iov, unsigned char *control, BIO_MSG *msg)
{
    bio_dgram_data *data;
...
static int
dgram_sendmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
               size_t *num_processed)
{
...
static int
dgram_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
               size_t *num_processed)
{
#if M_METHOD != M_METHOD_NONE && M_METHOD != M_METHOD_RECVMSG
    int ret;
...
# ifdef SCTP_AUTH_DEACTIVATE_KEY
            sockopt_len = sizeof(struct sctp_authkeyid);
            ret =
                setsockopt(b->num, IPPROTO_SCTP, SCTP_AUTH_DEACTIVATE_KEY, &authkeyid, sockopt_len);
            if (ret < 0)
                break;
# endif
...
int
BIO_dgram_sctp_notification_cb(BIO *b, BIO_dgram_sctp_notification_handler_fn handle_notifications,
                               void *context)
{
    bio_dgram_sctp_data *data = (bio_dgram_sctp_data *)b->ptr;
...
# else
    ...
    ret =
        setsockopt(b->num, IPPROTO_SCTP, SCTP_EVENTS, &event, sizeof(struct sctp_event_subscribe));
# endif
    if (ret < 0)
        return -1;
