 ...
 */
static long
bio_call_callback(BIO *b, int oper, const char *argp, size_t len, int argi, long argl, long inret,
                  size_t *processed)
{
    long ret = inret;
#ifndef OPENSSL_NO_DEPRECATED_3_0
...
bio_read_intern(BIO *b, void *data, size_t dlen, size_t *readbytes)
{
    ...
    if (HAS_CALLBACK(b))
        ret = (int)bio_call_callback(b, BIO_CB_READ | BIO_CB_RETURN, data, dlen, 0, 0L, ret,
                                     readbytes);
...
bio_write_intern(BIO *b, const void *data, size_t dlen, size_t *written)
{
    ...
    if (HAS_CALLBACK(b))
        ret = (int)bio_call_callback(b, BIO_CB_WRITE | BIO_CB_RETURN, data, dlen, 0, 0L, ret,
                                     &local_written);
...
int
BIO_sendmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
             size_t *msgs_processed)
{
    ...
    if (HAS_CALLBACK(b))
        ret = (size_t)bio_call_callback(b, BIO_CB_SENDMMSG | BIO_CB_RETURN, (void *)&args, ret, 0,
                                        0, ret, NULL);
...
int
BIO_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
             size_t *msgs_processed)
{
    size_t ret;
    BIO_MMSG_CB_ARGS args;
    ...
    if (HAS_CALLBACK(b))
        ret = (size_t)bio_call_callback(b, BIO_CB_RECVMMSG | BIO_CB_RETURN, (void *)&args, ret, 0,
                                        0, ret, NULL);
...
BIO_gets(BIO *b, char *buf, int size)
{
    ...
    if (HAS_CALLBACK(b))
        ret = (int)bio_call_callback(b, BIO_CB_GETS | BIO_CB_RETURN, buf, size, 0, 0L, ret,
                                     &readbytes);
