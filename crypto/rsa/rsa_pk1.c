...
int
RSA_padding_add_PKCS1_type_1(unsigned char *to, int tlen, const unsigned char *from, int flen)
{
    int j;
    unsigned char *p;
...
int
RSA_padding_check_PKCS1_type_1(unsigned char *to, int tlen, const unsigned char *from, int flen,
                               int num)
{
    int i, j;
...
int
ossl_rsa_padding_add_PKCS1_type_2_ex(OSSL_LIB_CTX *libctx, unsigned char *to, int tlen,
...
int
RSA_padding_add_PKCS1_type_2(unsigned char *to, int tlen, const unsigned char *from, int flen)
{
    return ossl_rsa_padding_add_PKCS1_type_2_ex(NULL, to, tlen, from, flen);
}

int
RSA_padding_check_PKCS1_type_2(unsigned char *to, int tlen, const unsigned char *from, int flen,
                               int num)
{
    int i;
...
static int
ossl_rsa_prf(OSSL_LIB_CTX *ctx, unsigned char *to, int tlen, const char *label, int llen,
 ...
 */
int
ossl_rsa_padding_check_PKCS1_type_2(OSSL_LIB_CTX *ctx, unsigned char *to, int tlen,
...
#define MAX_LEN_GEN_TRIES 128
    ...
    for (i = 0; i < MAX_LEN_GEN_TRIES * (int)sizeof(len_candidate); i += sizeof(len_candidate)) {
        ...
        synthetic_length = constant_time_select_int(constant_time_lt(len_candidate, max_sep_offset),
 ...
 */
int
ossl_rsa_padding_check_PKCS1_type_2_TLS(OSSL_LIB_CTX *libctx, unsigned char *to, size_t tlen,
                                        ...
                                        int alt_version)
{
     ...
     */
    version_good =
        constant_time_eq(from[flen - SSL_MAX_MASTER_KEY_LENGTH], (client_version >> 8) & 0xff);
    version_good &=
        constant_time_eq(from[flen - SSL_MAX_MASTER_KEY_LENGTH + 1], client_version & 0xff);
    ...
    if (alt_version > 0) {
        ...
        workaround_good =
            constant_time_eq(from[flen - SSL_MAX_MASTER_KEY_LENGTH], (alt_version >> 8) & 0xff);
        workaround_good &=
            constant_time_eq(from[flen - SSL_MAX_MASTER_KEY_LENGTH + 1], alt_version & 0xff);
    ...
    for (i = 0; i < SSL_MAX_MASTER_KEY_LENGTH; i++) {
        to[i] = constant_time_select_8(good, from[flen - SSL_MAX_MASTER_KEY_LENGTH + i],
