...
static int rsa_ossl_public_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                                   int padding);
static int rsa_ossl_private_encrypt(int flen, const unsigned char *from, unsigned char *to,
                                    RSA *rsa, int padding);
static int rsa_ossl_public_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                                   int padding);
...
static int
rsa_ossl_public_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                        int padding)
{
    BIGNUM *f, *ret;
    int i, num = 0, r = -1;
    ...
    case RSA_PKCS1_OAEP_PADDING:
        i = ossl_rsa_padding_add_PKCS1_OAEP_mgf1_ex(rsa->libctx, buf, num, from, flen, NULL, 0,
                                                    NULL, NULL);
        break;
    case RSA_NO_PADDING:
...
/* signing */
static int
rsa_ossl_private_encrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                         int padding)
{
    BIGNUM *f, *ret, *res;
    int i, num = 0, r = -1;
...
static int
derive_kdk(int flen, const unsigned char *from, RSA *rsa, unsigned char *buf, int num,
           unsigned char *kdk)
{
    int ret = 0;
    HMAC_CTX *hmac = NULL;
...
static int
rsa_ossl_private_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                         int padding)
{
    BIGNUM *f, *ret;
    int j, num = 0, r = -1;
...
static int
rsa_ossl_public_decrypt(int flen, const unsigned char *from, unsigned char *to, RSA *rsa,
                        int padding)
{
    BIGNUM *f, *ret;
    int i, num = 0, r = -1;
...
rsa_ossl_mod_exp(BIGNUM *r0, const BIGNUM *I, RSA *rsa, BN_CTX *ctx)
{
    ...
    if (smooth) {
        ...
        if (/* m1 = I moq q */
             ...
             *    r1 = r1^dmp1 mod p
             */
            || !BN_mod_exp_mont_consttime_x2(m1, m1, rsa->dmq1, rsa->q, rsa->_method_mod_q, r1, r1,
