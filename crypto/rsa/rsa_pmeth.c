...
static int
pkey_rsa_sign(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
              size_t tbslen)
{
    int ret;
    RSA_PKEY_CTX *rctx = ctx->data;
    ...
    if (rctx->md) {
        ...
        if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
            ...
            if (!setup_tbuf(rctx, ctx))
                return -1;
            if (!RSA_padding_add_PKCS1_PSS_mgf1(rsa, rctx->tbuf, tbs, rctx->md, rctx->mgf1md,
                                                rctx->saltlen))
                return -1;
                       ...
                       const unsigned char *sig, size_t siglen)
{
    ...
    if (rctx->md) {
        if (rctx->pad_mode == RSA_X931_PADDING) {
            ...
            size_t sltmp;
            ret =
                ossl_rsa_verify(EVP_MD_get_type(rctx->md), NULL, 0, rout, &sltmp, sig, siglen, rsa);
            if (ret <= 0)
                return 0;
            ret = sltmp;
        } else {
                ...
                const unsigned char *tbs, size_t tbslen)
{
    ...
    if (rctx->md) {
        ...
        if (rctx->pad_mode == RSA_X931_PADDING) {
            ...
            if (ret <= 0)
                return 0;
            ret = RSA_verify_PKCS1_PSS_mgf1(rsa, tbs, rctx->md, rctx->mgf1md, rctx->tbuf,
                                            rctx->saltlen);
            if (ret <= 0)
                return 0;
            return 1;
...
static int
pkey_rsa_encrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                 size_t inlen)
{
    int ret;
    RSA_PKEY_CTX *rctx = ctx->data;
    ...
    if (rctx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
        ...
        if (!setup_tbuf(rctx, ctx))
            return -1;
        if (!RSA_padding_add_PKCS1_OAEP_mgf1(rctx->tbuf, klen, in, inlen, rctx->oaep_label,
...
static int
pkey_rsa_decrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                 size_t inlen)
{
    int ret;
    int pad_mode;
...
rsa_set_pss_param(RSA *rsa, EVP_PKEY_CTX *ctx)
{
    ...
    if (rctx->md == NULL && rctx->mgf1md == NULL && rctx->saltlen == -2)
        return 1;
    rsa->pss =
        ossl_rsa_pss_params_create(rctx->md, rctx->mgf1md, rctx->saltlen == -2 ? 0 : rctx->saltlen);
    if (rsa->pss == NULL)
        return 0;
    return 1;
}
