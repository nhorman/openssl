...
static int
pem_bytes_read_bio_flags(unsigned char **pdata, long *plen, char **pnm, const char *name, BIO *bp,
...
int
PEM_bytes_read_bio_secmem(unsigned char **pdata, long *plen, char **pnm, const char *name, BIO *bp,
                          pem_password_cb *cb, void *u)
{
...
#ifndef OPENSSL_NO_STDIO
int
PEM_ASN1_write(i2d_of_void *i2d, const char *name, FILE *fp, const void *x, const EVP_CIPHER *enc,
...
static int
PEM_ASN1_write_bio_internal(i2d_of_void *i2d, OSSL_i2d_of_void_ctx *i2d_ctx, void *vctx,
...
int
PEM_ASN1_write_bio_ctx(OSSL_i2d_of_void_ctx *i2d, void *vctx, const char *name, BIO *bp,
...
int
PEM_do_header(EVP_CIPHER_INFO *cipher, unsigned char *data, long *plen, pem_password_cb *callback,
              void *u)
{
    int ok;
    int keylen;
    ...
    if (!EVP_BytesToKey(cipher->cipher, EVP_md5(), &(cipher->iv[0]), (unsigned char *)buf, keylen,
                        1, key, NULL))
        return 0;
 ...
 */
static int
get_header_and_data(BIO *bp, BIO **header, BIO **data, char *name, unsigned int flags)
{
    BIO *tmp = *header;
    char *linebuf, *p;
 ...
 */
int
PEM_read_bio_ex(BIO *bp, char **name_out, char **header, unsigned char **data, long *len_out,
                unsigned int flags)
{
    ...
    *data = NULL;
    ...
    EVP_DecodeInit(ctx);
    if (EVP_DecodeUpdate(ctx, (unsigned char *)buf_mem->data, &len, (unsigned char *)buf_mem->data,
                         len) < 0 ||
