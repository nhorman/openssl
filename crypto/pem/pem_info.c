...
/* A TJH addition */
int
PEM_X509_INFO_write_bio(BIO *bp, const X509_INFO *xi, EVP_CIPHER *enc, const unsigned char *kstr,
                        int klen, pem_password_cb *cb, void *u)
{
    ...
    if (xi->x_pkey != NULL) {
        if ((xi->enc_data != NULL) && (xi->enc_len > 0)) {
            ...
            if (PEM_write_bio_RSAPrivateKey(bp, EVP_PKEY_get0_RSA(xi->x_pkey->dec_pkey), enc, kstr,
                                            klen, cb, u) <= 0)
                goto err;
        }
    }
