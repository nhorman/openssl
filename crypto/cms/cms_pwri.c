...
int
CMS_RecipientInfo_set0_password(CMS_RecipientInfo *ri, unsigned char *pass, ossl_ssize_t passlen)
{
...
CMS_RecipientInfo *
CMS_add0_recipient_password(CMS_ContentInfo *cms, int iter, int wrap_nid, int pbe_nid,
...
static int
kek_unwrap_key(unsigned char *out, size_t *outlen, const unsigned char *in, size_t inlen,
               EVP_CIPHER_CTX *ctx)
{
    ...
    if (!EVP_DecryptUpdate(ctx, tmp + inlen - 2 * blocklen, &outl, in + inlen - 2 * blocklen,
                           blocklen * 2)
        /*
...
static int
kek_wrap_key(unsigned char *out, size_t *outlen, const unsigned char *in, size_t inlen,
             EVP_CIPHER_CTX *ctx, const CMS_CTX *cms_ctx)
{
    ...
    if (out) {
        ...
        if (olen > inlen + 4 && RAND_bytes_ex(ossl_cms_ctx_get0_libctx(cms_ctx), out + 4 + inlen,
                                              olen - 4 - inlen, 0) <= 0)
            return 0;
        /* Encrypt twice */
...
int
ossl_cms_RecipientInfo_pwri_crypt(const CMS_ContentInfo *cms, CMS_RecipientInfo *ri, int en_de)
{
    CMS_EncryptedContentInfo *ec;
    CMS_PasswordRecipientInfo *pwri;
    int r = 0;
    ...
    kekcipher =
        EVP_CIPHER_fetch(ossl_cms_ctx_get0_libctx(cms_ctx), name, ossl_cms_ctx_get0_propq(cms_ctx));
    ...
    if (EVP_PBE_CipherInit_ex(algtmp->algorithm, (char *)pwri->pass, pwri->passlen,
                              algtmp->parameter, kekctx, en_de, cms_ctx->libctx,
                              cms_ctx->propq) < 0) {
        ERR_raise(ERR_LIB_CMS, ERR_R_EVP_LIB);
        goto err;
    }
    ...
    if (en_de) {
        ...
        if (key == NULL)
            goto err;
        if (!kek_unwrap_key(key, &keylen, pwri->encryptedKey->data, pwri->encryptedKey->length,
                            kekctx)) {
