...
cms_copy_messageDigest(CMS_ContentInfo *cms, CMS_SignerInfo *si)
{
    ...
    for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
        ...
        if (OBJ_cmp(si->digestAlgorithm->algorithm, sitmp->digestAlgorithm->algorithm))
            continue;
        messageDigest = CMS_signed_get0_data_by_OBJ(sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3,
                                                    V_ASN1_OCTET_STRING);
        if (!messageDigest) {
        ...
        if (CMS_signed_add1_attr_by_NID(si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING,
                                        messageDigest, -1))
            return 1;
        else
            return 0;
...
int
ossl_cms_set1_SignerIdentifier(CMS_SignerIdentifier *sid, X509 *cert, int type, const CMS_CTX *ctx)
{
    switch (type) {
...
int
ossl_cms_SignerIdentifier_get0_signer_id(CMS_SignerIdentifier *sid, ASN1_OCTET_STRING **keyid,
...
ossl_cms_add1_signing_cert(CMS_SignerInfo *si, const ESS_SIGNING_CERT *sc)
{
    ...
    OPENSSL_free(pp);
    ret = CMS_signed_add1_attr_by_NID(si, NID_id_smime_aa_signingCertificate, V_ASN1_SEQUENCE, seq,
                                      -1);
    ASN1_STRING_free(seq);
    return ret;
}
...
ossl_cms_add1_signing_cert_v2(CMS_SignerInfo *si, const ESS_SIGNING_CERT_V2 *sc)
{
    ...
    OPENSSL_free(pp);
    ret = CMS_signed_add1_attr_by_NID(si, NID_id_smime_aa_signingCertificateV2, V_ASN1_SEQUENCE,
                                      seq, -1);
    ASN1_STRING_free(seq);
    return ret;
}
...
CMS_SignerInfo *
CMS_add1_signer(CMS_ContentInfo *cms, X509 *signer, EVP_PKEY *pk, const EVP_MD *md,
                unsigned int flags)
{
    CMS_SignedData *sd;
    ...
    if (!(flags & CMS_KEY_PARAM) && !cms_sd_asn1_ctrl(si, 0)) {
        ERR_raise_data(ERR_LIB_CMS, CMS_R_UNSUPPORTED_SIGNATURE_ALGORITHM, "pkey nid=%d",
                       EVP_PKEY_get_id(pk));
        goto err;
    }
    if (!(flags & CMS_NOATTR)) {
    ...
    if (flags & CMS_KEY_PARAM) {
        if (flags & CMS_NOATTR) {
            si->pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey,
...
int
CMS_SignerInfo_get0_signer_id(CMS_SignerInfo *si, ASN1_OCTET_STRING **keyid, X509_NAME **issuer,
                              ASN1_INTEGER **sno)
{
...
int
CMS_set1_signers_certs(CMS_ContentInfo *cms, STACK_OF(X509) * scerts, unsigned int flags)
{
    CMS_SignedData *sd;
    CMS_SignerInfo *si;
    CMS_CertificateChoices *cch;
...
void
CMS_SignerInfo_get0_algs(CMS_SignerInfo *si, EVP_PKEY **pk, X509 **signer, X509_ALGOR **pdig,
                         X509_ALGOR **psig)
{
    if (pk != NULL)
        *pk = si->pkey;
                            ...
                            const unsigned char *md, unsigned int mdlen)
{
    ...
    if (CMS_signed_get_attr_count(si) >= 0) {
        ...
        if (!CMS_signed_add1_attr_by_NID(si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING, md,
                                         mdlen))
            goto err;
...
int
ossl_cms_SignedData_final(CMS_ContentInfo *cms, BIO *chain, const unsigned char *precomp_md,
...
CMS_SignerInfo_sign(CMS_SignerInfo *si)
{
    ...
    if (si->pctx) {
        ...
        if (EVP_DigestSignInit_ex(mctx, &pctx, md_name, ossl_cms_ctx_get0_libctx(ctx),
    ...
    if (md_name == NULL) {
        if (ASN1_item_sign_ctx(ASN1_ITEM_rptr(CMS_Attributes_Sign), NULL, NULL, si->signature,
...
CMS_SignerInfo_verify(CMS_SignerInfo *si)
{
    ...
    if (cms_signature_nomd(si->pkey)) {
        r = ASN1_item_verify_ex(ASN1_ITEM_rptr(CMS_Attributes_Sign), si->signatureAlgorithm,
                                si->signature, si->signedAttrs, NULL, si->pkey, libctx, propq);
        if (r <= 0)
    ...
    if (EVP_DigestVerifyInit_ex(mctx, &si->pctx, EVP_MD_get0_name(md), libctx, propq, si->pkey,
                                NULL) <= 0) {
        si->pctx = NULL;
        goto err;
    }
    ...
    alen =
        ASN1_item_i2d((ASN1_VALUE *)si->signedAttrs, &abuf, ASN1_ITEM_rptr(CMS_Attributes_Verify));
    if (abuf == NULL || alen < 0)
        goto err;
...
CMS_SignerInfo_verify_content(CMS_SignerInfo *si, BIO *chain)
{
    ...
    if (CMS_signed_get_attr_count(si) >= 0) {
        os = CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_pkcs9_messageDigest), -3,
                                         V_ASN1_OCTET_STRING);
        if (os == NULL) {
            ERR_raise(ERR_LIB_CMS, CMS_R_ERROR_READING_MESSAGEDIGEST_ATTRIBUTE);
            goto err;
...
BIO *
CMS_SignedData_verify(CMS_SignedData *sd, BIO *detached_data, STACK_OF(X509) * scerts,
