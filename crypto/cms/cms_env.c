...
CMS_ContentInfo *
CMS_EnvelopedData_create_ex(const EVP_CIPHER *cipher, OSSL_LIB_CTX *libctx, const char *propq)
{
    CMS_ContentInfo *cms;
    ...
    if (!ossl_cms_EncryptedContent_init(env->encryptedContentInfo, cipher, NULL, 0,
...
BIO *
CMS_EnvelopedData_decrypt(CMS_EnvelopedData *env, BIO *detached_data, EVP_PKEY *pkey, X509 *cert,
                          ASN1_OCTET_STRING *secret, unsigned int flags, OSSL_LIB_CTX *libctx,
                          const char *propq)
{
    CMS_ContentInfo *ci;
    BIO *bio = NULL;
    ...
    if (secret != NULL &&
        ...
        goto end;
    res = CMS_decrypt(ci, secret == NULL ? pkey : NULL, secret == NULL ? cert : NULL, detached_data,
                      bio, flags);
...
CMS_AuthEnvelopedData_create_ex(const EVP_CIPHER *cipher, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (aenv == NULL)
        goto merr;
    if (!ossl_cms_EncryptedContent_init(aenv->authEncryptedContentInfo, cipher, NULL, 0,
                                        ossl_cms_get0_cmsctx(cms)))
        goto merr;
    return cms;
merr:
...
static int
cms_RecipientInfo_ktri_init(CMS_RecipientInfo *ri, X509 *recip, EVP_PKEY *pk, unsigned int flags,
                            const CMS_CTX *ctx)
{
    ...
    if (flags & CMS_KEY_PARAM) {
        ktri->pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), ktri->pkey,
                                                ossl_cms_ctx_get0_propq(ctx));
        if (ktri->pctx == NULL)
            return 0;
...
CMS_RecipientInfo *
CMS_add1_recipient(CMS_ContentInfo *cms, X509 *recip, EVP_PKEY *originatorPrivKey, X509 *originator,
                   unsigned int flags)
{
    ...
    case CMS_RECIPINFO_AGREE:
        if (!ossl_cms_RecipientInfo_kari_init(ri, recip, pk, originator, originatorPrivKey, flags,
                                              ctx))
            goto err;
        break;
...
int
CMS_RecipientInfo_ktri_get0_algs(CMS_RecipientInfo *ri, EVP_PKEY **pk, X509 **recip,
                                 X509_ALGOR **palg)
{
    CMS_KeyTransRecipientInfo *ktri;
    if (ri->type != CMS_RECIPINFO_TRANS) {
...
int
CMS_RecipientInfo_ktri_get0_signer_id(CMS_RecipientInfo *ri, ASN1_OCTET_STRING **keyid,
...
static int
cms_RecipientInfo_ktri_encrypt(const CMS_ContentInfo *cms, CMS_RecipientInfo *ri)
{
    CMS_KeyTransRecipientInfo *ktri;
...
cms_RecipientInfo_ktri_decrypt(CMS_ContentInfo *cms, CMS_RecipientInfo *ri)
{
    ...
    if (evp_pkey_decrypt_alloc(ktri->pctx, &ek, &eklen, fixlen, ktri->encryptedKey->data,
...
CMS_RecipientInfo *
CMS_add0_recipient_key(CMS_ContentInfo *cms, int nid, unsigned char *key, size_t keylen,
                       ...
                       ASN1_OBJECT *otherTypeId, ASN1_TYPE *otherType)
{
    ...
    (void)X509_ALGOR_set0(kekri->keyEncryptionAlgorithm, OBJ_nid2obj(nid), V_ASN1_UNDEF,
                          NULL); /* cannot fail */
...
int
CMS_RecipientInfo_kekri_get0_id(CMS_RecipientInfo *ri, X509_ALGOR **palg, ASN1_OCTET_STRING **pid,
                                ASN1_GENERALIZEDTIME **pdate, ASN1_OBJECT **potherid,
...
static int
cms_RecipientInfo_kekri_encrypt(const CMS_ContentInfo *cms, CMS_RecipientInfo *ri)
{
    CMS_EncryptedContentInfo *ec;
    CMS_KEKRecipientInfo *kekri;
...
static int
cms_env_encrypt_content_key(const CMS_ContentInfo *cms, STACK_OF(CMS_RecipientInfo) * ris)
{
    int i;
    CMS_RecipientInfo *ri;
...
cms_EnvelopedData_Decryption_init_bio(CMS_ContentInfo *cms)
{
     ...
     */
    if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx)) & EVP_CIPH_FLAG_CIPHER_WITH_MAC) !=
            0 &&
