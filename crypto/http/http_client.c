...
void
OSSL_HTTP_REQ_CTX_set_max_response_length(OSSL_HTTP_REQ_CTX *rctx, unsigned long len)
{
    if (rctx == NULL) {
        ERR_raise(ERR_LIB_HTTP, ERR_R_PASSED_NULL_PARAMETER);
        return;
 ...
 */
int
OSSL_HTTP_REQ_CTX_set_request_line(OSSL_HTTP_REQ_CTX *rctx, int method_POST, const char *server,
...
int
OSSL_HTTP_REQ_CTX_add1_header(OSSL_HTTP_REQ_CTX *rctx, const char *name, const char *value)
{
...
int
OSSL_HTTP_REQ_CTX_set_expected(OSSL_HTTP_REQ_CTX *rctx, const char *content_type, int asn1,
...
int
OSSL_HTTP_REQ_CTX_set1_req(OSSL_HTTP_REQ_CTX *rctx, const char *content_type, const ASN1_ITEM *it,
...
void
OSSL_HTTP_REQ_CTX_set_max_response_hdr_lines(OSSL_HTTP_REQ_CTX *rctx, size_t count)
{
    if (rctx == NULL) {
        ERR_raise(ERR_LIB_HTTP, ERR_R_PASSED_NULL_PARAMETER);
        return;
...
static int
add1_headers(OSSL_HTTP_REQ_CTX *rctx, const STACK_OF(CONF_VALUE) * headers, const char *host)
{
    int i;
...
http_req_ctx_new(int free_wbio, BIO *wbio, BIO *rbio, OSSL_HTTP_bio_cb_t bio_update_fn, void *arg,
                 int use_ssl, const char *proxy, const char *server, const char *port, int buf_size,
                 int overall_timeout)
{
...
check_max_len(const char *desc, size_t max_len, size_t len)
{
    if (max_len != 0 && len > max_len) {
        ERR_raise_data(ERR_LIB_HTTP, HTTP_R_MAX_RESP_LEN_EXCEEDED, "%s length=%zu, max=%zu", desc,
                       len, max_len);
        return 0;
    }
    return 1;
        ...
        if (value != NULL && line_end != NULL) {
            ...
            if (OPENSSL_strcasecmp(key, "Connection") == 0) {
                ...
                if (line_end == value || *line_end != '\0') {
                    ERR_raise_data(ERR_LIB_HTTP, HTTP_R_ERROR_PARSING_CONTENT_LENGTH, "input=%s",
                                   value);
                    return 0;
                }
...
int
OSSL_HTTP_REQ_CTX_nbio_d2i(OSSL_HTTP_REQ_CTX *rctx, ASN1_VALUE **pval, const ASN1_ITEM *it)
{
    const unsigned char *p;
    int rv;
...
OSSL_HTTP_REQ_CTX *
OSSL_HTTP_open(const char *server, const char *port, const char *proxy, const char *no_proxy,
               ...
               int buf_size, int overall_timeout)
{
    ...
    if (bio != NULL) {
        ...
        if (proxy != NULL &&
            !OSSL_HTTP_parse_url(proxy, NULL /* use_ssl */, NULL /* user */, &proxy_host,
                                 &proxy_port, NULL /* num */, NULL /* path */, NULL, NULL))
            return NULL;
    ...
    rctx = http_req_ctx_new(bio == NULL, cbio, rbio != NULL ? rbio : cbio, bio_update_fn, arg,
                            use_ssl, proxy, server, port, buf_size, overall_timeout);
                       ...
                       int timeout, int keep_alive)
{
    ...
    return OSSL_HTTP_REQ_CTX_set_request_line(
               rctx, req != NULL, use_http_proxy ? rctx->server : NULL, rctx->port, path) &&
           add1_headers(rctx, headers, rctx->server) &&
           OSSL_HTTP_REQ_CTX_set_expected(rctx, expected_content_type, expect_asn1, timeout,
                                          keep_alive) &&
...
BIO *
OSSL_HTTP_get(const char *url, const char *proxy, const char *no_proxy, BIO *bio, BIO *rbio,
              ...
              size_t max_resp_len, int timeout)
{
    ...
    for (;;) {
        ...
        if (!OSSL_HTTP_parse_url(current_url, &use_ssl, NULL /* user */, &host, &port,
                                 NULL /* port_num */, &path, NULL, NULL))
            break;

        rctx = OSSL_HTTP_open(host, port, proxy, no_proxy, use_ssl, bio, rbio, bio_update_fn, arg,
                              buf_size, timeout);
    new_rpath:
        redirection_url = NULL;
        if (rctx != NULL) {
            if (!OSSL_HTTP_set1_request(rctx, path, headers, NULL /* content_type */,
                                        NULL /* req */, expected_ct, expect_asn1, max_resp_len,
...
BIO *
OSSL_HTTP_transfer(OSSL_HTTP_REQ_CTX **prctx, const char *server, const char *port,
                   const char *path, int use_ssl, const char *proxy, const char *no_proxy, BIO *bio,
                   ...
                   int keep_alive)
{
    ...
    if (rctx == NULL) {
        rctx = OSSL_HTTP_open(server, port, proxy, no_proxy, use_ssl, bio, rbio, bio_update_fn, arg,
                              buf_size, timeout);
        timeout = -1; /* Already set during opening the connection */
    }
    if (rctx != NULL) {
        if (OSSL_HTTP_set1_request(rctx, path, headers, content_type, req, expected_ct, expect_asn1,
 ...
 */
int
OSSL_HTTP_proxy_connect(BIO *bio, const char *server, const char *port, const char *proxyuser,
