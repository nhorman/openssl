...
static int
set_trace_data(int category, int type, BIO **channel, const char **prefix, const char **suffix,
...
ossl_trace_cleanup(void)
{
    ...
    set_trace_data(OSSL_TRACE_CATEGORY_TRACE, 0, &channel, &prefix, &suffix, trace_attach_cb,
                   trace_detach_cb);
...
OSSL_trace_set_channel(int category, BIO *channel)
{
    ...
    if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
        return set_trace_data(category, SIMPLE_CHANNEL, &channel, NULL, NULL, trace_attach_cb,
                              trace_detach_cb);
#endif
    return 0;
}
...
OSSL_trace_begin(int category)
{
    ...
    if (channel != NULL) {
        ...
        case CALLBACK_CHANNEL:
            (void)BIO_ctrl(channel, OSSL_TRACE_CTRL_BEGIN, prefix == NULL ? 0 : strlen(prefix),
                           prefix);
            break;
        }
    }
...
OSSL_trace_end(int category, BIO *channel)
{
    ...
    if (channel != NULL && ossl_assert(channel == current_channel)) {
        ...
        case SIMPLE_CHANNEL:
            ...
            break;
        case CALLBACK_CHANNEL:
            (void)BIO_ctrl(channel, OSSL_TRACE_CTRL_END, suffix == NULL ? 0 : strlen(suffix),
                           suffix);
            break;
        }
...
int
OSSL_trace_string(BIO *out, int text, int full, const unsigned char *data, size_t size)
{
