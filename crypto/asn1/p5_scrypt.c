...
ASN1_SEQUENCE(SCRYPT_PARAMS) =
    {
        ...
        IMPLEMENT_ASN1_FUNCTIONS(SCRYPT_PARAMS)
            ...
            static X509_ALGOR
    * pkcs5_scrypt_set(const unsigned char *salt, size_t saltlen, size_t keylen, uint64_t N,
...
X509_ALGOR *
PKCS5_pbe2_set_scrypt(const EVP_CIPHER *cipher, const unsigned char *salt, int saltlen,
...
static X509_ALGOR *
pkcs5_scrypt_set(const unsigned char *salt, size_t saltlen, size_t keylen, uint64_t N, uint64_t r,
                 uint64_t p)
{
    ...
    if (ASN1_TYPE_pack_sequence(ASN1_ITEM_rptr(SCRYPT_PARAMS), sparam, &keyfunc->parameter) ==
        NULL) {
        ERR_raise(ERR_LIB_ASN1, ERR_R_ASN1_LIB);
        goto err;
    }
...
int
PKCS5_v2_scrypt_keyivgen_ex(EVP_CIPHER_CTX *ctx, const char *pass, int passlen, ASN1_TYPE *param,
                            ...
                            const char *propq)
{
    ...
    if (EVP_PBE_scrypt_ex(pass, passlen, salt, saltlen, N, r, p, 0, key, keylen, libctx, propq) ==
        0)
        goto err;
...
int
PKCS5_v2_scrypt_keyivgen(EVP_CIPHER_CTX *ctx, const char *pass, int passlen, ASN1_TYPE *param,
