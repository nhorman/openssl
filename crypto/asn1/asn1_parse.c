...
static int asn1_parse2(BIO *bp, const unsigned char **pp, long length, int offset, int depth,
                       int indent, int dump);
static int
asn1_print_info(BIO *bp, long offset, int depth, int hl, long len, int tag, int xclass,
                int constructed, int indent)
{
    char str[128];
    const char *p;
    int pop_f_prefix = 0;
    ...
    if (constructed != (V_ASN1_CONSTRUCTED | 1)) {
        if (BIO_snprintf(str, sizeof(str), "%5ld:d=%-2d hl=%ld l=%4ld %s", offset, depth, (long)hl,
                         len, p) <= 0)
            goto err;
    } else {
        if (BIO_snprintf(str, sizeof(str), "%5ld:d=%-2d hl=%ld l=inf  %s", offset, depth, (long)hl,
                         p) <= 0)
            goto err;
    }
    if (bp != NULL) {
...
static int
asn1_parse2(BIO *bp, const unsigned char **pp, long length, int offset, int depth, int indent,
            int dump)
{
    ...
    while (length > 0) {
         ...
         */
        if (!asn1_print_info(bp, (long)offset + (long)(op - *pp), depth, hl, len, tag, xclass, j,
                             (indent) ? depth : 0))
            goto end;
        if (j & V_ASN1_CONSTRUCTED) {
            ...
            if ((j == 0x21) && (len == 0)) {
                for (;;) {
                    r = asn1_parse2(bp, &p, (long)(tot - p), offset + (p - *pp), depth + 1, indent,
                                    dump);
                    if (r == 0)
                        goto end;
                    if ((r == 2) || (p >= tot)) {
            ...
            if ((tag == V_ASN1_PRINTABLESTRING) || (tag == V_ASN1_T61STRING) ||
                ...
                if (BIO_dump_indent(bp, (const char *)p, ((dump == -1 || dump > len) ? len : dump),
                                    dump_indent) <= 0)
                    goto end;
                nl = 1;
            }
