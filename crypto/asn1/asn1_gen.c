...
generate_v3(const char *str, X509V3_CTX *cnf, int depth, int *perr)
{
    ...
    for (i = 0, etmp = asn1_tags.exp_list + asn1_tags.exp_count - 1; i < asn1_tags.exp_count;
         i++, etmp--) {
...
static ASN1_TYPE *
asn1_multi(int utype, const char *section, X509V3_CTX *cnf, int depth, int *perr)
{
    ASN1_TYPE *ret = NULL;
...
static int
append_exp(tag_exp_arg *arg, int exp_tag, int exp_class, int exp_constructed, int exp_pad,
           int imp_ok)
{
...
asn1_str2type(const char *str, int format, int utype)
{
    ...
    case V_ASN1_NUMERICSTRING:
        ...
        if (ASN1_mbstring_copy(&atmp->value.asn1_string, (unsigned char *)str, -1, format,
