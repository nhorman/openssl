    ...
    if (ret != NULL) {
        ...
        if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
            if (src->pub_encoding != NULL) {
                ...
                if ((ret->pub_encoding = OPENSSL_memdup(src->pub_encoding, src->params->pk_len)) ==
                    NULL)
                    goto err;
            }
 ...
 */
static int
public_from_private(const ML_DSA_KEY *key, EVP_MD_CTX *md_ctx, VECTOR *t1, VECTOR *t0)
{
    ...
    if (sk == NULL) {
        ...
        if ((ret = keygen_internal(out)) != 0 &&
            ...
            ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                           "explicit %s private key does not match seed", out->params->alg);
        }
        OPENSSL_free(sk);
    }
    return ret;
