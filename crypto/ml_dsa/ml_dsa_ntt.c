    ...
    for (step = 1; step < ML_DSA_NUM_POLY_COEFFICIENTS; step <<= 1) {
        ...
        for (i = 0; i < step; i++) {
            ...
            for (j = k; j < k + offset; j++) {
                ...
                uint32_t t_odd =
                    reduce_montgomery((uint64_t)z_step_root * (uint64_t)p->coeff[j + offset]);
...
ossl_ml_dsa_poly_ntt_inverse(POLY *p)
{
    ...
    for (offset = 1; offset < ML_DSA_NUM_POLY_COEFFICIENTS; offset <<= 1) {
        ...
        for (i = 0; i < step; i++) {
            ...
            for (j = k; j < k + offset; j++) {
                ...
                p->coeff[j + offset] =
                    reduce_montgomery((uint64_t)step_root * (uint64_t)(ML_DSA_Q + even - odd));
            }
            k += 2 * offset;
        }
    }
    for (i = 0; i < ML_DSA_NUM_POLY_COEFFICIENTS; i++)
        p->coeff[i] =
            reduce_montgomery((uint64_t)p->coeff[i] * (uint64_t)inverse_degree_montgomery);
}
