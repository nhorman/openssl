 ...
 */
static int
OSSL_CRMF_MSG_push0_regCtrl(OSSL_CRMF_MSG *crm, OSSL_CRMF_ATTRIBUTETYPEANDVALUE *ctrl)
{
    int new = 0;
...
int
OSSL_CRMF_MSG_set0_SinglePubInfo(OSSL_CRMF_SINGLEPUBINFO *spi, int method, GENERAL_NAME *nm)
{
...
int
OSSL_CRMF_MSG_set_PKIPublicationInfo_action(OSSL_CRMF_PKIPUBLICATIONINFO *pi, int action)
{
 ...
 */
static int
OSSL_CRMF_MSG_push0_regInfo(OSSL_CRMF_MSG *crm, OSSL_CRMF_ATTRIBUTETYPEANDVALUE *ri)
{
...
int
OSSL_CRMF_MSG_set0_validity(OSSL_CRMF_MSG *crm, ASN1_TIME *notBefore, ASN1_TIME *notAfter)
{
...
static int
create_popo_signature(OSSL_CRMF_POPOSIGNINGKEY *ps, const OSSL_CRMF_CERTREQUEST *cr, EVP_PKEY *pkey,
...
int
OSSL_CRMF_MSG_create_popo(int meth, OSSL_CRMF_MSG *crm, EVP_PKEY *pkey, const EVP_MD *digest,
...
int
OSSL_CRMF_MSGS_verify_popo(const OSSL_CRMF_MSGS *reqs, int rid, int acceptRAVerified,
                           OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    case OSSL_CRMF_POPO_SIGNATURE:
        ...
        if (ASN1_item_verify_ex(it, sig->algorithmIdentifier, sig->signature, asn, NULL,
                                X509_PUBKEY_get0(pubkey), libctx, propq) < 1)
            return 0;
        break;
    case OSSL_CRMF_POPO_KEYENC:
        /*
...
int
OSSL_CRMF_MSG_centralkeygen_requested(const OSSL_CRMF_MSG *crm, const X509_REQ *p10cr)
{
    X509_PUBKEY *pubkey = NULL;
...
EVP_PKEY *
OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(const OSSL_CRMF_ENCRYPTEDKEY *encryptedKey, X509_STORE *ts,
                                 ...
                                 ASN1_OCTET_STRING *secret, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (encryptedKey->type != OSSL_CRMF_ENCRYPTEDKEY_ENVELOPEDDATA) {
        ...
        p = OSSL_CRMF_ENCRYPTEDVALUE_decrypt(encryptedKey->value.encryptedValue, libctx, propq,
                                             pkey, &len);
        if ((p_copy = p) != NULL)
    ...
    if ((bio = CMS_EnvelopedData_decrypt(encryptedKey->value.envelopedData, NULL, pkey, cert,
                                         secret, 0, libctx, propq)) == NULL) {
    ...
    if ((purpose_id = X509_PURPOSE_get_by_sname(SN_cmKGA)) < 0) {
        ...
        if (!X509_PURPOSE_add(purpose_id, X509_TRUST_COMPAT, 0, check_cmKGA, LN_cmKGA, SN_cmKGA,
                              NULL))
            goto end;
    }
    ...
    pkey_bio =
        CMS_SignedData_verify(sd, NULL, NULL /* scerts */, ts, extra, NULL, 0, libctx, propq);
...
unsigned char *
OSSL_CRMF_ENCRYPTEDVALUE_decrypt(const OSSL_CRMF_ENCRYPTEDVALUE *enc, OSSL_LIB_CTX *libctx,
                                 const char *propq, EVP_PKEY *pkey, int *outlen)
{
    ...
    *outlen = 0;
    if (enc == NULL || enc->symmAlg == NULL || enc->encSymmKey == NULL || enc->encValue == NULL ||
        pkey == NULL) {
        ERR_raise(ERR_LIB_CRMF, CRMF_R_NULL_ARGUMENT);
        return NULL;
    }
    ...
    if (pkctx != NULL && EVP_PKEY_decrypt_init(pkctx) > 0) {
        ...
        failure =
            ~constant_time_is_zero_s(constant_time_msb(retval) | constant_time_is_zero(retval));
 ...
 */
X509 *
OSSL_CRMF_ENCRYPTEDVALUE_get1_encCert(const OSSL_CRMF_ENCRYPTEDVALUE *ecert, OSSL_LIB_CTX *libctx,
 ...
 */
X509 *
OSSL_CRMF_ENCRYPTEDKEY_get1_encCert(const OSSL_CRMF_ENCRYPTEDKEY *ecert, OSSL_LIB_CTX *libctx,
                                    const char *propq, EVP_PKEY *pkey, unsigned int flags)
{
#ifndef OPENSSL_NO_CMS
    BIO *bio;
    X509 *cert = NULL;
    ...
    if (ecert->type != OSSL_CRMF_ENCRYPTEDKEY_ENVELOPEDDATA)
        return OSSL_CRMF_ENCRYPTEDVALUE_get1_encCert(ecert->value.encryptedValue, libctx, propq,
                                                     pkey);
#ifndef OPENSSL_NO_CMS
    bio = CMS_EnvelopedData_decrypt(ecert->value.envelopedData, NULL, pkey, NULL /* cert */, NULL,
                                    flags, libctx, propq);
    if (bio == NULL)
        return NULL;
