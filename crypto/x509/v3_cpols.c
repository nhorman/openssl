...
ASN1_ITEM_TEMPLATE(CERTIFICATEPOLICIES) = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0,
    ...
    ASN1_ITEM_TEMPLATE_END(CERTIFICATEPOLICIES)
        ...
        IMPLEMENT_ASN1_FUNCTIONS(CERTIFICATEPOLICIES)
            ...
            ASN1_SEQUENCE(
                POLICYINFO) = {ASN1_SIMPLE(POLICYINFO, policyid, ASN1_OBJECT),
...
ASN1_ADB(POLICYQUALINFO) = {ADB_ENTRY(NID_id_qt_cps,
...
static int
i2r_certpol(X509V3_EXT_METHOD *method, STACK_OF(POLICYINFO) * pol, BIO *out, int indent)
{
    int i;
    POLICYINFO *pinfo;
...
print_notice(BIO *out, USERNOTICE *notice, int indent)
{
    ...
    if (notice->exptext)
        BIO_printf(out, "%*sExplicit Text: %.*s", indent, "", notice->exptext->length,
                   notice->exptext->data);
}
