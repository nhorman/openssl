...
ASN1_SEQUENCE(X509_REVOKED) = {ASN1_EMBED(X509_REVOKED, serialNumber, ASN1_INTEGER),
...
ASN1_SEQUENCE_enc(X509_CRL_INFO, enc,
                  crl_inf_cb) = {ASN1_OPT(X509_CRL_INFO, version, ASN1_INTEGER),
...
ASN1_SEQUENCE_ref(X509_CRL, crl_cb) = {ASN1_EMBED(X509_CRL, crl, X509_CRL_INFO),
                                       ...
                                       ASN1_EMBED(X509_CRL, signature,
                                                  ASN1_BIT_STRING)} ASN1_SEQUENCE_END_ref(X509_CRL,
                                                                                          X509_CRL)
...
{
    return (
        ASN1_STRING_cmp((ASN1_STRING *)&(*a)->serialNumber, (ASN1_STRING *)&(*b)->serialNumber));
}
...
int
X509_CRL_get0_by_serial(X509_CRL *crl, X509_REVOKED **ret, const ASN1_INTEGER *serial)
{
...
{
    return ASN1_item_verify_ex(ASN1_ITEM_rptr(X509_CRL_INFO), &crl->sig_alg, &crl->signature,
                               &crl->crl, NULL, r, crl->libctx, crl->propq);
}
...
X509_CRL_METHOD *
X509_CRL_METHOD_new(int (*crl_init)(X509_CRL *crl), int (*crl_free)(X509_CRL *crl),
