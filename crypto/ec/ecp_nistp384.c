...
static ossl_inline void felem_mul_reduce(felem out, const felem in1,
                                         const felem in2)
{
    widefelem tmp;
...
static void felem_contract(felem out, const felem in)
{
    ...
    cond[0] =
        ((tmp[6] | 0xff000000000000) & tmp[5] & tmp[4] & tmp[3] &
         (tmp[2] | 0x0000000001ffff)) + 1;
...
int ossl_ec_GFp_nistp384_precompute_mult(EC_GROUP *group, BN_CTX *ctx)
{
    ...
    for (i = 1; i <= 4; i <<= 1) {
        point_double(pre->g_pre_comp[2 * i][0], pre->g_pre_comp[2 * i][1],
                     ...
                     pre->g_pre_comp[i][0],  pre->g_pre_comp[i][1],
                     pre->g_pre_comp[i][2]);
        for (j = 0; j < 94; ++j) {
            point_double(pre->g_pre_comp[2 * i][0], pre->g_pre_comp[2 * i][1],
                         pre->g_pre_comp[2 * i][2],
    ...
    point_add(pre->g_pre_comp[6][0],  pre->g_pre_comp[6][1],
              ...
              pre->g_pre_comp[4][2], 0,
              pre->g_pre_comp[2][0],  pre->g_pre_comp[2][1],
              pre->g_pre_comp[2][2]);
    /* 2^95*G + 2^285*G */
    point_add(pre->g_pre_comp[10][0], pre->g_pre_comp[10][1],
              pre->g_pre_comp[10][2],
              pre->g_pre_comp[8][0],  pre->g_pre_comp[8][1],
              pre->g_pre_comp[8][2], 0,
              pre->g_pre_comp[2][0],  pre->g_pre_comp[2][1],
              pre->g_pre_comp[2][2]);
    /* 2^190*G + 2^285*G */
    ...
    point_add(pre->g_pre_comp[14][0], pre->g_pre_comp[14][1],
              pre->g_pre_comp[14][2],
              pre->g_pre_comp[12][0], pre->g_pre_comp[12][1],
              pre->g_pre_comp[12][2], 0,
              pre->g_pre_comp[2][0],  pre->g_pre_comp[2][1],
              pre->g_pre_comp[2][2]);
    for (i = 1; i < 8; ++i) {
        /* odd multiples: add G */
