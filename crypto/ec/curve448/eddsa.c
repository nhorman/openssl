...
static c448_error_t
oneshot_hash(OSSL_LIB_CTX *ctx, uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen,
             const char *propq)
{
...
static c448_error_t
hash_init_with_dom(OSSL_LIB_CTX *ctx, EVP_MD_CTX *hashctx, uint8_t prehashed, uint8_t for_prehash,
...
c448_error_t
ossl_c448_ed448_convert_private_key_to_x448(OSSL_LIB_CTX *ctx, uint8_t x[X448_PRIVATE_BYTES],
...
c448_error_t
ossl_c448_ed448_derive_public_key(OSSL_LIB_CTX *ctx, uint8_t pubkey[EDDSA_448_PUBLIC_BYTES],
...
c448_error_t
ossl_c448_ed448_sign(OSSL_LIB_CTX *ctx, uint8_t signature[EDDSA_448_SIGNATURE_BYTES],
...
c448_error_t
ossl_c448_ed448_sign_prehash(OSSL_LIB_CTX *ctx, uint8_t signature[EDDSA_448_SIGNATURE_BYTES],
                             const uint8_t privkey[EDDSA_448_PRIVATE_BYTES],
                             const uint8_t pubkey[EDDSA_448_PUBLIC_BYTES], const uint8_t hash[64],
                             const uint8_t *context, size_t context_len, const char *propq)
{
    return ossl_c448_ed448_sign(ctx, signature, privkey, pubkey, hash, 64, 1, context, context_len,
                                propq);
}
...
c448_error_t
ossl_c448_ed448_verify(OSSL_LIB_CTX *ctx, const uint8_t signature[EDDSA_448_SIGNATURE_BYTES],
                       const uint8_t pubkey[EDDSA_448_PUBLIC_BYTES], const uint8_t *message,
                       size_t message_len, uint8_t prehashed, const uint8_t *context,
                       uint8_t context_len, const char *propq)
{
...

    {
        ...
        if (hashctx == NULL ||
            !hash_init_with_dom(ctx, hashctx, prehashed, 0, context, context_len, propq) ||
    ...
    ossl_curve448_scalar_decode_long(response_scalar, &signature[EDDSA_448_PUBLIC_BYTES],
                                     EDDSA_448_PRIVATE_BYTES);

    /* pk_point = -c(x(P)) + (cx + k)G = kG */
    ossl_curve448_base_double_scalarmul_non_secret(pk_point, response_scalar, pk_point,
                                                   challenge_scalar);
    return c448_succeed_if(ossl_curve448_point_eq(pk_point, r_point));
}

c448_error_t
ossl_c448_ed448_verify_prehash(OSSL_LIB_CTX *ctx,
...
int
ossl_ed448_sign(OSSL_LIB_CTX *ctx, uint8_t *out_sig, const uint8_t *message, size_t message_len,
...
{
    return ossl_c448_ed448_sign(ctx, out_sig, private_key, public_key, message, message_len, phflag,
...
ossl_ed448_verify(OSSL_LIB_CTX *ctx, const uint8_t *message, size_t message_len,
                  ...
                  const uint8_t *context, size_t context_len, const uint8_t phflag,
                  const char *propq)
{
...
{
    return ossl_c448_ed448_derive_public_key(ctx, out_public_key, private_key, propq) ==
           C448_SUCCESS;
}
