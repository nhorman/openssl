...
static int
ec_group_explicit_todata(const EC_GROUP *group, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[],
                         BN_CTX *bnctx, unsigned char **genbuf)
{
    ...
    if (tmpl != NULL || param != NULL) {
        if (!ossl_param_build_set_utf8_string(tmpl, params, OSSL_PKEY_PARAM_EC_FIELD_TYPE,
                                              field_type)) {
            ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
            goto err;
        }
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (!ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_EC_GENERATOR, *genbuf,
                                               genbuf_len)) {
            ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
            goto err;
        }
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (cofactor != NULL &&
            !ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_COFACTOR, cofactor)) {
            ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
            goto err;
        }
    }
    ...
    if (tmpl != NULL || param != NULL) {
        ...
        if (seed != NULL && seed_len > 0 &&
            !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_EC_SEED, seed,
                                               seed_len)) {
            ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
            goto err;
        }
...
int
ossl_ec_group_todata(const EC_GROUP *group, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[],
                     OSSL_LIB_CTX *libctx, const char *propq, BN_CTX *bnctx, unsigned char **genbuf)
{
    ...
    if (pt_form_name == NULL ||
        !ossl_param_build_set_utf8_string(tmpl, params, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT,
                                          pt_form_name)) {
    ...
    if (encoding_name == NULL || !ossl_param_build_set_utf8_string(
    ...
    if (curve_nid != NID_undef) {
        ...
        if (curve_name == NULL || !ossl_param_build_set_utf8_string(
...
ossl_ec_group_fromdata(EC_KEY *ec, const OSSL_PARAM params[])
{
    ...
    group =
        EC_GROUP_new_from_params(params, ossl_ec_key_get_libctx(ec), ossl_ec_key_get0_propq(ec));
...
EC_KEY *
ossl_ec_key_param_from_x509_algor(const X509_ALGOR *palg, OSSL_LIB_CTX *libctx, const char *propq)
{
    int ptype = 0;
    const void *pval = NULL;
    EC_KEY *eckey = NULL;
...
EC_KEY *
ossl_ec_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO *p8inf, OSSL_LIB_CTX *libctx, const char *propq)
{
