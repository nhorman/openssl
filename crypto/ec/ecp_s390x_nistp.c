...
static int
ec_GFp_s390x_nistp_mul(const EC_GROUP *group, EC_POINT *r, const BIGNUM *scalar, size_t num,
                       ...
                       unsigned int fc, int len)
{
    ...
    if ((scalar != NULL && num == 0 && BN_is_negative(scalar) == 0) ||
        ...
        if (group->meth->point_get_affine_coordinates(group, point_ptr, x, y, ctx) != 1 ||
            ...
            s390x_pcc(fc, param) != 0 || BN_bin2bn(param + S390X_OFF_RES_X(len), len, x) == NULL ||
...
static ECDSA_SIG *
ecdsa_s390x_nistp_sign_sig(const unsigned char *dgst, int dgstlen, const BIGNUM *kinv,
...
static int
ecdsa_s390x_nistp_verify_sig(const unsigned char *dgst, int dgstlen, const ECDSA_SIG *sig,
                             EC_KEY *eckey, unsigned int fc, int len)
{
    unsigned char param[S390X_SIZE_PARAM];
    int rc = -1;
    BN_CTX *ctx;
...
#define EC_GFP_S390X_NISTP_METHOD(bits)                                                            \
    ...
    {                                                                                              \
        return ec_GFp_s390x_nistp_mul(group, r, scalar, num, points, scalars, ctx,                 \
                                      S390X_SCALAR_MULTIPLY_P##bits, S390X_SIZE_P##bits);          \
    }                                                                                              \
                                                                                                   \
    static ECDSA_SIG *ecdsa_s390x_nistp##bits##_sign_sig(const unsigned char *dgst, int dgstlen,   \
    ...
    {                                                                                              \
        return ecdsa_s390x_nistp_sign_sig(dgst, dgstlen, kinv, r, eckey, S390X_ECDSA_SIGN_P##bits, \
                                          S390X_SIZE_P##bits);                                     \
                                                                                                   ...
                                                                                                   \
    static int ecdsa_s390x_nistp##bits##_verify_sig(const unsigned char *dgst, int dgstlen,        \
