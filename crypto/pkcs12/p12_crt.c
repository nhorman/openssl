...
PKCS12 *
PKCS12_create_ex2(const char *pass, const char *name, EVP_PKEY *pkey, X509 *cert,
...
PKCS12 *
PKCS12_create_ex(const char *pass, const char *name, EVP_PKEY *pkey, X509 *cert,
                 STACK_OF(X509) * ca, int nid_key, int nid_cert, int iter, int mac_iter,
                 int keytype, OSSL_LIB_CTX *ctx, const char *propq)
{
    return PKCS12_create_ex2(pass, name, pkey, cert, ca, nid_key, nid_cert, iter, mac_iter, keytype,
                             ctx, propq, NULL, NULL);
}
...
{
    return PKCS12_create_ex(pass, name, pkey, cert, ca, nid_key, nid_cert, iter, mac_iter, keytype,
                            NULL, NULL);
}

static PKCS12_SAFEBAG *
pkcs12_add_cert_bag(STACK_OF(PKCS12_SAFEBAG) * *pbags, X509 *cert, const char *name, int namelen,
...
PKCS12_SAFEBAG *
PKCS12_add_key_ex(STACK_OF(PKCS12_SAFEBAG) * *pbags, EVP_PKEY *key, int key_usage, int iter,
                  int nid_key, const char *pass, OSSL_LIB_CTX *ctx, const char *propq)
{
    ...
    if (nid_key != -1) {
        ...
        bag = PKCS12_SAFEBAG_create_pkcs8_encrypt_ex(nid_key, pass, -1, NULL, 0, iter, p8, ctx,
                                                     propq);
    } else {
...
PKCS12_SAFEBAG *
PKCS12_add_key(STACK_OF(PKCS12_SAFEBAG) * *pbags, EVP_PKEY *key, int key_usage, int iter,
...
PKCS12_SAFEBAG *
PKCS12_add_secret(STACK_OF(PKCS12_SAFEBAG) * *pbags, int nid_type, const unsigned char *value,
                  int len)
{
    PKCS12_SAFEBAG *bag = NULL;
...
int
PKCS12_add_safe_ex(STACK_OF(PKCS7) * *psafes, STACK_OF(PKCS12_SAFEBAG) * bags, int nid_safe,
...
int
PKCS12_add_safe(STACK_OF(PKCS7) * *psafes, STACK_OF(PKCS12_SAFEBAG) * bags, int nid_safe, int iter,
                const char *pass)
{
...
PKCS12 *
PKCS12_add_safes_ex(STACK_OF(PKCS7) * safes, int nid_p7, OSSL_LIB_CTX *ctx, const char *propq)
{
    PKCS12 *p12;
