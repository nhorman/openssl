...
EVP_PKEY *
evp_pkcs82pkey_legacy(const PKCS8_PRIV_KEY_INFO *p8, OSSL_LIB_CTX *libctx, const char *propq)
{
    EVP_PKEY *pkey = NULL;
...
EVP_PKEY *
EVP_PKCS82PKEY_ex(const PKCS8_PRIV_KEY_INFO *p8, OSSL_LIB_CTX *libctx, const char *propq)
{
    EVP_PKEY *pkey = NULL;
    ...
    dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, "DER", "PrivateKeyInfo", keytype, selection, libctx,
                                         propq);
    ...
    if (dctx != NULL && OSSL_DECODER_CTX_get_num_decoders(dctx) == 0) {
         ...
         * keytype
         */
        dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, "DER", "PrivateKeyInfo", NULL, selection,
                                             libctx, propq);
    }
...
EVP_PKEY2PKCS8(const EVP_PKEY *pkey)
{
    ...
    if (evp_pkey_is_provided(pkey)) {
        ...
        if ((ctx = OSSL_ENCODER_CTX_new_for_pkey(pkey, selection, "DER", "PrivateKeyInfo", NULL)) ==
                NULL ||
...
int
EVP_PKEY_get_attr_by_OBJ(const EVP_PKEY *key, const ASN1_OBJECT *obj, int lastpos)
{
...
int
EVP_PKEY_add1_attr_by_NID(EVP_PKEY *key, int nid, int type, const unsigned char *bytes, int len)
{
...
int
EVP_PKEY_add1_attr_by_txt(EVP_PKEY *key, const char *attrname, int type, const unsigned char *bytes,
                          int len)
{
