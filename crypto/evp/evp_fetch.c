...
static int
put_evp_method_in_store(void *store, void *method, const OSSL_PROVIDER *prov, const char *names,
 ...
 */
static void *
construct_evp_method(const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov, void *data)
{
    /*
...
static void *
inner_evp_generic_fetch(struct evp_method_data_st *methdata, OSSL_PROVIDER *prov, int operation_id,
                        ...
                        int (*up_ref_method)(void *), void (*free_method)(void *))
{
    ...
    if (meth_id == 0 || !ossl_method_store_cache_get(store, prov, meth_id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = {get_tmp_evp_method_store,   reserve_evp_method_store,
        ...
        if ((method = ossl_method_construct(methdata->libctx, operation_id, &prov,
            ...
            if (name_id == 0) {
                ERR_raise_data(ERR_LIB_EVP, ERR_R_FETCH_FAILED, "Algorithm %s cannot be found",
                               name);
                free_method(method);
                method = NULL;
            } else {
                ...
                if (meth_id != 0)
                    ossl_method_store_cache_set(store, prov, meth_id, propq, method, up_ref_method,
                                                free_method);
            }
        }
    ...
    if ((name_id != 0 || name != NULL) && method == NULL) {
        ...
        OSSL_TRACE4(QUERY, "%s, Algorithm (%s : %d), Properties (%s)\n",
                    ossl_lib_ctx_get_descriptor(methdata->libctx), name == NULL ? "<null>" : name,
                    name_id, properties == NULL ? "<null>" : properties);
    }
...
void *
evp_generic_fetch(OSSL_LIB_CTX *libctx, int operation_id, const char *name, const char *properties,
                  void *(*new_method)(int name_id, const OSSL_ALGORITHM *algodef,
                                      OSSL_PROVIDER *prov),
                  int (*up_ref_method)(void *), void (*free_method)(void *))
{
    ...
    method = inner_evp_generic_fetch(&methdata, NULL, operation_id, name, properties, new_method,
 ...
 */
void *
evp_generic_fetch_from_prov(OSSL_PROVIDER *prov, int operation_id, const char *name,
                            ...
                            int (*up_ref_method)(void *), void (*free_method)(void *))
{
    ...
    methdata.tmp_store = NULL;
    method = inner_evp_generic_fetch(&methdata, prov, operation_id, name, properties, new_method,
                                     up_ref_method, free_method);
    dealloc_tmp_evp_method_store(methdata.tmp_store);
    return method;
}
...
static int
evp_set_parsed_default_properties(OSSL_LIB_CTX *libctx, OSSL_PROPERTY_LIST *def_prop,
...
int
evp_set_default_properties_int(OSSL_LIB_CTX *libctx, const char *propq, int loadconfig,
                               int mirrored)
{
...
static int
evp_default_properties_merge(OSSL_LIB_CTX *libctx, const char *propq, int loadconfig)
{
    OSSL_PROPERTY_LIST **plp = ossl_ctx_global_properties(libctx, loadconfig);
...
int
evp_default_properties_enable_fips_int(OSSL_LIB_CTX *libctx, int enable, int loadconfig)
{
...
void
evp_generic_do_all(OSSL_LIB_CTX *libctx, int operation_id, void (*user_fn)(void *method, void *arg),
                   void *user_arg,
                   void *(*new_method)(int name_id, const OSSL_ALGORITHM *algodef,
                                       OSSL_PROVIDER *prov),
                   int (*up_ref_method)(void *), void (*free_method)(void *))
{
    ...
    methdata.libctx = libctx;
    methdata.tmp_store = NULL;
    (void)inner_evp_generic_fetch(&methdata, NULL, operation_id, NULL, NULL, new_method,
                                  up_ref_method, free_method);
...
int
evp_is_a(OSSL_PROVIDER *prov, int number, const char *legacy_name, const char *name)
{
    /*
...
int
evp_names_do_all(OSSL_PROVIDER *prov, int number, void (*fn)(const char *name, void *data),
                 void *data)
{
    OSSL_LIB_CTX *libctx = ossl_provider_libctx(prov);
    OSSL_NAMEMAP *namemap = ossl_namemap_stored(libctx);
