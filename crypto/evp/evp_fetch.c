    ...
    void *method)
{
    ...
    if (ossl_unlikely(cache == NULL)) {
        ...
        case OSSL_OP_CIPHER:
            ...
            break;
        case OSSL_OP_MAC:
            TL_INSERT(EVP_MAC, method, cache->cache, key, &mac);
            ret = mac;
            break;
        case OSSL_OP_KDF:
            TL_INSERT(EVP_KDF, method, cache->cache, key, &kdf);
            ret = kdf;
            break;
        case OSSL_OP_RAND:
            TL_INSERT(EVP_RAND, method, cache->cache, key, &rand);
            ret = rand;
            break;
        ...
        case OSSL_OP_KEYEXCH:
            TL_INSERT(EVP_KEYEXCH, method, cache->cache, key, &kex);
            ret = kex;
            break;
        case OSSL_OP_SIGNATURE:
            TL_INSERT(EVP_SIGNATURE, method, cache->cache, key, &sig);
            ret = sig;
            break;
        case OSSL_OP_ASYM_CIPHER:
            TL_INSERT(EVP_ASYM_CIPHER, method, cache->cache, key, &acp);
            ret = acp;
            break;
        case OSSL_OP_KEM:
            TL_INSERT(EVP_KEM, method, cache->cache, key, &kem);
            ret = kem;
            break;
