...
static void *
evp_rand_from_algorithm(int name_id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
...
EVP_RAND *
EVP_RAND_fetch(OSSL_LIB_CTX *libctx, const char *algorithm, const char *properties)
{
    return evp_generic_fetch(libctx, OSSL_OP_RAND, algorithm, properties, evp_rand_from_algorithm,
...
EVP_RAND_CTX_new(EVP_RAND *rand, EVP_RAND_CTX *parent)
{
    ...
    if ((ctx->algctx = rand->newctx(ossl_provider_ctx(rand->prov), parent_ctx, parent_dispatch)) ==
            NULL ||
...
void
EVP_RAND_do_all_provided(OSSL_LIB_CTX *libctx, void (*fn)(EVP_RAND *rand, void *arg), void *arg)
{
...
int
EVP_RAND_names_do_all(const EVP_RAND *rand, void (*fn)(const char *name, void *data), void *data)
{
    if (rand->prov != NULL)
...
static int
evp_rand_instantiate_locked(EVP_RAND_CTX *ctx, unsigned int strength, int prediction_resistance,
                            const unsigned char *pstr, size_t pstr_len, const OSSL_PARAM params[])
{
    return ctx->meth->instantiate(ctx->algctx, strength, prediction_resistance, pstr, pstr_len,
                                  params);
}

int
EVP_RAND_instantiate(EVP_RAND_CTX *ctx, unsigned int strength, int prediction_resistance,
                     const unsigned char *pstr, size_t pstr_len, const OSSL_PARAM params[])
{
    int res;
                         ...
                         const unsigned char *addin, size_t addin_len)
{
    ...
    for (; outlen > 0; outlen -= chunk, out += chunk) {
        ...
        if (!ctx->meth->generate(ctx->algctx, out, chunk, strength, prediction_resistance, addin,
                                 addin_len)) {
...
int
EVP_RAND_generate(EVP_RAND_CTX *ctx, unsigned char *out, size_t outlen, unsigned int strength,
                  int prediction_resistance, const unsigned char *addin, size_t addin_len)
{
    ...
    if (!evp_rand_lock(ctx))
        return 0;
    res = evp_rand_generate_locked(ctx, out, outlen, strength, prediction_resistance, addin,
                                   addin_len);
    evp_rand_unlock(ctx);
    return res;
}
...
static int
evp_rand_reseed_locked(EVP_RAND_CTX *ctx, int prediction_resistance, const unsigned char *ent,
                       size_t ent_len, const unsigned char *addin, size_t addin_len)
{
    if (ctx->meth->reseed != NULL)
        return ctx->meth->reseed(ctx->algctx, prediction_resistance, ent, ent_len, addin,
                                 addin_len);
    return 1;
}

int
EVP_RAND_reseed(EVP_RAND_CTX *ctx, int prediction_resistance, const unsigned char *ent,
                size_t ent_len, const unsigned char *addin, size_t addin_len)
{
    int res;
...
static size_t
evp_rand_get_seed_locked(EVP_RAND_CTX *ctx, unsigned char **buffer, int entropy, size_t min_len,
...
size_t
evp_rand_get_seed(EVP_RAND_CTX *ctx, unsigned char **buffer, int entropy, size_t min_len,
                  size_t max_len, int prediction_resistance, const unsigned char *adin,
                  size_t adin_len)
{
    int res;
...
static void
evp_rand_clear_seed_locked(EVP_RAND_CTX *ctx, unsigned char *buffer, size_t b_len)
{
