...
int
EVP_PBE_scrypt_ex(const char *pass, size_t passlen, const unsigned char *salt, size_t saltlen,
                  ...
                  size_t keylen, OSSL_LIB_CTX *ctx, const char *propq)
{
    ...
    *z++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (unsigned char *)pass, passlen);
...
int
EVP_PBE_scrypt(const char *pass, size_t passlen, const unsigned char *salt, size_t saltlen,
               uint64_t N, uint64_t r, uint64_t p, uint64_t maxmem, unsigned char *key,
               size_t keylen)
{
    return EVP_PBE_scrypt_ex(pass, passlen, salt, saltlen, N, r, p, maxmem, key, keylen, NULL,
                             NULL);
}
