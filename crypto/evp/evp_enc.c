...
static int
evp_cipher_init_internal(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, ENGINE *impl,
...
nonlegacy:
...
#else
        EVP_CIPHER *provciph =
            EVP_CIPHER_fetch(NULL, cipher->nid == NID_undef ? "NULL" : OBJ_nid2sn(cipher->nid), "");
    ...
    if (enc) {
        ...
        return ctx->cipher->einit(ctx->algctx, key,
    ...
    if (ctx->cipher->dinit == NULL) {
        ...
        if (key == NULL && ctx->cipher->dinit_skey != NULL) {
            return ctx->cipher->dinit_skey(
                ctx->algctx, NULL, iv, iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx), params);
        }
        ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
        return 0;
    }

    return ctx->cipher->dinit(ctx->algctx, key,
                              key == NULL ? 0 : EVP_CIPHER_CTX_get_key_length(ctx), iv,
                              iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx), params);
...
evp_cipher_init_skey_internal(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const EVP_SKEY *skey,
                              const unsigned char *iv, size_t iv_len, int enc,
                              const OSSL_PARAM params[])
{
    ...
    if (enc) {
        if (ctx->cipher->einit_skey == NULL) {
            ...
            ret = ctx->cipher->einit_skey(ctx->algctx, skey == NULL ? NULL : skey->keydata, iv,
                                          iv_len, params);
        }
    } else {
        if (ctx->cipher->dinit_skey == NULL) {
            ...
            ret = ctx->cipher->dinit_skey(ctx->algctx, skey == NULL ? NULL : skey->keydata, iv,
                                          iv_len, params);
        }
    }
...
int
EVP_CipherInit_SKEY(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, EVP_SKEY *skey,
...
int
EVP_CipherInit_ex2(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const unsigned char *key,
...
int
EVP_CipherInit(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const unsigned char *key,
...
EVP_CipherPipelineEncryptInit(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
                              const unsigned char *key, size_t keylen, size_t numpipes,
...
int
EVP_CipherUpdate(EVP_CIPHER_CTX *ctx, unsigned char *out, int *outl, const unsigned char *in,
                 int inl)
{
    if (ctx->encrypt)
...
int
EVP_CipherPipelineUpdate(EVP_CIPHER_CTX *ctx, unsigned char **out, size_t *outl,
...
int
EVP_CipherPipelineFinal(EVP_CIPHER_CTX *ctx, unsigned char **out, size_t *outl,
...
int
EVP_EncryptInit(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const unsigned char *key,
...
int
EVP_EncryptInit_ex2(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const unsigned char *key,
...
int
EVP_DecryptInit(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const unsigned char *key,
                const unsigned char *iv)
{
...
int
EVP_DecryptInit_ex2(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const unsigned char *key,
                    const unsigned char *iv, const OSSL_PARAM params[])
{
...
ossl_is_partially_overlapping(const void *ptr1, const void *ptr2, int len)
{
     ...
     */
    int overlapped =
        (len > 0) & (diff != 0) & ((diff < (PTRDIFF_T)len) | (diff > (0 - (PTRDIFF_T)len)));
...
int
EVP_EncryptUpdate(EVP_CIPHER_CTX *ctx, unsigned char *out, int *outl, const unsigned char *in,
                  int inl)
{
    int ret;
...
int
EVP_DecryptUpdate(EVP_CIPHER_CTX *ctx, unsigned char *out, int *outl, const unsigned char *in,
                  int inl)
{
...
EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_AEAD_SET_IV_FIXED:
        params[0] =
            OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TLS1_IV_FIXED, ptr, sz);
        break;
    case EVP_CTRL_GCM_IV_GEN:
        ...
        params[0] =
            OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TLS1_GET_IV_GEN, ptr, sz);
        break;
    case EVP_CTRL_GCM_SET_IV_INV:
        if (arg < 0)
            return 0;
        params[0] =
            OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TLS1_SET_IV_INV, ptr, sz);
        break;
    ...
    case EVP_CTRL_TLS1_1_MULTIBLOCK_MAX_BUFSIZE:
        params[0] =
            OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_MAX_SEND_FRAGMENT, &sz);
        ret = evp_do_ciph_ctx_setparams(ctx->cipher, ctx->algctx, params);
        if (ret <= 0)
            return 0;
    ...
    case EVP_CTRL_TLS1_1_MULTIBLOCK_AAD: {
        ...
        params[0] = OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_AAD,
                                                      (void *)p->inp, p->len);
        params[1] =
            OSSL_PARAM_construct_uint(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_INTERLEAVE, &p->interleave);
        ret = evp_do_ciph_ctx_setparams(ctx->cipher, ctx->algctx, params);
        if (ret <= 0)
            return ret;
    ...
    case EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT: {
        ...
        params[0] = OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC, p->out,
                                                      p->len);
...
static void *
evp_cipher_from_algorithm(const int name_id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
    ...
    if ((fnciphcnt != 0 && fnciphcnt != 3 && fnciphcnt != 4) ||
        ...
        (fnpipecnt != 0 &&
         (fnpipecnt < 3 || cipher->p_cupdate == NULL || cipher->p_cfinal == NULL)) ||
        fnctxcnt != 2) {
        /*
...
EVP_CIPHER *
EVP_CIPHER_fetch(OSSL_LIB_CTX *ctx, const char *algorithm, const char *properties)
{
    EVP_CIPHER *cipher =
        evp_generic_fetch(ctx, OSSL_OP_CIPHER, algorithm, properties, evp_cipher_from_algorithm,
...
EVP_CIPHER *
evp_cipher_fetch_from_prov(OSSL_PROVIDER *prov, const char *algorithm, const char *properties)
{
    return evp_generic_fetch_from_prov(prov, OSSL_OP_CIPHER, algorithm, properties,
...
void
EVP_CIPHER_do_all_provided(OSSL_LIB_CTX *libctx, void (*fn)(EVP_CIPHER *mac, void *arg), void *arg)
{
