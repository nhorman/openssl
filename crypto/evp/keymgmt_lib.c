...
int
evp_keymgmt_util_assign_pkey(EVP_PKEY *pkey, EVP_KEYMGMT *keymgmt, void *keydata)
{
...
int
evp_keymgmt_util_export(const EVP_PKEY *pk, int selection, OSSL_CALLBACK *export_cb,
                        void *export_cbarg)
{
...
void *
evp_keymgmt_util_export_to_provider(EVP_PKEY *pk, EVP_KEYMGMT *keymgmt, int selection)
{
    ...
    if (pk->keymgmt == keymgmt ||
        (pk->keymgmt->name_id == keymgmt->name_id && pk->keymgmt->prov == keymgmt->prov))
        return pk->keydata;
...
OP_CACHE_ELEM *
evp_keymgmt_util_find_operation_cache(EVP_PKEY *pk, EVP_KEYMGMT *keymgmt, int selection)
{
...
int
evp_keymgmt_util_cache_keydata(EVP_PKEY *pk, EVP_KEYMGMT *keymgmt, void *keydata, int selection)
{
...
void *
evp_keymgmt_util_fromdata(EVP_PKEY *target, EVP_KEYMGMT *keymgmt, int selection,
...
void *
evp_keymgmt_util_gen(EVP_PKEY *target, EVP_KEYMGMT *keymgmt, void *genctx, OSSL_CALLBACK *cb,
                     void *cbarg)
{
    void *keydata = NULL;
 ...
 */
int
evp_keymgmt_util_get_deflt_digest_name(EVP_KEYMGMT *keymgmt, void *keydata, char *mdname,
                                       size_t mdname_sz)
{
    OSSL_PARAM params[3];
    char mddefault[100] = "";
    ...
    params[0] = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_DEFAULT_DIGEST, mddefault,
                                                 sizeof(mddefault));
    params[1] = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_MANDATORY_DIGEST, mdmandatory,
                                                 sizeof(mdmandatory));
