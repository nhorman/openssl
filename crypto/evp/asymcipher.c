...
static int
evp_pkey_asym_cipher_init(EVP_PKEY_CTX *ctx, int operation, const OSSL_PARAM params[])
{
    int ret = 0;
    void *provkey = NULL;
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
        ...
        case 2:
            ...
            cipher = evp_asym_cipher_fetch_from_prov((OSSL_PROVIDER *)tmp_prov, supported_ciph,
                                                     ctx->propquery);
            if (cipher == NULL)
                goto legacy;
            break;
...
int
EVP_PKEY_encrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                 size_t inlen)
{
    int ret;
...
legacy:
    ...
    M_check_autoarg(ctx, out, outlen,
                    EVP_F_EVP_PKEY_ENCRYPT) return ctx->pmeth->encrypt(ctx, out, outlen, in, inlen);
}
...
int
EVP_PKEY_decrypt(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen, const unsigned char *in,
                 size_t inlen)
{
    int ret;
...
legacy:
    ...
    M_check_autoarg(ctx, out, outlen,
                    EVP_F_EVP_PKEY_DECRYPT) return ctx->pmeth->decrypt(ctx, out, outlen, in, inlen);
}

/* decrypt to new buffer of dynamic size, checking any pre-determined size */
int
evp_pkey_decrypt_alloc(EVP_PKEY_CTX *ctx, unsigned char **outp, size_t *outlenp,
...
static void *
evp_asym_cipher_from_algorithm(int name_id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
...
EVP_ASYM_CIPHER *
EVP_ASYM_CIPHER_fetch(OSSL_LIB_CTX *ctx, const char *algorithm, const char *properties)
{
...
EVP_ASYM_CIPHER *
evp_asym_cipher_fetch_from_prov(OSSL_PROVIDER *prov, const char *algorithm, const char *properties)
{
    return evp_generic_fetch_from_prov(prov, OSSL_OP_ASYM_CIPHER, algorithm, properties,
                                       evp_asym_cipher_from_algorithm, evp_asym_cipher_up_ref,
                                       evp_asym_cipher_free);
}
...
{
    evp_generic_do_all(libctx, OSSL_OP_ASYM_CIPHER, (void (*)(void *, void *))fn, arg,
                       evp_asym_cipher_from_algorithm, evp_asym_cipher_up_ref,
                       evp_asym_cipher_free);
}
