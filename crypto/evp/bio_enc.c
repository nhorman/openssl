    ...
    while (outl > 0) {
        ...
        if (i <= 0) {
            ...
            if (outl > ENC_MIN_CHUNK) {
                ...
                if (!EVP_CipherUpdate(ctx->cipher, (unsigned char *)out, &buf_len, ctx->read_start,
                                      i > j ? j : i)) {
...
int
BIO_set_cipher(BIO *b, const EVP_CIPHER *c, const unsigned char *k, const unsigned char *i, int e)
{
    BIO_ENC_CTX *ctx;
    ...
    if (callback_ex != NULL)
        return callback_ex(b, BIO_CB_CTRL | BIO_CB_RETURN, (const char *)c, 0, BIO_CTRL_SET, e, 1,
                           NULL);
#ifndef OPENSSL_NO_DEPRECATED_3_0
