...
static void *
evp_keyexch_from_algorithm(int name_id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
...
EVP_KEYEXCH *
EVP_KEYEXCH_fetch(OSSL_LIB_CTX *ctx, const char *algorithm, const char *properties)
{
    return evp_generic_fetch(ctx, OSSL_OP_KEYEXCH, algorithm, properties,
                             evp_keyexch_from_algorithm, evp_keyexch_up_ref, evp_keyexch_free);
}

EVP_KEYEXCH *
evp_keyexch_fetch_from_prov(OSSL_PROVIDER *prov, const char *algorithm, const char *properties)
{
    return evp_generic_fetch_from_prov(prov, OSSL_OP_KEYEXCH, algorithm, properties,
...
EVP_PKEY_derive_init_ex(EVP_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
        ...
        case 2:
            ...
            exchange = evp_keyexch_fetch_from_prov((OSSL_PROVIDER *)tmp_prov, supported_exch,
                                                   ctx->propquery);
            if (exchange == NULL)
                goto legacy;
            break;
...
int
EVP_PKEY_derive_set_peer_ex(EVP_PKEY_CTX *ctx, EVP_PKEY *peer, int validate_peer)
{
    int ret = 0, check;
    void *provkey = NULL;
...
EVP_PKEY_derive(EVP_PKEY_CTX *ctx, unsigned char *key, size_t *pkeylen)
{
    ...
    ret =
        ctx->op.kex.exchange->derive(ctx->op.kex.algctx, key, pkeylen, key != NULL ? *pkeylen : 0);
...
legacy:
    ...
    M_check_autoarg(ctx, key, pkeylen, EVP_F_EVP_PKEY_DERIVE) return ctx->pmeth->derive(ctx, key,
                                                                                        pkeylen);
}
...
void
EVP_KEYEXCH_do_all_provided(OSSL_LIB_CTX *libctx, void (*fn)(EVP_KEYEXCH *keyexch, void *arg),
                            void *arg)
{
...
int
EVP_KEYEXCH_names_do_all(const EVP_KEYEXCH *keyexch, void (*fn)(const char *name, void *data),
                         void *data)
{
