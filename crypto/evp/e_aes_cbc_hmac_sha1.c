...
void aesni256_cbc_sha1_dec(const void *inp, void *out, size_t blocks, const AES_KEY *key,
                           unsigned char iv[16], SHA_CTX *ctx, const void *in0);
...
static size_t
tls1_1_multi_block_encrypt(EVP_AES_HMAC_SHA1 *key, unsigned char *out, const unsigned char *inp,
...
static int
aesni_cbc_hmac_sha1_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in,
                           size_t len)
{
    EVP_AES_HMAC_SHA1 *key = data(ctx);
    unsigned int l;
...
# if defined(STITCHED_DECRYPT_CALL)
            if (stitch) {
                ...
                aesni256_cbc_sha1_dec(in + aes_off, out + aes_off, blocks, &key->ks, ctx->iv,
                                      &key->md, out + sha_off);
...
# if !defined(OPENSSL_NO_MULTIBLOCK)
    ...
    case EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT: {
        ...
        return (int)tls1_1_multi_block_encrypt(key, param->out, param->inp, param->len,
                                               param->interleave / 4);
    }
