...
# define ARIA_SBOX_LAYER1_WITH_PRE_DIFF(T0, T1, T2, T3)                                            \
     do {                                                                                          \
         (T0) = S1[GET_U8_BE(T0, 0)] ^ S2[GET_U8_BE(T0, 1)] ^ X1[GET_U8_BE(T0, 2)] ^               \
                X2[GET_U8_BE(T0, 3)];                                                              \
         (T1) = S1[GET_U8_BE(T1, 0)] ^ S2[GET_U8_BE(T1, 1)] ^ X1[GET_U8_BE(T1, 2)] ^               \
                X2[GET_U8_BE(T1, 3)];                                                              \
         (T2) = S1[GET_U8_BE(T2, 0)] ^ S2[GET_U8_BE(T2, 1)] ^ X1[GET_U8_BE(T2, 2)] ^               \
                X2[GET_U8_BE(T2, 3)];                                                              \
         (T3) = S1[GET_U8_BE(T3, 0)] ^ S2[GET_U8_BE(T3, 1)] ^ X1[GET_U8_BE(T3, 2)] ^               \
                X2[GET_U8_BE(T3, 3)];                                                              \
     } while (0)
...
# define ARIA_SBOX_LAYER2_WITH_PRE_DIFF(T0, T1, T2, T3)                                            \
     do {                                                                                          \
         (T0) = X1[GET_U8_BE(T0, 0)] ^ X2[GET_U8_BE(T0, 1)] ^ S1[GET_U8_BE(T0, 2)] ^               \
                S2[GET_U8_BE(T0, 3)];                                                              \
         (T1) = X1[GET_U8_BE(T1, 0)] ^ X2[GET_U8_BE(T1, 1)] ^ S1[GET_U8_BE(T1, 2)] ^               \
                S2[GET_U8_BE(T1, 3)];                                                              \
         (T2) = X1[GET_U8_BE(T2, 0)] ^ X2[GET_U8_BE(T2, 1)] ^ S1[GET_U8_BE(T2, 2)] ^               \
                S2[GET_U8_BE(T2, 3)];                                                              \
         (T3) = X1[GET_U8_BE(T3, 0)] ^ X2[GET_U8_BE(T3, 1)] ^ S1[GET_U8_BE(T3, 2)] ^               \
                S2[GET_U8_BE(T3, 3)];                                                              \
     } while (0)
...
# define _ARIA_GSRK(RK, X, Y, Q, R)                                                                \
     do {                                                                                          \
         ...
         (RK)->u[2] =                                                                              \
             ((X)[2]) ^ (((Y)[((Q) + 2) % 4]) >> (R)) ^ (((Y)[((Q) + 1) % 4]) << (32 - (R)));      \
         (RK)->u[3] =                                                                              \
             ((X)[3]) ^ (((Y)[((Q) + 3) % 4]) >> (R)) ^ (((Y)[((Q) + 2) % 4]) << (32 - (R)));      \
     } while (0)
...
void
ossl_aria_encrypt(const unsigned char *in, unsigned char *out, const ARIA_KEY *key)
{
...
int
ossl_aria_set_encrypt_key(const unsigned char *userKey, const int bits, ARIA_KEY *key)
{
    register uint32_t reg0, reg1, reg2, reg3;
...
int
ossl_aria_set_decrypt_key(const unsigned char *userKey, const int bits, ARIA_KEY *key)
{
    ARIA_u128 *rk_head;
    ARIA_u128 *rk_tail;
...
static const ARIA_u128 c1 = {{0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab,
                              0xe8, 0xfa, 0x9a, 0x6e, 0xe0}};

static const ARIA_u128 c2 = {{0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1,
                              0xd5, 0xef, 0x5d, 0xe2, 0xb0}};

static const ARIA_u128 c3 = {{0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97,
...
rotnr(unsigned int n, ARIA_u128 *o, const ARIA_u128 * xor, const ARIA_u128 *z)
{
    ...
    for (i = 0; i < ARIA_BLOCK_SIZE; i++)
        o->c[i] =
            ((t.c[i] >> bits) | (t.c[i ? i - 1 : ARIA_BLOCK_SIZE - 1] << (8 - bits))) ^ xor->c[i];
}
 ...
 */
static void
do_encrypt(unsigned char *o, const unsigned char *pin, unsigned int rounds, const ARIA_u128 *keys)
{
    ARIA_u128 p;
    unsigned int i;
