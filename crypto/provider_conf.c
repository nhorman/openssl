 ...
 */
static int
provider_conf_params_internal(OSSL_PROVIDER *prov, OSSL_PROVIDER_INFO *provinfo, const char *name,
                              ...
                              STACK_OF(OPENSSL_CSTRING) * visited)
{
    ...
    if (sect != NULL) {
        ...
        for (i = 0; i < sk_CONF_VALUE_num(sect); i++) {
            ...
            rc = provider_conf_params_internal(prov, provinfo, buffer, sectconf->value, cnf,
                                               visited);
            if (rc < 0) {
                sk_OPENSSL_CSTRING_pop(visited);
                return rc;
 ...
 */
static int
provider_conf_params(OSSL_PROVIDER *prov, OSSL_PROVIDER_INFO *provinfo, const char *name,
                     const char *value, const CONF *cnf)
{
    int rc;
 ...
 */
static int
provider_conf_activate(OSSL_LIB_CTX *libctx, const char *name, const char *value, const char *path,
...
static int
provider_conf_parse_bool_setting(const char *confname, const char *confvalue, int *val)
{
    ...
    if ((strcmp(confvalue, "1") == 0) || (strcmp(confvalue, "yes") == 0) ||
        ...
        *val = 1;
               ...
               (strcmp(confvalue, "NO") == 0) || (strcmp(confvalue, "false") == 0) ||
               (strcmp(confvalue, "FALSE") == 0) || (strcmp(confvalue, "off") == 0) ||
...
static int
provider_conf_load(OSSL_LIB_CTX *libctx, const char *name, const char *value, const CONF *cnf)
{
    int i;
    ...
    if (!ecmds) {
        ERR_raise_data(ERR_LIB_CRYPTO, CRYPTO_R_PROVIDER_SECTION_ERROR, "section=%s not found",
                       value);
        return 0;
    }
