...
static int
ocsp_verify_signer(X509 *signer, int response, X509_STORE *st, unsigned long flags,
...
static int
ocsp_verify(OCSP_REQUEST *req, OCSP_BASICRESP *bs, X509 *signer, unsigned long flags)
{
    EVP_PKEY *skey;
    int ret = 1;
...
int
OCSP_resp_get0_signer(OCSP_BASICRESP *bs, X509 **signer, STACK_OF(X509) * extra_certs)
{
    return ocsp_find_signer(signer, bs, extra_certs, 0) > 0;
}

static int
ocsp_find_signer(X509 **psigner, OCSP_BASICRESP *bs, STACK_OF(X509) * certs, unsigned long flags)
{
    X509 *signer;
 ...
 */
static int
ocsp_match_issuerid(X509 *cert, OCSP_CERTID *cid, STACK_OF(OCSP_SINGLERESP) * sresp)
{
    int ret = -1;
    EVP_MD *dgst = NULL;
...
static int
ocsp_req_find_signer(X509 **psigner, OCSP_REQUEST *req, const X509_NAME *nm, STACK_OF(X509) * certs,
                     unsigned long flags)
{
    X509 *signer;
