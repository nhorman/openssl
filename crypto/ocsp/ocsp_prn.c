...
OCSP_REQUEST_print(BIO *bp, OCSP_REQUEST *o, unsigned long flags)
{
    ...
    for (i = 0; i < sk_OCSP_ONEREQ_num(inf->requestList); i++) {
        ...
        if (!X509V3_extensions_print(bp, "Request Single Extensions", one->singleRequestExtensions,
                                     flags, 8))
            goto err;
    }
    ...
    for (i = 0; i < sk_OCSP_SINGLERESP_num(rd->responses); i++) {
        ...
        if (cst->type == V_OCSP_CERTSTATUS_REVOKED) {
            ...
            if (rev->revocationReason) {
                ...
                if (BIO_printf(bp, "\n    Revocation Reason: %s (0x%lx)", OCSP_crl_reason_str(l),
                               l) <= 0)
                    goto err;
            }
        }
        ...
        if (BIO_write(bp, "\n", 1) <= 0)
            goto err;
        if (!X509V3_extensions_print(bp, "Response Single Extensions", single->singleExtensions,
                                     flags, 8))
            goto err;
        if (BIO_write(bp, "\n", 1) <= 0)
            goto err;
