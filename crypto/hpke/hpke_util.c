...
static const OSSL_HPKE_KEM_INFO hpke_kem_tab[] = {
    ...
    {OSSL_HPKE_KEM_ID_P256, "EC", OSSL_HPKE_KEMSTR_P256, LN_sha256, SHA256_DIGEST_LENGTH, 65, 65,
     32, 0xFF},
    {OSSL_HPKE_KEM_ID_P384, "EC", OSSL_HPKE_KEMSTR_P384, LN_sha384, SHA384_DIGEST_LENGTH, 97, 97,
     48, 0xFF},
    {OSSL_HPKE_KEM_ID_P521, "EC", OSSL_HPKE_KEMSTR_P521, LN_sha512, SHA512_DIGEST_LENGTH, 133, 133,
     66, 0x01},
# ifndef OPENSSL_NO_ECX
    {OSSL_HPKE_KEM_ID_X25519, OSSL_HPKE_KEMSTR_X25519, NULL, LN_sha256, SHA256_DIGEST_LENGTH,
     X25519_KEYLEN, X25519_KEYLEN, X25519_KEYLEN, 0x00},
    {OSSL_HPKE_KEM_ID_X448, OSSL_HPKE_KEMSTR_X448, NULL, LN_sha512, SHA512_DIGEST_LENGTH,
...
ossl_hpke_kdf_extract(EVP_KDF_CTX *kctx, unsigned char *prk, size_t prklen,
                      ...
                      size_t ikmlen)
{
    return kdf_derive(kctx, prk, prklen, EVP_KDF_HKDF_MODE_EXTRACT_ONLY, salt, saltlen, ikm, ikmlen,
                      NULL, 0);
}

/* Common code to perform a HKDF expand */
int
ossl_hpke_kdf_expand(EVP_KDF_CTX *kctx, unsigned char *okm, size_t okmlen, const unsigned char *prk,
                     size_t prklen, const unsigned char *info, size_t infolen)
{
    return kdf_derive(kctx, okm, okmlen, EVP_KDF_HKDF_MODE_EXPAND_ONLY, NULL, 0, prk, prklen, info,
                      infolen);
}
 ...
 */
int
ossl_hpke_labeled_extract(EVP_KDF_CTX *kctx, unsigned char *prk, size_t prklen,
                          ...
                          const unsigned char *ikm, size_t ikmlen)
{
    ...
    if (!WPACKET_init_static_len(&pkt, labeled_ikm, labeled_ikmlen, 0) ||
        ...
        !WPACKET_memcpy(&pkt, suiteid, suiteidlen) || !WPACKET_memcpy(&pkt, label, labellen) ||
        !WPACKET_memcpy(&pkt, ikm, ikmlen) || !WPACKET_get_total_written(&pkt, &labeled_ikmlen) ||
        !WPACKET_finish(&pkt)) {
                         ...
                         const unsigned char *info, size_t infolen)
{
    ...
    labellen = strlen(label);
    labeled_infolen =
        2 + okmlen + prklen + label_hpkev1len + protocol_labellen + suiteidlen + labellen + infolen;
...
EVP_KDF_CTX *
ossl_kdf_ctx_create(const char *kdfname, const char *mdname, OSSL_LIB_CTX *libctx,
                    const char *propq)
{
    EVP_KDF *kdf;
    ...
    while (st != NULL && labels < 3) {
        ...
        if (labels == 0 && (kem = synonyms_name2id(st, kemstrtab, OSSL_NELEM(kemstrtab))) == 0)
            ...
            goto fail;
                 ...
                 (aead = synonyms_name2id(st, aeadstrtab, OSSL_NELEM(aeadstrtab))) == 0)
            goto fail;
