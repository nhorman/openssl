...
# ifndef OPENSSL_NO_DEPRECATED_3_0
    ...
    if (!ret) {
        ...
        RAND_POOL *pool = ossl_rand_pool_new(RAND_DRBG_STRENGTH, 1, (RAND_DRBG_STRENGTH + 7) / 8,
        ...
        if (meth->add == NULL || meth->add(ossl_rand_pool_buffer(pool), ossl_rand_pool_length(pool),
 ...
 */
int
RAND_priv_bytes_ex(OSSL_LIB_CTX *ctx, unsigned char *buf, size_t num, unsigned int strength)
{
    RAND_GLOBAL *dgbl;
    EVP_RAND_CTX *rand;
    ...
    if (dgbl->random_provider != NULL)
        return ossl_provider_random_bytes(dgbl->random_provider, OSSL_PROV_RANDOM_PRIVATE, buf, num,
                                          strength);
#endif /* !FIPS_MODULE */
...
#ifndef FIPS_MODULE
    if (dgbl->random_provider != NULL)
        return ossl_provider_random_bytes(dgbl->random_provider, OSSL_PROV_RANDOM_PUBLIC, buf, num,
                                          strength);
#endif /* !FIPS_MODULE */
...
static EVP_RAND_CTX *
rand_new_drbg(OSSL_LIB_CTX *libctx, EVP_RAND_CTX *parent, unsigned int reseed_interval,
              time_t reseed_time_interval)
{
    ...
    if (prov_name != NULL)
        *p++ =
            OSSL_PARAM_construct_utf8_string(OSSL_PROV_PARAM_CORE_PROV_NAME, (char *)prov_name, 0);
...
rand_get0_public(OSSL_LIB_CTX *ctx, RAND_GLOBAL *dgbl)
{
    ...
    if (rand == NULL) {
        ...
        if (CRYPTO_THREAD_get_local(&dgbl->private) == NULL &&
            ...
            return NULL;
        rand =
            rand_new_drbg(ctx, primary, SECONDARY_RESEED_INTERVAL, SECONDARY_RESEED_TIME_INTERVAL);
...
int
RAND_set_DRBG_type(OSSL_LIB_CTX *ctx, const char *drbg, const char *propq, const char *cipher,
                   const char *digest)
{
    RAND_GLOBAL *dgbl = rand_get_global(ctx);
...
int
RAND_set_seed_source_type(OSSL_LIB_CTX *ctx, const char *seed, const char *propq)
{
    RAND_GLOBAL *dgbl = rand_get_global(ctx);
 ...
 */
int
ossl_rand_check_random_provider_on_unload(OSSL_LIB_CTX *ctx, OSSL_PROVIDER *prov)
{
    RAND_GLOBAL *dgbl = rand_get_global(ctx);
