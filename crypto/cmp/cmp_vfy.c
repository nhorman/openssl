...
static int
verify_signature(const OSSL_CMP_CTX *cmp_ctx, const OSSL_CMP_MSG *msg, X509 *cert)
{
    ...
    if (ASN1_item_verify_ex(ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART), msg->header->protectionAlg,
                            msg->protection, &prot_part, NULL, pubkey, cmp_ctx->libctx,
                            cmp_ctx->propq) > 0) {
        res = 1;
        goto end;
    }
 ...
 */
int
OSSL_CMP_validate_cert_path(const OSSL_CMP_CTX *ctx, X509_STORE *trusted_store, X509 *cert)
{
    int valid = 0;
...
check_name(const OSSL_CMP_CTX *ctx, int log_success, const char *actual_desc,
           const X509_NAME *actual_name, const char *expect_desc, const X509_NAME *expect_name)
{
    char *str;
...
static int
check_kid(const OSSL_CMP_CTX *ctx, const ASN1_OCTET_STRING *ckid, const ASN1_OCTET_STRING *skid)
{
    char *str;
 ...
 */
static int
cert_acceptable(const OSSL_CMP_CTX *ctx, const char *desc1, const char *desc2, X509 *cert,
                ...
                const OSSL_CMP_MSG *msg)
{
    ...
    ossl_cmp_log3(INFO, ctx, " considering %s%s %s with..", self_issued ? "self-issued " : "",
                  desc1, desc2);
 ...
 */
static int
check_cert_path_3gpp(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, X509 *scrt)
{
    int valid = 0;
    X509_STORE *store;
...
static int
check_msg_given_cert(const OSSL_CMP_CTX *ctx, X509 *cert, const OSSL_CMP_MSG *msg)
{
 ...
 */
static int
check_msg_with_certs(OSSL_CMP_CTX *ctx, const STACK_OF(X509) * certs, const char *desc,
...
check_msg_all_certs(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, int mode_3gpp)
{
    ...
    if (check_msg_with_certs(ctx, msg->extraCerts, "extraCerts", NULL, NULL, msg, mode_3gpp))
        return 1;
    if (check_msg_with_certs(ctx, ctx->untrusted, "untrusted certs", msg->extraCerts, NULL, msg,
                             mode_3gpp))
        return 1;
    ...
    default:
        ...
        if (scrt == NULL) {
            if (ctx->trusted == NULL && ctx->secretValue != NULL) {
                ossl_cmp_info(ctx, "no trust store nor pinned server cert available for verifying "
                                   "signature-based CMP message protection");
                ERR_raise(ERR_LIB_CMP, CMP_R_MISSING_TRUST_ANCHOR);
                return 0;
            }
            if (check_msg_find_cert(ctx, msg)) {
                ossl_cmp_debug(ctx, "successfully validated signature-based CMP message protection "
                                    "using trust store");
                return 1;
            }
...
static int
check_transactionID_or_nonce(ASN1_OCTET_STRING *expected, ASN1_OCTET_STRING *actual, int reason)
{
...
int
ossl_cmp_verify_popo(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, int acceptRAVerified)
{
    ...
    case OSSL_CMP_PKIBODY_KUR:
        if (!OSSL_CRMF_MSGS_verify_popo(msg->body->value.ir, OSSL_CMP_CERTREQID, acceptRAVerified,
