 ...
 */
int
ossl_cmp_msg_set0_libctx(OSSL_CMP_MSG *msg, OSSL_LIB_CTX *libctx, const char *propq)
{
    if (msg != NULL) {
        msg->libctx = libctx;
...
OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX *ctx, int for_KUR, int rid)
{
    ...
    if (!OSSL_CRMF_MSG_set_certReqId(crm, rid)
         ...
         */
        || !OSSL_CRMF_CERTTEMPLATE_fill(OSSL_CRMF_MSG_get0_tmpl(crm), rkey, subject, issuer,
                                        NULL /* serial */))
        goto err;
    if (rkey != NULL && central_keygen)
    ...
    if (!ctx->SubjectAltName_nodefault && !HAS_SAN(ctx) && refcert != NULL &&
        (default_sans = X509V3_get_d2i(X509_get0_extensions(refcert), NID_subject_alt_name, NULL,
                                       NULL)) != NULL &&
    ...
    if (for_KUR && refcert != NULL) {
        OSSL_CRMF_CERTID *cid =
            OSSL_CRMF_CERTID_gen(X509_get_issuer_name(refcert), X509_get0_serialNumber(refcert));
        int ret;
...
ossl_cmp_certreq_new(OSSL_CMP_CTX *ctx, int type, const OSSL_CRMF_MSG *crm)
{
    ...
    if (type != OSSL_CMP_PKIBODY_P10CR) {
        ...
        if (crm == NULL) {
            local_crm =
                OSSL_CMP_CTX_setup_CRM(ctx, type == OSSL_CMP_PKIBODY_KUR, OSSL_CMP_CERTREQID);
            if (local_crm == NULL ||
                !OSSL_CRMF_MSG_create_popo(ctx->popoMethod, local_crm, privkey, ctx->digest,
...
enc_privkey(OSSL_CMP_CTX *ctx, const EVP_PKEY *pkey)
{
    ...
    envData = ossl_cms_sign_encrypt(privbio, ctx->cert, ctx->chain, ctx->pkey, CMS_BINARY,
                                    encryption_recips, cipher, CMS_BINARY, ctx->libctx, ctx->propq);
...
OSSL_CMP_MSG *
ossl_cmp_certrep_new(OSSL_CMP_CTX *ctx, int bodytype, int certReqId, const OSSL_CMP_PKISI *si,
                     ...
                     STACK_OF(X509) * chain, STACK_OF(X509) * caPubs, int unprotectedErrors)
{
    ...
    if (status != OSSL_CMP_PKISTATUS_rejection && status != OSSL_CMP_PKISTATUS_waiting &&
        cert != NULL) {
...
OSSL_CMP_MSG *
ossl_cmp_rp_new(OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si, const OSSL_CRMF_CERTID *cid,
...
int
ossl_cmp_msg_gen_push1_ITAVs(OSSL_CMP_MSG *msg, const STACK_OF(OSSL_CMP_ITAV) * itavs)
{
    int i;
 ...
 */
static OSSL_CMP_MSG *
gen_new(OSSL_CMP_CTX *ctx, const STACK_OF(OSSL_CMP_ITAV) * itavs, int body_type, int err_code)
{
    OSSL_CMP_MSG *msg = NULL;
...
OSSL_CMP_MSG *
ossl_cmp_error_new(OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si, int64_t errorCode,
 ...
 */
int
ossl_cmp_certstatus_set0_certHash(OSSL_CMP_CERTSTATUS *certStatus, ASN1_OCTET_STRING *hash)
{
...
OSSL_CMP_MSG *
ossl_cmp_certConf_new(OSSL_CMP_CTX *ctx, int certReqId, int fail_info, const char *text)
{
    OSSL_CMP_MSG *msg = NULL;
...
ossl_cmp_certrepmessage_get0_certresponse(const OSSL_CMP_CERTREPMESSAGE *crm, int rid)
{
 ...
 */
X509 *
ossl_cmp_certresponse_get1_cert(const OSSL_CMP_CTX *ctx, const OSSL_CMP_CERTRESPONSE *crep)
{
    ...
    if (encr_key != NULL) {
        ...
        pkey =
            OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(encr_key, ctx->trusted, ctx->untrusted, ctx->pkey,
    ...
    if ((coec = crep->certifiedKeyPair->certOrEncCert) != NULL) {
        ...
        case OSSL_CMP_CERTORENCCERT_ENCRYPTEDCERT:
            ...
            crt = OSSL_CRMF_ENCRYPTEDKEY_get1_encCert(coec->value.encryptedCert, ctx->libctx,
                                                      ctx->propq, pkey, 0);
            break;
        default:
