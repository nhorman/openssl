...
static void
cert_msg(const char *func, const char *file, int lineno, OSSL_CMP_severity level, OSSL_CMP_CTX *ctx,
...
static int
ossl_X509_check(OSSL_CMP_CTX *ctx, const char *source, X509 *cert, int type_CA,
...
static int
ossl_X509_check_all(OSSL_CMP_CTX *ctx, const char *source, STACK_OF(X509) * certs, int type_CA,
                    const X509_VERIFY_PARAM *vpm)
{
    int i;
    int ret = 1;
              ...
              int expected, const char *desc)
{
    ...
    if (n > 1)
        ossl_cmp_log2(
            WARN, ctx,
            "response on genm contains %d ITAVs; will use the first ITAV with infoType id-it-%s", n,
            desc);
    for (i = 0; i < n; i++) {
...
static int
verify_ss_cert(OSSL_LIB_CTX *libctx, const char *propq, X509_STORE *ts, STACK_OF(X509) * untrusted,
               X509 *target)
{
                     ...
                     const char *desc)
{
    ...
    res = verify_ss_cert(OSSL_CMP_CTX_get0_libctx(ctx), OSSL_CMP_CTX_get0_propq(ctx), ts, untrusted,
                         target);
    if (!res)
...
int
OSSL_CMP_get1_rootCaKeyUpdate(OSSL_CMP_CTX *ctx, const X509 *oldWithOld, X509 **newWithNew,
...
int
OSSL_CMP_get1_crlUpdate(OSSL_CMP_CTX *ctx, const X509 *crlcert, const X509_CRL *last_crl,
                        X509_CRL **crl)
{
    ...
    *crl = NULL;
    ...
    if (sk_X509_CRL_num(crls) != 1) {
        ERR_raise_data(ERR_LIB_CMP, CMP_R_INVALID_GENP, "Unexpected number of CRLs in genp: %d",
...
int
OSSL_CMP_get1_certReqTemplate(OSSL_CMP_CTX *ctx, OSSL_CRMF_CERTTEMPLATE **certTemplate,
