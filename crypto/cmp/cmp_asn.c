...
ASN1_SEQUENCE(OSSL_CMP_REVANNCONTENT) =
    {
        ...
        IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_REVANNCONTENT)
            ...
            ASN1_SEQUENCE(
                OSSL_CMP_CHALLENGE) = {ASN1_OPT(OSSL_CMP_CHALLENGE, owf, X509_ALGOR),
                                       ASN1_SIMPLE(OSSL_CMP_CHALLENGE, witness, ASN1_OCTET_STRING),
                                       ASN1_SIMPLE(
                                           OSSL_CMP_CHALLENGE, challenge,
                ...
                IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_CHALLENGE)
                    ...
                    ASN1_ITEM_TEMPLATE(OSSL_CMP_POPODECKEYCHALLCONTENT) = ASN1_EX_TEMPLATE_TYPE(
                        ...
                        ASN1_ITEM_TEMPLATE(OSSL_CMP_POPODECKEYRESPCONTENT) = ASN1_EX_TEMPLATE_TYPE(
                            ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_POPODECKEYRESPCONTENT,
                            ASN1_INTEGER) ASN1_ITEM_TEMPLATE_END(OSSL_CMP_POPODECKEYRESPCONTENT)
                            ...
                            ASN1_SEQUENCE(OSSL_CMP_CAKEYUPDANNCONTENT) =
                                {
                                    ...
                                    /* OSSL_CMP_CMPCERTIFICATE is effectively X509 so it is used
                                       directly */
                                    ASN1_SIMPLE(
                                        OSSL_CMP_CAKEYUPDANNCONTENT, newWithNew,
                                    ...
                                    IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_CAKEYUPDANNCONTENT)
                                        ...
                                        ASN1_SEQUENCE(OSSL_CMP_ERRORMSGCONTENT) =
                                            {ASN1_SIMPLE(OSSL_CMP_ERRORMSGCONTENT, pKIStatusInfo,
                                                         OSSL_CMP_PKISI),
                                             ASN1_OPT(OSSL_CMP_ERRORMSGCONTENT, errorCode,
                                                      ASN1_INTEGER),
                                             ...
                                             ASN1_SEQUENCE_OF_OPT(
                                                ...
                                                IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_ERRORMSGCONTENT)
                                                    ...
                                                    ASN1_ADB_TEMPLATE(infotypeandvalue_default) =
                                                        ASN1_OPT(OSSL_CMP_ITAV, infoValue.other,
                                                                 ASN1_ANY);
        ...
        ASN1_SEQUENCE(OSSL_CMP_ROOTCAKEYUPDATE) =
            {
                ...
                IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_ROOTCAKEYUPDATE)
                    ...
                    ASN1_ITEM_TEMPLATE(OSSL_CMP_ATAVS) = ASN1_EX_TEMPLATE_TYPE(
                        ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_ATAVS,
                        OSSL_CRMF_ATTRIBUTETYPEANDVALUE) ASN1_ITEM_TEMPLATE_END(OSSL_CMP_ATAVS)
                                 ...
                                 ASN1_SEQUENCE_OF_OPT(
                                    ...
                                    IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_CERTREQTEMPLATE)
                                        ...
                                        ASN1_CHOICE(OSSL_CMP_CRLSOURCE) =
                                            {
                                                ASN1_EXP(OSSL_CMP_CRLSOURCE, value.dpn,
                                                         DIST_POINT_NAME, 0),
                                                ASN1_EXP(OSSL_CMP_CRLSOURCE, value.issuer,
                                                         GENERAL_NAMES, 1),
} ASN1_CHOICE_END(OSSL_CMP_CRLSOURCE) IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_CRLSOURCE)
#define OSSL_CMP_CRLSOURCE_DPN 0
#define OSSL_CMP_CRLSOURCE_ISSUER 1

                                                ASN1_SEQUENCE(OSSL_CMP_CRLSTATUS) =
                                                    {ASN1_SIMPLE(OSSL_CMP_CRLSTATUS, source,
                                                                 OSSL_CMP_CRLSOURCE),
                                                     ASN1_OPT(
                                                         OSSL_CMP_CRLSTATUS, thisUpdate,
                                                         ASN1_TIME)} ASN1_SEQUENCE_END(OSSL_CMP_CRLSTATUS)
                                                        IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_CRLSTATUS)

                                                            OSSL_CMP_ITAV
                                                    * OSSL_CMP_ITAV_create(ASN1_OBJECT * type,
                                                                           ASN1_TYPE *value)
{
    OSSL_CMP_ITAV *itav;
...
int
OSSL_CMP_ITAV_push0_stack_item(STACK_OF(OSSL_CMP_ITAV) * *itav_sk_p, OSSL_CMP_ITAV *itav)
{
    int created = 0;
...
int
OSSL_CMP_ITAV_get0_certProfile(const OSSL_CMP_ITAV *itav, STACK_OF(ASN1_UTF8STRING) * *out)
{
...
OSSL_CMP_ITAV *
OSSL_CMP_ITAV_new_rootCaKeyUpdate(const X509 *newWithNew, const X509 *newWithOld,
...
int
OSSL_CMP_ITAV_get0_rootCaKeyUpdate(const OSSL_CMP_ITAV *itav, X509 **newWithNew, X509 **newWithOld,
                                   X509 **oldWithNew)
{
...
int
OSSL_CMP_ITAV_get1_certReqTemplate(const OSSL_CMP_ITAV *itav, OSSL_CRMF_CERTTEMPLATE **certTemplate,
...
OSSL_CMP_ATAV_new_rsaKeyLen(int len)
{
    ...
    if (!ASN1_INTEGER_set(aint, len) ||
        (res = OSSL_CMP_ATAV_create(OBJ_nid2obj(NID_id_regCtrl_rsaKeyLen), (ASN1_TYPE *)aint)) ==
            NULL)
...
OSSL_CMP_ATAV_push1(OSSL_CMP_ATAVS **sk_p, const OSSL_CMP_ATAV *atav)
{
    ...
    if ((dup = OSSL_CRMF_ATTRIBUTETYPEANDVALUE_dup((OSSL_CRMF_ATTRIBUTETYPEANDVALUE *)atav)) ==
        NULL)
        goto err;
...
int
OSSL_CMP_ITAV_get0_crlStatusList(const OSSL_CMP_ITAV *itav, STACK_OF(OSSL_CMP_CRLSTATUS) * *out)
{
    if (itav == NULL || out == NULL) {
        ERR_raise(ERR_LIB_CMP, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
...
OSSL_CMP_CRLSTATUS *
OSSL_CMP_CRLSTATUS_new1(const DIST_POINT_NAME *dpn, const GENERAL_NAMES *issuer,
                        const ASN1_TIME *thisUpdate)
{
    ...
    if (dpn != NULL) {
        ...
        if ((crlsource->value.issuer =
                 sk_GENERAL_NAME_deep_copy(issuer, GENERAL_NAME_dup, GENERAL_NAME_free)) == NULL)
            goto err;
    }
...
int
OSSL_CMP_CRLSTATUS_get0(const OSSL_CMP_CRLSTATUS *crlstatus, DIST_POINT_NAME **dpn,
                        GENERAL_NAMES **issuer, ASN1_TIME **thisUpdate)
{
    OSSL_CMP_CRLSOURCE *crlsource;
...
OSSL_CMP_ITAV_new_crls(const X509_CRL *crl)
{
    ...
    if (crl != NULL) {
        if ((crls = sk_X509_CRL_new_reserve(NULL, 1)) == NULL ||
            (crl_copy = X509_CRL_dup(crl)) == NULL || !sk_X509_CRL_push(crls, crl_copy))
            goto err;
...
static int
ossl_cmp_msg_cb(int operation, ASN1_VALUE **pval, ossl_unused const ASN1_ITEM *it, void *exarg)
{
