 ...
 */
static int
unprotected_exception(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *rep, int invalid_protection,
 ...
 */
static int
send_receive_check(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req, OSSL_CMP_MSG **rep,
                   int expected_type)
{
    ...
    *rep = (*transfer_cb)(ctx, req);
    ...
    if (bt != OSSL_CMP_PKIBODY_ERROR) {
        ...
        if (emc->errorCode != NULL && BIO_snprintf(buf, sizeof(buf), "; errorCode: %08lX",
                                                   ASN1_INTEGER_get(emc->errorCode)) > 0)
            ERR_add_error_data(1, buf);
        if (emc->errorDetails != NULL) {
            char *text =
                ossl_sk_ASN1_UTF8STRING2text(emc->errorDetails, ", ", OSSL_CMP_PKISI_BUFLEN - 1);
 ...
 */
static int
poll_for_response(OSSL_CMP_CTX *ctx, int sleep, int rid, OSSL_CMP_MSG **rep, int *checkAfter)
{
    ...
    for (;;) {
        ...
        if (OSSL_CMP_MSG_get_bodytype(prep) == OSSL_CMP_PKIBODY_POLLREP) {
            ...
            if (pollRep->reason == NULL ||
                (len = BIO_snprintf(str, OSSL_CMP_PKISI_BUFLEN, " with reason = '")) < 0) {
                *str = '\0';
            } else {
                char *text =
                    ossl_sk_ASN1_UTF8STRING2text(pollRep->reason, ", ", sizeof(str) - len - 2);

                if (text == NULL || BIO_snprintf(str + len, sizeof(str) - len, "%s'", text) < 0)
                    *str = '\0';
                OPENSSL_free(text);
            }
            ossl_cmp_log2(INFO, ctx, "received polling response%s; checkAfter = %ld seconds", str,
                          check_after);
 ...
 */
static int
send_receive_also_delayed(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req, OSSL_CMP_MSG **rep,
                          int expected_type)
{
 ...
 */
int
ossl_cmp_exchange_certConf(OSSL_CMP_CTX *ctx, int certReqId, int fail_info, const char *txt)
{
...
int
ossl_cmp_exchange_error(OSSL_CMP_CTX *ctx, int status, int fail_info, const char *txt,
...
get1_cert_status(OSSL_CMP_CTX *ctx, int bodytype, OSSL_CMP_CERTRESPONSE *crep)
{
    ...
    case OSSL_CMP_PKISTATUS_revocationNotification:
        ossl_cmp_warn(
            ctx, "received \"revocationNotification\" - a revocation of the cert has occurred");
        break;
 ...
 */
int
OSSL_CMP_certConf_cb(OSSL_CMP_CTX *ctx, X509 *cert, int fail_info, const char **text)
{
    ...
    if (out_trusted != NULL) {
        ...
        ossl_cmp_warn(ctx, "could not build approximate chain for newly enrolled cert, resorting "
                           "to received extraCerts");
 ...
 */
static int
cert_response(OSSL_CMP_CTX *ctx, int sleep, int rid, OSSL_CMP_MSG **resp, int *checkAfter,
...
static int
initial_certreq(OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm, OSSL_CMP_MSG **p_rep,
                int rep_type)
{
    OSSL_CMP_MSG *req;
    int res;
...
int
OSSL_CMP_try_certreq(OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm, int *checkAfter)
{
    ...
    if (ctx->status != OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
        ...
        if (req_type < 0)
            return ossl_cmp_exchange_error(ctx, OSSL_CMP_PKISTATUS_rejection, 0, "polling aborted",
