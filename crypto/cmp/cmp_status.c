 ...
 */
static char *
snprint_PKIStatusInfo_parts(int status, int fail_info, const OSSL_CMP_PKIFREETEXT *status_strings,
...
#define ADVANCE_BUFFER                                                                             \
    ...
    if (n_status_strings > 0) {
        printed_chars =
            BIO_snprintf(write_ptr, bufsize, "; StatusString%s: ", n_status_strings > 1 ? "s" : "");
        ADVANCE_BUFFER;
        for (i = 0; i < n_status_strings; i++) {
            text = sk_ASN1_UTF8STRING_value(status_strings, i);
            printed_chars =
                BIO_snprintf(write_ptr, bufsize, "\"%.*s\"%s", ASN1_STRING_length(text),
...
char *
OSSL_CMP_snprint_PKIStatusInfo(const OSSL_CMP_PKISI *statusInfo, char *buf, size_t bufsize)
{
    int failure_info;
    ...
    return snprint_PKIStatusInfo_parts(ASN1_INTEGER_get(statusInfo->status), failure_info,
                                       statusInfo->statusString, buf, bufsize);
}

char *
OSSL_CMP_CTX_snprint_PKIStatus(const OSSL_CMP_CTX *ctx, char *buf, size_t bufsize)
{
    if (ctx == NULL) {
        ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
        return NULL;
