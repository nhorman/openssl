...
static int dh_bn_mod_exp(const DH *dh, BIGNUM *r, const BIGNUM *a, const BIGNUM *p, const BIGNUM *m,
...
static int
dh_bn_mod_exp(const DH *dh, BIGNUM *r, const BIGNUM *a, const BIGNUM *p, const BIGNUM *m,
              BN_CTX *ctx, BN_MONT_CTX *m_ctx)
{
#ifdef S390X_MOD_EXP
...
int
ossl_dh_generate_public_key(BN_CTX *ctx, const DH *dh, const BIGNUM *priv_key, BIGNUM *pub_key)
{
    int ret = 0;
    BIGNUM *prk = BN_new();
    BN_MONT_CTX *mont = NULL;
...
generate_key(DH *dh)
{
    ...
    if (generate_new_key) {
        ...
        if (DH_get_nid(dh) != NID_undef) {
            ...
            if (!ossl_ffc_generate_private_key(ctx, &dh->params, dh->length, max_strength,
                                               priv_key))
                goto err;
        } else {
#ifdef FIPS_MODULE
...
#endif
            {
                ...
                if (!ossl_ffc_params_simple_validate(dh->libctx, &dh->params, FFC_PARAM_TYPE_DH,
                                                     NULL))
                    goto err;
                /*
                 ...
                 */
                if (!ossl_ffc_generate_private_key(ctx, &dh->params, BN_num_bits(dh->params.q),
