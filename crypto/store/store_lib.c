...
static int
loader_set_params(OSSL_STORE_LOADER *loader, OSSL_STORE_LOADER_CTX *loader_ctx,
                   ...
                   OSSL_STORE_post_process_info_fn post_process, void *post_process_data)
{
    ...
    if (ui_method != NULL && (!ossl_pw_set_ui_method(&pwdata, ui_method, ui_data) ||
...
OSSL_STORE_open(const char *uri, const UI_METHOD *ui_method, void *ui_data,
                OSSL_STORE_post_process_info_fn post_process, void *post_process_data)
{
    return OSSL_STORE_open_ex(uri, NULL, NULL, ui_method, ui_data, NULL, post_process,
                              post_process_data);
}
    ...
    if (ctx->fetched_loader != NULL) {
        ...
        case OSSL_STORE_SEARCH_BY_NAME:
            if ((name_der_sz = i2d_X509_NAME(search->name, (unsigned char **)&name_der)) > 0 &&
                OSSL_PARAM_BLD_push_octet_string(bld, OSSL_STORE_PARAM_SUBJECT, name_der,
                                                 name_der_sz))
                ret = 1;
            break;
        case OSSL_STORE_SEARCH_BY_ISSUER_SERIAL:
            if ((name_der_sz = i2d_X509_NAME(search->name, (unsigned char **)&name_der)) > 0 &&
                ...
                OSSL_PARAM_BLD_push_octet_string(bld, OSSL_STORE_PARAM_ISSUER, name_der,
                                                 name_der_sz) &&
...
again:
    ...
    if (ctx->cached_info != NULL) {
        ...
        if (ctx->fetched_loader != NULL) {
            ...
            if (!ctx->fetched_loader->p_load(ctx->loader_ctx, ossl_store_handle_load_result,
                                             &load_data, ossl_pw_passphrase_callback_dec,
                                             &ctx->pwdata)) {
                ctx->error_flag = 1;
                return NULL;
            }
                  ...
                  const UI_METHOD *ui_method, void *ui_data, const OSSL_PARAM params[])
{
    ...
    if (fetched_loader != NULL && fetched_loader->p_delete != NULL) {
        ...
        res = fetched_loader->p_delete(provctx, uri, params, ossl_pw_passphrase_callback_dec,
                                       &pwdata);
    }
    /* Clear any internally cached passphrase */
    (void)ossl_pw_clear_passphrase_cache(&pwdata);
...
OSSL_STORE_SEARCH *
OSSL_STORE_SEARCH_by_key_fingerprint(const EVP_MD *digest, const unsigned char *bytes, size_t len)
{
    OSSL_STORE_SEARCH *search = OPENSSL_zalloc(sizeof(*search));
    int md_size;
    ...
    if (digest != NULL && len != (size_t)md_size) {
        ERR_raise_data(ERR_LIB_OSSL_STORE, OSSL_STORE_R_FINGERPRINT_SIZE_DOES_NOT_MATCH_DIGEST,
                       "%s size is %d, fingerprint size is %zu", EVP_MD_get0_name(digest), md_size,
                       len);
        OPENSSL_free(search);
        return NULL;
    }
...
OSSL_STORE_CTX *
OSSL_STORE_attach(BIO *bp, const char *scheme, OSSL_LIB_CTX *libctx, const char *propq,
                  const UI_METHOD *ui_method, void *ui_data, const OSSL_PARAM params[],
                  OSSL_STORE_post_process_info_fn post_process, void *post_process_data)
{
    const OSSL_STORE_LOADER *loader = NULL;
