...
static int
put_loader_in_store(void *store, void *method, const OSSL_PROVIDER *prov, const char *scheme,
...
static void *
loader_from_algorithm(int scheme_id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
    ...
    if ((loader->p_open == NULL && loader->p_attach == NULL) || loader->p_load == NULL ||
        loader->p_eof == NULL || loader->p_close == NULL) {
...
inner_loader_fetch(struct loader_data_st *methdata, const char *scheme, const char *properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = {get_tmp_loader_store,   reserve_loader_store,
    ...
    if ((id != 0 || scheme != NULL) && method == NULL) {
        ...
        ERR_raise_data(ERR_LIB_OSSL_STORE, code, "%s%s, Scheme (%s : %d), Properties (%s)",
                       helpful_msg, ossl_lib_ctx_get_descriptor(methdata->libctx),
...
OSSL_STORE_LOADER *
OSSL_STORE_LOADER_fetch(OSSL_LIB_CTX *libctx, const char *scheme, const char *properties)
{
