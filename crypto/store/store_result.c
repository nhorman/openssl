...
static int try_key(struct extracted_param_data_st *, OSSL_STORE_INFO **, OSSL_STORE_CTX *,
                   const OSSL_PROVIDER *, OSSL_LIB_CTX *, const char *);
static int try_cert(struct extracted_param_data_st *, OSSL_STORE_INFO **, OSSL_LIB_CTX *,
                    const char *);
static int try_crl(struct extracted_param_data_st *, OSSL_STORE_INFO **, OSSL_LIB_CTX *,
                   const char *);
static int try_pkcs12(struct extracted_param_data_st *, OSSL_STORE_INFO **, OSSL_STORE_CTX *,
            ...
            const OSSL_PROVIDER *provider, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    while (keymgmt != NULL && keydata == NULL && try_fallback-- > 0) {
        ...
        if (keydata == NULL && try_fallback > 0) {
            ...
            keymgmt =
                evp_keymgmt_fetch_from_prov((OSSL_PROVIDER *)provider, data->data_type, propq);
            if (keymgmt != NULL) {
                ERR_pop_to_mark();
                ERR_set_mark();
            }
...
static EVP_PKEY *
try_key_value_legacy(struct extracted_param_data_st *data, store_info_new_fn **store_info_new,
...
static int
try_key(struct extracted_param_data_st *data, OSSL_STORE_INFO **v, OSSL_STORE_CTX *ctx,
        const OSSL_PROVIDER *provider, OSSL_LIB_CTX *libctx, const char *propq)
{
...
static int
try_cert(struct extracted_param_data_st *data, OSSL_STORE_INFO **v, OSSL_LIB_CTX *libctx,
         const char *propq)
{
    if (data->object_type == OSSL_OBJECT_UNKNOWN || data->object_type == OSSL_OBJECT_CERT) {
        ...
        if (d2i_X509_AUX(&cert, (const unsigned char **)&data->octet_data, data->octet_data_size) ==
                NULL &&
            (!ignore_trusted || d2i_X509(&cert, (const unsigned char **)&data->octet_data,
...
static int
try_crl(struct extracted_param_data_st *data, OSSL_STORE_INFO **v, OSSL_LIB_CTX *libctx,
        const char *propq)
{
...
static int
try_pkcs12(struct extracted_param_data_st *data, OSSL_STORE_INFO **v, OSSL_STORE_CTX *ctx,
           OSSL_LIB_CTX *libctx, const char *propq)
{
    int ok = 1;
    ...
    if (data->object_type == OSSL_OBJECT_UNKNOWN) {
        ...
        if (p12 != NULL) {
            ...
            if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, NULL, 0)) {
                ...
                OSSL_PARAM pw_params[] = {OSSL_PARAM_utf8_string(OSSL_PASSPHRASE_PARAM_INFO,
                                                                 prompt_info,
                                                                 sizeof(prompt_info) - 1),
                                          OSSL_PARAM_END};

                if (!ossl_pw_get_passphrase(tpass, sizeof(tpass) - 1, &tpass_len, pw_params, 0,
                                            &ctx->pwdata)) {
