...
static const struct rx_test_op rx_script_1[] = {
    ...
    RX_OP_STEP_TIME(201 * OSSL_TIME_MS) RX_OP_RX(0, INIT_WINDOW_SIZE, 0)
        RX_OP_RETIRE(0, INIT_WINDOW_SIZE - 60, 50 * OSSL_TIME_MS, 0)
            RX_OP_CHECK_SWM(INIT_WINDOW_SIZE) RX_OP_CHECK_CHANGED(1, 0) RX_OP_CHECK_CHANGED(1, 1)
                RX_OP_CHECK_CHANGED(0, 0) RX_OP_CHECK_ERROR(0, 0)
    ...
    RX_OP_RX(0, INIT_WINDOW_SIZE + 1, 0) RX_OP_STEP_TIME(201 * OSSL_TIME_MS)
        RX_OP_RX(0, INIT_WINDOW_SIZE * 2, 0) RX_OP_RETIRE(0, INIT_WINDOW_SIZE, 50 * OSSL_TIME_MS, 0)
    ...
    RX_OP_RX(0, INIT_WINDOW_SIZE * 2 + 1, 0) RX_OP_STEP_TIME(199 * OSSL_TIME_MS)
        RX_OP_RX(0, INIT_WINDOW_SIZE * 3, 0) RX_OP_RETIRE(0, INIT_WINDOW_SIZE, 50 * OSSL_TIME_MS, 0)
...
static const struct rx_test_op rx_script_2[] = {
                                ...
                                RX_OP_CHECK_RWM_CONN(0) RX_OP_CHECK_RWM_STREAM(0, 0)
                                    RX_OP_CHECK_RWM_STREAM(1, 0)
                                        ...
                                        RX_OP_RX(1, 42, 0) RX_OP_RX(
                                            1, 42, 0) /* monotonic; equal or lower values ignored */
    RX_OP_RX(1, 35, 0) RX_OP_CHECK_CWM_CONN(INIT_WINDOW_SIZE)
        RX_OP_CHECK_CWM_STREAM(0, INIT_S_WINDOW_SIZE) RX_OP_CHECK_CWM_STREAM(1, INIT_S_WINDOW_SIZE)
            RX_OP_CHECK_SWM_CONN(52) RX_OP_CHECK_SWM_STREAM(0, 10) RX_OP_CHECK_SWM_STREAM(1, 42)
                RX_OP_CHECK_RWM_CONN(0) RX_OP_CHECK_RWM_STREAM(0, 0) RX_OP_CHECK_RWM_STREAM(1, 0)
    ...
    RX_OP_RETIRE(0, INIT_S_WINDOW_SIZE - 10, 50 * OSSL_TIME_MS, 0)
        RX_OP_CHECK_CWM_STREAM(0, INIT_S_WINDOW_SIZE * 2) RX_OP_CHECK_CHANGED_STREAM(0, 1, 0)
            RX_OP_CHECK_CHANGED_STREAM(0, 1, 1) RX_OP_CHECK_CHANGED_STREAM(0, 0, 0)
    ...
    RX_OP_CHECK_CWM_CONN(INIT_S_WINDOW_SIZE + INIT_WINDOW_SIZE + 42)
        RX_OP_CHECK_RWM_CONN(INIT_S_WINDOW_SIZE + 42) RX_OP_CHECK_RWM_STREAM(0, INIT_S_WINDOW_SIZE)
    ...
    RX_OP_RX(0, INIT_S_WINDOW_SIZE * 2 + 1,
             0) RX_OP_CHECK_ERROR_STREAM(0, OSSL_QUIC_ERR_FLOW_CONTROL_ERROR, 0)
        RX_OP_CHECK_ERROR_STREAM(0, OSSL_QUIC_ERR_FLOW_CONTROL_ERROR, 1)
            RX_OP_CHECK_ERROR_STREAM(0, 0, 0) RX_OP_CHECK_ERROR_CONN(0, 0) /* doesn't affect conn */
...
#define MAX_STREAMS 3
    ...
    for (; op->op != RX_OPC_END; ++op) {
        ...
        case RX_OPC_INIT_STREAM:
            ...
            if (!TEST_true(ossl_quic_rxfc_init(&stream_rxfc[op->stream_idx], &conn_rxfc, op->arg0,
        ...
        case RX_OPC_RX:
            ...
            if (!TEST_true(ossl_quic_rxfc_on_rx_stream_frame(&stream_rxfc[op->stream_idx], op->arg0,
                                                             (int)op->arg1)))
                goto err;
        ...
        case RX_OPC_RETIRE:
            ...
            if (!TEST_int_eq(ossl_quic_rxfc_on_retire(&stream_rxfc[op->stream_idx], op->arg0,
        ...
        case RX_OPC_CHECK_CHANGED_CONN:
            if (!TEST_true(conn_init_done))
                goto err;
            if (!TEST_int_eq(ossl_quic_rxfc_has_cwm_changed(&conn_rxfc, (int)op->arg1),
                             (int)op->arg0))
                goto err;
            break;
        ...
        case RX_OPC_CHECK_ERROR_STREAM:
            if (!TEST_true(op->stream_idx < OSSL_NELEM(stream_rxfc) &&
                           stream_init_done[op->stream_idx]))
                goto err;
            if (!TEST_int_eq(ossl_quic_rxfc_get_error(&stream_rxfc[op->stream_idx], (int)op->arg1),
                             (int)op->arg0))
                goto err;
            break;
