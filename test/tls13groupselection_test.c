...
static const struct tls13groupselection_test_st tls13groupselection_tests[] = {
    ...
    {"X25519:secp256r1:X448:secp521r1:-X448:-secp256r1:-X25519:-secp521r1", "", CLIENT_PREFERENCE,
     NEGOTIATION_FAILURE},
...
static void
server_response_check_cb(int write_p, int version, int content_type, const void *buf, size_t len,
                         SSL *ssl, void *arg)
{
...
static int
test_invalidsyntax(const struct tls13groupselection_test_st *current_test_vector, int ssl_or_ctx)
{
    int ok = 0;
    ...
    TEST_true_or_end(create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(),
                                         TLS1_VERSION, 0, &server_ctx, &client_ctx, cert, privkey));
...
static int
test_groupnegotiation(const struct tls13groupselection_test_st *current_test_vector, int ssl_or_ctx,
                      TEST_TYPE test_type)
{
    int ok = 0;
...
tls13groupselection_test(int i)
{
    ...
    else if (strncmp(tls13groupselection_tests[i].expected_group, NEGOTIATION_FAILURE,
