...
static int
pkey_get_bn_bytes(EVP_PKEY *pkey, const char *name, unsigned char **out, size_t *out_len)
{
...
static int
sig_gen(EVP_PKEY *pkey, OSSL_PARAM *params, const char *digest_name, const unsigned char *msg,
...
static int
ecdsa_create_pkey(EVP_PKEY **pkey, const char *curve_name, const unsigned char *pub, size_t pub_len,
                  int expected)
{
    int ret = 0;
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
        (curve_name != NULL &&
         !TEST_true(OSSL_PARAM_BLD_push_utf8_string(bld, OSSL_PKEY_PARAM_GROUP_NAME, curve_name,
                                                    0) > 0)) ||
        !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PUB_KEY, pub, pub_len) >
                   0) ||
...
static int
get_ecdsa_sig_rs_bytes(const unsigned char *sig, size_t sig_len, unsigned char **r,
                       unsigned char **s, size_t *rlen, size_t *slen)
{
    int ret = 0;
...
ecdsa_sigver_test(int id)
{
    ...
    if (!TEST_int_gt((sig_len = i2d_ECDSA_SIG(sign, &sig)), 0) ||
        ...
        !TEST_true(
            EVP_DigestVerifyInit_ex(md_ctx, NULL, tst->digest_alg, libctx, NULL, pkey, NULL)) ||
...
ecdh_cofactor_derive_test(int tstid)
{
    ...
    if (t->derive_cofactor_mode != COFACTOR_NOT_SET) {
        params[0] =
            OSSL_PARAM_construct_int(OSSL_EXCHANGE_PARAM_EC_ECDH_COFACTOR_MODE, &cofactor_mode);
        prms = params;
    }
...
static int
pkey_get_octet_bytes(EVP_PKEY *pkey, const char *name, unsigned char **out, size_t *out_len)
{
    size_t len = 0;
    unsigned char *buf = NULL;
...
static int
eddsa_create_pkey(EVP_PKEY **pkey, const char *algname, const unsigned char *pub, size_t pub_len,
                  int expected)
{
    int ret = 0;
    EVP_PKEY_CTX *ctx = NULL;
...
dsa_paramgen_test(int id)
{
    ...
    if (!dsasign_allowed) {
        ...
        if (!TEST_true(EVP_PKEY_paramgen(paramgen_ctx, &param_key)) ||
            ...
            !TEST_true(
                pkey_get_octet_bytes(param_key, OSSL_PKEY_PARAM_FFC_SEED, &seed, &seedlen)) ||
...
static int
dsa_create_pkey(EVP_PKEY **pkey, const unsigned char *p, size_t p_len, const unsigned char *q,
                size_t q_len, const unsigned char *g, size_t g_len, const unsigned char *seed,
                size_t seed_len, int counter, int validate_pq, int validate_g,
                const unsigned char *pub, size_t pub_len, BN_CTX *bn_ctx)
{
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) || !TEST_ptr(p_bn = BN_CTX_get(bn_ctx)) ||
        ...
        !TEST_ptr(q_bn = BN_CTX_get(bn_ctx)) || !TEST_ptr(BN_bin2bn(q, q_len, q_bn)) ||
...
dsa_pqver_test(int id)
{
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
        !TEST_true(dsa_create_pkey(&param_key, tst->p, tst->p_len, tst->q, tst->q_len, NULL, 0,
                                   tst->seed, tst->seed_len, tst->counter, 1, 0, NULL, 0,
                                   bn_ctx)) ||
...
static int
get_dsa_sig_rs_bytes(const unsigned char *sig, size_t sig_len, unsigned char **r, unsigned char **s,
...
dsa_siggen_test(int id)
{
    ...
    if (!dsasign_allowed) {
        ...
        if (!TEST_ptr(pkey = dsa_keygen(tst->L, tst->N)))
            goto err;
        if (!TEST_true(
                sig_gen(pkey, NULL, tst->digest_alg, tst->msg, tst->msg_len, &sig, &sig_len)) ||
...
dsa_sigver_test(int id)
{
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
        !TEST_true(dsa_create_pkey(&pkey, tst->p, tst->p_len, tst->q, tst->q_len, tst->g,
                                   tst->g_len, NULL, 0, 0, 0, 0, tst->pub, tst->pub_len, bn_ctx)))
        goto err;
...
static int
cipher_enc(const char *alg, const unsigned char *pt, size_t pt_len, const unsigned char *key,
           size_t key_len, const unsigned char *iv, size_t iv_len, const unsigned char *ct,
           size_t ct_len, int enc)
{
...
cipher_enc_dec_test(int id)
{
    ...
    return TEST_true(cipher_enc(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len, tst->iv,
                                tst->iv_len, tst->ct, tst->ct_len, enc)) &&
           TEST_true(cipher_enc(tst->alg, tst->ct, tst->ct_len, tst->key, tst->key_len, tst->iv,
                                tst->iv_len, tst->pt, tst->pt_len, !enc));
}

static int
aes_ccm_enc_dec(const char *alg, const unsigned char *pt, size_t pt_len, const unsigned char *key,
                ...
                size_t tag_len, int enc, int pass)
{
    ...
    if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
        ...
        !TEST_int_gt(
            EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, tag_len, enc ? NULL : (void *)tag),
            0) ||
    ...
    if (enc) {
        out_len += len;
        if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, tag_len, out + out_len),
                         0) ||
...
aes_ccm_enc_dec_test(int id)
{
    ...
    return aes_ccm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len, tst->iv,
                           tst->iv_len, tst->aad, tst->aad_len, tst->ct, ct_len, tag, tag_len, enc,
                           pass) &&
           aes_ccm_enc_dec(tst->alg, tst->ct, ct_len, tst->key, tst->key_len, tst->iv, tst->iv_len,
                           tst->aad, tst->aad_len, tst->pt, tst->pt_len, tag, tag_len, !enc, pass)
           /* test that it fails if the tag is incorrect */
           && aes_ccm_enc_dec(tst->alg, tst->ct, ct_len, tst->key, tst->key_len, tst->iv,
                              tst->iv_len, tst->aad, tst->aad_len, tst->pt, tst->pt_len, tag - 1,
                              tag_len, !enc, !pass);
}

static int
aes_gcm_enc_dec(const char *alg, const unsigned char *pt, size_t pt_len, const unsigned char *key,
                size_t key_len, const unsigned char *iv, size_t iv_len, const unsigned char *aad,
                size_t aad_len, const unsigned char *ct, size_t ct_len, const unsigned char *tag,
                size_t tag_len, int enc, int pass, unsigned char *out, int *out_len,
                unsigned char *outiv)
{
    int ret = 0;
    EVP_CIPHER_CTX *ctx;
...
aes_gcm_enc_dec_test(int id)
{
    ...
    return aes_gcm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len, tst->iv,
                           tst->iv_len, tst->aad, tst->aad_len, tst->ct, tst->ct_len, tst->tag,
                           tst->tag_len, enc, pass, out, NULL, NULL) &&
           aes_gcm_enc_dec(tst->alg, tst->ct, tst->ct_len, tst->key, tst->key_len, tst->iv,
                           tst->iv_len, tst->aad, tst->aad_len, tst->pt, tst->pt_len, tst->tag,
                           tst->tag_len, !enc, pass, out, NULL, NULL)
           /* Fail if incorrect tag passed to decrypt */
           && aes_gcm_enc_dec(tst->alg, tst->ct, tst->ct_len, tst->key, tst->key_len, tst->iv,
                              tst->iv_len, tst->aad, tst->aad_len, tst->pt, tst->pt_len, tst->aad,
                              tst->tag_len, !enc, !pass, out, NULL, NULL);
}
...
aes_gcm_gen_iv_internal_test(void)
{
    ...
    return aes_gcm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len, NULL,
                           tst->iv_len, tst->aad, tst->aad_len, NULL, tst->ct_len, NULL,
                           tst->tag_len, enc, pass, out, &out_len, iv) &&
           aes_gcm_enc_dec(tst->alg, out, out_len, tst->key, tst->key_len, iv, tst->iv_len,
                           tst->aad, tst->aad_len, tst->pt, tst->pt_len, out + out_len,
                           tst->tag_len, !enc, pass, out, NULL, NULL);
}

#ifndef OPENSSL_NO_DH
static int
dh_create_pkey(EVP_PKEY **pkey, const char *group_name, const unsigned char *pub, size_t pub_len,
               const unsigned char *priv, size_t priv_len, BN_CTX *bn_ctx, int pass)
{
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
        (group_name != NULL &&
         !TEST_int_gt(
             OSSL_PARAM_BLD_push_utf8_string(bld, OSSL_PKEY_PARAM_GROUP_NAME, group_name, 0), 0)))
        goto err;
...
dh_safe_prime_keygen_test(int id)
{
    ...
    params[0] =
        OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, (char *)tst->group_name, 0);
    params[1] = OSSL_PARAM_construct_end();
...
dh_safe_prime_keyver_test(int id)
{
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
        !TEST_true(dh_create_pkey(&pkey, tst->group_name, tst->pub, tst->pub_len, tst->priv,
                                  tst->priv_len, bn_ctx, 1)) ||
...
static int
rsa_create_pkey(EVP_PKEY **pkey, const unsigned char *n, size_t n_len, const unsigned char *e,
                size_t e_len, const unsigned char *d, size_t d_len, BN_CTX *bn_ctx)
{
    int ret = 0;
    EVP_PKEY_CTX *ctx = NULL;
    OSSL_PARAM_BLD *bld = NULL;
...
rsa_keygen_test(int id)
{
    ...
    if (!TEST_mem_eq(tst->p1, tst->p1_len, p1, p1_len) ||
        ...
        !TEST_mem_eq(tst->q, tst->q_len, q, q_len) || !TEST_mem_eq(tst->n, tst->n_len, n, n_len) ||
...
rsa_siggen_test(int id)
{
    ...
    *p++ =
        OSSL_PARAM_construct_utf8_string(OSSL_SIGNATURE_PARAM_DIGEST, (char *)tst->digest_alg, 0);
    if (salt_len >= 0)
...
rsa_sigver_test(int id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
        !TEST_true(
            rsa_create_pkey(&pkey, tst->n, tst->n_len, tst->e, tst->e_len, NULL, 0, bn_ctx)) ||
        !TEST_ptr(md_ctx = EVP_MD_CTX_new()) ||
        !TEST_true(EVP_DigestVerifyInit_ex(md_ctx, &pkey_ctx, tst->digest_alg, libctx, NULL, pkey,
                                           NULL)) ||
        !check_verify_message(pkey_ctx, 1) ||
        !TEST_true(EVP_PKEY_CTX_set_params(pkey_ctx, params)) ||
        !TEST_int_eq(EVP_DigestVerify(md_ctx, tst->sig, tst->sig_len, tst->msg, tst->msg_len),
                     tst->pass) ||
        !check_verify_message(pkey_ctx, 1) || !TEST_true(EVP_PKEY_verify_init(pkey_ctx)) ||
        !check_verify_message(pkey_ctx, 0))
        goto err;
...
drbg_test(int id)
{
    ...
    params[0] = OSSL_PARAM_construct_octet_string(
        OSSL_RAND_PARAM_TEST_ENTROPY, (void *)tst->entropy_input, tst->entropy_input_len);
    params[1] = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE, (void *)tst->nonce,
                                                  tst->nonce_len);
    params[2] = OSSL_PARAM_construct_end();
    if (!TEST_true(EVP_RAND_CTX_set_params(parent, params)))
        goto err;
