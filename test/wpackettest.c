...
test_WPACKET_init(void)
{
    ...
    if (!TEST_true(WPACKET_init_len(&pkt, buf, 1)) ||
        !TEST_true(WPACKET_put_bytes_u8(&pkt, 0xff)) || !TEST_true(WPACKET_finish(&pkt)) ||
        !TEST_true(WPACKET_get_total_written(&pkt, &written)) ||
...
test_WPACKET_start_sub_packet(void)
{
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf)) || !TEST_true(WPACKET_start_sub_packet_u8(&pkt)) ||
        ...
        !TEST_true(WPACKET_close(&pkt)) || !TEST_true(WPACKET_get_length(&pkt, &len)) ||
...
test_WPACKET_quic(void)
{
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf)) ||
        ...
        !TEST_true(WPACKET_close(&pkt)) || !TEST_false(WPACKET_close(&pkt)) ||
        !TEST_true(WPACKET_finish(&pkt)) || !TEST_true(WPACKET_get_total_written(&pkt, &written)) ||
