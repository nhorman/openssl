...
template_private_single_zero_test(void)
{
    ...
    /* Check BN (zero BN becomes unsigned integer) */
    if (!TEST_ptr(p = OSSL_PARAM_locate(params, "zeronumber")) ||
        !TEST_true(CRYPTO_secure_allocated(p->data)) || !TEST_str_eq(p->key, "zeronumber") ||
        !TEST_uint_eq(p->data_type, OSSL_PARAM_UNSIGNED_INTEGER) ||
        !TEST_true(OSSL_PARAM_get_BN(p, &zbn_res)) ||
        !TEST_int_eq(BN_get_flags(zbn, BN_FLG_SECURE), BN_FLG_SECURE) || !TEST_BN_eq(zbn_res, zbn))
        goto err;
    res = 1;
err:
    if (params != params_blt)
...
template_public_test(int tstid)
{
    ...
    if (!TEST_ptr(bld) || !TEST_true(OSSL_PARAM_BLD_push_long(bld, "l", 42)) ||
        ...
        !TEST_ptr(zbn = BN_new()) || !TEST_true(OSSL_PARAM_BLD_push_BN(bld, "zeronumber", zbn)) ||
    ...
    if (!TEST_ptr(p = OSSL_PARAM_locate(params, "i")) || !TEST_true(OSSL_PARAM_get_int(p, &i)) ||
        ...
        !TEST_true(OSSL_PARAM_get_int32(p, &i32)) || !TEST_str_eq(p->key, "i32") ||
        ...
        !TEST_uint_eq(p->data_type, OSSL_PARAM_INTEGER) ||
        !TEST_size_t_eq(p->data_size, sizeof(long int)) || !TEST_true(OSSL_PARAM_get_long(p, &l)) ||
        !TEST_long_eq(l, 42)
        /* Check time_t */
        || !TEST_ptr(p = OSSL_PARAM_locate(params, "t")) || !TEST_str_eq(p->key, "t") ||
        !TEST_uint_eq(p->data_type, OSSL_PARAM_INTEGER) ||
        !TEST_size_t_eq(p->data_size, sizeof(time_t)) || !TEST_true(OSSL_PARAM_get_time_t(p, &t)) ||
...
template_private_test(int tstid)
{
    ...
    if (!TEST_ptr(p = OSSL_PARAM_locate(params, "i")) ||
        !TEST_false(CRYPTO_secure_allocated(p->data)) || !TEST_true(OSSL_PARAM_get_uint(p, &i)) ||
        ...
        !TEST_false(CRYPTO_secure_allocated(p->data)) || !TEST_str_eq(p->key, "i64") ||
        !TEST_uint_eq(p->data_type, OSSL_PARAM_UNSIGNED_INTEGER) ||
        !TEST_size_t_eq(p->data_size, sizeof(int64_t)) ||
        ...
        !TEST_false(CRYPTO_secure_allocated(p->data)) || !TEST_str_eq(p->key, "l") ||
        !TEST_uint_eq(p->data_type, OSSL_PARAM_UNSIGNED_INTEGER) ||
        ...
        !TEST_false(CRYPTO_secure_allocated(p->data)) || !TEST_str_eq(p->key, "st") ||
        !TEST_uint_eq(p->data_type, OSSL_PARAM_UNSIGNED_INTEGER) ||
        ...
        !TEST_true(CRYPTO_secure_allocated(p->data)) || !TEST_str_eq(p->key, "oct_s") ||
        ...
        /* Check BN (positive BN becomes unsigned integer) */
        || !TEST_ptr(p = OSSL_PARAM_locate(params, "bignumber")) ||
        !TEST_true(CRYPTO_secure_allocated(p->data)) || !TEST_str_eq(p->key, "bignumber") ||
        !TEST_uint_eq(p->data_type, OSSL_PARAM_UNSIGNED_INTEGER) ||
        !TEST_true(OSSL_PARAM_get_BN(p, &pbn_res)) ||
        !TEST_int_eq(BN_get_flags(pbn, BN_FLG_SECURE), BN_FLG_SECURE) ||
        !TEST_BN_eq(pbn_res, pbn)
...
builder_merge_test(void)
{
    ...
    if (!TEST_ptr(bld) || !TEST_true(OSSL_PARAM_BLD_push_uint(bld, "i", 6)) ||
        ...
        !TEST_true(OSSL_PARAM_BLD_push_uint64(bld, "i64", 9999999)) ||
        !TEST_true(OSSL_PARAM_BLD_push_size_t(bld, "st", 65537)) ||
        !TEST_ptr(bn_priv = BN_secure_new()) || !TEST_true(BN_set_word(bn_priv, 1729)) ||
        !TEST_true(OSSL_PARAM_BLD_push_BN(bld, "bignumber_priv", bn_priv)) ||
        !TEST_ptr(params_blt = OSSL_PARAM_BLD_to_param(bld)))
        goto err;

    if (!TEST_ptr(bld2) ||
        !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld2, "oct_s", data1, sizeof(data1))) ||
