...
static int
fake_skeymgmt_export(void *keydata, int selection, OSSL_CALLBACK *param_callback, void *cbarg)
{
    OSSL_PARAM params[3];
    ...
    if (selection & OSSL_SKEYMGMT_SELECT_PARAMETERS) {
        *p = OSSL_PARAM_construct_utf8_string(FAKE_CIPHER_PARAM_KEY_NAME, ctx->key_name,
                                              strlen(ctx->key_name));
        p++;
    }
...
static int
fake_skey_init(PROV_CIPHER_FAKE_CTX *ctx, void *pkeyparam, const unsigned char *iv, size_t ivlen,
...
static int
fake_skey_einit(void *vctx, void *pkeyparam, const unsigned char *iv, size_t ivlen,
                const OSSL_PARAM params[])
{
...
static int
fake_skey_dinit(void *vctx, void *pkeyparam, const unsigned char *iv, size_t ivlen,
                const OSSL_PARAM params[])
{
    PROV_CIPHER_FAKE_CTX *ctx = (PROV_CIPHER_FAKE_CTX *)vctx;
...
static int
fake_cipher(void *vctx, unsigned char *out, size_t *outl, size_t outsize, const unsigned char *in,
            size_t inl)
{
    PROV_CIPHER_FAKE_CTX *ctx = (PROV_CIPHER_FAKE_CTX *)vctx;
    size_t i;
...
static const OSSL_ALGORITHM fake_cipher_algs[] = {
    {"fake_cipher", FAKE_CIPHER_FETCH_PROPS, ossl_fake_functions}, {NULL, NULL, NULL}};
...
static int
fake_cipher_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
