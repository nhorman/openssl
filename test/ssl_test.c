...
static int
check_ca_names(const char *name, STACK_OF(X509_NAME) * expected_names, STACK_OF(X509_NAME) * names)
{
    int i;
...
test_handshake(int idx)
{
    ...
    if (server2_ctx != NULL &&
        (!SSL_CTX_config(server2_ctx, "server2") || !SSL_CTX_set_dh_auto(server2_ctx, 1)))
        goto err;
    ...
    result = do_handshake(server_ctx, server2_ctx, client_ctx, resume_server_ctx, resume_client_ctx,
                          test_ctx);
