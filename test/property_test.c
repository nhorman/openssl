...
test_property_defn_cache(void)
{
    ...
    r = TEST_ptr(store = ossl_method_store_new(NULL)) && add_property_names("red", "blue", NULL) &&
        ...
        TEST_ptr(blue = ossl_parse_property(NULL, "blue")) && TEST_ptr_ne(red, blue) &&
...
static const struct {
    ...
    {"alpha", "alpha=yes", 1},         {"alpha=no", "alpha", -1},       {"alpha=1", "alpha=1", 1},
...
test_register_deregister(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(impls); i++)
        if (!TEST_true(ossl_method_store_add(store, &prov, impls[i].nid, impls[i].prop,
...
test_property(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(impls); i++)
        if (!TEST_true(ossl_method_store_add(store, *impls[i].prov, impls[i].nid, impls[i].prop,
                                             impls[i].impl, &up_ref, &down_ref))) {
            TEST_note("iteration %zd", i + 1);
            goto err;
        }
    ...
    for (i = 0; i < OSSL_NELEM(queries); i++) {
        ...
        if (!TEST_true(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop, &nullprov,
                                               &result)) ||
    ...
    for (i = 0; i < OSSL_NELEM(queries); i++) {
        ...
        if (queries[i].prov == &fake_prov1) {
            if (!TEST_true(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop,
                                                   &fake_prov1, &result)) ||
            ...
            if (!TEST_false(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop,
                                                    &fake_prov1, &result)) ||
...
test_query_cache_stochastic(void)
{
    ...
    for (i = 1; i <= max; i++) {
        ...
        if (!TEST_true(ossl_method_store_add(store, &prov, i, buf, "abc", &up_ref, &down_ref)) ||
            ...
            !TEST_true(ossl_method_store_cache_set(store, &prov, i, "n=1234", "miss", &up_ref,
                                                   &down_ref))) {
...
test_property_list_to_string(int i)
{
    ...
    if (!TEST_ptr(buf) ||
        !TEST_size_t_eq(ossl_property_list_to_string(NULL, pl, buf, bufsize), bufsize) ||
