...
#if !defined(OPENSSL_NO_DH) || !defined(OPENSSL_NO_DSA) || !defined(OPENSSL_NO_EC) ||              \
    !defined(OPENSSL_NO_ML_DSA) || !defined(OPENSSL_NO_ML_KEM) || !defined(OPENSSL_NO_SLH_DSA)
static EVP_PKEY *
...
typedef int(encoder)(const char *file, const int line, void **encoded, long *encoded_len,
                     void *object, int selection, const char *output_type,
                     const char *output_structure, const char *pass, const char *pcipher);
typedef int(decoder)(const char *file, const int line, void **object, void *encoded,
                     long encoded_len, const char *input_type, const char *structure_type,
                     const char *keytype, int selection, const char *pass);
typedef int(tester)(const char *file, const int line, const void *data1, size_t data1_len,
                    const void *data2, size_t data2_len);
typedef int(checker)(const char *file, const int line, const char *type, const void *data,
                     size_t data_len);
...
static int
test_encode_decode(const char *file, const int line, const char *type, EVP_PKEY *pkey,
                   int selection, const char *output_type, const char *output_structure,
                   const char *pass, const char *pcipher, encoder *encode_cb, decoder *decode_cb,
                   tester *test_cb, checker *check_cb, dumper *dump_cb, int flags)
{
    void *encoded = NULL;
    long encoded_len = 0;
    EVP_PKEY *pkey2 = NULL;
     ...
     * dumping purposes.
     */
    if (!TEST_true(encode_cb(file, line, &encoded, &encoded_len, pkey, selection, output_type,
                             output_structure, pass, pcipher)))
        goto end;

    if ((flags & FLAG_FAIL_IF_FIPS) != 0 && is_fips && !is_fips_3_0_0) {
        if (TEST_false(decode_cb(file, line, (void **)&pkey2, encoded, encoded_len, output_type,
    ...
    if (!TEST_true(check_cb(file, line, type, encoded, encoded_len)) ||
        !TEST_true(decode_cb(file, line, (void **)&pkey2, encoded, encoded_len, output_type,
                             output_structure, (flags & FLAG_DECODE_WITH_TYPE ? type : NULL),
                             selection, pass)) ||
        ((output_structure == NULL || strcmp(output_structure, "type-specific") != 0) &&
         !TEST_true(decode_cb(file, line, (void **)&pkey3, encoded, encoded_len, output_type,
                              output_structure, (flags & FLAG_DECODE_WITH_TYPE ? type : NULL), 0,
                              pass))) ||
        !TEST_true(encode_cb(file, line, &encoded2, &encoded2_len, pkey2, selection, output_type,
                             output_structure, pass, pcipher)))
        goto end;
...
static int
encode_EVP_PKEY_prov(const char *file, const int line, void **encoded, long *encoded_len,
                     void *object, int selection, const char *output_type,
                     const char *output_structure, const char *pass, const char *pcipher)
{
    ...
    if (!TEST_FL_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(pkey, selection, output_type,
                                                          output_structure, testpropq)) ||
        !TEST_FL_int_gt(OSSL_ENCODER_CTX_get_num_encoders(ectx), 0) ||
        (pass != NULL &&
         !TEST_FL_true(OSSL_ENCODER_CTX_set_passphrase(ectx, upass, strlen(pass)))) ||
...
static int
encode_EVP_PKEY_i2d(const char *unused_file, const int unused_line, void **encoded,
                    long *encoded_len, void *object, int unused_selection,
                    const char *unused_output_type, const char *unused_output_structure,
...
static int
decode_EVP_PKEY_prov(const char *file, const int line, void **object, void *encoded,
                     long encoded_len, const char *input_type, const char *structure_type,
                     const char *keytype, int selection, const char *pass)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (!TEST_FL_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&testpkey, testtype, structure_type,
...
static int
encode_EVP_PKEY_legacy_PEM(const char *file, const int line, void **encoded, long *encoded_len,
                           ...
                           const char *pcipher)
{
    ...
    if (!TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem())) ||
        !TEST_FL_true(PEM_write_bio_PrivateKey_traditional(mem_ser, pkey, cipher, upass, passlen,
                                                           NULL, NULL)) ||
        !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
        !TEST_FL_ptr(*encoded = mem_buf->data) ||
        !TEST_FL_long_gt(*encoded_len = mem_buf->length, 0))
...
static int
encode_EVP_PKEY_MSBLOB(const char *file, const int line, void **encoded, long *encoded_len,
                       void *object, int selection, ossl_unused const char *output_type,
...
static int
encode_EVP_PKEY_PVK(const char *file, const int line, void **encoded, long *encoded_len,
                    void *object, int selection, ossl_unused const char *output_type,
                    ossl_unused const char *output_structure, const char *pass,
                    ossl_unused const char *pcipher)
{
    ...
    if (!TEST_FL_true(ossl_assert((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)) ||
        !TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem())) ||
        !TEST_FL_int_ge(
            i2b_PVK_bio_ex(mem_ser, pkey, enc, pass_pw, (void *)pass, testctx, testpropq), 0) ||
        !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
        !TEST_FL_ptr(*encoded = mem_buf->data) ||
        !TEST_FL_long_gt(*encoded_len = mem_buf->length, 0))
        goto end;
...
static int
test_text(const char *file, const int line, const void *data1, size_t data1_len, const void *data2,
          size_t data2_len)
{
...
static int
test_mem(const char *file, const int line, const void *data1, size_t data1_len, const void *data2,
         size_t data2_len)
{
...
static int
check_unprotected_PKCS8_DER(const char *file, const int line, const char *type, const void *data,
                            size_t data_len)
{
...
test_unprotected_via_DER(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(__FILE__, __LINE__, type, key,
                              ...
                              "DER", "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
...
test_unprotected_via_i2d(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(__FILE__, __LINE__, type, key,
                              OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS,
                              "DER", "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_i2d,
                              decode_EVP_PKEY_prov, test_mem, check_unprotected_PKCS8_DER, dump_der,
                              fips ? 0 : FLAG_FAIL_IF_FIPS);
}

static int
check_unprotected_PKCS8_PEM(const char *file, const int line, const char *type, const void *data,
                            size_t data_len)
{
...
test_unprotected_via_PEM(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(__FILE__, __LINE__, type, key,
                              OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS,
                              "PEM", "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
                              decode_EVP_PKEY_prov, test_text, check_unprotected_PKCS8_PEM,
                              dump_pem, fips ? 0 : FLAG_FAIL_IF_FIPS);
}

#ifndef OPENSSL_NO_KEYPARAMS
static int
check_params_DER(const char *file, const int line, const char *type, const void *data,
                 size_t data_len)
{
    const unsigned char *datap = data;
    int ok = 0;
...
static int
check_params_PEM(const char *file, const int line, const char *type, const void *data,
                 size_t data_len)
{
    static char expected_pem_header[80];

    return TEST_FL_int_gt(BIO_snprintf(expected_pem_header, sizeof(expected_pem_header),
...
test_params_via_DER(const char *type, EVP_PKEY *key)
{
    return test_encode_decode(__FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
                              "DER", "type-specific", NULL, NULL, encode_EVP_PKEY_prov,
                              decode_EVP_PKEY_prov, test_mem, check_params_DER, dump_der,
...
static int
check_unprotected_legacy_PEM(const char *file, const int line, const char *type, const void *data,
                             size_t data_len)
{
    static char expected_pem_header[80];
...
test_unprotected_via_legacy_PEM(const char *type, EVP_PKEY *key)
{
    ...
    return test_encode_decode(__FILE__, __LINE__, type, key,
                              ...
                              decode_EVP_PKEY_prov, test_text, check_unprotected_legacy_PEM,
                              dump_pem, 0);
}

static int
check_MSBLOB(const char *file, const int line, const char *type, const void *data, size_t data_len)
{
    const unsigned char *datap = data;
...
static int
check_PVK(const char *file, const int line, const char *type, const void *data, size_t data_len)
{
...
static int
check_protected_PKCS8_DER(const char *file, const int line, const char *type, const void *data,
                          size_t data_len)
{
    const unsigned char *datap = data;
...
static int
check_protected_PKCS8_PEM(const char *file, const int line, const char *type, const void *data,
                          size_t data_len)
{
...
static int
check_protected_legacy_PEM(const char *file, const int line, const char *type, const void *data,
                           size_t data_len)
{
    static char expected_pem_header[80];
...
static int
check_public_DER(const char *file, const int line, const char *type, const void *data,
                 size_t data_len)
{
    const unsigned char *datap = data;
...
static int
check_public_PEM(const char *file, const int line, const char *type, const void *data,
                 size_t data_len)
{
...
static int
check_public_MSBLOB(const char *file, const int line, const char *type, const void *data,
                    size_t data_len)
{
    const unsigned char *datap = data;
...
static int
do_create_ec_explicit_prime_params(OSSL_PARAM_BLD *bld, const unsigned char *gen, size_t gen_len)
{
    BIGNUM *a, *b, *prime, *order;

    /* Curve prime256v1 */
    static const unsigned char prime_data[] = {0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x01,
                                               0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                               0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                               0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    static const unsigned char a_data[] = {0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x01,
                                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                           0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
                                           0xff, 0xff, 0xff, 0xff, 0xff, 0xfc};
    static const unsigned char b_data[] = {0x5a, 0xc6, 0x35, 0xd8, 0xaa, 0x3a, 0x93, 0xe7,
    ...
    static const unsigned char order_data[] = {0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00,
    ...
    return TEST_ptr(a = BN_CTX_get(bnctx)) && TEST_ptr(b = BN_CTX_get(bnctx)) &&
           TEST_ptr(prime = BN_CTX_get(bnctx)) && TEST_ptr(order = BN_CTX_get(bnctx)) &&
           ...
           TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_EC_SEED, seed,
                                                      sizeof(seed))) &&
...
# ifndef OPENSSL_NO_EC2M
static int
do_create_ec_explicit_trinomial_params(OSSL_PARAM_BLD *bld, const unsigned char *gen,
                                       size_t gen_len)
{
    ...
    return TEST_ptr(a = BN_CTX_get(bnctx)) && TEST_ptr(b = BN_CTX_get(bnctx)) &&
           TEST_ptr(poly = BN_CTX_get(bnctx)) && TEST_ptr(order = BN_CTX_get(bnctx)) &&
...
ec_encode_to_data_multi(void)
{
    ...
    ret =
        TEST_ptr(key = EVP_PKEY_Q_keygen(testctx, "", "EC", "P-256")) &&
...
setup_tests(void)
{
    ...
    OSSL_PARAM EC_params[] = {OSSL_PARAM_utf8_string("group", groupname, sizeof(groupname) - 1),
                              OSSL_PARAM_END};
#endif
