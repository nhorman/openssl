...
# endif
    ...
    p = pt(HMAC(EVP_md5(), test[idx].key, test[idx].key_len, test[idx].data, test[idx].data_len,
                NULL, NULL),
           MD5_DIGEST_LENGTH);
...
test_hmac_copy(void)
{
    ...
    if (!TEST_true(HMAC_Init_ex(ctx, test[7].key, test[7].key_len, EVP_sha1(), NULL)) ||
        ...
        !TEST_true(HMAC_CTX_copy(ctx2, ctx)) || !TEST_true(HMAC_Final(ctx2, buf, &len)))
        goto err;
...
test_hmac_copy_uninited(void)
{
    ...
    if (!TEST_ptr(ctx = EVP_MD_CTX_new()) ||
        ...
        !TEST_ptr(ctx_tmp = EVP_MD_CTX_new()) || !TEST_true(EVP_MD_CTX_copy(ctx_tmp, ctx)))
        goto err;
    EVP_MD_CTX_free(ctx);
    ctx = ctx_tmp;
    ctx_tmp = NULL;
