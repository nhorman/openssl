...
execute_errormsg_create_test(CMP_MSG_TEST_FIXTURE *fixture)
{
    EXECUTE_MSG_CREATION_TEST(
        ossl_cmp_error_new(fixture->cmp_ctx, fixture->si, fixture->err_code, "details", 0));
}
...
{
    EXECUTE_MSG_CREATION_TEST(
        ossl_cmp_certConf_new(fixture->cmp_ctx, OSSL_CMP_CERTREQID, fixture->fail_info, NULL));
}
...
test_cmp_create_p10cr(void)
{
    ...
    if (!TEST_ptr(p10cr = load_csr_der(pkcs10_f, libctx)) ||
        !TEST_true(set1_newPkey(ctx, newkey)) || !TEST_true(OSSL_CMP_CTX_set1_p10CSR(ctx, p10cr))) {
        tear_down(fixture);
        fixture = NULL;
    }
    X509_REQ_free(p10cr);
...
execute_rp_create(CMP_MSG_TEST_FIXTURE *fixture)
{
    ...
    if (!X509_NAME_add_entry_by_txt(issuer, "CN", MBSTRING_ASC, (unsigned char *)"The Issuer", -1,
                                    -1, 0) ||
