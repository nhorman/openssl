...
static int
read_text_file(const char *dir, const char *file, char *buffer, int buffer_length)
{
    int len = -1;
    char *file_path = test_mk_file_path(dir, file);
...
static int
compare_extension_printout(X509_EXTENSION *extension, const char *expected_output)
{
    BIO *text_buffer = NULL;
    char *actual_output = NULL;
    int result = 0;
...
static int
assert_validity(CT_TEST_FIXTURE *fixture, STACK_OF(SCT) * scts, CT_POLICY_EVAL_CTX *policy_ctx)
{
...
execute_cert_test(CT_TEST_FIXTURE *fixture)
{
    ...
    if (fixture->sct_text_file != NULL) {
        sct_text_len = read_text_file(fixture->sct_dir, fixture->sct_text_file, expected_sct_text,
...
test_encode_tls_sct(void)
{
    ...
    if (!TEST_ptr(sct = SCT_new_from_base64(SCT_VERSION_V1, log_id, CT_LOG_ENTRY_TYPE_X509,
