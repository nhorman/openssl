...
/* test data */
# define NUM_TESTS 34
static unsigned char key_data[NUM_TESTS][8] = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
...
static unsigned char plain_data[NUM_TESTS][8] = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
                                                 {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
...
static unsigned char cipher_data[NUM_TESTS][8] = {{0x8C, 0xA6, 0x4D, 0xE9, 0xC1, 0xB1, 0x23, 0xA7},
...
static unsigned char cbc3_ok[32] = {
    0x3F, 0xE3, 0x01, 0xC9, 0x62, 0xAC, 0x01, 0xD0, 0x22, 0x13, 0x76, 0x3C, 0x1C, 0xBD, 0x4C, 0xDC,
    0x79, 0x96, 0x57, 0xC0, 0x64, 0xEC, 0xF5, 0xD4, 0x1C, 0x67, 0x38, 0x12, 0xCF, 0xDE, 0x96, 0x75};

static unsigned char pcbc_ok[32] = {
    0xcc, 0xd1, 0x73, 0xff, 0xab, 0x20, 0x39, 0xf4, 0x6d, 0xec, 0xb4, 0x70, 0xa0, 0xe5, 0x6b, 0x15,
    0xae, 0xa6, 0xbf, 0x61, 0xed, 0x7d, 0x9c, 0x9f, 0xf7, 0x17, 0x46, 0x3b, 0x8a, 0xb3, 0xcc, 0x88};

static unsigned char cfb_key[8] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef};
static unsigned char cfb_iv[8] = {0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef};
static unsigned char cfb_buf1[40], cfb_buf2[40], cfb_tmp[8];
static unsigned char plain[24] = {0x4e, 0x6f, 0x77, 0x20, 0x69, 0x73, 0x20, 0x74,
                                  0x68, 0x65, 0x20, 0x74, 0x69, 0x6d, 0x65, 0x20,
                                  0x66, 0x6f, 0x72, 0x20, 0x61, 0x6c, 0x6c, 0x20};

static unsigned char cfb_cipher8[24] = {0xf3, 0x1f, 0xda, 0x07, 0x01, 0x14, 0x62, 0xee,
                                        0x18, 0x7f, 0x43, 0xd8, 0x0a, 0x7c, 0xd9, 0xb5,
                                        0xb0, 0xd2, 0x90, 0xda, 0x6e, 0x5b, 0x9a, 0x87};

static unsigned char cfb_cipher16[24] = {0xF3, 0x09, 0x87, 0x87, 0x7F, 0x57, 0xF7, 0x3C,
                                         0x36, 0xB6, 0xDB, 0x70, 0xD8, 0xD5, 0x34, 0x19,
                                         0xD3, 0x86, 0xB2, 0x23, 0xB7, 0xB2, 0xAD, 0x1B};

static unsigned char cfb_cipher32[24] = {0xF3, 0x09, 0x62, 0x49, 0xA4, 0xDF, 0xA4, 0x9F,
                                         0x33, 0xDC, 0x7B, 0xAD, 0x4C, 0xC8, 0x9F, 0x64,
                                         0xE4, 0x53, 0xE5, 0xEC, 0x67, 0x20, 0xDA, 0xB6};

static unsigned char cfb_cipher48[24] = {0xF3, 0x09, 0x62, 0x49, 0xC7, 0xF4, 0x30, 0xB5,
                                         0x15, 0xEC, 0xBB, 0x85, 0x97, 0x5A, 0x13, 0x8C,
                                         0x68, 0x60, 0xE2, 0x38, 0x34, 0x3C, 0xDC, 0x1F};

static unsigned char cfb_cipher64[24] = {0xF3, 0x09, 0x62, 0x49, 0xC7, 0xF4, 0x6E, 0x51,
...
static unsigned char ofb_cipher[24] = {0xf3, 0x09, 0x62, 0x49, 0xc7, 0xf4, 0x6e, 0x51,
...
test_des_cfb64(void)
{
    ...
    DES_cfb64_encrypt(&plain[12], &cfb_buf1[12], sizeof(plain) - 12, &ks, &cfb_tmp, &n,
                      DES_ENCRYPT);
    ...
    DES_cfb64_encrypt(&cfb_buf1[17], &cfb_buf2[17], sizeof(plain) - 17, &ks, &cfb_tmp, &n,
                      DES_DECRYPT);
...
test_des_ede_cfb64(void)
{
    ...
    DES_ede3_cfb64_encrypt(&plain[12], &cfb_buf1[12], sizeof(plain) - 12, &ks, &ks, &ks, &cfb_tmp,
                           &n, DES_ENCRYPT);
    if (!TEST_mem_eq(cfb_cipher64, sizeof(plain), cfb_buf1, sizeof(plain)))
        return 0;
    memcpy(cfb_tmp, cfb_iv, sizeof(cfb_iv));
    ...
    DES_ede3_cfb64_encrypt(&cfb_buf1[17], &cfb_buf2[17], sizeof(plain) - 17, &ks, &ks, &ks,
...
test_des_quad_cksum(void)
{
    ...
    cs = DES_quad_cksum(cbc_data, (DES_cblock *)lqret, (long)strlen((char *)cbc_data), 2,
                        (DES_cblock *)cbc_iv);
...
test_des_two_key(void)
{
    ...
    if (!TEST_ptr(cipher = EVP_CIPHER_fetch(NULL, "DES-EDE-ECB", NULL)) ||
        ...
        !EVP_CIPHER_CTX_set_key_length(ctx, sizeof(key)) || !EVP_CIPHER_CTX_rand_key(ctx, key))
        goto err;
