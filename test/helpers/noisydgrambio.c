...
static size_t
bandwidth_limit(struct bw_limiter_st *limit, OSSL_TIME now, BIO_MSG *msg, size_t num_msg)
{
    size_t i;
...
static int
noisy_dgram_sendmmsg(BIO *bio, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
...
static void
get_noise(int noise_rate, int long_header, uint64_t *reinject, int *should_drop, uint16_t *flip,
          size_t *flip_offset)
{
    uint32_t type;
...
static int
noisy_dgram_recvmmsg(BIO *bio, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags,
                     size_t *msgs_processed)
{
    BIO *next = BIO_next(bio);
    ...
    for (i = 0, thismsg = msg; i < msg_cnt; i++, thismsg++, data->this_dgram++) {
        ...
        get_noise(data->noise_rate,
                  /* long header */ (((uint8_t *)thismsg->data)[0] & 0x80) != 0, &reinject,
                  &should_drop, &flip, &flip_offset);
        if (data->backoff) {
            /*
