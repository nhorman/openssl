...
static STACK_OF(PKCS12_SAFEBAG) *decode_contentinfo(STACK_OF(
                                                        PKCS7) *safes, int idx,
...
static int add_attributes(PKCS12_SAFEBAG *bag, const PKCS12_ATTR *attr)
{
    ...
    while (p_attr->oid != NULL) {
        ...
        if (attr_nid == NID_friendlyName) {
            ...
            x509_attr = X509_ATTRIBUTE_create(attr_nid, V_ASN1_OBJECT, OBJ_txt2obj(
                                                  p_attr->value,
                                                  0));
            ...
            if (!TEST_true(PKCS12_add1_attr_by_txt(bag, p_attr->oid,
                                                   MBSTRING_ASC,
    ...
    switch (PKCS12_SAFEBAG_get_nid(bag)) {
        ...
        case NID_pkcs8ShroudedKeyBag:
            if (legacy)
                ...
                p8 = PKCS12_decrypt_skey_ex(bag, enc->pass, strlen(
                                                enc->pass), test_ctx,
                                            test_propq);
            if (!TEST_ptr(p8)) {
                pb->success = 0;
