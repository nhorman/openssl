...
__owur static int
parse_enum(const test_enum *enums, size_t num_enums, int *value, const char *name)
{
    size_t i;
...
static const test_enum ssl_protocols[] = {
    {"TLSv1.3", TLS1_3_VERSION},   {"TLSv1.2", TLS1_2_VERSION}, {"TLSv1.1", TLS1_1_VERSION},
    {"TLSv1", TLS1_VERSION},       {"SSLv3", SSL3_VERSION},     {"DTLSv1", DTLS1_VERSION},
...
{
    return parse_enum(ssl_protocols, OSSL_NELEM(ssl_protocols), &test_ctx->expected_protocol,
                      value);
}
...
__owur static int
parse_client_verify_callback(SSL_TEST_CLIENT_CONF *client_conf, const char *value)
{
    int ret_value;
...
{
    int ret_value;
    if (!parse_enum(ssl_servername_callbacks, OSSL_NELEM(ssl_servername_callbacks), &ret_value,
                    value)) {
        return 0;
    }
...
{
    int ret_value;
    if (!parse_enum(ssl_ct_validation_modes, OSSL_NELEM(ssl_ct_validation_modes), &ret_value,
                    value)) {
        return 0;
    }
...
__owur static int
parse_max_fragment_len_mode(SSL_TEST_CLIENT_CONF *client_conf, const char *value)
{
    int ret_value;

    if (!parse_enum(ssl_max_fragment_len_mode, OSSL_NELEM(ssl_max_fragment_len_mode), &ret_value,
                    value)) {
        return 0;
    }
...
__owur static int
parse_expected_ca_names(STACK_OF(X509_NAME) * *pnames, const char *value, OSSL_LIB_CTX *libctx)
{
    if (value == NULL)
        return 0;
...
static int
parse_client_options(SSL_TEST_CLIENT_CONF *client, const CONF *conf, const char *client_section)
{
...
static int
parse_server_options(SSL_TEST_SERVER_CONF *server, const CONF *conf, const char *server_section)
{
    STACK_OF(CONF_VALUE) * sk_conf;
    int i;
    size_t j;
...
SSL_TEST_CTX *
SSL_TEST_CTX_create(const CONF *conf, const char *test_section, OSSL_LIB_CTX *libctx)
{
