...
test_basic(void)
{
    ...
    if (!TEST_true(qtest_create_quic_objects(NULL, cctx, NULL, cert, privkey, 0, &qtserv, &cssl,
                                             NULL, NULL)))
        goto err;
 ...
 */
static int
add_unknown_frame_cb(QTEST_FAULT *fault, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len,
                     void *cbarg)
{
 ...
 */
static int
drop_extensions_cb(QTEST_FAULT *fault, QTEST_ENCRYPTED_EXTENSIONS *ee, size_t eelen,
                   void *encextcbarg)
{
...
static int
on_packet_cipher_cb(QTEST_FAULT *fault, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len,
                    void *cbarg)
{
    if (!docorrupt || len == 0)
        return 1;
...
test_corrupted_data(int idx)
{
    ...
    if (!TEST_true(qtest_create_quic_objects(NULL, cctx, NULL, cert, privkey, QTEST_FLAG_FAKE_TIME,
    ...
    if (!TEST_true(ossl_quic_tserver_write(qtserv, sid, (unsigned char *)msg + 5, msglen - 5,
                                           &byteswritten)))
        goto err;
