...
static void *
fake_pipeline_newctx(void *provctx, char *ciphername, size_t kbits, size_t ivbits)
{
...
static int
fake_pipeline_init(void *vctx, const unsigned char *key, size_t keylen, size_t numpipes,
...
int
fake_pipeline_einit(void *vctx, const unsigned char *key, size_t keylen, size_t numpipes,
...
int
fake_pipeline_dinit(void *vctx, const unsigned char *key, size_t keylen, size_t numpipes,
                    const unsigned char **iv, size_t ivlen, const OSSL_PARAM params[])
{
...
int
fake_pipeline_update(void *vctx, size_t numpipes, unsigned char **out, size_t *outl,
                     const size_t *outsize, const unsigned char **in, const size_t *inl)
{
    ...
    for (i = 0; i < numpipes; i++) {
        inl_ = (int)inl[i];
        if (!EVP_CipherUpdate(ctx->cipher_ctxs[i], (out != NULL) ? out[i] : NULL, &ioutl, in[i],
                              inl_))
            return 0;
        outl[i] = (size_t)ioutl;
    }
...
int
fake_pipeline_final(void *vctx, size_t numpipes, unsigned char **out, size_t *outl,
...
const OSSL_PARAM *
fake_pipeline_aead_gettable_ctx_params(ossl_unused void *cctx, ossl_unused void *provctx)
{
...
static const OSSL_PARAM fake_pipeline_aead_known_settable_ctx_params[] = {
    OSSL_PARAM_octet_ptr(OSSL_CIPHER_PARAM_PIPELINE_AEAD_TAG, NULL, 0), OSSL_PARAM_END};
const OSSL_PARAM *
fake_pipeline_aead_settable_ctx_params(ossl_unused void *cctx, ossl_unused void *provctx)
{
...
static int
fake_pipeline_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
