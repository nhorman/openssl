...
static const struct puny_test {
    ...
    {/* Russian (Cyrillic) */
     {0x043F, 0x043E, 0x0447, 0x0435, 0x043C, 0x0443, 0x0436, 0x0435, 0x043E, 0x043D,
    ...
    {/* Spanish */
     {0x0050, 0x006F, 0x0072, 0x0071, 0x0075, 0x00E9, 0x006E, 0x006F, 0x0070, 0x0075,
...
test_a2ulabel_bad_decode(int tst)
{
    ...
    return TEST_int_eq(
        ossl_a2ulabel(bad_decode_tests[tst].input, out, bad_decode_tests[tst].outlen), -1);
}
...
test_a2ulabel(void)
{
    ...
    if (!TEST_int_eq(ossl_a2ulabel("xn--a.b.c", out, 6), 0) ||
        !TEST_int_eq(ossl_a2ulabel("xn--a.b.c", out, 7), 1) || !TEST_str_eq(out, "\xc2\x80.b.c"))
        return 0;
    ...
    /* Test that we dont overflow the fixed internal buffer of 512 bytes when the starting bytes are
     * copied */
    strcpy(in, "xn--");
    memset(in + 4, 'e', 513);
    memcpy(in + 517, "-3ya", 4);
