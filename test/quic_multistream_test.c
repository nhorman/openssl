...
struct helper {
    ...
    int (*qtf_packet_plain_cb)(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf,
                               size_t buf_len);
...
static int
helper_init(struct helper *h, const char *script_name, int free_order, int blocking,
            int need_injector)
{
...
static int
helper_local_set_c_stream(struct helper_local *hl, const char *stream_name, SSL *c_stream)
{
...
static int
helper_set_s_stream(struct helper *h, const char *stream_name, uint64_t s_stream_id)
{
    STREAM_INFO *info;
...
static int
helper_packet_plain_listener(QTEST_FAULT *qtf, QUIC_PKT_HDR *hdr, unsigned char *buf,
...
static int
helper_handshake_listener(QTEST_FAULT *fault, unsigned char *buf, size_t buf_len, void *arg)
{
    struct helper *h = arg;
...
static int
helper_datagram_listener(QTEST_FAULT *fault, BIO_MSG *msg, size_t stride, void *arg)
{
    struct helper *h = arg;
...
check_consistent_want(SSL *s, int ret)
{
    ...
    int ok = TEST_true((ec == SSL_ERROR_NONE && w == SSL_NOTHING) ||
...
static int
run_script_worker(struct helper *h, const struct script_op *script, const char *script_name,
                  int thread_idx)
{
    int testresult = 0;
...
#define C_SPIN_AGAIN()                                                                             \
    ...
    for (;;) {
        ...
        case OPK_S_WRITE: {
            ...
            if (!TEST_true(ossl_quic_tserver_write(ACQUIRE_S(), s_stream_id, op->arg0, op->arg1,
                                                   &bytes_written)) ||
                !TEST_size_t_eq(bytes_written, op->arg1))
                goto out;
        } break;
        ...
        case OPK_S_READ_EXPECT: {
            ...
            if (!TEST_true(ossl_quic_tserver_read(ACQUIRE_S(), s_stream_id, tmp_buf + offset,
        ...
        case OPK_C_ACCEPT_STREAM_NONE: {
            ...
            if (!TEST_ptr_null(c_stream =
                                   SSL_accept_stream(h->c_conn, SSL_ACCEPT_STREAM_NO_BLOCK))) {
                SSL_free(c_stream);
                goto out;
            }
        } break;
        ...
        case OPK_C_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (h->blocking &&
                !TEST_true(SSL_shutdown_ex(c_tgt, SSL_SHUTDOWN_FLAG_WAIT_PEER, NULL, 0)))
                goto out;
        ...
        case OPK_S_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (!TEST_uint64_t_eq(error_code, tc->error_code) ||
                !TEST_int_eq(expect_app, tc->app) || !TEST_int_eq(expect_remote, tc->remote))
                goto out;
        } break;
        ...
        case OPK_S_WRITE_FAIL: {
            ...
            if (!TEST_false(ossl_quic_tserver_write(
                    ACQUIRE_S(), s_stream_id, (const unsigned char *)"apple", 5, &bytes_written)))
                goto out;
        } break;
        ...
        case OPK_NEW_THREAD: {
            ...
            for (i = 0; i < op->arg1; ++i) {
                ...
                h->threads[i].t =
                    ossl_crypto_thread_native_start(run_script_child_thread, &h->threads[i], 1);
...
static int
run_script(const struct script_op *script, const char *script_name, int free_order, int blocking)
{
    int testresult = 0;
    struct helper h;
...
static const struct script_op script_1[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_WRITE(DEFAULT, "apple", 5)
        OP_C_CONCLUDE(DEFAULT) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_INCOMING_STREAM_POLICY(
        ...
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE(
            ...
            OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "flamingo", 8) OP_C_CONCLUDE(
                b) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1)) OP_S_READ_EXPECT(b, "flamingo", 8)
                        ...
                        OP_C_ACCEPT_STREAM_NONE()
                            ...
                            OP_S_NEW_STREAM_BIDI(
                                ...
                                OP_C_ACCEPT_STREAM_WAIT(d) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
                                    ...
                                    OP_S_NEW_STREAM_BIDI(
                                        e, S_BIDI_ID(
                                               1)) OP_S_WRITE(e, "mixture", 7) OP_S_CONCLUDE(e)

                                        OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(
                                            e, "mixture", 7) OP_C_EXPECT_FIN(e)
...
static const struct script_op script_7[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI) OP_C_WRITE(DEFAULT, "apple",
                                                                                  5)
...
static const struct script_op script_11_child[] = {
    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "foo", 3) OP_SLEEP(10) OP_C_EXPECT_FIN(a)
...
static const struct script_op script_12_child[] = {
    OP_C_NEW_STREAM_BIDI(a, ANY_ID) OP_C_WRITE(a, "foo", 3) OP_C_CONCLUDE(a) OP_C_FREE_STREAM(a)
...
static const struct script_op script_15[] = {
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_C_NEW_STREAM_BIDI_EX(a, ANY_ID, SSL_STREAM_FLAG_ADVANCE) OP_C_WRITE(a, "foo", 3)
                    ...
                    OP_S_WRITE(a, "strawberry", 10) OP_C_READ_EXPECT(DEFAULT, "strawberry", 10)
                        ...
                        OP_CHECK(check_key_update_lt, 1) OP_CHECK(trigger_key_update, 0)
                            ...
                            OP_C_WRITE(DEFAULT, "orange",
                                       6) OP_S_READ_EXPECT(a, "orange", 6) OP_S_WRITE(a, "ok", 2)
...
static int
script_21_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
...
static int
script_22_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    if (h->inject_word0 == 0)
        return 1;
...
static int
script_23_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
...
static int
script_24_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
...
static int
script_28_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
...
static int
script_32_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
            ...
            OP_C_NEW_STREAM_UNI(a, C_UNI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_S_NEW_STREAM_UNI(b, S_UNI_ID(0)) OP_SET_INJECT_WORD(
                        C_UNI_ID(0) + 1, OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED)
...
static int
script_39_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[64];
...
static int
script_41_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
...
static void
script_41_trace(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl,
                void *arg)
{
...
static const struct script_op script_41[] = {
    OP_S_SET_INJECT_PLAIN(script_41_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_CHECK(script_41_setup, 0)
            ...
            OP_C_WRITE(DEFAULT, "apple",
                       5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
...
static int
script_42_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
...
static int
script_44_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
...
static int
script_46_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
...
static int
script_52_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
...
static int
script_53_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    size_t written;
    WPACKET wpkt;
...
static int
script_58_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
...
static int
script_61_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[32];
...
static int
script_65_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
...
static int
script_66_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[64];
 ...
 */
static int
script_74_alter_version(const QUIC_PKT_HDR *hdrin, const OSSL_QTX_IOVEC *iovecin, size_t numin,
                        QUIC_PKT_HDR **hdrout, const OSSL_QTX_IOVEC **iovecout, size_t *numout,
                        void *arg)
{
...
script_74_arm_packet_mutator(struct helper *h, struct helper_local *hl)
{
    ...
    do_mutation = 1;
    if (!ossl_quic_channel_set_mutator(ch, script_74_alter_version, script_74_finish_mutation,
                                       NULL))
        return 0;
    return 1;
}
...
script_76_check(struct helper *h, struct helper_local *hl)
{
    if (!TEST_false(SSL_shutdown_ex(
            h->c_conn, SSL_SHUTDOWN_FLAG_WAIT_PEER | SSL_SHUTDOWN_FLAG_NO_BLOCK, NULL, 0)))
        return 0;
                        ...
                        OP_CHECK(trigger_late_session_ticket, 0)
                            ...
                            OP_S_WRITE(a, "Strawberry", 10) OP_C_READ_EXPECT(a, "Strawberry", 10)
                                ...
                                OP_CHECK(check_got_session_ticket, 0) OP_CHECK2(
                                    check_idle_timeout, SSL_VALUE_CLASS_FEATURE_NEGOTIATED, 30000)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_WRITE_EX2(DEFAULT, "apple", 5, SSL_WRITE_FLAG_CONCLUDE)
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a)
                OP_S_WRITE(a, "orange", 6) OP_S_CONCLUDE(a) OP_C_READ_EXPECT(DEFAULT, "orange", 6)
                    OP_C_EXPECT_FIN(DEFAULT) OP_END};

/* 80. Stateless reset detection test */
static QUIC_STATELESS_RESET_TOKEN test_reset_token = {{0xde, 0xad, 0xbe, 0xef, 0xde, 0xad, 0xbe,
 ...
 */
static int
script_80_send_stateless_reset(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    ...
    if (!TEST_int_eq(
            SSL_inject_net_dgram(h->c_conn, databuf, sizeof(databuf), NULL, h->s_net_bio_addr), 1))
        return 0;
...
static int
script_80_gen_new_conn_id(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int rc = 0;
    size_t l;
    unsigned char frame_buf[64];
...
static int
script_80_inject_pkt(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    if (h->inject_word1 == 1)
...
modify_idle_timeout(struct helper *h, struct helper_local *hl)
{
    ...
    /* Set value. */
    if (!TEST_true(SSL_set_feature_request_uint(h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT,
                                                hl->check_op->arg2)))
        return 0;
            ...
            OP_CHECK2(check_avail_streams, 0, 100) OP_CHECK2(check_avail_streams, 1, 100) OP_CHECK2(
                ...
                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0))
                    ...
                    OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
                        check_avail_streams, 1, 100) OP_CHECK2(check_avail_streams, 2,
...
static const struct script_op script_87[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0))
        OP_C_WRITE(a, "apple", 5) OP_C_CONCLUDE(a) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
            OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a) OP_S_WRITE(a, "orange", 6)
                OP_C_READ_EXPECT(a, "orange", 6) OP_S_CONCLUDE(a) OP_C_EXPECT_FIN(a) OP_SLEEP(1000)
                    OP_C_STREAM_RESET_FAIL(a, 42) OP_END};

static const struct script_op *const scripts[] = {
    script_1,  script_2,  script_3,  script_4,  script_5,  script_6,  script_7,  script_8,
    script_9,  script_10, script_11, script_12, script_13, script_14, script_15, script_16,
    script_17, script_18, script_19, script_20, script_21, script_22, script_23, script_24,
    script_25, script_26, script_27, script_28, script_29, script_30, script_31, script_32,
    script_33, script_34, script_35, script_36, script_37, script_38, script_39, script_40,
    script_41, script_42, script_43, script_44, script_45, script_46, script_47, script_48,
    script_49, script_50, script_51, script_52, script_53, script_54, script_55, script_56,
    script_57, script_58, script_59, script_60, script_61, script_62, script_63, script_64,
    script_65, script_66, script_67, script_68, script_69, script_70, script_71, script_72,
    script_73, script_74, script_75, script_76, script_77, script_78, script_79, script_80,
