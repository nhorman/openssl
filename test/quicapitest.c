    ...
    for (k = 0; k < 2; k++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            if (!TEST_true(SSL_read_ex(clientquic, buf, 1, &numbytes)) ||
                !TEST_size_t_eq(numbytes, 1) || !TEST_true(SSL_has_pending(clientquic)) ||
...
test_fin_only_blocking(void)
{
    ...
    if (!TEST_ptr(cctx) ||
        !TEST_true(qtest_create_quic_objects(libctx, cctx, sctx, cert, privkey, QTEST_FLAG_BLOCK,
...
test_version(void)
{
    ...
    if (!TEST_ptr(cctx) ||
        !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey, 0, &qtserv,
                                             &clientquic, NULL, NULL)) ||
...
test_ssl_trace(void)
{
    ...
    if (!TEST_ptr(cctx = SSL_CTX_new_ex(libctx, NULL, OSSL_QUIC_client_method())) ||
        ...
        !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey,
                                             QTEST_FLAG_FAKE_TIME, &qtserv, &clientquic, NULL,
                                             NULL)))
        goto err;
...
test_new_token(void)
{
    ...
    if (!TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey,
                                             QTEST_FLAG_FAKE_TIME, &qtserv2, &clientquic2, NULL,
                                             NULL)))
        goto err;
...
static void
dgram_cb(int write_p, int version, int content_type, const void *buf, size_t msglen, SSL *ssl,
         void *arg)
{
    if (!write_p)
        return;
...
test_non_io_retry(int idx)
{
    ...
    if (!TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey, flags, &qtserv,
                                             &clientquic, NULL, NULL)) ||
        !TEST_true(
...
test_quic_psk(void)
{
    ...
    if (!TEST_ptr(cctx)
        ...
        || !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, NULL, NULL, 0, &qtserv,
                                                &clientquic, NULL, NULL)))
        goto end;
...
test_client_auth(int idx)
{
    ...
    SSL_CTX_set_verify(
        sctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT | SSL_VERIFY_CLIENT_ONCE, NULL);
    ...
    if (!TEST_true(qtest_create_quic_objects(libctx, cctx, sctx, cert, privkey, 0, &qtserv,
                                             &clientquic, NULL, NULL)))
        goto err;
    ...
    if (!TEST_true(SSL_read_ex(clientquic, buf, sizeof(buf), &numbytes)) ||
        !TEST_size_t_eq(numbytes, msglen) || !TEST_mem_eq(buf, numbytes, msg, msglen))
        goto err;
...
test_alpn(int idx)
{
    ...
    if (!TEST_ptr(cctx) || !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey,
 ...
 */
static int
unreliable_client_read(SSL *clientquic, SSL **stream, void *buf, size_t buflen, size_t *readbytes,
                       QUIC_TSERVER *qtserv)
{
    int abortctr;
...
static int
unreliable_server_read(QUIC_TSERVER *qtserv, uint64_t sid, void *buf, size_t buflen,
                       size_t *readbytes, SSL *clientquic)
{
    int abortctr;
...
test_noisy_dgram(int idx)
{
    ...
    if (!TEST_ptr(cctx) ||
        !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey, flags, &qtserv,
    ...
    for (j = 0; j < 2; j++) {
        ...
        for (i = 0; i < 20; i++) {
             ...
             * handle that
             */
            if (!TEST_true(unreliable_client_read(clientquic, &stream[j], buf, sizeof(buf),
                                                  &readbytes, qtserv)) ||
                !TEST_mem_eq(msg, msglen, buf, readbytes))
                goto err;
        }
        ...
        for (i = 0; i < 20; i++) {
             ...
             * handle that
             */
            if (!TEST_true(unreliable_server_read(qtserv, sid, buf, sizeof(buf), &readbytes,
                                                  clientquic)) ||
                !TEST_mem_eq(msg, msglen, buf, readbytes))
                goto err;
        }
...
static const struct tparam_test tparam_tests[] = {
    ...
    TPARAM_CHECK_DUP(INITIAL_MAX_STREAMS_BIDI, "INITIAL_MAX_STREAMS_BIDI appears multiple times"),
    TPARAM_CHECK_DUP(INITIAL_MAX_STREAMS_UNI, "INITIAL_MAX_STREAMS_UNI appears multiple times"),
    TPARAM_CHECK_DUP(MAX_IDLE_TIMEOUT, "MAX_IDLE_TIMEOUT appears multiple times"),
    TPARAM_CHECK_DUP(MAX_UDP_PAYLOAD_SIZE, "MAX_UDP_PAYLOAD_SIZE appears multiple times"),
    TPARAM_CHECK_DUP(ACTIVE_CONN_ID_LIMIT, "ACTIVE_CONN_ID_LIMIT appears multiple times"),
    ...
    TPARAM_CHECK_INJECT(UNKNOWN_1, NULL, 0, NULL),
    ...
    TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_4,
                          "STATELESS_RESET_TOKEN is malformed"),
    TPARAM_CHECK_INJECT(STATELESS_RESET_TOKEN, NULL, 0, "STATELESS_RESET_TOKEN is malformed"),
    ...
    TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_4, "PREFERRED_ADDR is malformed"),
    ...
    TPARAM_CHECK_INJECT_A(ACK_DELAY_EXP, excess_ack_delay_exp, "ACK_DELAY_EXP is malformed"),
    ...
    TPARAM_CHECK_INT(INITIAL_MAX_STREAM_DATA_UNI, "INITIAL_MAX_STREAM_DATA_UNI is malformed"),
    TPARAM_CHECK_INT(ACK_DELAY_EXP, "ACK_DELAY_EXP is malformed"),
    TPARAM_CHECK_INT(MAX_ACK_DELAY, "MAX_ACK_DELAY is malformed"),
    TPARAM_CHECK_INT(INITIAL_MAX_STREAMS_BIDI, "INITIAL_MAX_STREAMS_BIDI is malformed"),
    TPARAM_CHECK_INT(INITIAL_MAX_STREAMS_UNI, "INITIAL_MAX_STREAMS_UNI is malformed"),
...
static int
tparam_handle(struct tparam_ctx *ctx, uint64_t id, unsigned char *data, size_t data_len,
              WPACKET *wpkt)
{
    ...
    case TPARAM_OP_DUP:
         ...
         */
        if (id == t->id &&
            !TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(wpkt, id, data, data_len)))
            return 0;
...
static int
tparam_on_enc_ext(QTEST_FAULT *qtf, QTEST_ENCRYPTED_EXTENSIONS *ee, size_t ee_len, void *arg)
{
    int rc = 0;
    ...
    if (!TEST_true(qtest_fault_delete_extension(qtf, TLSEXT_TYPE_quic_transport_parameters,
                                                ee->extensions, &ee->extensionslen, old_bufm)))
        goto err;
    ...
    if (ctx->t->op == TPARAM_OP_INJECT || ctx->t->op == TPARAM_OP_DROP_INJECT ||
        ...
        if (!TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(&wpkt, ctx->t->id, ctx->t->buf,
                                                                  ctx->t->buf_len)))
            goto err;

        if (ctx->t->op == TPARAM_OP_INJECT_TWICE &&
            !TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(&wpkt, ctx->t->id, ctx->t->buf,
                                                                  ctx->t->buf_len)))
            goto err;
...
test_tparam(int idx)
{
    ...
    if (!TEST_true(qtest_create_quic_objects(libctx, c_ctx, NULL, cert, privkey, 0, &s, &c_ssl,
                                             &qtf, NULL)))
        goto err;
...
test_domain_flags(void)
{
    ...
    if (!TEST_ptr(ctx = SSL_CTX_new_ex(libctx, NULL, OSSL_QUIC_client_method())) ||
        ...
        !TEST_uint64_t_ne(
            domain_flags & (SSL_DOMAIN_FLAG_SINGLE_THREAD | SSL_DOMAIN_FLAG_MULTI_THREAD), 0) ||
        ...
        !TEST_uint64_t_eq(domain_flags, SSL_DOMAIN_FLAG_SINGLE_THREAD) ||
        !TEST_true(SSL_is_domain(domain)) || !TEST_false(SSL_is_domain(other_conn)) ||
            ...
            unsigned int in_len, void *arg)
{
    ...
    if (SSL_select_next_proto((unsigned char **)out, out_len, alpn, sizeof(alpn), in, in_len) ==
