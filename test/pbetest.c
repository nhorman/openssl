...
static const unsigned char pbe_ciphertext_des_sha1[] = {
    ...
    !defined OPENSSL_NO_DES && !defined OPENSSL_NO_SHA1
static int
test_pkcs5_pbe(const EVP_CIPHER *cipher, const EVP_MD *md, const unsigned char *exp,
               const int exp_len)
{
    int ret = 0;
    EVP_CIPHER_CTX *ctx;
    ...
    if (!TEST_true(PKCS5_pbe_set0_algor(algor, EVP_CIPHER_nid(cipher), pbe_iter, pbe_salt,
                                        sizeof(pbe_salt))) ||
        !TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter,
                                      cipher, md, 1)) ||
    ...
    if (!TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password, strlen(pbe_password), algor->parameter,
                                      cipher, md, 0)) ||
