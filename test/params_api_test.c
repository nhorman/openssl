...
static const struct {
    ...
    {16,
     {0x61, 0xe8, 0x7e, 0x31, 0xe9, 0x33, 0x83, 0x3d, 0x87, 0x99, 0xc7, 0xd8, 0x5d, 0xa9, 0x8b,
      0x42}},
    {16,
     {0xee, 0x6e, 0x8b, 0xc3, 0xec, 0xcf, 0x37, 0xcc, 0x89, 0x67, 0xf2, 0x68, 0x33, 0xa0, 0x14,
      0xb0}},
};
...
test_param_uint(int n)
{
    ...
    const size_t len =
        raw_values[n].len >= sizeof(unsigned int) ? sizeof(unsigned int) : raw_values[n].len;
...
test_param_ulong(int n)
{
    ...
    const size_t len = raw_values[n].len >= sizeof(unsigned long int) ? sizeof(unsigned long int)
                                                                      : raw_values[n].len;
...
test_param_construct(int tstid)
{
    ...
    if (!TEST_ptr(cp = OSSL_PARAM_locate(p, "double")) ||
        !TEST_true(OSSL_PARAM_set_double(cp, 3.14)) || !TEST_true(OSSL_PARAM_get_double(cp, &d2)) ||
        !TEST_size_t_eq(cp->return_size, sizeof(double)) || !TEST_double_eq(d2, 3.14) ||
    ...
    if (!TEST_ptr(cp = OSSL_PARAM_locate(p, "utf8ptr")) ||
        ...
        !TEST_str_eq(bufp2, "tuvwxyz") || (tstid <= 1 && !TEST_ptr_eq(bufp2, bufp)))
        goto err;
    /* OCTET string */
...
test_param_copy_null(void)
{
    ...
    ret = TEST_ptr_null(OSSL_PARAM_dup(NULL)) && TEST_ptr(cp1 = OSSL_PARAM_merge(NULL, param)) &&
          TEST_ptr(p = OSSL_PARAM_locate(cp1, "a")) && TEST_true(OSSL_PARAM_get_int(p, &val)) &&
          TEST_int_eq(val, 1) && TEST_ptr(p = OSSL_PARAM_locate(cp1, "b")) &&
          TEST_true(OSSL_PARAM_get_int(p, &val)) && TEST_int_eq(val, 2) &&
          TEST_ptr(cp2 = OSSL_PARAM_merge(param, NULL)) &&
          TEST_ptr(p = OSSL_PARAM_locate(cp2, "a")) && TEST_true(OSSL_PARAM_get_int(p, &val)) &&
          TEST_int_eq(val, 1) && TEST_ptr(p = OSSL_PARAM_locate(cp2, "b")) &&
          TEST_true(OSSL_PARAM_get_int(p, &val)) && TEST_int_eq(val, 2) &&
