...
static int
compare_iov(const unsigned char *ref, size_t ref_len, const OSSL_QTX_IOVEC *iov, size_t iov_len)
{
...
static int
test_single_copy_read(QUIC_RSTREAM *qrs, unsigned char *buf, size_t size, size_t *readbytes,
                      int *fin)
{
...
test_rstream_simple(int idx)
{
    ...
    if (!TEST_true(ossl_quic_rstream_queue_data(rstream, NULL, 5, simple_data + 5, 10, 0)) ||
        ...
        !TEST_true(ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)) ||
        ...
        !TEST_true(ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes, &fin)) ||
        !TEST_false(fin) || !TEST_size_t_eq(readbytes, 1) || !TEST_mem_eq(buf, 1, simple_data, 1) ||
        ...
        !TEST_true(ossl_quic_rstream_queue_data(rstream, NULL, 15, simple_data + 15,
        ...
        !TEST_true(read_fn(rstream, buf, 2, &readbytes, &fin)) || !TEST_false(fin) ||
        !TEST_size_t_eq(readbytes, 2) || !TEST_mem_eq(buf, 2, simple_data, 2) ||
        ...
        !TEST_true(read_fn(rstream, buf + 14 + 5, sizeof(buf) - 14 - 5, &readbytes, &fin)) ||
