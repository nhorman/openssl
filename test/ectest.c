...
#endif
        || !TEST_true(EC_POINT_mul(group, Q, order, NULL, NULL, ctx)) ||
        !TEST_true(EC_POINT_is_at_infinity(group, Q)) || !TEST_true(EC_POINT_copy(P, G)) ||
        !TEST_true(BN_one(n1)) || !TEST_true(EC_POINT_mul(group, Q, n1, NULL, NULL, ctx)) ||
        !TEST_int_eq(0, EC_POINT_cmp(group, Q, P, ctx)) || !TEST_true(BN_sub(n1, order, n1)) ||
...
prime_field_tests(void)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_true(BN_hex2bn(&p, "17")) ||
        !TEST_true(BN_hex2bn(&a, "1")) || !TEST_true(BN_hex2bn(&b, "1")) ||
    ...
    if (!TEST_ptr(P = EC_POINT_new(group)) || !TEST_ptr(Q = EC_POINT_new(group)) ||
        ...
        !TEST_true(EC_POINT_is_at_infinity(group, P)) || !TEST_ptr(x = BN_new()) ||
        !TEST_ptr(y = BN_new()) || !TEST_ptr(z = BN_new()) || !TEST_ptr(yplusone = BN_new()) ||
    ...
    if (!TEST_true(BN_hex2bn(&z, "118"
        ...
        || !TEST_true(EC_POINT_set_affine_coordinates(group, P, x, y, ctx)) ||
        !TEST_true(EC_POINT_copy(Q, P)) || !TEST_false(EC_POINT_is_at_infinity(group, Q)) ||
    ...
    if (!TEST_true(EC_GROUP_get_order(group, z, ctx)) || !TEST_true(BN_add(y, z, BN_value_one())) ||
    ...
    if (!TEST_true(EC_POINTs_mul(group, P, NULL, 2, points, scalars, ctx)) ||
        !TEST_true(EC_POINT_is_at_infinity(group, P)) ||
        !TEST_true(BN_rand(x, BN_num_bits(y) - 1, 0, 0)) || !TEST_true(BN_add(z, x, y)))
        goto err;
    BN_set_negative(z, 1);
    scalars[0] = x;
    scalars[1] = y;
...
static struct c2_curve_test {
    ...
    {"NIST curve B-163", "0800000000000000000000000000000000000000C9", "1",
     ...
     "00D51FBC6C71A0094FA2CDD545B11C5C0C797324F1", 1, "040000000000000000000292FE77E70C12A4234C33",
     "2", 163},
...
# endif
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_ptr(x = BN_new()) || !TEST_ptr(y = BN_new()) ||
        !TEST_ptr(z = BN_new()) || !TEST_ptr(yplusone = BN_new()) ||
        !TEST_true(BN_hex2bn(&p, test->p)) || !TEST_true(BN_hex2bn(&a, test->a)) ||
...
# ifdef OPENSSL_EC_BIN_PT_COMP
    ...
    if (!TEST_false(EC_POINT_set_affine_coordinates(group, P, x, yplusone, ctx)) ||
        ...
        !TEST_int_gt(EC_POINT_is_on_curve(group, P, ctx), 0) ||
        !TEST_true(BN_hex2bn(&z, test->order)) || !TEST_true(BN_hex2bn(&cof, test->cof)) ||
...
char2_field_tests(void)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_true(BN_hex2bn(&p, "13")) ||
        !TEST_true(BN_hex2bn(&a, "3")) || !TEST_true(BN_hex2bn(&b, "1")))
        goto err;
    ...
    if (!TEST_true(EC_POINT_oct2point(group, Q, buf, 1, ctx)) ||
        !TEST_true(EC_POINT_add(group, P, P, Q, ctx)) ||
        !TEST_true(EC_POINT_is_at_infinity(group, P)) || !TEST_ptr(x = BN_new()) ||
        !TEST_ptr(y = BN_new()) || !TEST_ptr(z = BN_new()) || !TEST_ptr(cof = BN_new()) ||
...
hybrid_point_encoding_test(void)
{
    ...
    if (!TEST_true(BN_dec2bn(&x, "0")) || !TEST_true(BN_dec2bn(&y, "1")) ||
        ...
        !TEST_size_t_ne(
            0, (len = EC_POINT_point2oct(group, point, POINT_CONVERSION_HYBRID, NULL, 0, NULL))) ||
...
nistp_single_test(int idx)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_ptr(x = BN_new()) || !TEST_ptr(y = BN_new()) ||
        !TEST_ptr(m = BN_new()) || !TEST_ptr(n = BN_new()) || !TEST_ptr(order = BN_new()) ||
        !TEST_ptr(yplusone = BN_new())

        || !TEST_ptr(NISTP = EC_GROUP_new_by_curve_name(test->nid)) ||
        !TEST_true(BN_hex2bn(&p, test->p)) || !TEST_int_eq(1, BN_check_prime(p, ctx, NULL)) ||
        !TEST_true(BN_hex2bn(&a, test->a)) || !TEST_true(BN_hex2bn(&b, test->b)) ||
        !TEST_true(EC_GROUP_set_curve(NISTP, p, a, b, ctx)) || !TEST_ptr(G = EC_POINT_new(NISTP)) ||
        !TEST_ptr(P = EC_POINT_new(NISTP)) || !TEST_ptr(Q = EC_POINT_new(NISTP)) ||
        !TEST_ptr(Q_CHECK = EC_POINT_new(NISTP)) || !TEST_true(BN_hex2bn(&x, test->Qx)) ||
        ...
        !TEST_true(BN_hex2bn(&x, test->Gx)) || !TEST_true(BN_hex2bn(&y, test->Gy)) ||
    ...
    if (!TEST_true(BN_set_word(m, 32)) || !TEST_true(BN_set_word(n, 31)) ||
        !TEST_true(EC_POINT_copy(P, G)) || !TEST_true(EC_POINT_invert(NISTP, P, ctx)) ||
...
check_named_curve_test(int id)
{
    ...
    if (field_nid == NID_X9_62_characteristic_two_field) {
         ...
         */
        if (!TEST_ptr(BN_copy(other_p, BN_ucmp(BN_get0_nist_prime_192(), other_p) == 0
...
ossl_parameter_test(void)
{
    ...
    if (EC_GROUP_get0_seed(group_nmd) != NULL) {
        if (!TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_EC_SEED,
...
static const unsigned char params_cf_pass[] = {
    };
...
static const unsigned char params_cf_fail[] = {
    };
...
cardinality_test(int n)
{
    ...
    BIGNUM *g1_p = NULL, *g1_a = NULL, *g1_b = NULL, *g1_x = NULL, *g1_y = NULL, *g1_order = NULL,
           *g1_cf = NULL, *g2_cf = NULL;
...
static int
do_test_custom_explicit_fromdata(EC_GROUP *group, BN_CTX *ctx, unsigned char *gen, int gen_size)
{
    int ret = 0, i_out;
    ...
    if (!TEST_false(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_GROUP_NAME, name,
                                                   sizeof(name), &name_len)))
        goto err;

    /* The encoding should be explicit as it has no group */
    if (!TEST_true(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_ENCODING, name,
                                                  sizeof(name), &name_len)) ||
        !TEST_str_eq(name, OSSL_PKEY_EC_ENCODING_EXPLICIT))
        goto err;

    if (!TEST_true(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_FIELD_TYPE, name,
                                                  sizeof(name), &name_len)) ||
        !TEST_str_eq(name, field_name))
        goto err;

    if (!TEST_true(EVP_PKEY_get_octet_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_GENERATOR, buf,
                                                   sizeof(buf), &buf_len)) ||
    ...
    if (!TEST_true(EVP_PKEY_get_bn_param(pkeyparam, OSSL_PKEY_PARAM_EC_P, &p_out)) ||
        !TEST_BN_eq(p_out, p) ||
        !TEST_true(EVP_PKEY_get_bn_param(pkeyparam, OSSL_PKEY_PARAM_EC_A, &a_out)) ||
        !TEST_BN_eq(a_out, a) ||
    ...
    if (EC_GROUP_get0_seed(group) != NULL) {
        if (!TEST_true(EVP_PKEY_get_octet_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_SEED, buf,
                                                       sizeof(buf), &buf_len)) ||
    ...
    if (EC_GROUP_get_field_type(group) == NID_X9_62_prime_field) {
        ...
        if (!TEST_false(EVP_PKEY_get_int_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_M, &i_out)) ||
            ...
            !TEST_false(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_TYPE,
        ...
        if (!TEST_true(EVP_PKEY_get_int_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_M, &i_out)) ||
            ...
            !TEST_true(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_TYPE,
                                                      name, sizeof(name), &name_len)) ||
...
custom_generator_test(int id)
{
    ...
    if (!TEST_ptr(k = BN_CTX_get(ctx))
        ...
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx),
                        bsize) ||
        !TEST_ptr(b1 = OPENSSL_malloc(bsize)) ||
        !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, b1, bsize, ctx),
                     bsize)
        ...
        !TEST_int_eq(EC_POINT_point2oct(group, Q2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx),
                     bsize) ||
        !TEST_ptr(b2 = OPENSSL_malloc(bsize)) ||
        !TEST_int_eq(EC_POINT_point2oct(group, Q2, POINT_CONVERSION_UNCOMPRESSED, b2, bsize, ctx),
                     bsize)
...
custom_params_test(int id)
{
    ...
    if (!TEST_true(EC_GROUP_get_curve(group, p, a, b, ctx)) ||
        ...
        || !TEST_true(EC_POINT_dbl(group, G2, EC_GROUP_get0_generator(group), ctx))
        /* pull out the bytes of that */
        || !TEST_int_eq(EC_POINT_point2oct(group, G2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx),
                        bsize) ||
        !TEST_ptr(buf1 = OPENSSL_malloc(bsize)) ||
        !TEST_int_eq(EC_POINT_point2oct(group, G2, POINT_CONVERSION_UNCOMPRESSED, buf1, bsize, ctx),
                     bsize) ||
    ...
    if (/* allocate temporary points on group and altgroup */
        ...
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx),
                        bsize) ||
        !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, buf1, bsize, ctx),
                     bsize)
        /* k := k/2 */
        || !TEST_true(BN_rshift1(k, k))
        ...
        /* pull out the bytes of that */
        ||
        !TEST_int_eq(EC_POINT_point2oct(altgroup, Q2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx),
                     bsize) ||
        !TEST_ptr(buf2 = OPENSSL_malloc(bsize)) ||
        !TEST_int_eq(
    ...
    if (!TEST_true(EC_POINT_mul(group, Q1, priv1, NULL, NULL, ctx)) ||
        ...
        !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, pub1, bsize, ctx),
                     bsize))
        goto err;

    /* retrieve bytes for pub2 for later */
    if (!TEST_ptr(Q = EC_KEY_get0_public_key(eckey2)) ||
        !TEST_int_eq(EC_POINT_point2oct(altgroup, Q, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx),
                     bsize) ||
    ...
    if (!TEST_ptr(pctx2 = EVP_PKEY_CTX_new(pkey2, NULL)) ||
        ...
        !TEST_int_eq(EVP_PKEY_derive(pctx2, NULL, &t), 1) || !TEST_int_gt(bsize, t) ||
        !TEST_int_le(sslen, t) || !TEST_int_eq(EVP_PKEY_derive(pctx2, buf2, &t), 1))
        goto err;
    ...
    if (!TEST_ptr(param_bld = OSSL_PARAM_BLD_new()) ||
        !TEST_true(OSSL_PARAM_BLD_push_utf8_string(param_bld, OSSL_PKEY_PARAM_GROUP_NAME,
                                                   curve_name, 0)) ||
        !TEST_true(
    ...
    if (!TEST_ptr(pctx1 = EVP_PKEY_CTX_new(pkey1, NULL)) ||
        !TEST_int_eq(EVP_PKEY_derive_init(pctx1), 1) || !TEST_ptr(dctx = EVP_PKEY_CTX_dup(pctx1)) ||
        !TEST_int_eq(EVP_PKEY_derive_set_peer_ex(dctx, pkey2, 1), 1) ||
        !TEST_int_eq(EVP_PKEY_derive(dctx, NULL, &t), 1) || !TEST_int_gt(bsize, t) ||
        !TEST_int_le(sslen, t) ||
...
ec_d2i_publickey_test(void)
{
    ...
    if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_name(NULL, "EC", NULL)) ||
        ...
        !TEST_ptr(decoded_key = d2i_PublicKey(EVP_PKEY_EC, &decoded_key, &pk_enc, pklen)))
        goto err;
