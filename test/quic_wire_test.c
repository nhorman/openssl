...
static const OSSL_QUIC_FRAME_ACK encode_case_3_f = {(OSSL_QUIC_ACK_RANGE *)encode_case_3_ranges,
...
static const OSSL_QUIC_FRAME_CRYPTO encode_case_6_f = {0x1234, sizeof(encode_case_6_data),
                                                       encode_case_6_data};
...
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_new_token(pkt, encode_case_7_token,
...
encode_case_11_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_stream_data(pkt, &stream_id, &max_data),
                     fail < 0))
        return 0;
...
encode_case_14_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_stream_data_blocked(pkt, &stream_id, &max_data),
                     fail < 0))
        return 0;
...
encode_case_15_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_streams_blocked(pkt, &max_streams_2),
                     fail < 0 || fail >= 8))
        return 0;
...
static const OSSL_QUIC_FRAME_NEW_CONN_ID encode_case_16_f = {
    ...
    {{0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71, 0x78, 0x09, 0xbb, 0xe8, 0x50, 0x19, 0x12,
      0x9a}}};
...
static const OSSL_QUIC_FRAME_CONN_CLOSE encode_case_20_f = {
    0, 0x1234, 0x9781, (char *)encode_case_20_reason, sizeof(encode_case_20_reason)};
...
encode_case_22_enc(WPACKET *pkt)
{
    ...
    if (!TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(pkt, 0x1234, encode_case_22_data,
...
encode_case_22_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if ((fail < 0 || fail >= 8) &&
        !TEST_int_eq(ossl_quic_wire_peek_transport_param(pkt, &id), fail < 0 || fail >= 12))
        return 0;
...
#define ENCODE_CASE(n)                                                                             \
    {encode_case_##n##_enc, encode_case_##n##_expect, OSSL_NELEM(encode_case_##n##_expect),        \
     encode_case_##n##_dec},

static const struct encode_test_case encode_cases[] = {
    ENCODE_CASE(1) ENCODE_CASE(2) ENCODE_CASE(3) ENCODE_CASE(4) ENCODE_CASE(5) ENCODE_CASE(6)
        ENCODE_CASE(7) ENCODE_CASE(8) ENCODE_CASE(9) ENCODE_CASE(10) ENCODE_CASE(11) ENCODE_CASE(12)
            ENCODE_CASE(13) ENCODE_CASE(14) ENCODE_CASE(15) ENCODE_CASE(16) ENCODE_CASE(16b)
                ENCODE_CASE(17) ENCODE_CASE(18) ENCODE_CASE(19) ENCODE_CASE(20) ENCODE_CASE(21)
