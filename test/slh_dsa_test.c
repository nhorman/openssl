...
static EVP_PKEY *
slh_dsa_key_from_data(const char *alg, const unsigned char *data, size_t datalen, int public)
{
    int ret;
...
static int
slh_dsa_create_keypair(EVP_PKEY **pkey, const char *name, const uint8_t *priv, size_t priv_len,
                       const uint8_t *pub, size_t pub_len)
{
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
        !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PRIV_KEY, priv, priv_len) >
                   0) ||
...
do_gen_key(const char *alg, const uint8_t *seed, size_t seed_len)
{
    ...
    if (seed_len != 0)
        *p++ =
            OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_SLH_DSA_SEED, (char *)seed, seed_len);
    *p = OSSL_PARAM_construct_end();
...
slh_dsa_keygen_test(int tst_id)
{
    ...
    if (!TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY, priv,
                                                   sizeof(priv), &priv_len)))
        goto err;
    if (!TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PUB_KEY, pub, sizeof(pub),
                                                   &pub_len)))
        goto err;
...
slh_dsa_usage_test(void)
{
    ...
    if (!TEST_ptr(pub_bio = BIO_new(BIO_s_mem())) || !TEST_ptr(priv_bio = BIO_new(BIO_s_mem())) ||
        ...
        !TEST_true(PEM_write_bio_PrivateKey_ex(priv_bio, gkey, cipher, NULL, 0, NULL, (void *)pass,
                                               lib_ctx, NULL)))
        goto err;
...
slh_dsa_deterministic_usage_test(void)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(sig = OPENSSL_zalloc(sig_len * 2)) ||
        ...
        !TEST_int_eq(EVP_PKEY_sign(dupctx, sig + sig_len, &len, msg, msg_len), 1) ||
        !TEST_size_t_eq(sig_len, len))
        goto err;
    /* Read the public key and add to a verify ctx */
    if (!TEST_ptr(PEM_read_bio_PUBKEY_ex(pub_bio, &pub, NULL, NULL, lib_ctx, NULL)) ||
...
slh_dsa_digest_sign_verify_test(void)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_SIGNATURE_PARAM_CONTEXT_STRING, context,
                                             sizeof(context));
    *p++ = OSSL_PARAM_construct_end();
