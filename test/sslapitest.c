...
static int use_session_cb(SSL *ssl, const EVP_MD *md, const unsigned char **id, size_t *idlen,
                          SSL_SESSION **sess);
static int find_session_cb(SSL *ssl, const unsigned char *identity, size_t identity_len,
                           SSL_SESSION **sess);
...
hostname_cb(SSL *s, int *al, void *arg)
{
    ...
    if (hostname != NULL &&
        (strcmp(hostname, "goodhost") == 0 || strcmp(hostname, "altgoodhost") == 0))
...
static int
compare_hex_encoded_buffer(const char *hex_encoded, size_t hex_length, const uint8_t *raw,
                           size_t raw_length)
{
    size_t i, j;
    char hexed[3];
    ...
    for (token = strtok(buffer, " \n"); token != NULL; token = strtok(NULL, " \n")) {
        if (strcmp(token, "RSA") == 0) {
            ...
            if (!TEST_size_t_eq(strlen(token), 64))
                return 0;
            if (!TEST_false(compare_hex_encoded_buffer(token, 64, actual_client_random,
                                                       client_random_size)))
                return 0;
            ...
            if (!TEST_size_t_ne(master_key_size, 0))
                return 0;
            if (!TEST_false(compare_hex_encoded_buffer(token, strlen(token), actual_master_key,
                                                       master_key_size)))
                return 0;
            master_secret_count++;
...
test_keylog(void)
{
    ...
    if (!TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
        ...
        !TEST_false(error_writing_log) || !TEST_int_gt(client_log_buffer_index, 0) ||
    ...
    if (!TEST_true(test_keylog_output(client_log_buffer, clientssl, SSL_get_session(clientssl),
                                      &expected)))
        goto end;

    expected.rsa_key_exchange_count = 0;
    if (!TEST_true(test_keylog_output(server_log_buffer, serverssl, SSL_get_session(serverssl),
                                      &expected)))
        goto end;
...
test_keylog_no_master_key(void)
{
    ...
    if (!TEST_true(test_keylog_output(client_log_buffer, clientssl, SSL_get_session(clientssl),
                                      &expected)) ||
        !TEST_true(test_keylog_output(server_log_buffer, serverssl, SSL_get_session(serverssl),
                                      &expected)))
        goto end;
...
test_client_cert_verify_cb(void)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_VERSION, 0, &sctx, &cctx, NULL, NULL)))
        goto end;
...
test_no_ems(void)
{
    ...
    if (!create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION,
...
test_ccs_change_cipher(void)
{
    ...
    if (!TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
        ...
        !TEST_true(SSL_renegotiate(clientssl)) || !TEST_true(SSL_renegotiate_pending(clientssl)))
        goto end;
    ...
    if (!TEST_false(SSL_renegotiate_pending(clientssl)) ||
        ...
        !TEST_ptr(sesspost = SSL_get0_session(serverssl)) || !TEST_ptr_ne(sesspre, sesspost) ||
...
static int
execute_test_large_message(const SSL_METHOD *smeth, const SSL_METHOD *cmeth, int min_version,
                           int max_version, int read_ahead)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, smeth, cmeth, min_version, max_version, &sctx, &cctx,
                                       cert, privkey)))
        goto end;
...
static int
execute_test_ktls(int cis_ktls, int sis_ktls, int tls_version, const char *cipher)
{
    SSL_CTX *cctx = NULL, *sctx = NULL;
    SSL *clientssl = NULL, *serverssl = NULL;
...
# endif
     ...
     * read_ahead is set.
     */
    return execute_test_large_message(DTLS_server_method(), DTLS_client_method(), DTLS1_VERSION, 0,
                                      0);
}
#endif
...
static int
execute_cleanse_plaintext(const SSL_METHOD *smeth, const SSL_METHOD *cmeth, int min_version,
                          int max_version)
{
    size_t i;
    SSL_CTX *cctx = NULL, *sctx = NULL;
...
{
#if !defined(OPENSSL_NO_TLS1_2)
    if (!TEST_true(execute_cleanse_plaintext(TLS_server_method(), TLS_client_method(),
    ...
    if (!TEST_true(execute_cleanse_plaintext(DTLS_server_method(), DTLS_client_method(),
                                             DTLS1_VERSION, 0)))
        return 0;
#endif
    return 1;
...
test_tlsext_status_type(void)
{
    ...
    if (!create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0,
...
static int
execute_test_session(int maxprot, int use_int_cache, int use_ext_cache, long s_options)
{
...
# endif
    ...
    if (use_ext_cache &&
        (!TEST_int_eq(new_called, numnewsesstick) || !TEST_int_eq(remove_called, 0)))
        goto end;
...
# endif
    ...
    if (use_ext_cache) {
        ...
        if (!TEST_int_eq(new_called, numnewsesstick) || !TEST_int_eq(remove_called, 0) ||
...
test_extra_tickets(int idx)
{
    ...
    if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) || !TEST_size_t_eq(1, nbytes) ||
        ...
        !TEST_int_eq(idx * 2 + 4, new_called) || !TEST_int_eq(sizeof(buf), nbytes) ||
        !TEST_int_eq(c, buf[0]) || !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)))
        goto end;
    ...
    new_called = 0;
    if (!TEST_true(SSL_new_session_ticket(serverssl)) ||
        !TEST_true(SSL_new_session_ticket(serverssl)) || !TEST_true(SSL_do_handshake(serverssl)) ||
        !TEST_int_eq(2, new_called) ||
        !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
        !TEST_int_eq(4, new_called))
        goto end;
     ...
     */
    if (!TEST_true(SSL_new_session_ticket(serverssl)) ||
        !TEST_true(SSL_new_session_ticket(serverssl)) || !TEST_int_eq(0, new_called) ||
...
execute_test_ssl_bio(int pop_ssl, bio_change_t change_bio)
{
    ...
    if (!TEST_ptr(ctx = SSL_CTX_new_ex(libctx, NULL, TLS_method())) ||
        !TEST_ptr(ssl = SSL_new(ctx)) || !TEST_ptr(sslbio = BIO_new(BIO_f_ssl())) ||
...
static int
use_session_cb(SSL *ssl, const EVP_MD *md, const unsigned char **id, size_t *idlen,
               SSL_SESSION **sess)
{
...
static unsigned int
psk_client_cb(SSL *ssl, const char *hint, char *id, unsigned int max_id_len, unsigned char *psk,
...
static int
find_session_cb(SSL *ssl, const unsigned char *identity, size_t identity_len, SSL_SESSION **sess)
{
    find_session_cb_cnt++;
 ...
 */
static int
setupearly_data_test(SSL_CTX **cctx, SSL_CTX **sctx, SSL **clientssl, SSL **serverssl,
                     SSL_SESSION **sess, int idx, size_t mdsize)
{
    int artificial = (artificial_ticket_time > 0);

    if (*sctx == NULL &&
        !TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_VERSION, 0, sctx, cctx, cert, privkey)))
        return 0;
...
test_early_data_read_write(int idx)
{
    ...
    if (!TEST_true(BIO_read_ex(rbio, data, sizeof(data), &rawread)) ||
        !TEST_size_t_lt(rawread, sizeof(data)) || !TEST_size_t_gt(rawread, SSL3_RT_HEADER_LENGTH))
        goto end;
...
test_early_data_not_sent(int idx)
{
    ...
    if (!TEST_int_eq(SSL_read_early_data(serverssl, buf, sizeof(buf), &readbytes),
                     SSL_READ_EARLY_DATA_FINISH) ||
        !TEST_size_t_eq(readbytes, 0) ||
        !TEST_int_eq(SSL_get_early_data_status(serverssl), SSL_EARLY_DATA_NOT_SENT) ||
...
static int
alpn_select_cb(SSL *ssl, const unsigned char **out, unsigned char *outlen, const unsigned char *in,
...
# define BADALPN (alpnlist + GOODALPNLEN)
    ...
    if (!TEST_true(setupearly_data_test(&cctx, &sctx, &clientssl, &serverssl, &sess, 2,
                                        SHA384_DIGEST_LENGTH)))
        goto end;
...
test_early_data_tls1_2(int idx)
{
    ...
    if (!TEST_true(setupearly_data_test(&cctx, &sctx, &clientssl, &serverssl, NULL, idx,
                                        SHA384_DIGEST_LENGTH)))
        goto end;
...
static int ffdhe_kexch_groups[] = {NID_ffdhe2048, NID_ffdhe3072, NID_ffdhe4096, NID_ffdhe6144,
                                   NID_ffdhe8192};
# endif
static int
...
static int
set_ssl_groups(SSL *serverssl, SSL *clientssl, int clientmulti, int isecdhe, int idx)
{
    int kexch_alg;
    int *kexch_groups = &kexch_alg;
    int numec, numff;
...
# endif
        for (i = 0; i < OSSL_NELEM(t13_ciphers); i++) {
            ...
            if (max_ver == TLS1_3_VERSION &&
                !TEST_strn_eq(t13_cipher, negotiated_scipher, strlen(negotiated_scipher)))
                goto end;
...
test_tls13_psk(int idx)
{
    ...
    const unsigned char key[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_VERSION, 0, &sctx, &cctx, idx == 3 ? NULL : cert,
                                       idx == 3 ? NULL : privkey)))
        goto end;
...
test_tls13_no_dhe_kex(const int idx)
{
    ...
    uint64_t server_options = 0 | (server_allow_no_dhe ? SSL_OP_ALLOW_NO_DHE_KEX : 0) |
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_3_VERSION, 0, &sctx, &cctx, cert, privkey)))
        goto end;
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_3_VERSION, 0, NULL, &cctx, cert, privkey)))
        goto end;
...
static int
generate_stateless_cookie_callback(SSL *ssl, unsigned char *cookie, size_t *cookie_len)
{
    unsigned int temp;
...
static int
verify_stateless_cookie_callback(SSL *ssl, const unsigned char *cookie, size_t cookie_len)
{
...
static int
old_add_cb(SSL *s, unsigned int ext_type, const unsigned char **out, size_t *outlen, int *al,
           void *add_arg)
{
...
static void
old_free_cb(SSL *s, unsigned int ext_type, const unsigned char *out, void *add_arg)
{
    OPENSSL_free((unsigned char *)out);
}

static int
old_parse_cb(SSL *s, unsigned int ext_type, const unsigned char *in, size_t inlen, int *al,
             void *parse_arg)
{
...
static int
new_add_cb(SSL *s, unsigned int ext_type, unsigned int context, const unsigned char **out,
...
static void
new_free_cb(SSL *s, unsigned int ext_type, unsigned int context, const unsigned char *out,
            void *add_arg)
{
    OPENSSL_free((unsigned char *)out);
}
...
static int
new_parse_cb(SSL *s, unsigned int ext_type, unsigned int context, const unsigned char *in,
...
test_custom_exts(int tst)
{
    ...
    if (tst == 2 && !TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), NULL, TLS1_VERSION,
    ...
    if (tst == 0) {
        if (!TEST_true(SSL_CTX_add_client_custom_ext(cctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb,
                                                     &client, old_parse_cb, &client)))
            goto end;
    } else {
        if (!TEST_true(SSL_CTX_add_custom_ext(cctx, TEST_EXT_TYPE1, context, new_add_cb,
                                              new_free_cb, &client, new_parse_cb, &client)))
            goto end;
    }

    /* Should not be able to add duplicates */
    if (!TEST_false(SSL_CTX_add_client_custom_ext(cctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb,
                                                  &client, old_parse_cb, &client)) ||
        !TEST_false(SSL_CTX_add_custom_ext(cctx, TEST_EXT_TYPE1, context, new_add_cb, new_free_cb,
                                           &client, new_parse_cb, &client)))
        goto end;

    /* Create a server side custom extension */
    if (tst == 0) {
        if (!TEST_true(SSL_CTX_add_server_custom_ext(sctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb,
                                                     &server, old_parse_cb, &server)))
            goto end;
    } else {
        if (!TEST_true(SSL_CTX_add_custom_ext(sctx, TEST_EXT_TYPE1, context, new_add_cb,
                                              new_free_cb, &server, new_parse_cb, &server)))
            goto end;
        if (sctx2 != NULL &&
            !TEST_true(SSL_CTX_add_custom_ext(sctx2, TEST_EXT_TYPE1, context, new_add_cb,
                                              new_free_cb, &server, new_parse_cb, &server)))
            goto end;
    }

    /* Should not be able to add duplicates */
    if (!TEST_false(SSL_CTX_add_server_custom_ext(sctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb,
                                                  &server, old_parse_cb, &server)) ||
        !TEST_false(SSL_CTX_add_custom_ext(sctx, TEST_EXT_TYPE1, context, new_add_cb, new_free_cb,
...
# define TLS13CONTEXT                                                                              \
     (SSL_EXT_TLS1_3_CERTIFICATE | SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_CLIENT_HELLO)

# define SERVERINFO_CUSTOM                                                                         \
     0x00, (char)TLSEXT_TYPE_signed_certificate_timestamp, 0x00, 0x03, 0x04, 0x05, 0x06

static const unsigned char serverinfo_custom_tls13[] = {0x00, 0x00, (TLS13CONTEXT >> 8) & 0xff,
                                                        TLS13CONTEXT & 0xff, SERVERINFO_CUSTOM};
static const unsigned char serverinfo_custom_v2[] = {0x00, 0x00, (SYNTHV1CONTEXT >> 8) & 0xff,
                                                     SYNTHV1CONTEXT & 0xff, SERVERINFO_CUSTOM};
...
static int
serverinfo_custom_parse_cb(SSL *s, unsigned int ext_type, unsigned int context,
...
test_serverinfo_custom(const int idx)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_method(), TLS_method(), protocol_version,
                                       protocol_version, &sctx, &cctx, cert, privkey)))
        goto end;
...
test_export_key_mat(int tst)
{
     ...
     */
    if (!TEST_int_le(SSL_export_keying_material(clientssl, ckeymat1, sizeof(ckeymat1), label,
                                                SMALL_LABEL_LEN + 1, context, sizeof(context) - 1,
                                                1),
                     0))
        goto end;
    ...
    if (!TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat1, sizeof(ckeymat1), label,
                                                labellen, context, sizeof(context) - 1, 1),
                     1) ||
        !TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat2, sizeof(ckeymat2), label,
                                                labellen, emptycontext, 0, 1),
                     1) ||
        !TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat3, sizeof(ckeymat3), label,
                                                labellen, NULL, 0, 0),
                     1) ||
        (test_longcontext &&
         !TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat4, sizeof(ckeymat4), label,
                                                 labellen, longcontext, sizeof(longcontext), 1),
                      1)) ||
        !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat1, sizeof(skeymat1), label,
                                                labellen, context, sizeof(context) - 1, 1),
                     1) ||
        !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat2, sizeof(skeymat2), label,
                                                labellen, emptycontext, 0, 1),
                     1) ||
        !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat3, sizeof(skeymat3), label,
                                                labellen, NULL, 0, 0),
                     1) ||
        (test_longcontext &&
         !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat4, sizeof(skeymat4), label,
                                                 labellen, longcontext, sizeof(longcontext), 1),
                      1))
        /*
...
test_export_key_mat_early(int idx)
{
    ...
    if (!TEST_int_eq(SSL_export_keying_material_early(clientssl, ckeymat1, sizeof(ckeymat1), label,
...
test_key_update(void)
{
    ...
    for (j = 0; j < 2; j++) {
        ...
        for (i = 0; i < NUM_KEY_UPDATE_MESSAGES; i++) {
            if (!TEST_true(SSL_key_update(clientssl, (j == 0) ? SSL_KEY_UPDATE_NOT_REQUESTED
...
test_key_update_peer_in_write(int tst)
{
    ...
    if (!TEST_int_eq(SSL_write(peerwrite, mess, strlen(mess)), -1) ||
        ...
        !TEST_true(SSL_want_write(peerwrite)) || !TEST_true(SSL_net_write_desired(peerwrite)))
        goto end;
    ...
    if (!TEST_int_eq(SSL_read(peerwrite, buf, sizeof(buf)), -1) ||
        ...
        !TEST_true(SSL_want_read(peerwrite)) || !TEST_true(SSL_net_read_desired(peerwrite)))
        goto end;
...
test_max_fragment_len_ext(int idx_tst)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, NULL, TLS_client_method(), TLS1_VERSION, 0, NULL,
                                       &ctx, NULL, NULL)))
        return 0;
...
create_new_vfile(char *userid, char *password, const char *filename)
{
    ...
    gNid = SRP_create_verifier_ex(userid, password, &row[DB_srpsalt], &row[DB_srpverifier], NULL,
                                  NULL, libctx, NULL);
    if (!TEST_ptr(gNid))
        goto end;
...
create_new_vbase(char *userid, char *password)
{
    ...
    if (!TEST_true(SRP_create_verifier_BN_ex(userid, password, &salt, &verifier, lgN->N, lgN->g,
                                             libctx, NULL)))
        goto end;
...
/* We need either ECDHE or DHE for the TLSv1.2 test to work */
    ...
    if (tst >= 4 && tst < 6) {
        ...
        /* early_data tests */
        if (!TEST_true(setupearly_data_test(&cctx, &sctx, &clientssl, &serverssl, &sess, 0,
                                            SHA384_DIGEST_LENGTH)))
            goto end;
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), tlsvers,
...
#ifndef OPENSSL_NO_DTLS
        if (!TEST_true(create_ssl_ctx_pair(libctx, DTLS_server_method(), DTLS_client_method(),
 ...
 */
#if defined(OSSL_NO_USABLE_TLS1_3) || !defined(OPENSSL_NO_TLS1_2)
    {TLS1_2_VERSION, "AES128-SHA:AES256-SHA", NULL, "AES256-SHA:DHE-RSA-AES128-SHA", NULL,
...
# if !defined(OPENSSL_NO_CHACHA) && !defined(OPENSSL_NO_POLY1305) && !defined(OPENSSL_NO_EC)
    {TLS1_2_VERSION, "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305", NULL,
     "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305", NULL, "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305",
     "AES128-SHA"},
# endif
    ...
    {TLS1_2_VERSION, "AES128-SHA:AES256-SHA", NULL, "AES128-SHA:DHE-RSA-AES128-SHA", NULL,
...
int_test_ssl_get_shared_ciphers(int tst, int clnt)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_VERSION, shared_ciphers_data[tst].maxprot, &sctx, &cctx,
                                       cert, privkey)))
        goto end;
...
dec_tick_cb(SSL *s, SSL_SESSION *ss, const unsigned char *keyname, size_t keyname_length,
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static int
tick_key_cb(SSL *s, unsigned char key_name[16], unsigned char iv[EVP_MAX_IV_LENGTH],
...
static int
tick_key_evp_cb(SSL *s, unsigned char key_name[16], unsigned char iv[EVP_MAX_IV_LENGTH],
...
test_ticket_callbacks(int tst)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(
            ...
            ((tst % 2) == 0) ? TLS1_2_VERSION : TLS1_3_VERSION, &sctx, &cctx, cert, privkey)))
        goto end;
...
test_incorrect_shutdown(int tst)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), 0, 0,
                                       &sctx, &cctx, cert, privkey)))
        goto end;
...
static int
load_chain(const char *file, EVP_PKEY **pkey, X509 **x509, STACK_OF(X509) * chain)
{
...
# endif
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), prot, prot,
                                       &sctx, &cctx, NULL, NULL)))
        goto end;
...
test_ca_names_int(int prot, int tst)
{
    ...
    for (i = 0; i < OSSL_NELEM(name); i++) {
        ...
        if (!TEST_ptr(name[i]) ||
            !TEST_true(X509_NAME_add_entry_by_txt(name[i], "CN", MBSTRING_ASC,
                                                  (unsigned char *)strnames[i], -1, -1, 0)))
            goto end;
    }

    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_VERSION, prot, &sctx, &cctx, cert, privkey)))
        goto end;
...
#define TIMEOUT 10
    ...
    if (!TEST_ptr(ctx = SSL_CTX_new_ex(libctx, NULL, TLS_method())) ||
        !TEST_ptr(early = SSL_SESSION_new()) || !TEST_ptr(middle = SSL_SESSION_new()) ||
...
test_unknown_sigalgs_groups(void)
{
    ...
    if (!TEST_int_gt(SSL_CTX_set1_client_sigalgs_list(ctx, "RSA+SHA256:?nonexistent:?RSA+SHA512"),
                     0))
        goto end;
...
test_pluggable_group(int idx)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_3_VERSION, TLS1_3_VERSION, &sctx, &cctx, cert,
                                       privkey)) ||
        !TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)))
        goto end;
    ...
    if (!TEST_true(SSL_set1_groups_list(
            serverssl, "xorgroup:xorkemgroup:dummy1:dummy2:dummy3:dummy4:dummy5:dummy6:dummy7:"
                       ...
                       "dummy35:dummy36:dummy37:dummy38:dummy39:dummy40:dummy41:dummy42:dummy43"))
...
{
    EVP_PKEY_CTX *evpctx =
        EVP_PKEY_CTX_new_from_name(libctx, (idx == 0) ? "xorhmacsig" : "xorhmacsha2sig", NULL);
    EVP_PKEY *pkey = NULL;
    X509 *x509 = X509_new();
    X509_NAME *name = NULL;
    ...
    if (!TEST_ptr(evpctx) || !TEST_int_gt(EVP_PKEY_keygen_init(evpctx), 0) ||
        ...
        !TEST_true(X509_set_pubkey(x509, pkey)) || !TEST_ptr(name = X509_get_subject_name(x509)) ||
        !TEST_true(X509_NAME_add_entry_by_txt(name, "C", MBSTRING_ASC, (unsigned char *)"CH", -1,
                                              -1, 0)) ||
        !TEST_true(X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC, (unsigned char *)"test.org",
                                              -1, -1, 0)) ||
...
test_pluggable_signature(int idx)
{
    ...
    if (do_conf_cmd) {
        ...
        SSL_CONF_CTX_set_flags(
            confctx, SSL_CONF_FLAG_FILE | SSL_CONF_FLAG_SERVER | SSL_CONF_FLAG_CERTIFICATE |
...
static int
secret_cb(SSL *s, void *secretin, int *secret_len, STACK_OF(SSL_CIPHER) * peer_ciphers,
...
get_tmp_dh_params(void)
{
    if (tmp_dh_params == NULL) {
        ...
        if (!TEST_ptr(params) ||
            !TEST_int_eq(EVP_PKEY_fromdata(pctx, &dhpkey, EVP_PKEY_KEY_PARAMETERS, params), 1))
            goto end;
...
test_sni_tls13(void)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                                       TLS1_3_VERSION, 0, &sctx2, &cctx, cert, privkey)))
        goto end;
...
# endif
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), version,
    ...
    if (idx == 0) {
        ...
        if (!TEST_ulong_eq(SSL_SESSION_get_ticket_lifetime_hint(SSL_get_session(clientssl)),
                           TWO_WEEK_SEC))
            goto end;
    } else {
        /* TLSv1.3 uses the limited value */
        if (!TEST_ulong_le(SSL_SESSION_get_ticket_lifetime_hint(SSL_get_session(clientssl)),
                           ONE_WEEK_SEC))
            goto end;
    }
    testresult = 1;
...
test_load_dhfile(void)
{
    ...
    SSL_CONF_CTX_set_flags(cctx,
                           SSL_CONF_FLAG_CERTIFICATE | SSL_CONF_FLAG_SERVER | SSL_CONF_FLAG_FILE);
...
test_pipelining(int idx)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), 0,
                                       TLS1_2_VERSION, &sctx, &cctx, cert, privkey)))
        goto end;
...
test_version(int idx)
{
    ...
    if (is_fips &&
        (version == SSL3_VERSION || version == TLS1_VERSION || version == DTLS1_VERSION)) {
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, servmeth, clientmeth, version, version, &sctx, &cctx,
                                       cert, privkey)))
        goto end;
...
test_handshake_retry(int idx)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(), 0,
                                       maxversion, &sctx, &cctx, cert, privkey)))
        goto end;
...
test_select_next_proto(int idx)
{
    ...
    if (!TEST_int_eq(SSL_select_next_proto(&out, &outlen, server, serverlen, client, clientlen),
                     np->expected_ret))
        goto err;
...
static int
npn_advert_cb(SSL *ssl, const unsigned char **out, unsigned int *outlen, void *arg)
{
    int *idx = (int *)arg;
...
static int
npn_select_cb(SSL *s, unsigned char **out, unsigned char *outlen, const unsigned char *in,
              unsigned int inlen, void *arg)
{
    int *idx = (int *)arg;
...
static int
alpn_select_cb2(SSL *ssl, const unsigned char **out, unsigned char *outlen, const unsigned char *in,
                unsigned int inlen, void *arg)
{
    int *idx = (int *)arg;
...
static int
crypto_send_cb(SSL *s, const unsigned char *buf, size_t buf_len, size_t *consumed, void *arg)
{
    struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
    struct quic_tls_test_data *peer = data->peer;
    size_t max_len =
        sizeof(peer->rcd_data[data->wenc_level]) - peer->rcd_data_len[data->wenc_level];
...
static int
crypto_recv_rcd_cb(SSL *s, const unsigned char **buf, size_t *bytes_read, void *arg)
{
    struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
...
static int
yield_secret_cb(SSL *s, uint32_t prot_level, int direction, const unsigned char *secret,
...
static int
got_transport_params_cb(SSL *s, const unsigned char *params, size_t params_len, void *arg)
{
    struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
...
test_quic_tls(int idx)
{
    ...
    if (!TEST_false(sdata.alert) || !TEST_false(cdata.alert) || !TEST_false(sdata.err) ||
        !TEST_false(cdata.err))
        goto end;

    /* Check the secrets all match */
    for (i = OSSL_RECORD_PROTECTION_LEVEL_EARLY - 1; i < OSSL_RECORD_PROTECTION_LEVEL_APPLICATION;
         i++) {
...
static void
assert_no_end_of_early_data(int write_p, int version, int content_type, const void *buf,
...
setup_tests(void)
{
    ...
    if (strcmp(modulename, "fips") == 0) {
        ...
        if (prov != NULL) {
            ...
            params[0] =
                OSSL_PARAM_construct_int(OSSL_PROV_PARAM_TLS1_PRF_EMS_CHECK, &fips_ems_check);
            params[1] = OSSL_PARAM_construct_end();
