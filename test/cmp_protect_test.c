 ...
 */
static int
verify_signature(OSSL_CMP_MSG *msg, ASN1_BIT_STRING *protection, EVP_PKEY *pkey, EVP_MD *digest)
{
    OSSL_CMP_PROTECTEDPART prot_part;

    prot_part.header = OSSL_CMP_MSG_get0_header(msg);
    prot_part.body = msg->body;
    return ASN1_item_verify_ex(ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART), msg->header->protectionAlg,
                               protection, &prot_part, NULL, pkey, libctx, NULL) > 0;
}
...
test_MSG_protect_certificate_based_without_cert(void)
{
    ...
    fixture->expected = 0;
    if (!TEST_ptr(fixture->msg = OSSL_CMP_MSG_dup(ir_unprotected)) ||
        !TEST_true(SET_OPT_UNPROTECTED_SEND(ctx, 0)) || !TEST_true(EVP_PKEY_up_ref(server_key)) ||
...
execute_cmp_build_cert_chain_test(CMP_PROTECT_TEST_FIXTURE *fixture)
{
    ...
    X509_STORE *store;
    STACK_OF(X509) *chain = X509_build_chain(fixture->cert, fixture->certs, NULL, fixture->with_ss,
    ...
    if (TEST_ptr(store = X509_STORE_new()) && TEST_true(X509_STORE_add_cert(store, root))) {
        ...
        chain = X509_build_chain(fixture->cert, fixture->certs, store, fixture->with_ss,
                                 ctx->libctx, ctx->propq);
...
test_X509_STORE(void)
{
    ...
    if (!TEST_ptr(fixture->certs = sk_X509_new_null()) ||
        !sk_X509_push(fixture->certs, endentity1) || !sk_X509_push(fixture->certs, endentity2) ||
        !sk_X509_push(fixture->certs, root) || !sk_X509_push(fixture->certs, intermediate) ||
