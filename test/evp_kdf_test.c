...
static OSSL_PARAM *
construct_tls1_prf_params(const char *digest, const char *secret, const char *seed)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SECRET, (unsigned char *)secret,
                                             strlen(secret));
    *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (unsigned char *)seed, strlen(seed));
...
test_kdf_tls1_prf(void)
{
    ...
    OSSL_PARAM *params;
    static const unsigned char expected[sizeof(out)] = {0x8e, 0x4d, 0x93, 0x25, 0x30, 0xd7,
                                                        0x65, 0xa0, 0xaa, 0xe9, 0x74, 0xc3,
                                                        0x04, 0x73, 0x5e, 0xcc};
...
static OSSL_PARAM *
construct_hkdf_params(char *digest, char *key, size_t keylen, char *salt, char *info)
{
...
test_kdf_hkdf(void)
{
    ...
    OSSL_PARAM *params;
    static const unsigned char expected[sizeof(out)] = {0x2a, 0xc4, 0x36, 0x9f, 0x52,
...
do_kdf_hkdf_gettables(int extract_only, int has_digest)
{
    ...
    params_get[0] =
        OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE, mode_utf8, sizeof(mode_utf8));
...
static OSSL_PARAM *
construct_pbkdf1_params(char *pass, char *digest, char *salt, unsigned int *iter)
{
    OSSL_PARAM *params = OPENSSL_malloc(sizeof(OSSL_PARAM) * 5);
    OSSL_PARAM *p = params;
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (unsigned char *)pass,
                                             strlen(pass));
    *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (unsigned char *)salt, strlen(salt));
...
static OSSL_PARAM *
construct_pbkdf2_params(char *pass, char *digest, char *salt, unsigned int *iter, int *mode)
{
...
test_kdf_pbkdf2(void)
{
    ...
    params = construct_pbkdf2_params("passwordPASSWORDpassword", "sha256",
                                     "saltSALTsaltSALTsaltSALTsaltSALTsalt", &iterations, &mode);
...
test_kdf_pbkdf2_small_salt(void)
{
    ...
    params = construct_pbkdf2_params("passwordPASSWORDpassword", "sha256", "saltSALT", &iterations,
                                     &mode);
...
test_kdf_ss_hash(void)
{
    ...
    static unsigned char other[] = {0xa1, 0xb2, 0xc3, 0xd4, 0xe5, 0x43, 0x41, 0x56, 0x53, 0x69,
...
test_kdf_x963(void)
{
     ...
     */
    static unsigned char z[] = {0x00, 0xaa, 0x5b, 0xb7, 0x9b, 0x33, 0xe3, 0x89, 0xfa, 0x58, 0xce,
    ...
    static const unsigned char expected[sizeof(out)] = {
        0x44, 0x63, 0xf8, 0x69, 0xf3, 0xcc, 0x18, 0x76, 0x9b, 0x52, 0x26, 0x4b, 0x01, 0x12, 0xb5,
        0x85, 0x8f, 0x7a, 0xd3, 0x2a, 0x5a, 0x2d, 0x96, 0xd8, 0xcf, 0xfa, 0xbf, 0x7f, 0xa7, 0x33,
        0x63, 0x3d, 0x6e, 0x4d, 0xd2, 0xa5, 0x99, 0xac, 0xce, 0xb3, 0xea, 0x54, 0xa6, 0x21, 0x7c,
        0xe0, 0xb5, 0x0e, 0xef, 0x4f, 0x6b, 0x40, 0xa5, 0xc3, 0x02, 0x50, 0xa5, 0xa8, 0xee, 0xee,
        0x20, 0x80, 0x02, 0x26, 0x70, 0x89, 0xdb, 0xf3, 0x51, 0xf3, 0xf5, 0x02, 0x2a, 0xa9, 0x63,
        0x8b, 0xf1, 0xee, 0x41, 0x9d, 0xea, 0x9c, 0x4f, 0xf7, 0x45, 0xa2, 0x5a, 0xc2, 0x7b, 0xda,
        0x33, 0xca, 0x08, 0xbd, 0x56, 0xdd, 0x1a, 0x59, 0xb4, 0x10, 0x6c, 0xf2, 0xdb, 0xbc, 0x0a,
        0xb2, 0xaa, 0x8e, 0x2e, 0xfa, 0x7b, 0x17, 0x90, 0x2d, 0x34, 0x27, 0x69, 0x51, 0xce, 0xcc,
...
test_kdf_kbkdf_6803_128(void)
{
    ...
    static unsigned char outputs[][16] = {{0xD1, 0x55, 0x77, 0x5A, 0x20, 0x9D, 0x05, 0xF0, 0x2B,
                                           0x38, 0xD4, 0x2A, 0x38, 0x9E, 0x5A, 0x56},
                                          {0x64, 0xDF, 0x83, 0xF8, 0x5A, 0x53, 0x2F, 0x17, 0x57,
                                           0x7D, 0x8C, 0x37, 0x03, 0x57, 0x96, 0xAB},
                                          {0x3E, 0x4F, 0xBD, 0xF3, 0x0F, 0xB8, 0x25, 0x9C, 0x42,
                                           0x5C, 0xB6, 0xC9, 0x6F, 0x1F, 0x46, 0x35}};
    ...
    for (i = 0; i < 3; i++) {
        p = 0;
        params[p++] =
            OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_CIPHER, "CAMELLIA-128-CBC", 0);
        ...
        params[p++] =
            OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, input_key, sizeof(input_key));
        params[p++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, constants[i],
                                                        sizeof(constants[i]));
...
test_kdf_kbkdf_6803_256(void)
{
    ...
    for (i = 0; i < 3; i++) {
        p = 0;
        params[p++] =
            OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_CIPHER, "CAMELLIA-256-CBC", 0);
        params[p++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MAC, "CMAC", 0);
        params[p++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE, "FEEDBACK", 0);
        params[p++] =
            OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, input_key, sizeof(input_key));
...
static OSSL_PARAM *
construct_kbkdf_params(char *digest, char *mac, unsigned char *key, size_t keylen, char *salt,
                       char *info, int *r)
{
...
test_kdf_kbkdf_8009_prf1(void)
{
    ...
    params[i++] =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, prf_input, strlen(prf_input));
...
test_kdf_kbkdf_kmac(void)
{
    ...
    static unsigned char input_key[] = {
        0xDD, 0x81, 0xEF, 0xC8, 0x2C, 0xDD, 0xEC, 0x51, 0xC4, 0x09, 0xBD, 0x8C, 0xCB, 0xAF,
        0x94, 0xF6, 0x5F, 0xFA, 0x7B, 0x92, 0xF1, 0x11, 0xF9, 0x40, 0x2B, 0x0D, 0x6A, 0xE0,
        0x5E, 0x44, 0x92, 0x34, 0xF0, 0x3B, 0xBA, 0xF5, 0x4F, 0xEF, 0x19, 0x45, 0xDA};
    static unsigned char context[] = {
        };
    static unsigned char label[] = {
        0xA5, 0xDE, 0x2A, 0x0A, 0xF0, 0xDA, 0x59, 0x04, 0xCC, 0xFF, 0x50, 0xD3, 0xA5, 0xD2,
    ...
    static unsigned char output[] = {
        ,
...
test_kdf_ss_hmac(void)
{
    ...
    static const unsigned char expected[sizeof(out)] = {0x44, 0xf6, 0x76, 0xe8, 0x5c, 0x1b,
                                                        0x1a, 0x8b, 0xbc, 0x3d, 0x31, 0x92,
                                                        0x18, 0x63, 0x1c, 0xa3};
...
test_kdf_sshkdf(void)
{
    ...
    static unsigned char key[] = {
        };
    static unsigned char xcghash[] = {0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed, 0x7c, 0x2f,
                                      0xf0, 0x2d, 0xef, 0x23, 0xb2, 0x74, 0x5e, 0xbc,
                                      0x99, 0xb2, 0x67, 0xda, 0xa8, 0x6a, 0x4a, 0xa7,
                                      0x69, 0x72, 0x39, 0x08, 0x82, 0x53, 0xf6, 0x42};
    static unsigned char sessid[] = {0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed, 0x7c, 0x2f,
                                     0xf0, 0x2d, 0xef, 0x23, 0xb2, 0x74, 0x5e, 0xbc,
                                     0x99, 0xb2, 0x67, 0xda, 0xa8, 0x6a, 0x4a, 0xa7,
                                     0x69, 0x72, 0x39, 0x08, 0x82, 0x53, 0xf6, 0x42};
    ...
    *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SSHKDF_XCGHASH, xcghash, sizeof(xcghash));
    *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SSHKDF_SESSION_ID, sessid, sizeof(sessid));
...
test_kdf_krb5kdf(void)
{
    ...
    static const unsigned char expected[sizeof(out)] = {0x34, 0x28, 0x0A, 0x38, 0x2B, 0xC9,
                                                        0x27, 0x69, 0xB2, 0xDA, 0x2F, 0x9E,
                                                        0xF0, 0x66, 0x85, 0x4B};
...
test_kdf_hmac_drbg_settables(void)
{
    ...
    params[0] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_ENTROPY, (char *)ent,
                                                  sizeof(ent));
    params[1] =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_NONCE, (char *)ent, sizeof(ent));
    ...
    params[0] =
        OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, digestname, sizeof(digestname));
...
test_kbkdf_mac_change(void)
{
    ...
    int l = 0, sep = 0, r = 8;
    static /* const */ unsigned char key[] = {0x3e, 0xdc, 0x6b, 0x5b, 0x8f, 0x7a, 0xad, 0xbd,
    ...
    static /* const */ unsigned char info[] = {
        0x98, 0xe9, 0x98, 0x8b, 0xb4, 0xcc, 0x8b, 0x34, 0xd7, 0x92, 0x2e, 0x1c, 0x68, 0xad, 0x69,
        0x2b, 0xa2, 0xa1, 0xd9, 0xae, 0x15, 0x14, 0x95, 0x71, 0x67, 0x5f, 0x17, 0xa7, 0x7a, 0xd4,
        0x9e, 0x80, 0xc8, 0xd2, 0xa8, 0x5e, 0x83, 0x1a, 0x26, 0x44, 0x5b, 0x1f, 0x0f, 0xf4, 0x4d,
        0x70, 0x84, 0xa1, 0x72, 0x06, 0xb4, 0x89, 0x6c, 0x81, 0x12, 0xda, 0xad, 0x18, 0x60, 0x5a};
