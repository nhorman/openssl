...
static int
net_sim_init(struct net_sim *s, const OSSL_CC_METHOD *ccm, OSSL_CC_DATA *cc, uint64_t capacity,
             uint64_t latency)
{
    s->ccm = ccm;
    s->cc = cc;
...
dump_state(const OSSL_CC_METHOD *ccm, OSSL_CC_DATA *cc, struct net_sim *s)
{
    ...
    fprintf(logfile, "%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,%10lu,\"%c\"\n",
            ossl_time2ms(fake_time), ccm->get_tx_allowance(cc), cwnd_size, cur_bytes,
            s->total_acked, s->total_lost, s->capacity, s->spare_capacity, (char)state);
#endif
...
test_simulate(void)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    p = params;
    *p++ =
        OSSL_PARAM_construct_uint64(OSSL_CC_OPTION_CUR_BYTES_IN_FLIGHT, &diag_cur_bytes_in_flight);
    ...
    *p++ = OSSL_PARAM_construct_end();
     ...
     */
    {
        ...
        TEST_info("est = %6llu kB/s, act=%6llu kB/s (error=%.02f%%)\n",
                  (unsigned long long)estimated_capacity, (unsigned long long)actual_capacity,
                  error * 100.0);
