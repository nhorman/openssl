...
static int
do_PRF(const void *seed1, int seed1_len, const void *seed2, int seed2_len, const void *seed3,
       int seed3_len, unsigned char *out, int olen)
{
...
#define SS_SESSID_OFS 15        /* Session ID goes here */
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x30, /* OCTET_STRING, master secret
                                                                     */
...
static int
send_record(BIO *rbio, unsigned char type, uint64_t seqnr, const void *msg, size_t len)
{
    ...
    if (!EVP_MAC_init(ctx, mac_key, 20, params) || !EVP_MAC_update(ctx, epoch, 2) ||
        !EVP_MAC_update(ctx, seq, 6) || !EVP_MAC_update(ctx, &type, 1) ||
...
send_finished(SSL *s, BIO *rbio)
{
    ...
    /* Derive key material */
    do_PRF(TLS_MD_KEY_EXPANSION_CONST, TLS_MD_KEY_EXPANSION_CONST_SIZE, server_random,
    ...
    if (md_size <= 0)
        return 0;
    do_PRF(TLS_MD_SERVER_FINISH_CONST, TLS_MD_SERVER_FINISH_CONST_SIZE, handshake_hash, md_size,
           NULL, 0, finished_msg + DTLS1_HM_HEADER_LENGTH, TLS1_FINISH_MAC_LENGTH);
...
test_bad_dtls(void)
{
    ...
    for (i = 0; i < (int)OSSL_NELEM(tests); i++) {
        ...
        if (!TEST_true(send_record(rbio, SSL3_RT_APPLICATION_DATA, tests[i].seq, &tests[i].seq,
                                   sizeof(uint64_t)))) {
