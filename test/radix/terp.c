...
err:
    if (!ok) {
        if (gctx.error)
            TEST_error("script generation failed: %s (at %s:%d)", gctx.first_error_msg,
                       gctx.first_error_file, gctx.first_error_line);
...
static int
GEN_SCRIPT_print(GEN_SCRIPT *gen_script, BIO *bio, const SCRIPT_INFO *script_info)
{
    int ok = 0;
    size_t i;
...
static void
SCRIPT_INFO_print(SCRIPT_INFO *script_info, BIO *bio, int error, const char *msg)
{
    if (error)
