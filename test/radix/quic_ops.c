...
static int
ssl_ctx_select_alpn(SSL *ssl, const unsigned char **out, unsigned char *out_len,
...
static int
ssl_create_bound_socket(uint16_t listen_port, int *p_fd, uint16_t *p_result_port)
{
    int ok = 0;
    int fd = -1;
    BIO_ADDR *addr = NULL;
...
DEF_FUNC(hf_read_expect)
{
    ...
    r = SSL_read_ex(ssl, RT()->tmp_buf + RT()->tmp_buf_offset, buf_len - RT()->tmp_buf_offset,
                    &bytes_read);
...
#define OP_NEW_SSL_L_FROM(domain_name, listener_name, flags)                                       \
    (OP_SELECT_SSL(0, domain_name), OP_PUSH_PZ(#listener_name), OP_PUSH_U64(flags),                \
...
#define OP_SIMPLE_PAIR_CONN()                                                                      \
    (OP_NEW_SSL_L_LISTEN(L), OP_NEW_SSL_C(C), OP_SET_PEER_ADDR_FROM(C, L), OP_CONNECT_WAIT(C))
...
#define OP_SIMPLE_PAIR_CONN_ND()                                                                   \
    (OP_SIMPLE_PAIR_CONN(), OP_SET_DEFAULT_STREAM_MODE(C, SSL_DEFAULT_STREAM_MODE_NONE))

#define OP_NEW_STREAM(conn_name, stream_name, flags)                                               \
    (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_PUSH_U64(flags), OP_PUSH_U64(0),    \
     OP_FUNC(hf_new_stream))

#define OP_ACCEPT_STREAM_WAIT(conn_name, stream_name, flags)                                       \
    (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_PUSH_U64(flags), OP_PUSH_U64(1),    \
     OP_FUNC(hf_new_stream))
...
#define OP_ACCEPT_CONN_WAIT(listener_name, conn_name, flags)                                       \
    (OP_SELECT_SSL(0, listener_name), OP_PUSH_PZ(#conn_name), OP_PUSH_U64(flags),                  \
...
#define OP_SHUTDOWN_WAIT(name, flags, error_code, reason)                                          \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(flags), OP_PUSH_U64(error_code), OP_PUSH_PZ(reason),      \
...
#define OP_EXPECT_CONN_CLOSE_INFO(name, error_code, expect_app, expect_remote)                     \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(expect_app), OP_PUSH_U64(expect_remote),                  \
