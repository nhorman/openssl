...
execute_CTX_reinit_test(OSSL_CMP_CTX_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_true(ctx->status == -1 && ctx->failInfoCode == -1 && ctx->statusString == NULL &&
                   ctx->newCert == NULL && ctx->newChain == NULL && ctx->caPubs == NULL &&
...
static int
msg_total_size_log_cb(const char *func, const char *file, int line, OSSL_CMP_severity level,
                      const char *msg)
{
...
static int
test_log_cb(const char *func, const char *file, int line, OSSL_CMP_severity level, const char *msg)
{
    test_log_cb_res =
#ifndef PEDANTIC
...
#define DEFINE_SET_GET_TEST(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE)                                 \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get##M, DUP, FIELD, TYPE *, NULL, IS_0,     \
                             TYPE##_new(), TYPE##_free)

#define DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FIELD, ELEM_TYPE, DEFAULT, NEW, FREE)  \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get##M, 1, FIELD, STACK_OF(ELEM_TYPE) *,    \
                             NULL, DEFAULT, NEW, FREE)
#define DEFINE_SET_GET_SK_TEST(OSSL_CMP, CTX, N, M, FIELD, T)                                      \
    DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FIELD, T, IS_0, sk_##T##_new_null(),       \
                                   sk_##T##_free)
#define DEFINE_SET_GET_SK_X509_TEST(OSSL_CMP, CTX, N, M, FNAME)                                    \
    DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FNAME, X509, EMPTY_SK_X509,                \
                                   sk_X509_new_1(), sk_X509_pop_X509_free)

#define DEFINE_SET_GET_TEST_DEFAULT(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE, DEFAULT)                \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get##M, DUP, FIELD, TYPE *, NULL, DEFAULT,  \
                             TYPE##_new(), TYPE##_free)
...
#define DEFINE_SET_SK_TEST(OSSL_CMP, CTX, N, FIELD, TYPE)                                          \
    ...
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get0, 1, FIELD, STACK_OF(TYPE) *, NULL,     \
                             IS_0, sk_##TYPE##_new_null(), sk_##TYPE##_free)
...
#define DEFINE_SET_CB_TEST(FIELD)                                                                  \
    ...
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set, get, 0, FIELD, OSSL_CMP_##FIELD##_t, NULL, IS_0,   \
                             test_##FIELD, DROP)
#define DEFINE_SET_GET_P_VOID_TEST(FIELD)                                                          \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set, get, 0, FIELD, void *, NULL, IS_0, ((void *)1),    \
                             DROP)
...
#define DEFINE_SET_GET1_STR_FN(SETN, FIELD)                                                        \
    ...
    {                                                                                              \
        return OSSL_CMP_CTX_##SETN##_##FIELD(ctx, (unsigned char *)val, strlen(val));              \
    }                                                                                              \
                                                                                                   \
...
DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set0, get0, 0, trusted, X509_STORE *, NULL, DEFAULT_STORE,
                         X509_STORE_new_1(), X509_STORE_free)
