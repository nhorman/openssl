 ...
 */
static int
encode(unsigned const char *buf, unsigned buflen, char *encoded, int trunc, unsigned llen,
       unsigned wscnt, BIO *mem)
{
...
static int
genb64(char *prefix, char *suffix, unsigned const char *buf, unsigned buflen, int trunc,
       char *encoded, unsigned llen, unsigned wscnt, char **out)
{
    ...
    if ((*prefix &&
         (BIO_write(mem, prefix, preflen) != preflen || BIO_write(mem, &newline, 1) != 1)) ||
        encode(buf, buflen, encoded, trunc, llen, wscnt, mem) <= 0 ||
        (*suffix &&
         (BIO_write(mem, suffix, sufflen) != sufflen || BIO_write(mem, &newline, 1) != 1))) {
        BIO_free(mem);
        return -1;
    }
...
test_bio_base64_run(test_case *t, int llen, int wscnt)
{
     ...
     */
    if (t->trunc > 0 || ((t->bytes > 0 || t->no_nl) && *t->suffix && *t->suffix != '-') ||
        (t->no_nl && *t->prefix)) {
        if ((ret = ret < 0 ? 0 : -1) != 0)
