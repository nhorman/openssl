...
static int
do_encrypt(unsigned char *iv_gen, unsigned char *ct, int *ct_len, unsigned char *tag, int *tag_len)
{
    int ret = 0;
    ...
    *tag_len = 16;
    ret = TEST_ptr(ctx = EVP_CIPHER_CTX_new()) &&
          ...
          TEST_true(EVP_EncryptInit_ex(ctx, NULL, NULL, gcm_key, iv_gen != NULL ? NULL : gcm_iv) >
                    0) &&
...
static int
do_decrypt(const unsigned char *iv, const unsigned char *ct, int ct_len, const unsigned char *tag,
           int tag_len)
{
    int ret = 0;
    EVP_CIPHER_CTX *ctx = NULL;
