...
check_int_from_text(const struct int_from_text_test_st a)
{
    ...
    if (!OSSL_PARAM_allocate_from_text(&param, params_from_text, a.argname, a.strval, 0, NULL)) {
        if (a.expected_res)
            TEST_error("unexpected OSSL_PARAM_allocate_from_text() return for %s \"%s\"", a.argname,
                       a.strval);
    ...
    if (val != a.expected_intval) {
        TEST_error("unexpected result for %s \"%s\":  %li != %li", a.argname, a.strval,
                   a.expected_intval, val);
        return 0;
    }
...
check_octetstr_from_hexstr(void)
{
    ...
    for (i = 0; values[i] != NULL; i++) {
        ...
        if (expected)
            result = TEST_true(OSSL_PARAM_allocate_from_text(&param, params_from_text, "hexoctets",
                                                             values[i], 0, NULL));
        else
            result = TEST_false(OSSL_PARAM_allocate_from_text(&param, params_from_text, "hexoctets",
                                                              values[i], 0, NULL));
        if (!result) {
