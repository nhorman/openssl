...
typedef int PEM_write_bio_of_void_protected(BIO *out, const void *obj, const EVP_CIPHER *enc,
...
static const OSSL_PARAM EC_params[] = {
    OSSL_PARAM_utf8_string("group", groupname, sizeof(groupname) - 1), OSSL_PARAM_END};
#endif
                   ...
                   const char *structure)
{
    ...
    if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey, selection, "PEM", structure,
                                                       NULL)) ||
    ...
    if (pem_read_bio != NULL) {
        ...
        if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new()) ||
            !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&decoded_provided_pkey, "PEM", structure,
...
static int
test_DER(const char *keytype, int evp_type, const void *legacy_key, i2d_of_void *i2d,
         d2i_of_void *d2i, EVP_PKEY_eq_fn *evp_pkey_eq, EVP_PKEY_print_fn *evp_pkey_print,
         EVP_PKEY *provided_pkey, int selection, const char *structure)
{
    int ok = 0;
    ...
    if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey, selection, "DER", structure,
                                                       NULL)) ||
    ...
    if (d2i != NULL) {
        ...
        if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new()) ||
            !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&decoded_provided_pkey, "DER", structure,
                                                           keytype, selection, NULL, NULL)) ||
...
test_key(int idx)
{
    ...
    if (test_stanza->pem_write_bio_PublicKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_unprotected_PEM(key->keytype, key->evp_type, legacy_obj,
                                      ...
                                      test_stanza->pem_read_bio_PublicKey, EVP_PKEY_eq,
    ...
    if (test_stanza->i2d_PrivateKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_PrivateKey,
                          test_stanza->d2i_PrivateKey, EVP_PKEY_eq, EVP_PKEY_print_private, pkey,
                          selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->i2d_PublicKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_PublicKey,
                          test_stanza->d2i_PublicKey, EVP_PKEY_eq, EVP_PKEY_print_public, pkey,
                          selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->i2d_params != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            TEST_info("Test OSSL_ENCODER against i2d_{TYPE}params for %s, %s", test_stanza->keytype,
                      structure);
            if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_params,
                          test_stanza->d2i_params, EVP_PKEY_parameters_eq, EVP_PKEY_print_params,
    ...
    if (test_stanza->i2d_PUBKEY != NULL) {
        ...
        TEST_info("Test OSSL_ENCODER against i2d_{TYPE}_PUBKEY for %s, %s", test_stanza->keytype,
                  structure);
        if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_PUBKEY,
                      test_stanza->d2i_PUBKEY, EVP_PKEY_eq, EVP_PKEY_print_public, pkey, selection,
                      structure))
            ok = 0;
    }
end:
