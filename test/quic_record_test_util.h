...
static int
cmp_pkt_hdr(const QUIC_PKT_HDR *a, const QUIC_PKT_HDR *b, const unsigned char *b_data, size_t b_len,
            int cmp_data)
{
    int ok = 1;
    ...
    if (!TEST_int_eq(a->type, b->type) || !TEST_int_eq(a->spin_bit, b->spin_bit) ||
        ...
        !TEST_size_t_eq(a->token_len, b->token_len) || !TEST_uint64_t_eq(a->len, b->len))
        ok = 0;
