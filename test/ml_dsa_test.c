...
do_gen_key(const char *alg, const uint8_t *seed, size_t seed_len)
{
    ...
    if (seed_len != 0)
        *p++ =
            OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_ML_DSA_SEED, (char *)seed, seed_len);
...
static int
ml_dsa_create_keypair(EVP_PKEY **pkey, const char *name, const uint8_t *priv, size_t priv_len,
                      const uint8_t *pub, size_t pub_len, int expected)
{
    ...
    if (priv != NULL) {
        *p++ =
            OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_PRIV_KEY, (uint8_t *)priv, priv_len);
...
ml_dsa_keygen_test(int tst_id)
{
    ...
    if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->seed, tst->seed_len)) ||
        !TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ML_DSA_SEED, seed,
                                                   sizeof(seed), &seed_len)) ||
        !TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY, priv,
                                                   sizeof(priv), &priv_len)) ||
        !TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PUB_KEY, pub, sizeof(pub),
                                                   &pub_len)) ||
        ...
        !TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE, &sig_len)) ||
...
ml_dsa_siggen_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len, NULL, 0, 1)) ||
        ...
        !TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len, digest, &digest_len),
                     1) ||
...
ml_dsa_sigver_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1) ||
        !TEST_int_eq(EVP_PKEY_verify(vctx, td->sig, td->sig_len, td->msg, td->msg_len),
                     td->expected))
        goto err;
    ret = 1;
err:
...
from_data_invalid_public_test(void)
{
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv, tst->priv_len, tst->pub,
                                         tst->pub_len, 1)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv, tst->priv_len, pub,
                                         tst->pub_len, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv, tst->priv_len, tst->pub,
                                         tst->pub_len - 1, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv, tst->priv_len, tst->pub,
                                         tst->pub_len + 1, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv, tst->priv_len - 1, tst->pub,
                                         tst->pub_len, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv, tst->priv_len + 1, tst->pub,
                                         tst->pub_len, 0)))
        goto err;
...
ml_dsa_keygen_drbg_test(void)
{
    ...
    if (!TEST_ptr(pkey = do_gen_key("ML-DSA-44", NULL, 0)) ||
        ...
        !TEST_int_eq(EVP_PKEY_eq(pkey, pkey2), 0) || !TEST_int_eq(EVP_PKEY_eq(pkey, pkey3), -1) ||
        !TEST_int_eq(EVP_PKEY_get_raw_private_key(pkey, NULL, &len), 1) || !TEST_int_gt(len, 0) ||
...
do_ml_dsa_sign_verify(const char *alg, int tstid)
{
    ...
    *p++ = OSSL_PARAM_construct_int(OSSL_SIGNATURE_PARAM_MESSAGE_ENCODING, (int *)&sp->encoded);
    if (sp->ctx != NULL)
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_SIGNATURE_PARAM_CONTEXT_STRING, sp->ctx,
                                                 sp->ctx_len);
    *p++ = OSSL_PARAM_construct_end();
...
ml_dsa_digest_sign_verify_test(void)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(mctx = EVP_MD_CTX_new()) ||
        !TEST_int_eq(
            EVP_DigestSignInit_ex(mctx, NULL, "SHA256", lib_ctx, "?fips=true", key, params), 0) ||
        !TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx, "?fips=true", key, params),
                     1))
        goto err;
    if (sp->expected == 0) {
        ret = 1; /* return true as we expected to fail */
    ...
    if (!TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len), 0))
        goto err;
    sig_len++;
    if (!TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx, "?fips=true", key, params),
                     1) ||
        !TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len), 1) ||
        !TEST_int_eq(
            EVP_DigestVerifyInit_ex(mctx, NULL, "SHA256", lib_ctx, "?fips=true", key, params), 0) ||
        !TEST_int_eq(EVP_DigestVerifyInit_ex(mctx, NULL, NULL, lib_ctx, "?fips=true", key, params),
                     1) ||
