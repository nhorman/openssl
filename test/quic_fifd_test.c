...
static void (*regen_frame_p)(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
                             void *arg);

static void
regen_frame(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
    regen_frame_p(frame_type, stream_id, pkt, arg);
}

static void
confirm_frame(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
}
...
static void
regen_expect(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
...
test_generic(INFO *info, int kind)
{
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (!TEST_true(ossl_quic_sstream_append(info->sstream[i], (unsigned char *)"Test message",
                                                12, &consumed)) ||
        ...
        if (!TEST_true(
            ...
            !TEST_true(ossl_quic_sstream_mark_transmitted(info->sstream[i], hdr.offset,
...
test_fifd(int idx)
{
    ...
    if (!TEST_true(ossl_statm_init(&info.statm)) ||
        ...
        !TEST_ptr(info.ackm = ossl_ackm_new(fake_now, NULL, &info.statm, &ossl_cc_dummy_method,
                                            info.ccdata)) ||
        ...
        !TEST_true(ossl_quic_fifd_init(&info.fifd, info.cfq, info.ackm, info.txpim,
