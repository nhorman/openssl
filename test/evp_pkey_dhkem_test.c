...
test_dhkem_encapsulate(int tstid)
{
    ...
    if (t->spriv != NULL) {
        if (!TEST_ptr(
                spriv = new_raw_private_key(t->curve, t->spriv, t->sprivlen, t->spub, t->spublen)))
            goto err;
    }
...
test_dhkem_decapsulate(int tstid)
{
    ...
    if (!TEST_ptr(rpriv =
                      new_raw_private_key(t->curve, t->rpriv, t->rprivlen, t->rpub, t->rpublen)))
        goto err;
    if (t->spub != NULL) {
...
test_ec_dhkem_derivekey_fail(void)
{
    ...
    params[1] =
        OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_DHKEM_IKM, (char *)t->ikm, t->ikmlen);
    ...
    params[1] =
        OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_DHKEM_IKM, (char *)t->ikm, t->ikmlen - 1);
    params[2] = OSSL_PARAM_construct_end();
...
test_no_operation_set(int tstid)
{
    ...
    return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) &&
           ...
           TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1) &&
           TEST_int_eq(EVP_PKEY_decapsulate(ctx, NULL, &len, t->expected_enc, t->expected_enclen),
                       1);
}
...
test_ec_invalid_private_key(void)
{
    ...
    static const unsigned char order[] = {0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00,
                                          0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                                          0xBC, 0xE6, 0xFA, 0xAD, 0xA7, 0x17, 0x9E, 0x84,
                                          0xF3, 0xB9, 0xCA, 0xC2, 0xFC, 0x63, 0x25, 0x51};

    ret =
        TEST_ptr(priv = new_raw_private_key("P-256", order, sizeof(order), t->rpub, t->rpublen)) &&
...
test_ec_public_key_infinity(void)
{
    ...
    ret =
        TEST_ptr(key = new_raw_private_key(t->curve, t->rpriv, t->rprivlen, tmp, sizeof(tmp))) &&
...
test_null_params(int tstid)
{
    ...
    return TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, NULL, opparam), 0) &&
           ...
           TEST_int_eq(EVP_PKEY_decapsulate(ctx, NULL, NULL, t->expected_enc, t->expected_enclen),
                       0)
...
test_ec_dhkem_derivekey(int tstid)
{
    ...
    params[0] =
        OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, (char *)t->curvename, 0);
    params[1] =
        OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_DHKEM_IKM, (char *)t->ikm, t->ikmlen);
    params[2] = OSSL_PARAM_construct_end();
    ...
    ret = TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, "EC", NULL)) &&
          ...
          TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY,
...
test_ec_noikme(int tstid)
{
    ...
    ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, recip, NULL)) &&
          ...
          TEST_int_eq(EVP_PKEY_encapsulate(ctx, sender_pub, &sender_publen, sender_secret,
                                           &sender_secretlen),
                      1) &&
          (sender_auth == NULL ||
...
test_ecx_dhkem_derivekey(int tstid)
{
    ...
    ret = TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, t->curvename, NULL)) &&
          ...
          TEST_int_eq(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY,
                                                      pubkey, sizeof(pubkey), &pubkeylen),
                      1) &&
          TEST_int_eq(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY, privkey,
...
setup_tests(void)
{
    ...
    if (!TEST_ptr(rctx[TEST_KEYTYPE_P256] =
                      EVP_PKEY_CTX_new_from_pkey(libctx, rkey[TEST_KEYTYPE_P256], NULL)))
        goto err;
#ifndef OPENSSL_NO_ECX
    if (!TEST_ptr(rctx[TEST_KEYTYPE_X25519] =
                      EVP_PKEY_CTX_new_from_pkey(libctx, rkey[TEST_KEYTYPE_X25519], NULL)))
        goto err;
#endif
