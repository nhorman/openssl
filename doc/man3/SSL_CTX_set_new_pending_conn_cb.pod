...
SSL_set_new_pending_conn_cb_fn() must return an integer value.  A return value of
...
on a subsequent call to SSL_accept_connection().  Any other nonzero return
value is treated as success, allowing the new connection to be enqueued to the
accept queue.

=head1 HISTORY

SSL_CTX_set_new_pending_conn_cb() was added in OpenSSL 3.5.0
