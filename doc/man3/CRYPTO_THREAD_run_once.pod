...
=head1 SYNOPSIS
 ...
 CRYPTO_THREAD_rcu_derefrence(p)
 CRYPTO_THREAD_rcu_assign_pointer(p,v)
...
OpenSSL currently support both traditional Read/Write locks and
Read-Copy-Update (RCU) locks.  The latter provides both improved read side
performance, as well as lower writer-starvation than their read/write
counterparts, but require additional synchronization.  The read side operation
of RCU locks is simmilar to that of read/write locks, but the write side
requires that writers copy the data being modified, atomically updating a shared
read/write pointer, a synchronization period, folowed by freeing of the old data
...
CRYPTO_THREAD_rcu_lock_new() allocates a new rcu lock

=item *

CRYPTO_THREAD_rcu_read_lock() acquires a read side hold on data protected by
the lock

=item *

CRYPTO_THREAD_rcu_read_unlock() releases a read side hold on data protected by
the lock

=item *

CRYPTO_THREAD_rcu_write_lock() acquires a write side hold on data protected by
the lock.  Note only one writer per lock is permitted, as with read/write locks

=item *

CRYPTO_THREAD_rcu_write_unlock() releases a write side hold on data protected
by the lock

=item *

CRYPTO_THREAD_synchronize_rcu() blocks the calling thread until all read side
holds on the lock have been released, guaranteing that any old data updated by
the write side thread is safe to free

=item *

CRYPTO_THREAD_rcu_call() enqueues a callback function to the lock, to be called
...
CRYPTO_THREAD_rcu_derefrence(p) atomically reads a pointer under an RCU locks
protection

=item *

CRYPTO_THREAD_rcu_assign_pointer(p,v) atomically writes to a pointer under an
RCU locks protection

=item *

CRYPTO_THREAD_rcu_lock_free() frees an allocated RCU lock
