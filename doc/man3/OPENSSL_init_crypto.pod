...
=head1 SYNOPSIS
 ...
 int OPENSSL_add_library_user(OSSL_LIBRARY_TOKEN *token);
...
a call to OPENSSL_cleanup(), which drops the library reference count.  Only
...
while it is uninitalized (in the event OPENSSL_cleanup() is called more often
...
The OPENSSL_add_library_user() call was added in OpenSSL 4.0.0.

The parameter list of OPENSSL_cleanup was modified to include an
B<OSSL_LIBRARY_TOKEN> parameter in OpenSSL 4.0.0.
