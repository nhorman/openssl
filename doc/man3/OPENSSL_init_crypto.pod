...
while it is uninitalized (in the event OPENSSL_cleanup_ex() is called more often
...
legacy applications, but will preform no operation if any other software
...
mechansim available via OPENSSL_add_library_user() and OPENSSL_cleanup_ex(),
